...
```bash
agentnexus server --config server_config.yaml
```
...
policies:
  ...
  github_access:
    type: function
    handler: agentnexus.policies.builtins.github.github_policy
    factory_params:
      write_repos:
        - myorg/my-repo
      write_branches:
  ...
  google_policy:
    type: function
    handler: agentnexus.policies.builtins.google.gdrive_policy
    factory_params:
      read_all: true
      allow_create: true
```
...
AgentNexus session marker) plus the harness's own variable family, and nothing
...
- **A generic ACP agent** (`type: acp`) gets no vendor family at all, because
  AgentNexus cannot know which vendor an arbitrary agent belongs to. Gemini via
  ACP needs `env_passthrough: ["GEMINI_API_KEY"]`.
- **Goose** owns `GOOSE_*` only. Goose configured through AgentNexus's
...
daily_budget:
  type: function
  handler: agentnexus.policies.builtins.cost.user_daily_cost_budget
  factory_params:
    max_cost_usd: 25.00
...
policies:
  deny_trivial_opus:
    type: function
    handler: agentnexus.policies.builtins.routing.deny_trivial_to_expensive_model
    factory_params:
      expensive_models:
