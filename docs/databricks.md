# Running agentnexus on Databricks

A production deployment guide for running agentnexus agents on Databricks
infrastructure. Covers the four canonical integration points:

1. **Databricks Apps** as the managed runtime for the agentnexus server
...
agentnexus's fine standalone with any OTLP backend and any LLM
...
> (Beta) is a fully managed service: Databricks operates the agentnexus
> server for you, already wired to workspace identity, Foundation
> Models, AI Gateway, and MLflow Tracing. You enable the **AgentNexus**
...
> the agentnexus server yourself on Databricks Apps. Reach for it when the
...
This guide assumes you're new to both agentnexus and Databricks. Each
...
When agentnexus runs on Databricks with the four integration points
wired:
...
- **Managed runtime with workspace identity.** The agentnexus server
...
The integration is layered. The agentnexus server runs on Databricks
Apps. It exports OpenTelemetry traces (via the work landed in [PR
#1050](https://github.com/agentnexus-ai/agentnexus/pull/1050)) to MLflow
...
![agentnexus on Databricks architecture](images/databricks/architecture.png)

The boundary stays sharp. agentnexus itself remains a standalone Apache
...
3. **Python 3.11+** locally with `uv` installed (the agentnexus project
   standard).
...
guide, this is the fastest path. It runs agentnexus locally (not on
Apps) but wires up Foundation Models + Gateway + tracing to the
workspace.

```bash
pip install 'agentnexus[tracing]' openai

export DATABRICKS_HOST=https://<your-workspace>.cloud.databricks.com
export DATABRICKS_TOKEN=<personal-access-token>

# Point agentnexus at Mosaic AI Foundation Models as the LLM provider
export OPENAI_BASE_URL=$DATABRICKS_HOST/serving-endpoints
export OPENAI_API_KEY=$DATABRICKS_TOKEN

# Point agentnexus's OTel exporter at the MLflow OTLP receiver in UC
...
# Start the local agentnexus server with tracing
omni server
```
...
Databricks workspace. agentnexus's server is an HTTP app, so it fits
...
The `deploy/databricks/` directory in the agentnexus repository contains
a complete Databricks Asset Bundle for deploying the agentnexus server
to Apps backed by Lakebase. Use it as-is for a first deploy:

```bash
git clone https://github.com/agentnexus-ai/agentnexus
cd agentnexus
...
#   uv run python deploy/databricks/deploy.py --app-name agentnexus --profile <profile> ...
...
Self-hosting the agentnexus server (e.g., on a VM or in a container)
...
agentnexus's harnesses (`claude_sdk`, `openai_agents_sdk`, `pi`, and
...
Then any agentnexus agent spec that points at a `databricks-` model
...
agentnexus doesn't need to know it's calling a Gateway. The Gateway
endpoint speaks the OpenAI API, so the same `OPENAI_BASE_URL` knob
that points agentnexus at Foundation Models points it at the Gateway.
...
The OpenAI-style chat completions URL (what agentnexus uses) is:
...
### Point agentnexus at the Gateway endpoint
...
Then in the agentnexus agent spec, use the Gateway endpoint name as the
model:
...
then called it via the same OpenAI SDK pattern agentnexus uses. The
...
resp = client.chat.completions.create(
    model='agentnexus-docs-test-gateway',   # the Gateway endpoint, not the backing model
...
(`agentnexus-docs-test-gateway`) is what agentnexus calls. Swapping the
...
| Claude Max / ChatGPT Plus / Cursor Pro (OAuth subscription) | No | agentnexus session metadata only; LLM calls invisible | No — billed to the individual's consumer account |
...
If centralized governance is a hard requirement, the agentnexus host
...
| Capability | Raw provider call from agentnexus | Through AI Gateway |
|---|---|---|
...
agentnexus emits OpenTelemetry spans for every agent turn, LLM call, and
...
in [PR #1050](https://github.com/agentnexus-ai/agentnexus/pull/1050). Any
...
Three env vars on the agentnexus server:
...
agentnexus's `telemetry.init()` auto-detects the OTLP endpoint and wires
...
`mlflow.start_span` API agentnexus's `TracingContext` wraps) against the
...
Experiment:   id=3163592711242134 path=/Users/.../agentnexus-databricks-docs-verification
...
agentnexus does not capture message bodies into traces by default. Set:

```bash
export AGENTNEXUS_OTEL_CAPTURE_CONTENT=true
```
...
| `AGENTNEXUS_OTEL_CAPTURE_CONTENT` | `true` to include message bodies in traces | Apps env (default off) |

---

## Roadmap

The four sections above cover the V1 integration. The agentnexus +
Databricks story extends further. Planned follow-ups, each as a
separate PR:

- **Unity Catalog functions as agent tools.** A `databricks-tools`
  optional extra in agentnexus that exposes UC functions as
...
- **Mosaic AI Agent Evaluation integration.** Sample production
  traces from agentnexus for the managed Mosaic AI Agent Evaluation
  pipeline.
...
Track these on the [agentnexus issues
list](https://github.com/agentnexus-ai/agentnexus/issues) under the
`databricks` label.
...
fresh Lakebase project per agentnexus app rather than sharing one.
...
observability series shipped in PRs [#1050](https://github.com/agentnexus-ai/agentnexus/pull/1050),
[#1068](https://github.com/agentnexus-ai/agentnexus/pull/1068),
[#1070](https://github.com/agentnexus-ai/agentnexus/pull/1070),
[#1071](https://github.com/agentnexus-ai/agentnexus/pull/1071),
[#1072](https://github.com/agentnexus-ai/agentnexus/pull/1072), and
[#1083](https://github.com/agentnexus-ai/agentnexus/pull/1083).
...
- External Model (Gateway) endpoint created, called, and torn down:
  `agentnexus-docs-test-gateway` proxied to `databricks-claude-sonnet-4`
...
ping @debu-sinha on the agentnexus Slack channel.
