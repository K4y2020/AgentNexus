...
- CLI/server/host logs stay under the runtime data dir
  (`~/.agentnexus/logs` by default, `AGENTNEXUS_DATA_DIR` to isolate).
...
- `agentnexus diagnose` emits a sanitized environment snapshot described as
...
- `AGENTNEXUS_DATA_DIR` isolates a full environment (DB, artifacts, logs)
...
- Auth is disabled by default locally. When `AGENTNEXUS_AUTH_ENABLED` is on,
