...
So `agentnexus version | cat`, `agentnexus config list | jq`, and piped
...
`agentnexus.inner.mascots.MASCOT_ART_COLOR`, re-exported as `ui.ACCENT`.
The `scripts/install_oss.sh` installer mirrors the same accent
(`\033[38;2;244;59;166m`) so the installer and the tool agree.

## Helper API (`agentnexus.inner.ui`)

Status lines — consistent glyph + color, correct stream:

```python
ui.step("Installing AgentNexus")     # ==>  accent   (stdout)
ui.success("Verified agentnexus")    #  ✓   green    (stdout)
ui.info("Using ~/.agentnexus")       #  ·   dim      (stdout)
...
no-op off a TTY or when `AGENTNEXUS_NO_BANNER` is set):

- **Full lockup** — `ui.print_landing(...)` — Otto + wordmark, optional
  gradient / tagline / epilogue. The hero moment, reserved for the few
  landing surfaces:
  - `agentnexus --help` (the top-level group, via `_AgentNexusCLI.format_help`)
  - `agentnexus setup` (first-run experience)
...
`✦ agentnexus`) exists for opt-in use, but is intentionally **not** wired
onto any command today — add it only if a specific surface clearly wants
light branding.

The bare `agentnexus` invocation on a TTY launches the REPL (its own
...
| `AGENTNEXUS_NO_BANNER` truthy  | No banner/brandmark even on a TTY          |
| `AGENTNEXUS_NO_SPINNER` truthy | No startup spinner (pre-existing)          |

## Adding a new command — checklist

1. `from agentnexus.inner import ui` (import lazily inside the command body
