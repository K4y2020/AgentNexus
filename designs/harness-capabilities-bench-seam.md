...
|---|---|---|---|
| **Static** (use this) | `agentnexus.harness_plugins.harness_capabilities()` → `dict[str, HarnessCapabilities]` | *Declared* trait/claim, pre-spawn | Immediately, no subprocess |
| Runtime | `agentnexus.inner.executor.Executor.supports_streaming()` / `interrupt_session()` … | *Actual* in-subprocess behavior | Only after spawn |
...
- Type: `agentnexus/harness_capabilities.py` → `HarnessCapabilities` (frozen
...
| `auth` | `auth` | `AGENTNEXUS_CREDENTIAL` / `OWN_AUTH` / `SESSION_SCOPED_CONFIG`. The old free-text ("Anthropic key / Databricks gateway") is richer prose; keep a small enum→string map if you want the exact wording, or simplify. |
...
1. Add the field to `HarnessCapabilities` (+ `as_dict()`), in
   `agentnexus/harness_capabilities.py`.
