...
AgentNexus release features are deployment-wide, temporary rollout switches. They
are not authorization controls or user preferences.

## Configuration

Set the comma-separated `AGENTNEXUS_FEATURES` environment variable and restart or
redeploy the server:

```bash
AGENTNEXUS_FEATURES=usage_page,harness_install
```

Unset or empty means every release feature is off. Unknown names fail startup.
The former `AGENTNEXUS_HARNESS_INSTALL_ENABLED` switch is rejected with a
migration hint; use `AGENTNEXUS_FEATURES=harness_install` instead. The server resolves the set once at startup and publishes frontend-visible
...
`agentnexus/server/feature_flags.py` is the source of truth for known keys and
lifecycle metadata.
...
1. Deploy an immutable image with the feature absent from `AGENTNEXUS_FEATURES`.
