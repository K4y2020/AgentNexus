...
class SlackNotifier:
    ...
    def _session_web_link(self, session_id: str) -> str:
        ...
        # mount (``https://<ws>/api/2.0/agentnexus``), but the web UI lives on the
        # workspace SPA mount (``https://<ws>/agentnexus``) — linking to the API
        ...
        # import ``agentnexus.server_url`` — so the mount mapping is mirrored
        # here.)
        parts = urlsplit(self._server_url.rstrip("/"))
        # Legacy workspace mounts remain supported until 2.0.
        if parts.path in ("/api/2.0/agentnexus", "/api/2.0/omnigent"):
            ui_path = parts.path.removeprefix("/api/2.0")
            return urlunsplit(
                (parts.scheme, parts.netloc, f"{ui_path}/c/{session_id}", parts.query, "")
            )
