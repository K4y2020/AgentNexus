...
COMMAND_NAME = "/agentnexus"
# Existing Slack app registrations remain supported until 2.0.
LEGACY_COMMAND_NAME = "/omnigent"
...
class SetupFlow:
    ...
    Opening ``/agentnexus`` validates connectivity against that server,
    ...
    def register(self, app: AsyncApp) -> None:
        ...
        app.command(LEGACY_COMMAND_NAME)(self._handle_config_command)
    ...
    async def _handle_config_command(self, ack: Any, command: dict[str, Any], client: Any) -> None:
        # ``/agentnexus`` (or ``/agentnexus config``) opens setup against the fixed
        ...
        # ``/agentnexus logout`` revokes every server token and clears all saved
        # settings for the user. ``/agentnexus bind`` staffs the channel where the
        # command ran with a teammate; ``/agentnexus unbind`` removes the binding.
        await ack()
    ...
    async def _handle_logout(self, *, team_id: str, user_id: str, client: Any) -> None:
        """Handle ``/agentnexus logout`` — full reset for the user.
        ...
        await self._dm_user(
            ...
            text=(
                ...
                "cleared — run `/agentnexus` to set up again."
            ),
    ...
    async def _begin_setup(
        ...
        *,
        ...
        try:
            ...
            # the moment the user approves — no DM, no re-running /agentnexus.
        ...
        if not email:
            await client.views_update(
                ...
                view=login_failed_modal(
                    ...
                    "`users:read.email` scope, then run `/agentnexus` again.",
                ),
            )
            return
...
def host_unavailable_text(server_url: str) -> str:
    ...
    return (
        ...
        f"`agentnexus host --server {server_url}`"
    )
        ...
        "blocks": [
            {
                ...
                "text": {
                    ...
                    "text": (
                        ...
                        "Add an agent on the server, then run `/agentnexus` again."
                    ),
                },
            }
        ],
...
def login_failed_modal(server_url: str, reason: str) -> dict[str, Any]:
    ...
    # re-runs /agentnexus to try again.
