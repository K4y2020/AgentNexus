...
def _local_data_dir() -> Path:
    ...
    local copy rather than an import so the standalone ``agentnexus-slack``
...
class Settings(BaseSettings):
    ...
    # This mirrors ``agentnexus server`` / the core CLI, which load no ``.env``:
    ...
    def _normalize_server_url(cls, value: str) -> str:
        ...
        # (see agentnexus.py). Plaintext http:// would transmit that credential in
...
def load_settings() -> Settings:
    ...
    try:
        ...
        lines.append(
            ...
            "injects them — e.g. `uv run --env-file .env agentnexus integration slack`. "
