...
class HostUnavailableError(AgentNexusError):
    ...
    resolves by starting a host with ``agentnexus host --server <url>``.
    """
...
class HarnessNotConfiguredError(AgentNexusError):
    ...
    A precondition failure the user resolves by running ``agentnexus setup`` on the
...
async def _raise_for_status(response: httpx.Response) -> None:
    try:
        ...
        # an unauthenticated request to its login page — the agentnexus API itself
        ...
        if response.status_code == 412 and error_code == "harness_not_configured":
            ...
            # on the host — run `agentnexus setup` there). The server's structured
