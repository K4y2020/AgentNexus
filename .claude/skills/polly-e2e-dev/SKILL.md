...
- **guardrails** (`agentnexus.inner.nessie.policies`) — `blast_radius` (deny
...
```bash
uv run --frozen python -c "import agentnexus; print('ok')"   # builds .venv
```
...
The driver boots a throwaway local AgentNexus server (which carries
`agentnexus.inner.nessie.policies` — the module polly's guardrails resolve) plus
the repo's mock-LLM server, rewrites the polly bundle to the `openai-agents`
harness wired to the mock, then runs `agentnexus run` turns where the brain is
...
- **Guardrail policies:** `agentnexus/inner/nessie/policies.py`
- **Runner-side gate:** `agentnexus/runner/policy.py`; server-side tool-call
  enforcement: `agentnexus/server/routes/sessions.py`
...
`agentnexus.host._daemon_entry` / `runner._entry` / `harnesses._runner`
...
pgrep -af "$(pwd)/.venv/bin/python -m agentnexus" | grep -E "_entry|_runner|_daemon" || echo clean
```
