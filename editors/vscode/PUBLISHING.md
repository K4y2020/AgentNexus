# Publishing the AgentNexus VS Code extension

This directory holds the AgentNexus VS Code extension. It is published
...
Code extension publish flow (the one to adapt) and agentnexus's PyPI release
(which already checks out `agentnexus-ai/agentnexus` cross-org). Maintainers: the
...
5. **Smoke-test the `.vsix` locally.** Download the `.vsix` from the published
   ...
   ```bash
   gh release download vscode-v<version> --repo agentnexus-ai/agentnexus --pattern '*.vsix'
   code --install-extension agentnexus-vscode-<version>.vsix
   ```

   Reload VS Code, start a local server (`agentnexus server`), and run the
   **AgentNexus: Open** command — confirm it opens an editor pane showing the
   ...
   marketplaces.
6. **Publish to the marketplaces.** Dispatch `agentnexus-vscode.yml` in the
...
| 5   | Ask DECO to register `agentnexus-vscode` under the `databricks` publisher + add dedicated `OMNI_VSCE_TOKEN` / `OMNI_OVSX_PAT` secrets (and an `agentnexus-vscode-marketplace` environment for the reviewer gate)                                                                                                                                    | Slack `#dev-ecosystem-discuss` ([https://databricks.slack.com/archives/C01KSAWFXG8/p1782971196701749](https://databricks.slack.com/archives/C01KSAWFXG8/p1782971196701749)) | human approval      |
| 6   | Add an `agentnexus-vscode.yml` publish workflow in the secure repo, adapting the existing Databricks VS Code extension workflow — it already does download → scan → `vsce publish` + `ovsx publish` in one workflow | the internal secure-release repo                                                                                                                                            | DECO grant (step 5) |
...
`gh release download -R databricks/databricks-vscode`. The agentnexus variant
must point `-R` at `agentnexus-ai/agentnexus` and relax the filename check (it
hard-codes `databricks-*-${VERSION}.vsix`; ours is `agentnexus-vscode-*.vsix`).
- We author `agentnexus-vscode.yml` ourselves — it's our own pipeline, not a fork
...
(`vscode-extension-release.yml` and `agentnexus-vscode.yml`), and we control
both.
- Cross-org fetch from the public `agentnexus-ai/agentnexus` repo is already an
accepted pattern — `agentnexus.yml` checks out `agentnexus-ai/agentnexus` for the
PyPI release. So reading a public agentnexus release from the hardened runner is
not a new blocker.
...
`agentnexus-vscode-marketplace` environment for an independent reviewer gate.)
