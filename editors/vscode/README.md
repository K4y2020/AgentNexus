# AgentNexus for VS Code

A minimal VS Code extension that opens your **running local AgentNexus server** inside
...
[agentnexus-ai/agentnexus#1219](https://github.com/agentnexus-ai/agentnexus/issues/1219)):
...
- On activation the extension discovers a locally running server via
  `~/.agentnexus/local_server.pid` and a `/health` probe (or uses `agentnexus.serverUrl`
  when set to a localhost URL).
- The AgentNexus activity-bar view offers an **Open AgentNexus** button. The
  **AgentNexus: Open** command (`agentnexus.open`) — also on the editor title bar and in the
...
|---|---|---|
| `agentnexus.serverUrl` | `""` | Manual **localhost** server URL override (e.g. `http://127.0.0.1:6767`); empty = auto-discover. Non-localhost URLs are not supported in this build. |
...
pnpm run package      # @vscode/vsce package -> agentnexus-vscode-<version>.vsix
```
...
├── commands/openPanel.ts  # the agentnexus.open command
