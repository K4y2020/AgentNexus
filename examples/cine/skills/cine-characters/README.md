...
- **角色设定图** — **每个角色一张**：16:9 分三区，左侧约 34% 证件照式半身像（面部基准）、右上全身三视图、右下关键细节特写条。**画风可选**：默认半写实厚涂，也可以出吉卜力动画风。白底方便抠图，通过直接 V3 画布工具授权出图（可选）
...
```
/cine-characters ./book.txt --lang en
/cine-characters ./book.txt --lang ja
```
...
```bash
node scripts/cine-characters.mjs ui-template fr   # 打印待翻译的骨架
```
...
```
cine-outline    → outline.json （什么：结构与分集，谁进谁不进）
...
```bash
node scripts/cine-characters.mjs seed outline.json > seed.json
```
...
```
/cine-characters ./book.txt --style ghibli
```
...
```bash
node scripts/cine-characters.mjs styles          # 看所有预设
node scripts/cine-characters.mjs styles ghibli   # 看某一个的完整内容
```
...
```bash
node scripts/cine-characters.mjs seed outline.json              # 有大纲就从它预填角色表骨架
node scripts/cine-characters.mjs chunk book.txt /tmp/wk        # 切块
node scripts/cine-characters.mjs merge /tmp/wk                 # 归并 roster-*.json，附疑似同人候选
node scripts/cine-characters.mjs merge /tmp/wk --apply m.json   # 落地复核后的合并
node scripts/cine-characters.mjs assemble /tmp/wk --source 书名 # card-*.json 合成 cast.json，同档按戏份排序
node scripts/cine-characters.mjs validate cast.json book.txt   # 校验
node scripts/cine-characters.mjs render cast.json --html       # 出 report.html
node scripts/cine-characters.mjs slug "胡二爷"                  # 安全文件名
```
...
- 默认取戏份最重的 30 位角色，**按明确授权范围出设定图**——一个角色一次调用，所以角色多的时候这步最花时间。想少出就直接给个数，或者说只要主要角色
