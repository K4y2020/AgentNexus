...
```
cine-outline    → outline.json (what: structure & episodes, who is in)
...
```
/cine-characters ./your-novel.txt
```
...
```bash
node scripts/cine-characters.mjs styles          # list all presets
node scripts/cine-characters.mjs styles ghibli   # dump one in full
```
...
```bash
node scripts/cine-characters.mjs seed outline.json              # seed the roster from an outline, if you have one
node scripts/cine-characters.mjs chunk book.txt /tmp/wk        # split
node scripts/cine-characters.mjs merge /tmp/wk                 # merge roster-*.json, with merge candidates
node scripts/cine-characters.mjs merge /tmp/wk --apply m.json   # apply reviewed merges
node scripts/cine-characters.mjs assemble /tmp/wk --source Book # combine card-*.json into cast.json, prominence-ordered
node scripts/cine-characters.mjs validate cast.json book.txt   # validate
node scripts/cine-characters.mjs render cast.json --html       # build report.html
node scripts/cine-characters.mjs slug "胡二爷"                  # filesystem-safe name
```
...
- The top 30 characters by prominence are profiled by default, and **sheets are generated only within the explicitly authorized scope** — one call per character, so this is the slowest step on a large cast. Ask for a smaller number, or for leads only, if you want it shorter
...
references/
  ...
  sheet.md               the direct V3 canvas contract for model-sheet generation
