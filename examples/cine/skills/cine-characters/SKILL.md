...
description: |
  ...
  本地脚本零依赖；Cine 通过 seedance_read_canvas / seedance_edit_canvas 制作设定卡，授权后由 V3 workers 出图。
...
`{baseDir}` = 本文件所在目录。脚本 `{baseDir}/scripts/cine-characters.mjs`，零依赖，`node` 直接跑。

**运行环境**：Claude Code 和 codex 都能跑。第 8 步出图取决于 V3 工具、授权与真实资产是否可用——见 `references/sheet.md`。
...
- **其他任何语言** —— 你要现场翻一份。跑
  ...
  ```bash
  node {baseDir}/scripts/cine-characters.mjs ui-template <lang>
  ```
...
```bash
node {baseDir}/scripts/cine-characters.mjs styles   # 打印预设的完整内容
```
...
**有 `outline.json`（cine-outline 的产出）就复用，走 seed**——大纲是角色设定的上游，它的 `characters` 块已经定死了角色清单：

```bash
node {baseDir}/scripts/cine-characters.mjs seed <outline.json> > <workdir>/seed.json
```
...
```bash
node {baseDir}/scripts/cine-characters.mjs chunk <book.txt> <workdir>
```
...
```bash
node {baseDir}/scripts/cine-characters.mjs merge <workdir> | tee <workdir>/merged.json
```
...
```bash
node {baseDir}/scripts/cine-characters.mjs merge <workdir> --apply merges.json | tee <workdir>/merged.json
```
...
角色卡 JSON 写到 `<workdir>/card-<slug>.json`。**断点续跑**：`card-<slug>.json` 仅在核对当前上游 pins、内容和校验结果后可复用，存在本身不代表有效。
...
```bash
node {baseDir}/scripts/cine-characters.mjs assemble <workdir> \
...
```bash
node {baseDir}/scripts/cine-characters.mjs validate <cast.json> <book.txt>
```
...
- **V3 工具不可用或未获生成授权就跳过出图**，只交提示词并标图像缺失
- Cine 先 `seedance_read_canvas` 再 `seedance_edit_canvas` 创建/更新角色卡并连接真实参考资产；按上文并发版本与授权约束提交 V3 workers
- **一个角色一次调用，绝不批量**
- 单个失败就跳过，不阻断；最后汇总说明
- **断点续跑**：只有核对当前来源、上游 pins 且实际看图验收的文件可复用；旧文件和路径不算成功，缺图保持缺失
...
```bash
cd <输出目录>
node {baseDir}/scripts/cine-characters.mjs render <cast.json> --md   > <书名>-cast.md
node {baseDir}/scripts/cine-characters.mjs render <cast.json> --html > report.html
```

语言取 `cast.json` 里的 `lang`，要临时覆盖就加 `--lang <code>`。

`render` 会自动去 `images/<slug>-sheet.png` 找图。使用本次独立输出目录，仅放已核对并验收的本地图；找得到文件不等于图像已验收。缺图可渲染占位。
...
一句话说清：角色数、出图数、报告路径。校验一次没过的话，说明修了什么。有角色出图失败、被截断、或工具不可用或未获授权而没出图，明确说清楚。
...
- 出图仅通过直接画布工具提交 V3 workers；禁止 codex `$imagegen` 和 CLI fallback
