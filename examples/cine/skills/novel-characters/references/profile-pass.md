...
角色卡同时是下游编剧的行为依据，不只是形象与音色资产。用现有字段写出可用于写戏的小传：

- `oneLiner` 写角色在故事中的矛盾位置，不只写职业和外貌。
- `persona.motivation` 写外在欲望、失败代价及不愿承认的内在需要；原文未给出的部分须标注推断。
- `persona.temperament` 写可观察的惯常行为、掩饰方式、受压后的换招和说话习惯，避免只堆形容词。
- `persona.arc` 写起点信念、迫使其选择的压力和可能变化，不把结局口号当弧光。
- `persona.relationships[].relation` 写该角色对对方的态度、旧账或依赖、当前筹码，不能只写“同事”“朋友”。关系允许不对称。
- `persona.personality` 只作检索标签；不能替代上述行为信息。

小传只收纳原文证据和为制作所需的克制推断，不虚构与当前戏无关的童年、创伤或爱好。目标是让编剧知道这个人想要什么、如何施压或躲避、什么话说得出口，而不是把人物解释完。
