...
给 AI 短剧出**分镜**：把 novel-script 的节拍流切成可以直接下单给视频模型的生成任务单。这是管线里第一个直接面对视频模型的层，前提刻在骨子里：生成与重试有成本，切镜节奏由叙事需求决定；下述 2–5 秒只是短剧默认值。所以结构是三层：
...
- **分镜图是资产合成，不是凭空画** — 出图挂场景/角色/道具设定图当参考图，novel-art 和 novel-characters 的图在这一步真正被消费。经授权且工具可用时提交出图（可选，结果另行验收）
