...
给 AI 短剧写**剧本**：把 cine-outline 的分集梗概落成场次和台词。前提刻在骨子里：**剧本管戏，分镜管拍**——「爽不爽」和「怎么拍」是两种迭代节奏，台词要反复推翻重写，绑上镜头分解每改一句都得重排镜头。所以这层没有镜号、没有首帧提示词，那些是下一层分镜 skill 的活。

但守住一条底线：**台词是结构化数据，不是散文**。

- **节拍流** — 每场戏是动作节拍与台词行的交替序列：动作一拍一件事（叙述体），台词逐句带说话人与语气。台词直接对接 TTS 逐句生成，动作节拍就是画面要发生的事
- **中文口播复查** — 台词独立检查人物目的、身份关系、口头语序和听觉指代；遮住 `delivery` 朗读仍须像人话，同时冻结原句事实和说话功能，不用“更好听”的另一层意思冒充润色
- **人物小传驱动** — 有 `cast.json` 必须消费角色的欲望、代价、关系筹码、受压策略与语言指纹；短场没有角色卡也先写临时场内说话卡，再落对白
- **逐集时长预算** — 台词按语速折算（默认 4.5 字/秒）、动作按节拍估时（默认 2.5 秒/拍），首稿以目标 ±5% 为写作目标，±15% 仅作硬拦截带。**一集三分钟就是三分钟**，写超写欠当场拦下，不流到生成环节才发现
...
| 门 | 规则 |
| --- | --- |
| **保守参数** | `params` 只能保持默认值或收紧，禁止靠加快语速、缩短动作或放宽容差刷绿 |
| **每集时长** | 按自然对白的保守默认值预估，落在 `targetSeconds` ±15% 内 |
...
- **KPI 带**：集数 / 预估总时长 vs 目标 / 台词句数 / 台词占比 / 平均每场——换景次数只是统计不设门，生成与一致性维护仍有成本
...
```bash
node scripts/cine-script.mjs seed outline.json --eps 1-3        # 预填骨架
node scripts/cine-script.mjs validate script.json \
     --outline outline.json --art art.json                       # 校验
node scripts/cine-script.mjs checkup script.json                # 只跑质量门
node scripts/cine-script.mjs render script.json --html \
     --outline outline.json --art art.json \
     --cast cast.json > script-report.html                       # 出报告（--cast 带音色提示词）
node scripts/cine-script.mjs render script.json --html --lang en \
     --outline outline.json --art art.json > script-report.html  # 英文界面报告（默认中文）
node scripts/cine-script.mjs slug "渡口"                         # 安全文件名
```

## 边界

- 不分镜头、无镜号、不写画面生成提示词、不出图——分镜层的活一件不碰
- 时长是**估算不是秒表**；创作模型不能修改参数来消除超时，正文必须在保守默认值下合格
...
scripts/
  cine-script.mjs       seed / validate / checkup / render / slug
  selftest.mjs           158 项以上断言，不调模型
references/
