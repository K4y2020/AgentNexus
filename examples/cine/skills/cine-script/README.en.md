...
- **KPI band**: episodes / estimated total vs target / line count / dialogue ratio / average scene length — scene changes are a statistic, not a gate; generation and consistency work still have costs
...
```
cine-outline    → outline.json (what)
cine-characters → cast.json    (who)
cine-art        → art.json     (where + what's in their hands)
...
```bash
node scripts/cine-script.mjs seed outline.json --eps 1-3
node scripts/cine-script.mjs validate script.json --outline outline.json --art art.json
node scripts/cine-script.mjs checkup script.json
node scripts/cine-script.mjs render script.json --html --outline outline.json --art art.json --cast cast.json > script-report.html
node scripts/cine-script.mjs render script.json --html --lang en --outline outline.json --art art.json > script-report.html
```
