...
```bash
node scripts/cine-outline.mjs render outline.json --html --lang en > outline-report.html
```
...
```bash
node scripts/cine-outline.mjs checkup outline.json
node scripts/cine-outline.mjs render outline.json --html > outline-report.html
```
...
## Relationship to cine-characters

cine-outline owns adaptation structure (cutting, merging, beat placement, episodes); cine-characters owns character design (profiles, image/voice prompts, model sheets).
