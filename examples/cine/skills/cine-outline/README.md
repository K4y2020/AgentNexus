...
```bash
node scripts/cine-outline.mjs checkup outline.json    # 终端 ✓/✗
...
```bash
node scripts/cine-outline.mjs chunk book.txt /tmp/wk           # 按章分卷
node scripts/cine-outline.mjs validate outline.json            # 校验（--stage skeleton|beats|full）
node scripts/cine-outline.mjs checkup outline.json             # 只跑质量门
node scripts/cine-outline.mjs render outline.json --html       # 出报告（界面默认中文）
node scripts/cine-outline.mjs render outline.json --html --lang en > outline-report.html   # 英文界面报告
node scripts/cine-outline.mjs assets outline.json              # 资产清单 JSON
```

## 跟 cine-characters 的关系

分工：**cine-outline 管改编结构**（砍线/合人/排爽点/分集），**cine-characters 管角色设定**（画像/形象提示词/音色/设定图）。
...
scripts/
  cine-outline.mjs      chunk / validate / checkup / render / assets
