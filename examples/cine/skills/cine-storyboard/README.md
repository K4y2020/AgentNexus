...
跨 skill 管线以 [production-handoff.md](../film-analysis/references/production-handoff.md) 为权威，独立使用仍保留。Cine 负责制作：先 `seedance_read_canvas`，再 `seedance_edit_canvas`，更新/删除携带 `expected_revision`；明确授权后才提交 V3 workers。V3 Agent 仅定位 QA，但现有 `seedance_agent_message` 不具代码级只读隔离，因此目前只做本地/只读快照 QA，不通过泛用消息制作或冒充安全 QA。禁止 codex `$imagegen` 及 fallback。实际本地图像与视觉验收必须独立核对，远程 ID、旧文件或路径不算成图，缺失保持缺失；生成可能计费，阶段通过不等于 production PASS。

给 AI 短剧出**分镜**：把 cine-script 的节拍流切成可以直接下单给视频模型的生成任务单。这是管线里第一个直接面对视频模型的层，前提刻在骨子里：生成与重试有成本，切镜节奏由叙事需求决定；下述 2–5 秒只是短剧默认值。所以结构是三层：

```
段（segment）＝ 一次视频生成调用，≤ 15 秒，不跨场次
 ├─ 分镜（cut）× 3–5 ＝ 段内剪切，每切默认 2–5 秒（可配置），各自认领剧本节拍
...
- **分镜图是资产合成，不是凭空画** — 出图挂场景/角色/道具设定图当参考图，cine-art 和 cine-characters 的图在这一步真正被消费。经授权且工具可用时提交出图（可选，结果另行验收）

产出 `storyboard.json` + Markdown + 一个双击就能开的 `storyboard-report.html`：

![storyboard-report.html](assets/report.webp)

H3 导出与提示词校验仅支持 H3。Seedance 等不支持的提供商导出明确 blocked；可交结构化分镜草稿，但不得编造 `h3Prompt` 过门。短剧风格/钩子门不适用于慢节奏电影，原生门不兼容时如实报告。
...
| **分镜时长** | 默认每切 2–5 秒；`params.minCutSeconds` / `params.maxCutSeconds` 可配置，不作为所有电影的节奏规则 |
...
cine-characters → cast.json       （谁：角色设定图）
cine-art        → art.json        （哪里：场景/道具设定图）
cine-script     → script.json     （戏：场次、节拍、台词）
cine-storyboard → storyboard.json （怎么拍：段、分镜、分镜图、H3 提示词）
```
...
- 分镜图通过直接 V3 画布工具提交，引用已验收的场景/角色/道具资产；H3 导出包仅供 MiniMax H3 使用，另需生成授权

## 命令行直接用

```bash
node scripts/cine-storyboard.mjs seed script.json --eps 1     # 切镜底稿
node scripts/cine-storyboard.mjs validate sb.json \
...
node scripts/cine-storyboard.mjs validate sb.json --script script.json \
     --shots /path/to/cards                                              # 可选：开第 17 道配方门
node scripts/cine-storyboard.mjs render sb.json --html \
     --script script.json --outline outline.json --art art.json > storyboard-report.html
node scripts/cine-storyboard.mjs render sb.json --html --lang en \
     --script script.json --outline outline.json --art art.json > storyboard-report.html   # 英文界面报告
node scripts/cine-storyboard.mjs export sb.json --script script.json   # H3 投产包
```
...
- 分镜图获明确生成授权后先出第一段的整套（3–5 张）看效果，确认画风和构图再往后补——一集约 30–40 格，方向错了整批重来
...
scripts/
  cine-storyboard.mjs   seed / validate / checkup / render / export / slug
