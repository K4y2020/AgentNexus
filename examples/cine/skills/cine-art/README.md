...
- 画风预设与 cine-characters **同名对齐**（realistic / ghibli）但内容是环境版——真实感来自用旧的材质，不是皮肤毛孔

## 命令行直接用

```bash
node scripts/cine-art.mjs seed outline.json > art.json      # 从大纲预填场景骨架
node scripts/cine-art.mjs validate art.json --cast cast.json
node scripts/cine-art.mjs checkup art.json                  # 只跑质量门
node scripts/cine-art.mjs render art.json --html            # 出报告（界面默认中文）
node scripts/cine-art.mjs render art.json --html --lang en  # 英文界面报告
node scripts/cine-art.mjs styles                            # 看画风预设
```

## 出图（可选）

通过直接 V3 画布工具授权提交 workers。场景和道具各一张 16:9 设定图，版面都是**主视角大图 + 底部和右侧的 L 形细节边框**：场景细节格放锚点特写，道具细节格放锚点特写 + 其他状态 + 侧面。场景**无人**；道具另加**无手、纯白背景**。变体场景拿母场景成图当参考图。工具不可用或未授权就只交提示词，其余照常。调用契约见 `references/sheet.md`。
