...
DRAFT_ROOT = os.path.expanduser("~/Movies/JianyingPro/User Data/Projects/com.lveditor.draft")
ROOT_META = os.path.join(DRAFT_ROOT, "root_meta_info.json")


def jianying_running() -> bool:
    out = subprocess.run(
        ["pgrep", "-fl", "-i", "JianyingPro"], capture_output=True, text=True
    ).stdout
...
def _load_platform(draft_dir: str) -> dict | None:
    ...
    try:
        ...
        return None
    if isinstance(platform, dict) and platform.get("device_id") and platform.get("os") == "mac":
        return {
            k: platform[k]
            for k in ("os", "app_version", "device_id", "hard_disk_id", "mac_address")
            if k in platform
        }
    return None


def _mac_platform(donor_draft: str | None = None, allow_missing_fingerprint: bool = False) -> dict:
    ...
    if donor_draft:
        ...
        if platform is None:
            raise ValueError(f"指定的 donor 草稿不可用（非明文或缺 device_id）：{donor_draft}")
        return platform
    try:
            ...
            records.append(
                {
                    ...
                    "duration": m["duration"],
                    "extra_info": (
                        m.get("material_name") or m.get("name") or os.path.basename(m["path"])
                    ),
                    "file_Path": m["path"],
                    ...
                    "metetype": metetype or ("photo" if m.get("type") == "photo" else "video"),
...
def macify(
    ...
    if bundled_size is not None:
        ...
        materials_size = sum(
            os.path.getsize(p) for p in {r["file_Path"] for r in records} if os.path.exists(p)
        )
    ...
    if videos:
        ...
        if bundle_media:
            cover_src = os.path.join(draft_dir, "Resources", os.path.basename(cover_src))
        subprocess.run(
            [
                "ffmpeg",
                "-y",
...
def install(draft_dir: str, draft_name: str, info: dict) -> str:
    ...
    if os.path.exists(target):
        old = os.path.join(_trash_dir(), f"{draft_name}.replaced-{time.strftime('%Y%m%d-%H%M%S')}")
...
def uninstall(draft_name: str) -> None:
    ...
    if os.path.exists(target):
        tmp = os.path.join(
            _trash_dir(), f"{draft_name}.uninstall-{time.strftime('%Y%m%d-%H%M%S')}"
        )
    ...
    if tmp:
        try:
            ...
            print(
                f"[warn] 注册表已更新，残留在回收目录（不影响剪映显示），请手动清理：{tmp}（{e}）"
            )
