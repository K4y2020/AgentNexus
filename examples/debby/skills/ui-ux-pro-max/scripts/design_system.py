...
DIAL_TIERS = {
    "variance": [
        ...
        (
            4,
            7,
            {"label": "Balanced / Modern", "style_keywords": ["modern", "structured", "balanced"]},
        ),
        (
            8,
            10,
            {
                ...
                "style_keywords": ["Brutalism", "Bento Grids", "asymmetric", "experimental"],
            },
        ),
    ],
    "motion": [
...
def format_ascii_box(design_system: dict) -> str:
    ...
    if any(dials.get(k) is not None for k in ("variance", "motion", "density")):
        ...
        if dials.get("variance") is not None:
            lines.append(
                f"│  Variance: {dials['variance']}/10 — {dials['variance_label']}".ljust(BOX_WIDTH)
                + "│"
            )
        if dials.get("motion") is not None:
            lines.append(
                f"│  Motion:   {dials['motion']}/10 — {dials['motion_label']}".ljust(BOX_WIDTH)
                + "│"
            )
        if dials.get("density") is not None:
            lines.append(
                f"│  Density:  {dials['density']}/10 — {dials['density_label']}".ljust(BOX_WIDTH)
                + "│"
            )
    ...
    if motion_snippet:
        ...
        if motion_snippet.get("Framework Notes"):
            for line in wrap_text(
                f"Framework: {motion_snippet.get('Framework Notes', '')}", "│     ", BOX_WIDTH
            ):
                lines.append(line.ljust(BOX_WIDTH) + "│")
