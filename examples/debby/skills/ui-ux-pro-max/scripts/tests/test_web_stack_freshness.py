...
class TestWebStackFreshness(unittest.TestCase):
    ...
    def test_explicit_old_major_uses_only_curated_legacy_rows(self):
        ...
        for stack, query in cases.items():
            with self.subTest(stack=stack):
                ...
                self.assertEqual({row["Status"] for row in result["results"]}, {"deprecated"})
    ...
    def test_shadcn_named_base_excludes_incompatible_composition_apis(self):
        ...
        self.assertGreater(result["count"], 0)
        self.assertEqual(result["results"][0]["Guideline"], "Use render for Base UI composition")
        self.assertTrue(all("base=radix" not in row["Applies To"] for row in result["results"]))
