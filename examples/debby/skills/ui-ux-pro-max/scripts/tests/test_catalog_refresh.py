...
class CatalogRefreshTest(unittest.TestCase):
    ...
    def font_args(
        self, directory, api=None, metadata=None, approve=True, existing=None, overrides=None
    ):
        args = [
            FONT_SCRIPT,
            "--api-input",
    ...
    def test_font_refresh_is_deterministic_and_preserves_reviewed_fields(self):
        ...
        self.assertEqual(
            ["Alpha Sans", "Zeta Serif"], [item["name"] for item in licenses["families"]]
        )
    ...
    def test_icon_refresh_rejects_invalid_schema_size_and_curated_import(self):
        with tempfile.TemporaryDirectory() as raw:
            ...
            curated = (
                (FIXTURES / "icons-curated.csv").read_text().replace("{ Acorn }", "{ Horse }")
            )
