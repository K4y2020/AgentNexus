...
def _get_bm25(filepath, search_cols, data, signature=None, cache_variant=""):
    ...
    if signature is None:
        ...
        signature = (
            cached_rows[0] if cached_rows and cached_rows[1] is data else _file_signature(filepath)
        )
...
def _search_csv_detailed(
    ...
    abstain = (
        ...
        or (threshold["min_margin"] > 0 and top_score - runner_up_score < threshold["min_margin"])
    )
...
def _passes_threshold(index, query, threshold):
    ...
    return (
        ...
        and (
            threshold["min_margin"] <= 0 or top_score - runner_up_score >= threshold["min_margin"]
        )
    )
...
def _suggest_terms(bm25, query, limit=6, threshold=None):
    ...
    for term in bm25.vocabulary():
        ...
        if similarity >= 0.72 and (threshold is None or _passes_threshold(bm25, term, threshold)):
...
_DOMAIN_TIEBREAK_RANK = {domain: rank for rank, domain in enumerate(_DOMAIN_TIEBREAK_ORDER)}
...
def _style_identity(rows, query, allow_contained=True):
    ...
    for row in rows:
        ...
        for identity in identities:
            identity_tokens = set(re.findall(r"\w+", _normalize(identity.casefold()), re.UNICODE))
            if (
                identity_tokens
                ...
                candidates.append((len(distinctive), len(identity_tokens), len(identity), row))
    if not candidates:
        return None
...
def _style_search_destination(rows, matched):
    ...
    if domain == "style" and replacement_id:
        replacement = next((row for row in rows if row.get("Style ID") == replacement_id), None)
...
def search(query, domain=None, max_results=MAX_RESULTS, diagnostics=False):
    ...
    if not results:
        if search_domain == "landing":
            out["suggestions"] = _suggest_identities(landing_rows, query, _LANDING_IDENTITY_FIELDS)
        else:
...
def _stack_query_requests_legacy(query, stack):
    ...
    if current_version is not None and stack_name is not None:
        ...
        requested_versions = [
            tuple(int(value) for value in match.groups() if value is not None) for match in matches
        ]
        if stack == "threejs":
        ...
        if requested_versions:
            if migration_intent and any(
                requested >= current_version[: len(requested)] for requested in requested_versions
            ):
                return False
            return all(
                requested < current_version[: len(requested)] for requested in requested_versions
...
def _exact_stack_identifier(rows, query, row_filter):
    ...
    matches = [
        row
        for row in rows
        if row_filter(row) and any(pattern.search(row.get(field, "")) for field in fields)
    ]
    return matches[0] if len(matches) == 1 else None
...
def _legacy_successor_guidance(rows, query, stack, row_filter):
    ...
    matches = [
        ...
        and re.search(
            ...
            " ".join(
                (row.get("Guideline", ""), row.get("Description", ""), row.get("Do", ""))
            ).casefold(),
        )
    ]
    return matches[0] if len(matches) == 1 else None
...
def search_stack(query, stack, max_results=MAX_RESULTS, diagnostics=False):
    ...
    exact = _legacy_successor_guidance(rows, query, stack, row_filter) or _exact_stack_identifier(
        rows, query, row_filter
    )
