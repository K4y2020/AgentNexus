...
| **AgentNexus MCP** | A native harness calls the read-only `sys_session_list` relay tool through its generated `agentnexus` MCP server. | P1 |
...
| AgentNexus MCP | Not applicable | Generated `agentnexus` MCP relay when supported by the vendor | Not applicable |
...
Reusable production helpers live in `agentnexus.config` and the existing runtime
