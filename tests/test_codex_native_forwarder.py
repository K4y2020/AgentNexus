...
module) onto ``_CodexForwarderState.model`` and mirrors it to the AgentNexus server
...
model IS mirrored (so AgentNexus learns the session's model even when unchanged);
...
from agentnexus import codex_native_forwarder as fwd
...
from agentnexus.codex_native_forwarder import _persist_codex_compaction_item
...
async def test_sync_model_change_posts_on_change() -> None:
    ...
    The in-TUI ``/model`` switch (gpt-5.5 → gpt-5.4) must mirror to AgentNexus as
...
async def test_sync_model_change_no_post_when_unchanged() -> None:
    ...
    Prevents the spawn/startup model from being echoed back to AgentNexus as a
...
def test_refresh_prefers_pushed_settings_model_over_stale_config(tmp_path: Path) -> None:
    ...
    # AgentNexus pushes a routed model thread-level; the live notification wins.
...
def test_refresh_launch_race_ends_on_routed_model(tmp_path: Path) -> None:
    ...
    """
    from agentnexus.codex_native_bridge import write_codex_config_model
...
def test_note_resume_response_records_model_without_seeding_baseline() -> None:
    ...
    AgentNexus must learn the session's ACTUAL model — including the spawn default —
...
async def test_sync_after_resume_posts_spawn_model() -> None:
    ...
    spawned on gpt-5.4-mini, the model never "changed", yet AgentNexus must still
...
def test_thread_settings_updated_records_effort_and_collaboration_mode() -> None:
    ...
    no state to mirror, so AgentNexus would keep stale ``reasoning_effort`` and
...
async def test_sync_reasoning_effort_change_posts_and_dedupes() -> None:
    """
    Codex effort changes mirror to AgentNexus exactly once per observed value.
...
async def test_sync_reasoning_effort_change_posts_clear() -> None:
    """
    Codex clearing effort mirrors JSON null to AgentNexus.
    ...
    early on falsey ``None``, AgentNexus would keep a stale explicit effort.
    """
    client = _RecordingClient()
    state = fwd._CodexForwarderState(
        effort=None,
...
async def test_sync_codex_collaboration_mode_change_posts_and_dedupes() -> None:
    """
    Codex collaboration mode changes mirror to AgentNexus labels once.
...
async def test_post_user_message_image_only_posts_empty_content() -> None:
    """
    An image-only ``userMessage`` is posted with EMPTY AgentNexus content.
...
def test_terminal_turn_status_edge_empty_turn_idle_and_warns(
    ...
    with caplog.at_level("WARNING", logger="agentnexus.codex_native_forwarder"):
        edge = fwd._terminal_turn_status_edge(tmp_path, "turn/completed", params)
...
async def test_mcp_startup_event_records_and_posts(tmp_path: Path) -> None:
    ...
    from agentnexus.codex_native_bridge import read_mcp_startup
...
async def test_seed_mcp_startup_round_posts_config_servers(tmp_path: Path) -> None:
    ...
    try:
        assert read_mcp_startup(tmp_path) == {
            ...
            "agentnexus": {"status": "starting", "error": None},
        }
        assert client.posts == [
            (
                "/v1/sessions/conv_x/events",
...
async def test_seed_mcp_startup_round_skips_when_state_exists(tmp_path: Path) -> None:
    ...
    """
    from agentnexus.codex_native_bridge import read_mcp_startup, update_mcp_server_startup
...
def test_read_developer_instructions_collapsed_wrapper_matches_tri_state_value(
    ...
    from agentnexus.codex_native_bridge import read_codex_config_developer_instructions_from_home
...
def test_refresh_developer_instructions_from_config_clears_on_genuine_absence(
    ...
    information — e.g. the user's AgentNexus-appended directive was removed —
