...
import agentnexus.antigravity_native as _mod
import agentnexus.antigravity_native_bridge as bridge_mod
from agentnexus._wrapper_labels import ANTIGRAVITY_NATIVE_WRAPPER_VALUE, WRAPPER_LABEL_KEY
from agentnexus.antigravity_native import antigravity_terminal_resource_id
...
def _stub_agy_binary(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    """
    monkeypatch.setattr("agentnexus.antigravity_native_launch.agy_binary_path", lambda: "agy")
...
async def test_launch_and_record_isolates_gemini_dir_and_wires_relay(
    ...
    saw no AgentNexus relay (so no ``sys_*`` tools, #1194) and the survey/trust
    ...
    assert "sys_session_create" in payload["mcpServers"]["agentnexus"]["enabledTools"]
