...
from agentnexus.spec.types import AgentSpec, ExecutorSpec, RetryPolicy
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (ExecutorSpec(type="agentnexus", config={"harness": "codex"}), "codex"),
        (
            ExecutorSpec(type="agentnexus", config={"harness": "claude-native"}),
            "claude-native",
        ),
...
def test_retry_policy_from_json_drops_unknown_keys_for_forwards_compat() -> None:
    ...
    AgentNexus serializes it, an older harness wrap (still on the
...
def test_retry_policy_from_json_uses_defaults_for_missing_fields() -> None:
    ...
    Pin: backwards compat — an older AgentNexus serializing a partial
...
def test_retry_policy_from_json_rejects_non_dict_payload() -> None:
    ...
    Pin: defensive against AgentNexus regressions that might
...
def test_retry_policy_from_json_rejects_non_list_status_codes() -> None:
    ...
    Defensive: if AgentNexus serialized it as a string or dict
