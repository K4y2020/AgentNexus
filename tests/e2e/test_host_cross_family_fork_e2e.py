...
Claude transcript from the fork's copied AgentNexus items
...
SDK→native test needs only the Claude side. Set the matching gates::
    ...
    AGENTNEXUS_E2E_CLAUDE_NATIVE=1 AGENTNEXUS_E2E_CODEX_NATIVE=1 \\
...
_BOTH_NATIVE_GATE = pytest.mark.skipif(
    os.environ.get("AGENTNEXUS_E2E_CLAUDE_NATIVE") != "1"
    or os.environ.get("AGENTNEXUS_E2E_CODEX_NATIVE") != "1"
    ...
    reason=(
        ...
        "set AGENTNEXUS_E2E_CLAUDE_NATIVE=1 and AGENTNEXUS_E2E_CODEX_NATIVE=1 to run"
    ),
)
_CLAUDE_NATIVE_GATE = pytest.mark.skipif(
    ...
    reason=(
        ...
        "set AGENTNEXUS_E2E_CLAUDE_NATIVE=1 (and have `claude` installed + logged in) to run"
    ),
)
...
def _recall_marker_in_clone(
    ...
    *,
    ...
    The recall only succeeds if the source's AgentNexus items were rebuilt
    ...
    assert marker in text, (
        ...
        f"{source_harness} source's AgentNexus items were not rebuilt into the "
...
def test_fork_codex_native_into_claude_native_rebuilds_history(
    ...
    Claude transcript from the copied AgentNexus items, then ``--resume`` it.
...
def test_fork_openai_sdk_source_into_claude_native_rebuilds_history(
    ...
    clone's Claude transcript from the copied AgentNexus items and resume it.
