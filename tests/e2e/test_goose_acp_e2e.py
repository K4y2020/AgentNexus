...
``session/request_permission`` through AgentNexus's TOOL_CALL policy + human-consent
...
----------------------------------------------------------
* **Opt-in only**: set ``AGENTNEXUS_E2E_GOOSE=1`` to run. Needs a configured Goose
...
from agentnexus.inner.executor import ExecutorError, TextChunk, TurnComplete
from agentnexus.inner.goose_executor import GooseExecutor

pytestmark = pytest.mark.skipif(
    os.environ.get("AGENTNEXUS_E2E_GOOSE") != "1"
    or shutil.which("goose") is None
    or not os.environ.get("GOOSE_PROVIDER")
    or not os.environ.get("GOOSE_MODEL"),
    reason=(
        "headless goose ACP e2e is opt-in: set AGENTNEXUS_E2E_GOOSE=1 with a "
