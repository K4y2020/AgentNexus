...
----------------------------------------------------------
* **Opt-in only**: set ``AGENTNEXUS_E2E_OPENCODE_NATIVE=1`` and have a pinned
...
* Run it with::
    ...
    AGENTNEXUS_E2E_OPENCODE_NATIVE=1 \
...
pytestmark = pytest.mark.skipif(
    os.environ.get("AGENTNEXUS_E2E_OPENCODE_NATIVE") != "1" or shutil.which("opencode") is None,
    reason=(
        "opencode-native wire-contract e2e needs a pinned `opencode` binary on PATH; "
        "set AGENTNEXUS_E2E_OPENCODE_NATIVE=1 (and `npm i -g opencode-ai@1.17.7`) to run"
    ),
)
...
async def test_opencode_native_wire_contract_against_real_server() -> None:
    ...
    try:
        ...
        try:
            ...
            session = await client.create_session({"title": "agentnexus-e2e"})
