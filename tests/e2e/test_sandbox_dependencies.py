...
def test_pip_install_and_use_package(
    ...
    configure_mock_llm(
        ...
        [
                        ...
                        "arguments": json.dumps(
                            {
                                "command": (
                                    ...
                                    "\"import cowsay; cowsay.cow('hello from agentnexus')\""
                                ),
                            }
                        ),
                    }
            ...
            {"text": "Here is the cowsay output with hello from agentnexus."},
        ],
        key=model,
    )
    ...
    assert "hello from agentnexus" in combined, (
        f"Expected cowsay ASCII art with 'hello from agentnexus' "
...
def test_uv_pip_install_and_use_package(
    ...
    configure_mock_llm(
        ...
        [
                        ...
                        "arguments": json.dumps(
                            {
                                "command": (
                                    ...
                                    "\"import cowsay; cowsay.cow('hello from agentnexus via uv')\""
                                ),
                            }
                        ),
                    }
            ...
            {"text": "Here is the cowsay output with hello from agentnexus via uv."},
        ],
        key=model,
    )
    ...
    all_output = _tool_call_output_text(body)
    combined = (text + " " + all_output).lower()
    assert "hello from agentnexus via uv" in combined, (
        f"Expected cowsay output with 'hello from agentnexus via uv' -- proves "
