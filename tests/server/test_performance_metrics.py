...
from agentnexus.process_logging import DEFAULT_LOG_DATEFMT, DEFAULT_LOG_PREFIX_FORMAT
...
def test_otel_publisher_records_request_duration_histogram() -> None:
    ...
    records = meter.histograms["agentnexus.server.http.request.duration"].records
    assert records == [
        _MetricRecord(
            amount=0.125,
            attributes={
...
async def test_publish_server_metrics_periodically_exports_until_cancelled() -> None:
    ...
    async def wait_for_started_counter() -> None:
        ...
        """
        while not meter.counters["agentnexus.server.http.requests.started"].records:
...
def test_request_route_template_for_metrics_uses_sentinel_for_unmatched_routes() -> None:
    ...
    """
    from agentnexus.server.app import request_route_template_for_metrics
