...
from agentnexus.server.routes._codex_elicitation import (
...
class TestParseCodexElicitationRequest:
    ...
    def test_missing_method_raises(self) -> None:
        with pytest.raises(AgentNexusError, match="non-empty method"):
    ...
    def test_non_dict_params_raises(self) -> None:
        with pytest.raises(AgentNexusError, match="params must be an object"):
            parse_codex_elicitation_request(
                {"id": 1, "method": "mcpServer/elicitation/request", "params": "bad"}
            )

    def test_missing_id_raises(self) -> None:
        with pytest.raises(AgentNexusError, match="string or integer id"):
            parse_codex_elicitation_request(
                {"method": "mcpServer/elicitation/request", "params": {}}
            )

    def test_unsupported_method_raises(self) -> None:
        with pytest.raises(AgentNexusError, match="Unsupported"):
...
class TestExecpolicyAmendment:
    ...
    def test_empty_list_raises(self) -> None:
        with pytest.raises(AgentNexusError, match="non-empty list"):
            _execpolicy_amendment([])
