...
async def test_create_failure_never_removes_existing_worktree(
    ...
    for a *pre-existing* worktree without AgentNexus creating one. The
    create-rollback (``git worktree remove --force`` + ``git branch -D``)
    is gated on AgentNexus having created a worktree, NOT on ``git_branch``
...
async def test_create_failure_rolls_back_omnigent_created_worktree(
    ...
    """A create_conversation failure DOES clean up an AgentNexus-made worktree.

    The counterpart to the data-loss guard: when AgentNexus creates the
    ...
    rollback guard (gated on AgentNexus having created a worktree) still
    ...
    # AgentNexus created the worktree, so the rollback force-removed it: one
