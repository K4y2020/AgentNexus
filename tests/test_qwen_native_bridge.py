...
def bridge_dir(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> Path:
    ...
    """
    root = tmp_path / "agentnexus-test" / "qwen-native"
...
def test_write_mcp_config_writes_into_bridge_dir_not_workspace(bridge_dir: Path) -> None:
    ...
    server = data["mcpServers"]["agentnexus"]
    ...
    # trust:true auto-approves qwen's own MCP gate (AgentNexus gates separately).
...
def test_write_mcp_config_is_valid_for_qwen_mcp_config_flag(bridge_dir: Path) -> None:
    ...
    assert set(data["mcpServers"]) == {"agentnexus"}
...
def test_write_mcp_config_path_is_per_session(
    ...
    args_a = json.loads(path_a.read_text())["mcpServers"]["agentnexus"]["args"]
    args_b = json.loads(path_b.read_text())["mcpServers"]["agentnexus"]["args"]
...
def test_write_mcp_bridge_config_rejects_symlinked_ancestor(
    ...
    """
    real_root = tmp_path / "agentnexus-test"
