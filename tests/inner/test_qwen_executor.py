...
async def test_run_turn_emits_usage_on_turn_complete(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    notified: list[dict] = []
    monkeypatch.setattr(
        "agentnexus.inner.qwen_executor._notify_usage_from_dict",
...
async def test_run_turn_approval_does_not_count_against_idle_timeout(
    ...
    """
    monkeypatch.setattr("agentnexus.inner.qwen_executor._PROMPT_TIMEOUT_SECONDS", 0.05)
...
def test_qwen_in_harness_allowlist() -> None:
    """'qwen' must be in AGENTNEXUS_HARNESSES."""
    from agentnexus.spec._omnigent_compat import AGENTNEXUS_HARNESSES

    assert "qwen" in AGENTNEXUS_HARNESSES
...
def test_qwen_code_in_harness_aliases() -> None:
    """'qwen-code' must be in AGENTNEXUS_HARNESS_ALIASES."""
    from agentnexus.spec._omnigent_compat import AGENTNEXUS_HARNESS_ALIASES

    assert "qwen-code" in AGENTNEXUS_HARNESS_ALIASES
...
def test_qwen_harness_creates_fastapi_app() -> None:
    ...
    from agentnexus.inner.qwen_harness import create_app
...
def test_qwen_harness_module_importable() -> None:
    ...
    from agentnexus.inner import qwen_harness
...
def test_parse_image_data_uri_edge_cases() -> None:
    ...
    from agentnexus.inner.qwen_executor import _parse_image_data_uri
