...
from agentnexus.inner import claude_native_executor
from agentnexus.inner.claude_native_executor import ClaudeNativeExecutor
...
async def test_run_turn_injects_user_message_without_streaming_transcript(
    ...
    produce visible AgentNexus chat items. This fails if the executor
...
async def test_run_turn_does_not_advertise_active_omnigent_tools(
    ...
    from Claude's transcript rather than from a transient AgentNexus turn.
    """
    bridge_dir = tmp_path / "bridge"
    sent_messages: list[dict[str, Any]] = []
