...
- how to build an :class:`OSEnvSandboxSpec` for that backend with the
  ...
  ``import agentnexus.*`` from a tempdir cwd
...
from agentnexus.inner.datamodel import CredentialProxySpec, OSEnvSandboxSpec
...
def _repo_root_for_pythonpath() -> str:
    ...
    in these tests, so they can't ``import agentnexus`` unless the
...
def active_sandbox_spec_factory(
    ...
    helper subprocess can ``import agentnexus.*`` from a tempdir
...
def sandbox_pythonpath_env(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    bootstrap runs ``import agentnexus.inner.os_env`` in the helper.
