...
from agentnexus.codex_native_app_server import CodexAppServerResponseError
...
from agentnexus.inner.codex_native_executor import CodexNativeExecutor
...
def test_next_web_message_starts_new_codex_turn_after_forwarder_marks_idle(
    ...
    ``turn/completed``. Once that happens, the next AgentNexus dispatch must
    ...
    class _RacingSteerClient(_FakeCodexNativeClient):
        ...
        async def request(self, method: str, params: dict[str, Any]) -> dict[str, Any]:
            ...
            if method == "turn/steer" and params["expectedTurnId"] == "turn_a":
                from agentnexus.codex_native_bridge import update_active_turn_id
...
def test_web_model_pick_applied_via_thread_settings_update(
    ...
    A model/effort change made in the AgentNexus web UI reaches the runner
...
def test_model_settings_update_mirrors_model_into_config_toml(
    ...
    ``external_model_change`` back to AgentNexus, silently reverting a routed
...
def test_turn_start_is_not_gated_on_pending_mcp_startup(
    ...
    """
    from agentnexus.codex_native_bridge import update_mcp_server_startup
