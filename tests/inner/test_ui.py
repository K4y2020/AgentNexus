...
def test_accent_is_brand_magenta() -> None:
    """The shared accent is the AgentNexus brand magenta."""
...
def test_show_banner_respects_no_banner_env(value: str) -> None:
    """``AGENTNEXUS_NO_BANNER`` force-disables the banner even on a TTY."""
...
def test_status_lines_go_to_stdout(capsys: pytest.CaptureFixture[str]) -> None:
    ...
    ui.step("Installing AgentNexus")
    ui.success("Verified omnigent")
    ui.info("Using ~/.agentnexus")
    captured = capsys.readouterr()
    assert "Installing AgentNexus" in captured.out
