...
def test_spinner_disabled_by_env_var(value: str) -> None:
    """Any truthy ``AGENTNEXUS_NO_SPINNER`` value disables the spinner.
...
def test_spinner_not_disabled_by_falsy_env_var() -> None:
    """``AGENTNEXUS_NO_SPINNER=0`` (or empty) leaves the spinner on.
...
def test_runner_startup_progress_plain_mode_prints_to_stderr(
    ...
    assert "agentnexus: Starting local runner" in captured.err
    assert "agentnexus: Waiting for runner to register" in captured.err
...
def test_runner_startup_progress_plain_mode_does_not_swallow_exceptions(
    ...
    # suppress it.
    assert "agentnexus: Starting" in capsys.readouterr().err
