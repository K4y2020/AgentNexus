...
from agentnexus.cli import _strip_one_shot_flags, _strip_resume_flags
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (
            ["agentnexus", "run", "a.yaml", "--profile", "prf", "--resume"],
            ["agentnexus", "run", "a.yaml", "--profile", "prf"],
        ),
        # ``--resume`` with a conversation id: drop both tokens.
        (
            ["agentnexus", "run", "a.yaml", "--resume", "conv_abc"],
            ["agentnexus", "run", "a.yaml"],
        ),
        ...
        (
            ["agentnexus", "run", "a.yaml", "-r"],
            ["agentnexus", "run", "a.yaml"],
        ),
        # ``-r conv_id`` short form with value: drop both tokens.
        (
            ["agentnexus", "run", "a.yaml", "-r", "conv_abc"],
            ["agentnexus", "run", "a.yaml"],
        ),
        # Continue forms (always boolean).
        (
            ["agentnexus", "run", "a.yaml", "-c"],
            ["agentnexus", "run", "a.yaml"],
        ),
        (
            ["agentnexus", "run", "a.yaml", "--continue"],
            ["agentnexus", "run", "a.yaml"],
        ),
        ...
        (
            ["agentnexus", "run", "a.yaml", "--session", "conv_abc"],
            ["agentnexus", "run", "a.yaml"],
        ),
        (
            ["agentnexus", "run", "a.yaml", "-s", "conv_abc"],
            ["agentnexus", "run", "a.yaml"],
        ),
        (
            ["agentnexus", "run", "a.yaml", "--session=conv_abc"],
        ...
        (
            ...
            ["agentnexus", "run", "a.yaml", "--profile", "prf", "--model", "m"],
        ),
        # Empty argv → empty.
        ([], []),
        # Non-resume argv: identity.
        (
            ["agentnexus", "run", "a.yaml", "--model", "m", "--profile", "prf"],
            ["agentnexus", "run", "a.yaml", "--model", "m", "--profile", "prf"],
        ),
    ],
)
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (
            ["agentnexus", "run", "a.yaml", "-p", "hello there"],
            ["agentnexus", "run", "a.yaml"],
        ),
        # ``--prompt`` long form.
        (
            ["agentnexus", "run", "a.yaml", "--prompt", "hello"],
            ["agentnexus", "run", "a.yaml"],
        ),
        # ``--prompt=value``.
        (
        ...
        (
            ["agentnexus", "run", "a.yaml", "--system-prompt", "be terse"],
            ["agentnexus", "run", "a.yaml"],
        ),
    ],
)
