...
import pytest
from agentnexus_client import child_summary_busy
from agentnexus_ui_sdk.terminal._formatter import StreamingText
...
async def test_aenter_logs_stderr_redirect_failures(
    ...
    monkeypatch.setattr(
        "agentnexus.cli_diagnostics.redirect_stderr_to_log",
        _raise_redirect_failure,
    )
    host = TerminalHost(model_name="test")

    with caplog.at_level(logging.ERROR, logger="agentnexus_ui_sdk.terminal._host"):
        async with host:
            pass
...
async def test_aexit_logs_stderr_restore_failures(
    ...
    monkeypatch.setattr(
        "agentnexus.cli_diagnostics.restore_stderr",
...
def _patch_create_output(monkeypatch: pytest.MonkeyPatch, output: object) -> None:
    ...
    """
    monkeypatch.setattr(
        "agentnexus_ui_sdk.terminal._host.create_output",
        lambda: output,
    )
...
async def test_window_title_set_on_aenter_and_cleared_on_aexit(
    ...
    What this proves: a user running ``agentnexus run agent.yaml``
...
def test_default_history_file_matches_legacy_omnigent_path() -> None:
    ...
    persists input history to ``~/.agentnexus_history`` — the
    same path the legacy ``agentnexus run`` CLI uses
    (``omnigent/inner/cli.py:_cli_history_file_path``).

    What this proves: a user who flips between
    ``agentnexus run agent.yaml`` (legacy) and
    ``agentnexus run agent.yaml`` sees the same ↑ / Ctrl+R
    recall in both, instead of two divergent histories. If the
    default drifts (e.g. back to ``~/.agentnexus-history``,
    or to a fresh location), this test fails loud at the SDK
    boundary so the divergence doesn't ship silently. The
    history file's location is part of the AgentNexus mode-vs-legacy
    ...
    expected = os.path.expanduser("~/.agentnexus_history")
    actual = host._prompt.history.filename
    assert actual == expected, (
        f"Default history_file resolved to {actual!r}; expected "
        f"{expected!r}. If the path is ``~/.agentnexus-history`` "
...
async def test_window_title_swallows_set_title_failure(
    ...
    won't break ``agentnexus run`` — the REPL still boots,
...
def test_output_dispatches_stream_replace_to_replace_live_region(
    ...
    """
    from agentnexus_ui_sdk.terminal._formatter import StreamReplace
...
def test_compute_sidebar_scroll_offset_no_change_when_selection_in_view() -> None:
    ...
    """
    from agentnexus_ui_sdk.terminal._host import _compute_sidebar_scroll_offset
...
def test_output_stream_live_replaces_live_region(
    ...
    """
    from agentnexus_ui_sdk.terminal._formatter import StreamLive
...
def test_stream_live_then_replace_commits(
    ...
    """
    from agentnexus_ui_sdk.terminal._formatter import StreamLive, StreamReplace
...
def test_live_region_capped_to_viewport_ceiling(
    ...
    monkeypatch.setattr(
        "agentnexus_ui_sdk.terminal._host._term_height",
        lambda: 10,
    )
...
def test_subagent_badge_spinner_animates() -> None:
    ...
    from agentnexus_ui_sdk.terminal._host import _SPINNER_FRAMES
...
def test_subagent_menu_caps_visible_rows_at_five() -> None:
    ...
    from agentnexus_ui_sdk.terminal._host import _SUBAGENT_MENU_MAX_ROWS
