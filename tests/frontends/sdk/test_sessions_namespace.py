...
* ``test_*_404``: that the namespace propagates :class:`AgentNexusError`
...
from agentnexus_client._sessions import (
    Session,
    SessionsNamespace,
)
...
async def test_fork_404_raises() -> None:
    """``fork()`` raises ``AgentNexusError`` when the source session is missing.
