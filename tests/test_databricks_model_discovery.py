...
def test_truncated_pagination_warns(caplog: pytest.LogCaptureFixture) -> None:
    ...
    with caplog.at_level("WARNING", logger="agentnexus.databricks_model_discovery"):
        families = discover_databricks_claude_catalog(
            "https://workspace.example.com",
            "token",
            transport=httpx.MockTransport(_handler),
...
def test_family_shim_warns_and_delegates_to_the_catalog() -> None:
    from agentnexus.databricks_model_discovery import discover_databricks_claude_models
...
def _discover_codex(model_services: list[dict[str, str]]) -> tuple[str, ...]:
    ...
    from agentnexus.databricks_model_discovery import discover_databricks_codex_models
...
def test_select_servable_model_matches_legacy_spelling() -> None:
    ...
    from agentnexus.databricks_model_discovery import select_servable_model
