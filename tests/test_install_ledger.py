...
def test_write_install_ledger_from_env_records_profile_and_dep_provenance(
    ...
    monkeypatch.setenv("AGENTNEXUS_DATA_DIR", str(state))
    monkeypatch.setenv("AGENTNEXUS_LEDGER_PROFILE", str(profile))
    monkeypatch.setenv("AGENTNEXUS_LEDGER_DEP_UV", "agentnexus")
    ...
    assert ledger.entries.deps["uv"].installed_by == "agentnexus"
...
def test_installer_ledger_supersedes_backfill_and_preserves_dep_ownership(
    ...
    assert rewritten.entries.deps["uv"].installed_by == "agentnexus"
...
def test_install_ledger_merges_existing_and_observed_external_configs(
    ...
    old_config = install_ledger.ExternalConfigEntry(
        ...
        marker="mcpServers.agentnexus",
        format="json",
        allowlist=["mcpServers.agentnexus"],
    )
    ...
    observed_config.write_text('{"mcpServers": {"agentnexus": {"command": "python"}}}\n')
...
def test_deep_backfill_observes_external_config_and_launch_agents(
    ...
    cursor_config.write_text('{"mcpServers": {"agentnexus": {"command": "python"}}}\n')
    launch_agent = launch_dir / "ai.agentnexus.local.plist"
    ...
    assert ledger.entries.injected_external_config[0].marker == "mcpServers.agentnexus"
...
def test_record_and_remove_launch_agent_preserves_other_entries(
    ...
    current = install_ledger.LaunchAgentEntry(
        kind="launchd",
        path=str(home / "Library/LaunchAgents/ai.agentnexus.host.plist"),
        label="ai.agentnexus.host",
        confidence="certain",
    )
    ...
    install_ledger.remove_launch_agent(kind="launchd", label="ai.agentnexus.host")
