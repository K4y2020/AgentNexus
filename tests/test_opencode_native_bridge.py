...
from agentnexus import opencode_native_bridge as bridge
...
def test_write_opencode_policy_plugin(bridge_dir: Path) -> None:
    ...
    assert path.name == "agentnexus-policy.js"
    ...
    assert "AGENTNEXUS_POLICY_URL" in src and "AGENTNEXUS_SESSION_ID" in src
    assert "/policies/evaluate" in src
    # A function export so opencode's Object.values(mod) loader picks it up.
    assert "export const AgentNexusPolicyPlugin" in src
...
def test_policy_plugin_merges_routing_headers(bridge_dir: Path) -> None:
    """The generated policy plugin merges AGENTNEXUS_POLICY_HEADERS into its
    ...
    assert "AGENTNEXUS_POLICY_HEADERS" in src
    ...
    assert "AGENTNEXUS_POLICY_AUTH" not in src
