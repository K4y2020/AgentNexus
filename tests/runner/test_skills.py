...
Skills are resolved on the runner, not the AgentNexus server, because the
...
from agentnexus.runner.app import ResolvedSpec
...
class _ServerClient:
    """
    Fake AgentNexus server client whose session snapshot carries an agent_id and
...
async def test_resolve_session_skill_unknown_returns_404_with_available(tmp_path: Path) -> None:
    ...
    sorted list of available skill names (the error the AgentNexus server
...
async def test_session_skills_cache_ttl_expiry_rediscovers(
    ...
    (home / ".claude" / "skills").mkdir(parents=True)
    monkeypatch.setattr("pathlib.Path.home", lambda: home)
    monkeypatch.setattr("agentnexus.runner.app._SESSION_SKILLS_CACHE_TTL_SECONDS", 0.0)
