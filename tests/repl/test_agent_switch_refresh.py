...
import pytest
from agentnexus_client._sessions import Session as SessionSnapshot
from agentnexus_ui_sdk import RichBlockFormatter
from rich.text import Text

from agentnexus.repl._repl import _refresh_session_metadata, _SessionsChatReplAdapter
...
class _SnapshotClient:
    """
    AgentNexus client stub exposing only the ``sessions.get`` surface.
...
class _RaisingClient:
    """AgentNexus client stub whose ``sessions.get`` always fails."""
...
def _make_adapter(client: object, agent_name: str = "nessie") -> _SessionsChatReplAdapter:
    ...
    :param client: AgentNexus client (stub) handed to the adapter.
