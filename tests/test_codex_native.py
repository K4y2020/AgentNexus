...
from agentnexus import codex_native, codex_native_app_server, codex_native_forwarder
...
def test_codex_auth_unavailable_reason_provider_override_available(
    ...
    auth_path = tmp_path / "codex-home" / "auth.json"  # never created
    launch = codex_native_app_server.NativeCodexLaunch(
        config_overrides=['model_provider="agentnexus_databricks"'], model=None, profile=None
    )
    _point_codex_auth_check_at(monkeypatch, auth_path, binary_present=True, launch=launch)
...
def test_codex_auth_unavailable_reason_local_auth_failure_needs_auth(
    ...
    monkeypatch.setattr(
        "agentnexus.onboarding.codex_auth_readiness.codex_config_effective_auth",
        _boom,
    )
...
def test_preload_codex_thread_for_resume_resumes_and_closes(
    ...
    monkeypatch.setattr(
        "agentnexus.codex_native_app_server.CodexAppServerClient",
        fake_client_factory,
    )
...
def _expected_delta_data(
    ...
    *,
    ...
    """
    Build the AgentNexus event data expected for one Codex native text delta.
...
def _expected_status_data(status: str, turn_id: str) -> dict[str, Any]:
    """
    Build the AgentNexus event data expected for one Codex native status edge.

    :param status: AgentNexus session status, e.g. ``"running"``.
    :param turn_id: Codex turn id, e.g. ``"turn_123"``.
...
def _usage_coalescer(
    ...
    :param session_id: AgentNexus session id, e.g. ``"conv_123"``.
...
def test_materialized_codex_agent_spec_loads_as_valid_omnigent_yaml(
    ...
    """
    The generated wrapper spec passes AgentNexus YAML validation.
...
def test_codex_app_server_client_responds_to_server_requests(
    ...
    app-server to the AgentNexus client. After AP/web resolves the
...
def test_wait_for_thread_started_fails_when_stream_ends() -> None:
    ...
    of leaving the AgentNexus session without a bridge state.
    """
...
def test_supervise_forwarder_resumes_when_it_opens_client(
    ...
    monkeypatch.setattr(
        "agentnexus.codex_native_app_server.CodexAppServerClient", fake_client_factory
    )
...
def test_subscribe_until_ready_replays_completed_turn_status(
    ...
    turn state instead of leaving the AgentNexus session running forever.
    ...
    def handler(request: httpx.Request) -> httpx.Response:
        """
        Capture AgentNexus event posts from the forwarder.
...
def test_forwarder_ignores_thread_started_for_current_codex_thread(tmp_path: Path) -> None:
    """
    A duplicate ``thread/started`` notification does not rotate AgentNexus sessions.
...
def test_forwarder_rotates_session_on_new_codex_thread_and_posts_to_new_session(
    ...
    """
    Native Codex thread switches create a replacement AgentNexus session.
    ...
    def handler(request: httpx.Request) -> httpx.Response:
        """
        Serve AgentNexus calls made during Codex session rotation.

        :param request: HTTP request from the forwarder.
        :returns: Fake AgentNexus response.
        """
            ...
            return httpx.Response(
                ...
                json={
                    ...
                    "labels": {
                        ...
                        "agentnexus.codex_native.bridge_id": "bridge_shared",
                    },
                },
            )
...
def test_forwarder_rotation_failure_preserves_old_target(
    ...
    If AgentNexus rejects replacement-session creation, the forwarder logs the
    event-handler failure and continues. The old target must remain
    intact; closing its coalescer before the AgentNexus move succeeds would
    ...
    class _FakeCoalescer:
        ...
        :param session_id: AgentNexus session id represented by this fake,
            e.g. ``"conv_old"``.
        """
    ...
    async def fail_create_thread_replacement_session(**_kwargs: object) -> str:
        """
        Simulate AgentNexus rejecting the replacement-session operation.

        :returns: Never returns successfully.
        :raises RuntimeError: Always raised to model AgentNexus failure.
        """
...
def test_forwarder_posts_active_codex_agent_message_delta(tmp_path: Path) -> None:
    """
    Codex assistant deltas are forwarded as transient AgentNexus text deltas.
    ...
    item, so this asserts on the exact AgentNexus event envelope.
    """
    write_bridge_state(
        tmp_path,
        CodexNativeBridgeState(
...
def test_forwarder_persists_interrupted_codex_partial_agent_message(tmp_path: Path) -> None:
    ...
    Without this fallback, AgentNexus Web shows the streamed text live but loses it
...
def test_forwarder_posts_active_codex_plan_delta(tmp_path: Path) -> None:
    """
    Codex plan deltas are forwarded as transient AgentNexus text deltas.

    Plan mode uses ``item/plan/delta`` while rendering the visible
    plan. If this branch is missing, AgentNexus web stays blank even though the
...
def test_forwarder_coalesces_codex_agent_message_deltas(tmp_path: Path) -> None:
    """
    Native Codex streaming does not post one AgentNexus event per tiny delta.

    Breaking the coalescer would recreate the slow-drain failure where
    Codex finishes locally while the AgentNexus SSE stream is still serialized
...
def test_forwarder_posts_codex_usage_live_per_frame(
    ...
    def handler(request: httpx.Request) -> httpx.Response:
        """
        Capture AgentNexus event posts from both coalescers.
...
def test_output_text_delta_coalescer_auto_flushes(
    ...
    AgentNexus post directly instead of calling ``flush()``, so removing any
    ...
    :param expected_delta: Coalesced AgentNexus delta payload.
    :returns: None.
    """
    posted: list[dict[str, Any]] = []
    ...
    async def run() -> None:
        """
        Append deltas and wait for the automatic AgentNexus post.
        ...
        def handler(request: httpx.Request) -> httpx.Response:
            """
            Capture AgentNexus event posts from the coalescer.
...
def test_supervise_forwarder_continues_after_event_handler_failure(
    ...
    async def fake_handle_event(
        ...
        *,
        ...
        :param _client: AgentNexus HTTP client.
        :param session_id: AgentNexus session id, e.g. ``"conv_123"``.
...
def test_forwarder_sends_codex_mcp_elicitation_response_to_app_server(
    ...
    """
    Codex MCP elicitation requests are forwarded to AgentNexus and the AgentNexus hook
    ...
    def handler(request: httpx.Request) -> httpx.Response:
        """
        Capture the AgentNexus hook request and return an accepted MCP result.

        :param request: HTTP request sent by the forwarder.
        :returns: AgentNexus hook response.
        """
...
def test_forwarder_keeps_streaming_when_native_tui_answers_codex_elicitation(
    ...
    """
    Native TUI approval must not park the AgentNexus web mirror.

    The AgentNexus hook remains pending when a separate native Codex client
    answers the prompt first. Codex app-server emits
    ``serverRequest/resolved`` with the original request id; the
    forwarder must mirror that exact resolution to AgentNexus and still mirror
    ...
    async def handler(request: httpx.Request) -> httpx.Response:
        """
        Hold the hook open and capture subsequent AgentNexus event posts.

        :param request: HTTP request sent by the forwarder.
        :returns: AgentNexus event response for non-hook posts.
        """
...
def test_forwarder_sends_codex_request_user_input_response_to_app_server(
    ...
    """
    Codex requestUserInput frames use the same AgentNexus hook path and relay
    ...
    def handler(request: httpx.Request) -> httpx.Response:
        """
        Return a requestUserInput result from the AgentNexus hook.
...
def test_forwarder_flushes_plan_text_before_codex_request_user_input(
    ...
    """
    Buffered plan deltas reach AgentNexus before the final plan prompt.
    ...
    def handler(request: httpx.Request) -> httpx.Response:
        """
        Capture AgentNexus posts in arrival order.

        :param request: HTTP request sent by the forwarder.
        :returns: AgentNexus response appropriate to the endpoint.
        """
...
def test_forwarder_synthesizes_plan_implementation_prompt_after_completed_plan_turn(
    ...
    """
    Completed Plan-mode turns surface the final implementation prompt in AgentNexus Web.
    ...
    def handler(request: httpx.Request) -> httpx.Response:
        """
        Capture AgentNexus posts and decline the synthesized prompt.
...
def test_forwarder_starts_default_turn_from_plan_implementation_prompt(
    ...
    the answer back into Codex app-server actions, AgentNexus Web would look
    ...
    def handler(request: httpx.Request) -> httpx.Response:
        ...
        :param request: HTTP request sent by the forwarder.
        :returns: AgentNexus hook or event response.
        """
        if request.url.path.endswith("/events"):
            return httpx.Response(202, json={"queued": False})
        return httpx.Response(
...
def test_forwarder_starts_fresh_thread_from_clear_context_plan_prompt(
    ...
    This mirrors the terminal TUI action closely enough for AgentNexus Web:
...
def test_forwarder_sends_codex_command_approval_response_to_app_server(
    ...
    """
    Codex command-approval request frames use the AgentNexus hook path and
    ...
    def handler(request: httpx.Request) -> httpx.Response:
        """
        Return a command approval result from the AgentNexus hook.
...
def test_forwarder_sends_codex_permissions_response_to_app_server(
    ...
    """
    Codex permission-profile request frames are relayed through AgentNexus and
    ...
    def handler(request: httpx.Request) -> httpx.Response:
        """
        Return a permissions approval result from the AgentNexus hook.
...
def test_forwarder_leaves_codex_elicitation_pending_on_empty_hook_body(
    ...
    """
    Empty AgentNexus hook responses represent timeout/disconnect fallback, not
    ...
    def handler(_request: httpx.Request) -> httpx.Response:
        """
        Return the AgentNexus hook's fail-ask shape.
...
def test_forwarder_posts_codex_user_and_agent_messages(tmp_path: Path) -> None:
    ...
    external conversation items for the AgentNexus session stream.
    """
    posted: list[dict[str, Any]] = []
...
def test_forwarder_recovers_missed_user_message_before_assistant(tmp_path: Path) -> None:
    ...
    BEFORE the reply so AgentNexus assigns it the earlier position.
    """
    posted: list[dict[str, Any]] = []
...
def test_forwarder_posts_completed_codex_plan_item() -> None:
    """
    Completed Codex ``plan`` thread items are mirrored into AgentNexus history.
...
def _capture_handler(posted: list[dict[str, Any]]) -> Callable[[httpx.Request], httpx.Response]:
    ...
    def handler(request: httpx.Request) -> httpx.Response:
        """
        Capture one AgentNexus event post from the forwarder.
...
async def _replay_completed_item(
    ...
    :param handler: MockTransport handler capturing the AgentNexus posts.
    :returns: None.
    """
    async with httpx.AsyncClient(
        base_url="http://127.0.0.1:8000",
...
def test_forwarder_posts_codex_command_execution_tool_call() -> None:
    ...
    aggregated output) must be mirrored as the AgentNexus ``function_call`` /
...
def test_forwarder_retries_transient_external_item_rejection(
    ...
    """
    Transient AgentNexus failures do not drop the mirrored Codex item.
...
def test_forwarder_logs_rejected_external_item(
    ...
    """
    AgentNexus 4xx responses are logged so mirror failures are diagnosable.
    """
...
def test_forwarder_marks_codex_skill_user_message_as_meta(tmp_path: Path) -> None:
    ...
    for AgentNexus resume/history replay while tagging it ``is_meta`` so UI
    clients can hide it.
    """
    posted: list[dict[str, Any]] = []
...
def test_local_run_prints_resume_hint_after_attach(
    ...
    monkeypatch.setattr("agentnexus.chat._find_free_port", lambda: 23456)
...
def test_local_run_resume_hint_follows_native_new_rotation(
    ...
    AgentNexus ownership to a new conversation, recording it in bridge
...
def test_local_resume_does_not_print_redundant_resume_hint(
    ...
    monkeypatch.setattr("agentnexus.chat._find_free_port", lambda: 23457)
    monkeypatch.setattr("agentnexus.chat._start_local_server", fake_start_server)
    monkeypatch.setattr("agentnexus.chat._stop_local_server", lambda server: None)
    monkeypatch.setattr("agentnexus.chat._wait_for_server", lambda *a, **k: None)
    monkeypatch.setattr(codex_native, "_prepare_codex_terminal", fake_prepare)
...
def test_record_launch_for_fresh_session_persists_current_cwd(
    ...
    :returns: None.
    """
    from agentnexus.codex_native_state import read_launch_state

    workspace = tmp_path / "workspace"
    workspace.mkdir()
    monkeypatch.chdir(workspace)
    monkeypatch.setenv("AGENTNEXUS_CODEX_NATIVE_STATE_DIR", str(tmp_path / "state"))
...
def test_run_with_remote_server_aligns_cwd_before_daemon_prepare(
    ...
    import agentnexus.host.identity as identity_mod
...
async def test_prepare_codex_terminal_fresh_session_passes_developer_instructions(
    ...
    """
    monkeypatch.setattr("agentnexus.codex_native_bridge._BRIDGE_ROOT", tmp_path / "codex-bridge")
...
def test_run_with_local_server_threads_raw_instructions_to_prepare_terminal_fresh(
    ...
    monkeypatch.setattr("agentnexus.chat._find_free_port", lambda: 12401)
    monkeypatch.setattr("agentnexus.chat._start_local_server", fake_start_server)
    monkeypatch.setattr("agentnexus.chat._stop_local_server", lambda server: None)
    monkeypatch.setattr("agentnexus.chat._wait_for_server", lambda *a, **k: None)
    monkeypatch.setattr("agentnexus.chat._bundle_agent", lambda path: b"bundle")
...
def test_run_with_local_server_threads_raw_instructions_to_prepare_terminal_resume(
    ...
    monkeypatch.setattr("agentnexus.chat._find_free_port", lambda: 12402)
    monkeypatch.setattr("agentnexus.chat._start_local_server", fake_start_server)
    monkeypatch.setattr("agentnexus.chat._stop_local_server", lambda server: None)
    monkeypatch.setattr("agentnexus.chat._wait_for_server", lambda *a, **k: None)
    monkeypatch.setattr(
...
async def test_prepare_codex_terminal_via_daemon_creates_runner_and_ensures_terminal(
    ...
    against an ``httpx.MockTransport`` AgentNexus server. Removing terminal launch arg
    ...
    def handler(request: httpx.Request) -> httpx.Response:
        """
        Route AgentNexus requests issued by daemon preparation.
    ...
    def client_factory(*args: object, **kwargs: object) -> httpx.AsyncClient:
        """
        Inject the mock AgentNexus transport into clients created by the helper.
...
async def test_prepare_codex_terminal_via_daemon_live_resume_skips_config_patch(
    ...
    rollout from AgentNexus history while the app-server may be appending to it.
    ...
    calls: list[tuple[str, str, object]] = []
    thread_id = "019e96aa-0be2-7343-8d3b-6f914d60936b"
    monkeypatch.setattr("agentnexus.codex_native_bridge._BRIDGE_ROOT", tmp_path / "bridges")
    ...
    def handler(request: httpx.Request) -> httpx.Response:
        """
        Route AgentNexus requests for a live resume.
...
async def test_prepare_codex_terminal_hot_resume_does_not_rewrite_rollout(
    ...
    rewrite rollout files from AgentNexus history because Codex may be appending
    ...
    def handler(request: httpx.Request) -> httpx.Response:
        """
        Serve a codex-native session, live terminal, and AgentNexus item history.
        ...
        if request.method == "GET" and path == f"/v1/sessions/{session_id}":
            return httpx.Response(
                ...
                json={
                    "labels": {
                        "agentnexus.wrapper": "codex-native-ui",
                        "agentnexus.codex_native.bridge_id": bridge_id,
                    },
                    "external_session_id": thread_id,
                },
            )
...
async def test_find_running_codex_terminal_known_misses_relaunch(
    ...
    :param body: Structured AgentNexus error body for the lookup.
    :returns: None.
    """
...
def test_attach_with_forwarder_uses_direct_tmux_when_socket_is_local(
    ...
    monkeypatch.setattr("agentnexus.codex_native.shutil.which", lambda _name: "/usr/bin/tmux")
...
def test_attach_with_forwarder_closes_active_rotated_session_terminal(
    ...
    """
    Wrapper exit closes the terminal on the active rotated AgentNexus session.
    ...
    async def fake_attach_terminal_resource(**_kwargs: object) -> None:
        """
        Simulate ``/clear`` rotating AgentNexus ownership during attach.
...
def test_attach_terminal_resource_runner_owned_missing_socket_fails_loud(
    ...
    the AgentNexus terminal WebSocket would reintroduce CLI-owned terminal IO.
...
def test_attach_with_forwarder_falls_back_when_tmux_socket_is_not_local(
    ...
    """
    Non-local runner sockets keep using the AgentNexus terminal attach bridge.
...
def test_session_usage_data_extracts_cumulative_tokens() -> None:
    ...
    stays 0 (codex produces no ``response.completed`` for the AgentNexus relay).
    """
    params = {
        "threadId": "thread_123",
        "tokenUsage": {
...
def test_usage_coalescer_flush_attaches_model_to_every_post() -> None:
    ...
    def handler(request: httpx.Request) -> httpx.Response:
        """
        Capture AgentNexus usage posts from the coalescer.
...
def _transcript_posts(
    ...
    """
    Filter AgentNexus posts to the ``external_conversation_item`` events for one session.

    :param posted: All captured AgentNexus posts as ``(path, body)`` tuples.
    :param session_id: AgentNexus session id to filter for.
...
def _registration_posts(
    ...
    """
    Filter AgentNexus posts to the ``external_codex_subagent_start`` events for a parent.

    :param posted: All captured AgentNexus posts as ``(path, body)`` tuples.
    :param parent_session_id: Parent AgentNexus session id to filter for.
...
def _make_omnigent_handler(
    ...
    """
    Build an AgentNexus ``MockTransport`` handler that registers a child session on demand.

    :param posted: Mutable list collecting all captured AgentNexus requests.
    :param child_session_id: AgentNexus child session id to return for
...
def test_forwarder_registers_subagent_activity_before_child_events(
    ...
    assert registrations, "subAgentActivity(kind=started) must create an AgentNexus child session"
...
def test_forwarder_does_not_double_write_stable_id_item_delivered_twice(
    ...
    written to AgentNexus only once.
...
def test_forwarder_child_thread_started_does_not_rotate_parent_session(
    ...
    rotate the parent AgentNexus session.
    ...
    by the rotation check, otherwise the parent's AgentNexus session would be
    ...
    # No AgentNexus calls should have been made during rotation detection.
    assert ap_posts == []
...
def test_forwarder_routes_live_child_items_to_child_session(
    ...
    to the child AgentNexus session, not the parent.
...
def test_forwarder_resolves_child_thread_elicitation_on_child_session(
    ...
    published on the child AgentNexus session (``route_session_id``). Codex's
...
async def test_ensure_local_codex_resume_rollout_synthesizes_omnigent_history(
    ...
    """
    Cross-machine Codex resume rebuilds a local rollout from AgentNexus history.

    The server can know the AgentNexus conversation and Codex thread id while the
    current host has no ``$CODEX_HOME/sessions/.../rollout-*-<thread>.jsonl``.
    This helper must fetch committed AgentNexus items, follow pagination, and write
    ...
    assert records[0]["payload"]["model_provider"] == "agentnexus_databricks"
...
async def test_ensure_local_codex_resume_rollout_preserves_existing_rollout(
    ...
    def handler(request: httpx.Request) -> httpx.Response:
        """
        Fail if AgentNexus history is fetched despite a local rollout.

        :param request: Incoming mock HTTP request.
        :returns: Mock AgentNexus response.
        """
        del request
        raise AssertionError("existing rollout should avoid AgentNexus history fetch")
...
async def test_ensure_local_codex_resume_rollout_rejects_malformed_tool_history(
    ...
    """
    Codex rollout synthesis fails loudly for corrupt AgentNexus tool history.

    Tool call ``arguments`` and tool ``output`` are required AgentNexus string
    fields. Missing values must not be invented as ``{}`` or ``""``,
    because Codex would then resume a tool history that never happened.

    :param tmp_path: Temporary directory for isolated ``CODEX_HOME``.
    :param bad_item: Malformed AgentNexus item to serve.
    :param message: Expected diagnostic substring.
    """

    def handler(request: httpx.Request) -> httpx.Response:
        """
        Serve malformed AgentNexus history.

        :param request: Incoming mock HTTP request.
        :returns: Mock AgentNexus item page.
        """
        assert request.url.path == "/v1/sessions/conv_codex/items"
...
async def test_ensure_local_codex_resume_rollout_rejects_unsafe_thread_id(
    ...
    def handler(request: httpx.Request) -> httpx.Response:
        """
        Fail if AgentNexus history is fetched for an unsafe thread id.

        :param request: Incoming mock HTTP request.
        :returns: Never returns.
        """
        del request
        raise AssertionError("unsafe thread id should be rejected before AgentNexus fetch")
...
def test_resolve_native_codex_launch_no_provider_sets_login_fallback_summary(
    ...
    from agentnexus.onboarding import ambient, detected, provider_config
...
def test_codex_discover_thread_and_forward_writes_routing_summary_on_timeout(
    ...
    from agentnexus import codex_native_forwarder as _fwd
    from agentnexus.codex_native_bridge import read_bridge_startup_error
    from agentnexus.runner.native import orchestration as native_orch
