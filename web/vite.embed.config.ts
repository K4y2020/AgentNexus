...
// into an ESM entry (`dist-embed/agentnexus-embed.js`) plus a `chunks/` tree of
...
// stylesheet (`dist-embed/agentnexus-embed.css`). The universe monolith then
// ingests this graph into its OWN rspack graph (see
// `webapp/web/js/genai/agentnexus/embed/loadOmnigentEmbed.ts` + the
// `@agentnexus/embed` alias in `app.rsbuild.config.ts`) and emits the FINAL
...
//
// The CSS is post-processed to prefix every selector with `.agentnexus-app` so
...
const scopePlugin = (): postcss.Plugin => ({
  postcssPlugin: "scope-agentnexus",
  ...
  // scoped utility like `.agentnexus-app .text-[11px]`. Since every embed rule is
  // already scoped under `.agentnexus-app`, we don't need layers for internal
  // ordering; dropping them makes the embed compete on specificity, which it
  // wins (`.agentnexus-app .util` ≥ host's typical element selectors).
  AtRule(atRule) {
...
function resolveExternalCjsRequire(externals: readonly string[]): Plugin {
  ...
  const importName = (spec: string) => `__agentnexusExt_${spec.replace(/[^a-zA-Z0-9]/g, "_")}`;
  ...
  return {
    ...
    renderChunk(code) {
      ...
      const replacement =
        ...
        `\tconst __agentnexusExternals = { ${table} };\n` +
        `\tif (Object.prototype.hasOwnProperty.call(__agentnexusExternals, id)) return __agentnexusExternals[id];\n` +
...
export default defineConfig({
  ...
  // `/assets/...` string). Monaco is agentnexus_ui's own dep, so Vite builds the
  ...
  // content-hash on its own CDN (see the `@agentnexus/embed` wiring in
  ...
  build: {
    ...
    // agentnexus-embed.js:<line>. Consumed at the host build via source-map-loader.
    sourcemap: true,
    lib: {
    ...
    rollupOptions: {
      ...
      output: {
        ...
        entryFileNames: "agentnexus-embed.js",
        chunkFileNames: "chunks/[name]-[hash].js",
        assetFileNames: (assetInfo) => {
          const name = assetInfo.names?.[0] ?? "";
          return name.endsWith(".css") ? "agentnexus-embed.css" : "assets/[name].[ext]";
        },
      },
    },
  },
