...
describe("effortLevelsForConv", () => {
  it("returns the extended ladder (xhigh, max) for claude-code-native-ui", () => {
    ...
    expect(effortLevelsForConv({ labels: { "agentnexus.wrapper": NATIVE } })).toEqual([
      "low",
      "medium",
      "high",
      "xhigh",
  ...
  it("returns the base three levels for a non-native wrapper", () => {
    ...
    expect(effortLevelsForConv({ labels: { "agentnexus.wrapper": "codex-native" } })).toEqual([
      "low",
      "medium",
      "high",
    ]);
...
describe("shouldShowModelPicker", () => {
  it("shows the picker for the native wrappers that honor a model override", () => {
    ...
    expect(shouldShowModelPicker({ labels: { "agentnexus.wrapper": "opencode-native-ui" } })).toBe(
      true,
    );
  ...
  it("hides the picker for other wrappers and missing labels (fail closed)", () => {
    ...
    expect(shouldShowModelPicker({ labels: { "agentnexus.wrapper": "codex-native" } })).toBe(false);
...
describe("shouldShowEffortPicker", () => {
  it("shows effort controls only for claude-native sessions", () => {
    ...
    // Web UI effort dial.
    expect(shouldShowEffortPicker({ labels: { "agentnexus.wrapper": NATIVE } })).toBe(true);
  });
  ...
  it("hides effort controls for cursor-native (model switch only, for now)", () => {
    ...
    expect(shouldShowEffortPicker({ labels: { "agentnexus.wrapper": "cursor-native-ui" } })).toBe(
      false,
    );
  });
  ...
  it("hides effort controls for opencode-native (model indicator only)", () => {
    ...
    expect(shouldShowEffortPicker({ labels: { "agentnexus.wrapper": "opencode-native-ui" } })).toBe(
      false,
    );
  });
  ...
  it("hides effort controls for kiro-native (model selection only)", () => {
    ...
    expect(shouldShowEffortPicker({ labels: { "agentnexus.wrapper": "kiro-native-ui" } })).toBe(
      false,
    );
  });
});
