 ...
 * to `--agentnexus-viewport-height`; the app-shell sizes to that (see index.css).
...
export function useIOSViewportLock(): void {
  useEffect(() => {
    ...
    const apply = () => {
      frame = 0;
      root.style.setProperty("--agentnexus-viewport-height", `${Math.round(viewport.height)}px`);
      resetPan();
    };
    ...
    return () => {
      ...
      root.style.removeProperty("--agentnexus-viewport-height");
    };
  }, []);
}
