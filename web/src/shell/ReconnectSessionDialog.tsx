 ...
 *   down. The owner reconnects the host (`agentnexus host`); a
 ...
 *
 * 1. `host_offline` — `agentnexus host` re-registers the host
 ...
 *    (`wrapper === "claude-code-native-ui"`) use `agentnexus claude
 *    --resume <id>`; everything else uses the generic `agentnexus run
...
export function buildReconnectCommand({
  ...
  if (state === "host_offline") {
    return ["agentnexus host \\", `  --server ${quotedServerUrl}`].join("\n");
  }
 ...
 *
 * @param wrapper - The conversation's `agentnexus.wrapper` label
 *   (`"claude-code-native-ui"` for `agentnexus claude` sessions). Picks
