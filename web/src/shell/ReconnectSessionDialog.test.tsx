...
//
// What we lock:
//   1. host_offline owner → `agentnexus host` (no --resume / YAML),
...
//   4. claude-native local_stranded → `agentnexus claude --resume`.
...
describe("buildReconnectCommand", () => {
  ...
  it("prefers `agentnexus host` for a host_offline claude-native session", () => {
    ...
    expect(cmd).toContain("agentnexus host");
    expect(cmd).not.toContain("agentnexus claude");
  });
  ...
  it("emits the generic run form for a local_stranded session", () => {
    ...
    expect(cmd).toContain("agentnexus run path/to/agent.yaml");
  ...
  it("emits `agentnexus claude --resume` for a claude-native local_stranded session", () => {
    const cmd = buildReconnectCommand({
...
describe("<ReconnectSessionDialog />", () => {
  ...
  it("defaults to the Reconnect tab with the host command for a host_offline owner", () => {
    ...
    expect(block.textContent).toContain("agentnexus host");
  ...
  it("shows the run command for a local_stranded session (owner or not)", () => {
    ...
    const block = screen.getByTestId("reconnect-session-command");
    expect(block.textContent).toContain("agentnexus run path/to/agent.yaml");
  ...
  it("shows the claude reattach command for a claude-native local_stranded session", () => {
    ...
    const block = screen.getByTestId("reconnect-session-command");
    expect(block.textContent).toContain("agentnexus claude");
