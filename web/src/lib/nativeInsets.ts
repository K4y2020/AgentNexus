...
// `--agentnexus-native-top-bar` / `--agentnexus-native-bottom-bar`; index.css folds
// them (together with the web-owned visibility flags and `env(safe-area-*)`)
// into `--agentnexus-inset-top` / `--agentnexus-inset-bottom`, which `<PageScroll>`
...
export function initNativeInsets(): () => void {
  return onNativeInsets(({ topBar, bottomBar }) => {
    ...
    root.setProperty("--agentnexus-native-top-bar", `${topBar}px`);
    root.setProperty("--agentnexus-native-bottom-bar", `${bottomBar}px`);
  });
}
