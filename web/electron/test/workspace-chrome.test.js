...
describe("applyWorkspaceChromeHideCss", () => {
  it("injects the chrome-hide CSS even when the URL is not under the workspace path", () => {
    const injected = [];
    const webContents = {
      // A path variant the old guard would have skipped (not /agentnexus).
    ...
    assert.deepEqual(
      ...
      [
        ...
        "URL, but it did not fire for a non-/agentnexus path. A URL/path guard has likely",
        ...
        "must stay unconditional — the CSS only targets .agentnexus-app (workspace-embedded",
