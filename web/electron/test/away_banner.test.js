...
describe("registerServerAwayWatch", () => {
  it("notifies after the delay with the last on-server subpage URL", () => {
    ...
    // On-server navigation (a Databricks /agentnexus mount subpage with args)
    // is remembered as the return target.
    webContents.emit("did-navigate", {}, `${ORIGIN}/agentnexus/c/abc123?o=456`);
    ...
    timers.run();
    assert.deepEqual(events.away, [`${ORIGIN}/agentnexus/c/abc123?o=456`]);
  });

  it("does not notify when the window returns to the server before the delay", () => {
    const { webContents, timers, events } = makeWatch();
    webContents.emit("did-navigate", {}, `${ORIGIN}/agentnexus`);
  ...
  it("does not re-notify on further foreign navigations until it returns on-server", () => {
    ...
    webContents.url = `${ORIGIN}/agentnexus/c/xyz`;
    webContents.emit("did-navigate", {}, `${ORIGIN}/agentnexus/c/xyz`);
    webContents.url = `${FOREIGN}/login`;
    webContents.emit("did-navigate", {}, `${FOREIGN}/login`);
    timers.run();
    assert.deepEqual(events.away, [`${ORIGIN}/agentnexus`, `${ORIGIN}/agentnexus/c/xyz`]);
  });
  ...
  it("tracks in-page navigations on the server as return targets", () => {
    const { webContents, timers, events } = makeWatch();
    webContents.emit("did-navigate", {}, `${ORIGIN}/agentnexus`);
    webContents.emit("did-navigate-in-page", {}, `${ORIGIN}/agentnexus/c/new-chat?o=7`, true);
    webContents.url = `${FOREIGN}/login`;
    webContents.emit("did-navigate", {}, `${FOREIGN}/login`);
    timers.run();
    assert.deepEqual(events.away, [`${ORIGIN}/agentnexus/c/new-chat?o=7`]);
  });
  ...
  it("treats same-origin auth-gate pages as away, never as return targets", () => {
    ...
    webContents.url = `${ORIGIN}/login.html?next_url=%2Fagentnexus&o=1`;
    webContents.emit("did-navigate", {}, `${ORIGIN}/login.html?next_url=%2Fagentnexus&o=1`);
    assert.equal(timers.pending(), 1);
    ...
    webContents.emit("did-navigate", {}, `${FOREIGN}/sso`);
    timers.run();
    assert.deepEqual(events.away, [`${ORIGIN}/agentnexus`]);
  });
...
describe("isAuthGatePath", () => {
  it("matches the platform auth namespace, not app pages", () => {
    assert.equal(isAuthGatePath("https://h/login.html?next_url=%2Fagentnexus"), true);
    ...
    assert.equal(isAuthGatePath("https://h/agentnexus"), false);
