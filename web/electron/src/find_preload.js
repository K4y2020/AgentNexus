...
contextBridge.exposeInMainWorld("agentnexusFind", {
  /**
   ...
   */
  query: (text, opts) => {
    ipcRenderer.send("agentnexus:find-query", {
      text: String(text ?? ""),
  ...
  close: () => {
    ipcRenderer.send("agentnexus:find-close");
  },
  /**
   ...
   */
  onResult: (callback) => {
    ipcRenderer.on("agentnexus:find-result", (_event, result) => callback(result));
  },
  /** Fires when Cmd/Ctrl+F re-activates an already-open bar. */
  onActivate: (callback) => {
    ipcRenderer.on("agentnexus:find-activate", () => callback());
  },
});
