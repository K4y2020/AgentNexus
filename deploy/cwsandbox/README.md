...
disposable cloud machines for running AgentNexus hosts, two ways:
...
- **No local port forward.** CoreWeave Sandbox can't forward a sandbox→laptop
  callback port, so the interactive in-sandbox `agentnexus login` / App OAuth step
...
```bash
pip install 'agentnexus[cwsandbox]'
```
...
with AgentNexus and its dependencies preinstalled — including the coding-harness
...
Then point AgentNexus at it — `AGENTNEXUS_CWSANDBOX_HOST_IMAGE` for the CLI flow, or
...
```bash
agentnexus sandbox create --provider cwsandbox --server https://your-host
```
...
```bash
agentnexus sandbox connect --provider cwsandbox \
...
`AGENTNEXUS_CWSANDBOX_SANDBOX_ENV` in your shell to a comma-separated list of
...
`connect` runs `agentnexus host` inside the sandbox, and that host must present
credentials when it dials back to a server that requires authentication. The
interactive `agentnexus login` browser flow can't run inside a sandbox (no callback
port forward), so inject the keys for the relevant server instead — name them in
`AGENTNEXUS_CWSANDBOX_SANDBOX_ENV` before `create`:

```bash
export AGENTNEXUS_CWSANDBOX_SANDBOX_ENV=DATABRICKS_HOST,DATABRICKS_TOKEN
agentnexus sandbox create --provider cwsandbox --server https://your-host
```
...
`accounts` provider is fine for CLI-launched hosts (you `agentnexus login`, and
...
`AGENTNEXUS_CWSANDBOX_SANDBOX_ENV` (CLI) or `sandbox.cwsandbox.env` (managed); the
...
`AGENTNEXUS_CWSANDBOX_SANDBOX_ENV` / `sandbox.cwsandbox.env`. The host image's git
...
- **The launch token's lifetime tracks the sandbox lifetime.** CW Sandbox's
  lifetime is operator-overridable (`AGENTNEXUS_CWSANDBOX_MAX_LIFETIME_S`, 24h
...
- Sandboxes are reaped at `max_lifetime_seconds` (24h default; override with
  `AGENTNEXUS_CWSANDBOX_MAX_LIFETIME_S`). The managed launch-token TTL is set above
...
- **"managed host did not come online within 120s"** — the server waits up to two
  ...
  host log: `/tmp/agentnexus-host.log`.
...
- **Agent has no credentials** — verify the injected var names match the
  forwarded set (or are named in `AGENTNEXUS_RUNNER_ENV_PASSTHROUGH`), and that each
...
| `AGENTNEXUS_CWSANDBOX_HOST_IMAGE` | CLI machine / server | Override the host image ref (`sandbox.cwsandbox.image` takes precedence for managed) |
| `AGENTNEXUS_CWSANDBOX_SANDBOX_ENV` | CLI machine / server | Comma-separated launcher-side env var names to inject (`sandbox.cwsandbox.env` takes precedence for managed) |
| `AGENTNEXUS_CWSANDBOX_MAX_LIFETIME_S` | CLI machine / server | Sandbox lifetime cap in seconds (default 24h); also derives the managed launch-token TTL |
...
Validate the API primitives directly (no AgentNexus or SDK install needed — stdlib
+ curl only):
