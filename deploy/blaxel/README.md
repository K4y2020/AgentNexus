...
Run AgentNexus hosts in Blaxel sandboxes from your terminal or let the AgentNexus server create one for each managed session.

- **CLI-launched:** `agentnexus sandbox create` ships your local checkout, then `connect` registers the sandbox as a host.
...
```bash
pip install 'agentnexus[blaxel]'
brew tap blaxel-ai/blaxel
brew install blaxel
bl login your-workspace
```

The process that launches the sandbox needs Blaxel control credentials. This is your shell for a CLI launch and the server process for a managed launch. Web users log in to AgentNexus, not Blaxel.
...
Run `create` from an AgentNexus checkout. Use a server URL that the Blaxel sandbox can reach:

```bash
agentnexus sandbox create \
  --provider blaxel \
...
Register that sandbox as an AgentNexus host:

```bash
agentnexus sandbox connect \
  --provider blaxel \
...
Blaxel does not expose local callback port forwarding. AgentNexus therefore skips the in-sandbox browser login automatically. If the host or agent needs environment credentials, list their variable names before `create`:

```bash
export OPENAI_API_KEY=your-openai-key
export AGENTNEXUS_BLAXEL_SANDBOX_ENV=OPENAI_API_KEY
```

The launcher copies each named value into the sandbox. For a protected CLI target, include the credentials that `agentnexus host` uses, such as `DATABRICKS_HOST` and `DATABRICKS_TOKEN`. Never include `BL_API_KEY` or `BL_CLIENT_CREDENTIALS` because Blaxel control credentials must stay outside the sandbox.
...
All managed sandboxes use the Blaxel workspace and credentials of the server process. This integration does not map each AgentNexus user to a separate Blaxel workspace.

## Image and limits

AgentNexus uses `blaxel/agentnexus-host:latest` by default. This public Blaxel Hub image combines the standard AgentNexus host runtime with Blaxel's `sandbox-api`, which provides process, file, streaming, and lifecycle control. Use `sandbox.blaxel.image` or `AGENTNEXUS_BLAXEL_HOST_IMAGE` to select a fixed published tag.

| Setting | Default | Purpose |
| --- | --- | --- |
| `image` | `blaxel/agentnexus-host:latest` | Host image from Blaxel Hub |
...
The host uses Blaxel keep-alive mode until the TTL or managed teardown. Process output through the provider API is limited to 4 MiB per command. AgentNexus stops commands that cross this limit.
...
The Blaxel SDK can send SDK error events to its vendor Sentry endpoint when tracking is enabled in Blaxel configuration. Tracking is off by default. Set `DO_NOT_TRACK=1` on the AgentNexus server to disable Blaxel SDK telemetry.

## Run the live smoke test

The smoke test refuses known production workspace names. It creates one unique sandbox and always attempts deletion. Set `AGENTNEXUS_BLAXEL_HOST_IMAGE` only to test an image override.

```bash
export AGENTNEXUS_BLAXEL_LIVE_TEST=1
export BL_WORKSPACE=your-non-production-workspace
export AGENTNEXUS_BLAXEL_TEST_WORKSPACE=your-non-production-workspace
# Optional image override:
# export AGENTNEXUS_BLAXEL_HOST_IMAGE=blaxel/agentnexus-host:<tag>
