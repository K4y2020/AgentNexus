...
# required secrets (POSTGRES_PASSWORD, AGENTNEXUS_OIDC_COOKIE_SECRET)
...
#   - If AGENTNEXUS_OIDC_COOKIE_SECRET is unset OR commented out,
...
if [[ -z "$cookie_current" || "$cookie_current" == "<64-hex-chars>" ]]; then
  ...
  echo "→ AGENTNEXUS_OIDC_COOKIE_SECRET already set, leaving alone"
fi
...
if [[ -z "$accounts_cookie_current" || "$accounts_cookie_current" == "<64-hex-chars>" ]]; then
  ...
  echo "→ AGENTNEXUS_ACCOUNTS_COOKIE_SECRET already set, leaving alone"
fi

echo
echo "✓ deploy/docker/.env is ready. Next:"
echo "    docker compose up -d && docker compose logs agentnexus"
echo
...
echo "    AGENTNEXUS_ACCOUNTS_BASE_URL=<your public URL>"
