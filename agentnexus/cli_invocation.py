...
``AGENTNEXUS_WRAPPER_COMMAND`` and refuses naked ``omnigent`` calls (see the
...
DEFAULT_CLI_NAME = "agentnexus"
WRAPPER_COMMAND_ENV = "AGENTNEXUS_WRAPPER_COMMAND"
...
def cli_invocation(*, name: str = DEFAULT_CLI_NAME, env: Mapping[str, str] | None = None) -> str:
    ...
    ``AGENTNEXUS_WRAPPER_COMMAND=isaac omni``.
    """
    if env is None:
        env = os.environ
