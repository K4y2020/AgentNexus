"""Read AgentNexus's user and project configuration."""
...
_GLOBAL_CONFIG_PATH = Path.home() / ".agentnexus" / "config.yaml"
_LOCAL_CONFIG_RELPATH = Path(".agentnexus") / "config.yaml"
...
def _merge_effective_config(
    ...
    :param global_cfg: User-level config (``~/.agentnexus/config.yaml``).
    :param local_cfg: Project-level config (``.agentnexus/config.yaml``).
