...
* **RPC mirroring (read path) and RPC web-turn delivery (write path).** agy's
  conversation mirrors into the AgentNexus chat view via the RPC read driver
...
from agentnexus.antigravity_native_launch import (
    agy_binary_path,
    build_agy_launch,
...
def run_antigravity_native(
    *,
    ...
    """
    Launch the Antigravity (agy) TUI in an AgentNexus terminal and attach.

    :param server: Resolved AgentNexus server URL, e.g.
        ``"http://127.0.0.1:8123"``. ``None`` starts a local AgentNexus
        server using the existing chat-server machinery.
    :param session_id: Optional existing AgentNexus conversation id to
    ...
    :param permission_mode: Optional AgentNexus permission mode, e.g.
    ...
    with TemporaryDirectory(prefix="agentnexus-antigravity-native-") as tmpdir:
...
def _materialize_antigravity_agent_spec(tmpdir: Path) -> Path:
    ...
    raw: dict[str, object] = {
        ...
        # sessions. The AgentNexus MCP relay (wired in #1194 — see
        ...
        # Declare a default shell terminal so the AgentNexus MCP relay advertises
...
def _run_with_local_server(
    ...
    *,
    ...
    """
    Start a local AgentNexus server, launch agy, and attach to it.
...
def _run_with_remote_server(
    ...
    *,
    ...
    """
    Launch agy on a remote AgentNexus server via a daemon-spawned runner.
    ...
    :returns: None.
    """
    from agentnexus.chat import _bundle_agent, _remote_headers
...
async def _prepare_antigravity_terminal(
    *,
    ...
    :param command: agy executable to run.
    :param model: Optional agy model id.
    :param permission_mode: Optional AgentNexus permission mode threaded into the
        agy argv assembly.
    :param headless: ``True`` when no interactive client will attach (forces
...
async def _launch_and_record(
    ...
    *,
    ...
    # ~/.gemini: agy saw no AgentNexus relay (hence no sys_* tools, #1194), and the
...
async def _attach_terminal(
    ...
    **Read/write wiring on the CLI fallback only.** When the runner produced no
    ...
    which mirrors agy's conversation into the AgentNexus chat view and surfaces
...
async def _cold_start_agy_conversation(
    ...
    *timeout_s*, ``StartCascade`` erroring, or the PATCH failing) leaves the
    ...
    :param base_url: AgentNexus server base URL for the ``external_session_id``
        PATCH.
...
async def _attach_direct_tmux(socket_path: Path, tmux_target: str) -> _AttachOutcome:
    ...
    attach to AgentNexus's private tmux server. After the attach child
    ...
    """
    from agentnexus.terminals.ws_common import _tmux_session_alive
...
async def _create_antigravity_session(
    ...
    *,
    ...
    :returns: New AgentNexus session id, e.g. ``"conv_abc123"``.
...
async def _fetch_antigravity_session(
    ...
    """
    Fetch an existing AgentNexus session snapshot.
