...
from agentnexus.codex_native_bridge import write_policy_hook_config
...
from agentnexus.process_logging import log_info_once, log_once
...
# AgentNexus policy hook. Codex discovers it as a ``user``-layer hook
...
# might declare.
_POLICY_HOOK_MODULE = "agentnexus.codex_native_hook"
...
def _toml_table_header_name(line: str) -> str | None:
    ...
    :param line: One config line, e.g.
        ``"[mcp_servers.agentnexus] # generated\n"``.
    :returns: The table name, e.g. ``"mcp_servers.agentnexus"``, or
...
def _remove_toml_table(text: str, table_name: str) -> str:
    ...
    AgentNexus MCP server table. This avoids accumulating duplicate
    ``[mcp_servers.agentnexus]`` sections across terminal relaunches.
...
#: AgentNexus tools the framework calls on every session's behalf, pre-approved
...
def framework_approved_tools(*, routed_spawns: bool) -> tuple[str, ...]:
    """
    Name the AgentNexus tools this session pre-approves in codex.
...
def _codex_mcp_server_config_section(
    ...
    *,
    ...
    :returns: TOML text for ``[mcp_servers.agentnexus]`` and its
...
def _sync_codex_developer_instructions(
    ...
    config_path = codex_home / "config.toml"
    base_path = codex_home / ".agentnexus-developer-instructions-base"
    if config_path.is_symlink():
        target = config_path.resolve()
        config_path.unlink()
        if target.is_file():
    ...
    if base_path.exists():
        ...
        # A previous AgentNexus build may have appended the same instructions
...
def _inject_mcp_server_config(
    ...
    *,
    ...
    """
    Upsert AgentNexus MCP server config into ``config.toml``.

    Writes a ``[mcp_servers.agentnexus]`` section that points Codex
    ...
    if config_path.exists():
        ...
        existing = ""
    updated = _remove_toml_table(existing, "mcp_servers.agentnexus")
...
class CodexAppServerClient:
    ...
    :param client_name: App-server client name for the initialize
        handshake, e.g. ``"agentnexus-codex-native"``.
    """
    ...
    async def respond(self, request_id: int | str, result: CodexParams) -> None:
        ...
        The forwarder handles those requests through AgentNexus and replies
...
async def discover_codex_model_options(*, codex_path: str | None = None) -> list[_JsonObject]:
    ...
    with tempfile.TemporaryDirectory(prefix="agentnexus-codex-model-discovery-") as raw_dir:
        root = Path(raw_dir)
        ...
        try:
            ...
            client = CodexAppServerClient(
                ws_url=listen_url,
                client_name="agentnexus-codex-model-discovery",
            )
...
def _probe_codex_home(config_overrides: Sequence[str]) -> Path:
    ...
    home = Path.home() / ".agentnexus" / "cache" / "codex-model-probe" / key
...
async def probe_codex_model_options(*, codex_path: str | None = None) -> list[_JsonObject]:
    ...
    try:
        ...
        client = CodexAppServerClient(
            ws_url=listen_url,
            client_name="agentnexus-codex-model-probe",
        )
        await client.connect()
...
class CodexNativeAppServer:
    ...
    :param bridge_dir: Native Codex bridge directory, e.g.
        ``Path("~/.agentnexus/codex-native/<hash>")``. The policy hook
        subprocess is pointed at it via ``--bridge-dir`` and reads the
        session id + AgentNexus coordinates from it.
    :param ap_server_url: AgentNexus server base URL the policy hook POSTs tool
        calls to, e.g. ``"http://127.0.0.1:8787"``. ``None`` registers
        and trusts the hook but writes no AgentNexus coordinates, so the hook
        no-ops (no enforcement) until coordinates exist.
    :param ap_auth_headers: Outbound auth headers for the policy hook's
        AgentNexus requests, e.g. ``{"Authorization": "Bearer <token>"}``.
    ...
    :param policy_notice_pending: One-shot flag: ``True`` once a degrade
        ...
        surfaces it to AgentNexus (which posts a single durable banner). Prevents
    ...
    async def start(self) -> None:
        ...
        if codex_version is not None and not policy_hooks_supported:
            ...
            # Register the AgentNexus policy hook in this private CODEX_HOME
            # *before* launching the app-server so codex discovers it at
            # config load. The AgentNexus coordinates the hook subprocess
    ...
    async def _trust_policy_hooks(self) -> None:
        """
        Mark the registered AgentNexus policy hook as trusted.
        ...
        if self.listen_url and self.listen_url.startswith("ws://"):
            client = CodexAppServerClient(
                ...
                client_name="agentnexus-policy-trust",
            )
        else:
            client = CodexAppServerClient(
                self.socket_path,
    ...
    def _warn_policy_hook_disabled(self) -> None:
        ...
        :attr:`policy_hook_disabled_reason`. When AgentNexus coordinates are
...
def _our_policy_hooks_from_list(listed: _JsonObject, cwd: str) -> list[_JsonObject]:
    ...
    :param cwd: The cwd whose hook set to read, e.g.
        ``"/home/user/repo"``.
    :returns: The matching AgentNexus policy-hook metadata dicts.
    """
...
def _untrusted_hook_detail(hooks: Sequence[_JsonObject]) -> str:
    ...
    :param hooks: Untrusted AgentNexus hook metadata dicts from
        ``hooks/list``.
...
async def trust_native_policy_hooks(client: CodexAppServerClient, *, cwd: str) -> None:
    """
    Trust the AgentNexus policy hook so codex actually runs it.
    ...
    verifies every AgentNexus hook ended ``trusted``/``managed``.

    :param client: A connected Codex app-server client.
    :param cwd: The session cwd the hooks are scoped to, e.g.
        ``"/home/user/repo"``.
    :returns: None.
    :raises RuntimeError: If no AgentNexus hook is discovered (it was
    ...
    if not ours:
        raise RuntimeError(
            f"AgentNexus policy hook was not discovered for cwd {cwd!r}; "
    ...
    if still_untrusted:
        ...
        raise RuntimeError(
            "AgentNexus policy hook still untrusted after config/batchWrite; "
...
def _trust_codex_project(codex_home: Path, cwd: Path) -> None:
    ...
    until AgentNexus reports a timeout. The config is already a private copy;
...
def build_codex_native_server(
    *,
    ...
    :param bridge_dir: Native Codex bridge directory; the policy hook is
        pointed at it and reads the session id + AgentNexus coordinates from it.
    :param ap_server_url: AgentNexus server base URL the policy hook POSTs tool
        calls to, e.g. ``"http://127.0.0.1:8787"``. ``None`` registers
        the hook but writes no AgentNexus coordinates (hook no-ops).
    :param ap_auth_headers: Outbound auth headers for the policy hook's
        AgentNexus requests, e.g. ``{"Authorization": "Bearer <token>"}``.
    :param python_executable: Python executable the policy hook command
    ...
    :param extra_config_overrides: Additional ``-c`` config overrides
        ...
        registration for the AgentNexus tool relay.
    :param developer_instructions: Optional raw ``AgentSpec.instructions``
    ...
    if not resolved_codex:
        raise ImportError(
            ...
            "nvm-managed bin dir), set AGENTNEXUS_CODEX_PATH=/path/to/codex."
        )
    env = _clean_codex_env()
    config_overrides: list[str] = []
    pinned_model = model
...
def _config_default_provider_base_url() -> str | None:
    ...
    from agentnexus.onboarding.detected import codex_config_provider_dismissed
...
def _cli_config_provider_base_url(provider_name: str) -> str | None:
    ...
    Codex's own login (no pinned AIGW) and ``"agentnexus_databricks"`` is the
    ...
    :returns: The provider table's ``base_url``, or ``None`` when it cannot be
        read.
    """
    if provider_name in ("openai", "agentnexus_databricks"):
        return None
    return _config_toml_provider_base_url(provider_name)
...
def _codex_login_usable() -> bool:
    ...
    """
    from agentnexus.onboarding.ambient import codex_auth_has_credential
...
def resolve_native_codex_launch(
    *, model: str | None, spec: AgentSpec | None = None
    ...
    :returns: The resolved :class:`NativeCodexLaunch`.
    """
    from agentnexus.onboarding.ambient import codex_config_detection
    ...
    if (
        ...
        # detection without being marked the AgentNexus default. Codex still
...
def client_for_transport(
    ...
    *,
    ...
    :param transport: App-server transport from bridge state, e.g.
        ...
        ``"/home/u/.agentnexus/codex-native/x/app-server.sock"``.
    :param client_name: App-server initialize-handshake client name,
        e.g. ``"agentnexus-codex-native"``.
...
async def preload_codex_thread_for_resume(
    ...
    *,
    ...
    client = client_for_transport(
        transport,
        client_name="agentnexus-codex-native-preload",
    )
    await client.connect()
    try:
...
def build_codex_remote_args(
    ...
    *separate* process that loads its own config from ``CODEX_HOME`` and
    ...
    through makes the TUI resolve the AgentNexus provider
    ...
    :param remote_url: App-server endpoint the TUI attaches to, e.g.
        ``"unix:///home/user/.agentnexus/codex-native/x/app-server.sock"``
        or ``"ws://127.0.0.1:9876"``.
    :param config_overrides: Codex ``-c`` config override values to apply
        to the TUI, e.g.
        ``('model="databricks-gpt-5-5"', 'model_provider="agentnexus_databricks"')``.
    ...
    :param bypass_hook_trust: When ``True``, emit
        ...
        ``CODEX_HOME`` is provisioned by AgentNexus and there is no terminal
