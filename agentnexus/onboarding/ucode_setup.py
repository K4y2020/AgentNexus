"""Setup helpers for invoking ucode from AgentNexus."""
...
def build_ucode_configure_command(
    ...
    *,
    ...
    """Build the ``ucode configure`` command AgentNexus runs.
...
def configure_ucode_for_workspace(
    ...
    *,
    ...
    ``kind: databricks`` provider via ``agentnexus setup --no-internal-beta``.
    ucode writes ``~/.ucode/state.json``, which AgentNexus then reads for
