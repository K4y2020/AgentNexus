...
``agentnexus setup --no-internal-beta`` command writes a provider's API key here
...
- **OS keychain** (macOS Keychain, GNOME Keyring, Windows Credential
  ...
  actually works and ``AGENTNEXUS_DISABLE_KEYRING`` is not set. Secrets
...
- **A ``0600`` JSON file** at ``<config_home>/secrets.json`` (a flat
  ...
  ``AGENTNEXUS_DISABLE_KEYRING`` or a keyring call raises
...
_DISABLE_KEYRING_ENV = "AGENTNEXUS_DISABLE_KEYRING"
...
def _keyring_disabled() -> bool:
    ...
    :returns: ``True`` when ``AGENTNEXUS_DISABLE_KEYRING`` is truthy, e.g.
        with ``AGENTNEXUS_DISABLE_KEYRING=1`` set.
    """
...
def _use_keyring() -> bool:
    ...
    :returns: ``True`` unless ``AGENTNEXUS_DISABLE_KEYRING`` is truthy.
    """
...
def _config_home() -> str:
    ...
    :returns: The config home path, e.g. ``"/home/u/.agentnexus"`` or the
        value of ``$AGENTNEXUS_CONFIG_HOME`` when set.
    """
    config_home = os.environ.get("AGENTNEXUS_CONFIG_HOME")
    if config_home:
        return config_home
    return os.path.join(os.path.expanduser("~"), ".agentnexus")
...
def _secrets_path() -> str:
    ...
    :returns: Path to ``secrets.json`` under the config home, e.g.
        ``"/home/u/.agentnexus/secrets.json"``.
    """
