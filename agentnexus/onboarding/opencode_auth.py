"""OpenCode readiness + credential reporting for ``agentnexus setup``.

Like :mod:`omnigent.onboarding.goose_auth`, AgentNexus stores **no** OpenCode
...
read-only reporter so ``agentnexus setup`` can show which providers OpenCode can
...
from agentnexus.onboarding.harness_install import OPENCODE_KEY, harness_cli_installed
