"""
Databricks profile onboarding for AgentNexus.
...
SKIP_ENV_VAR = "AGENTNEXUS_SKIP_ONBOARD"
...
class ProfileSpec:
    """
    One Databricks profile AgentNexus depends on.
...
def run_onboarding() -> bool:
    ...
    from agentnexus.onboarding.internal_beta import DEFAULT_PROFILES
    ...
    console.print(
        Panel(
            "[bold]AgentNexus onboarding[/bold]\n\n"
            "Setting up the Databricks profiles AgentNexus needs "
...
def maybe_run_onboarding() -> None:
    ...
    Skipped when ``$AGENTNEXUS_SKIP_ONBOARD`` is set or stdin isn't a
    ...
    if actions.aliasable:
        ...
        if n:
            console.print(
                f"agentnexus: aliased {n} existing profile(s) "
                f"to AgentNexus names: "
    ...
    console.print(
        f"\n  [yellow]AgentNexus needs Databricks profiles: {', '.join(missing_parts)}[/yellow]"
    )
    try:
