"""Cursor API-key credential storage for ``agentnexus setup`` and the runtime.
...
register a ``CURSOR_API_KEY`` once through ``agentnexus setup`` rather than
...
from a dedicated top-level ``cursor:`` block in ``~/.agentnexus/config.yaml``::
...
from agentnexus.onboarding.extra_install import extra_install_command
from agentnexus.onboarding.provider_config import load_config, resolve_secret
