"""GitHub Copilot token storage for ``agentnexus setup`` and the runtime.
...
to register a Copilot token once through ``agentnexus setup`` rather than
...
dedicated top-level ``copilot:`` block in ``~/.agentnexus/config.yaml``::
