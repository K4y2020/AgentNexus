"""Read/write the kind-typed model-provider config in ``~/.agentnexus/config.yaml``.
...
block in ``~/.agentnexus/config.yaml`` is the source of truth for the
...
from agentnexus.env_credentials import (
    _ENV_REF_RE,
...
_HARNESS_FAMILY: dict[str, str] = {
    ...
    # per-spawn provider override flag, so AgentNexus cannot thread a generic
...
class ProviderEntry:
    ...
    def family(self, name: str) -> FamilyConfig | None:
        ...
        :raises AgentNexusError: If the family references an unset
...
def resolve_secret(ref: str) -> str:
    ...
    :raises AgentNexusError: If an ``env:`` / ``$VAR`` reference names an
...
def _config_path() -> str:
    ...
    Respects ``$AGENTNEXUS_CONFIG_HOME`` for test isolation (matching the
    rest of the onboarding layer).

    :returns: Path to ``config.yaml``, e.g.
        ``"/home/u/.agentnexus/config.yaml"``.
    """
    config_home = os.environ.get("AGENTNEXUS_CONFIG_HOME")
    if config_home:
        return os.path.join(config_home, "config.yaml")
    return os.path.join(os.path.expanduser("~"), ".agentnexus", "config.yaml")
...
def _expand(key: str, value: str) -> str:
    ...
    :raises AgentNexusError: If a referenced variable is unset.
    """
...
def _expand_family(provider_name: str, family_name: str, family: FamilyConfig) -> FamilyConfig:
    ...
    :returns: A new :class:`FamilyConfig` with ``base_url`` expanded and,
        ...
        ``api_key_ref`` cleared).
    :raises AgentNexusError: If a referenced environment variable is unset,
...
def _parse_family(provider_name: str, family_name: str, raw: dict[str, object]) -> FamilyConfig:
    ...
    :raises AgentNexusError: If ``base_url`` is missing, the credential
...
def _parse_default_families(
    ...
    :raises AgentNexusError: If ``default`` is an unsupported type, or names
...
def _parse_provider(name: str, raw: dict[str, object]) -> ProviderEntry:
    ...
    :raises AgentNexusError: If ``kind`` is missing/invalid or the
...
def load_config() -> dict[str, object]:
    """Load the global ``~/.agentnexus/config.yaml`` mapping.
    ...
    ``$AGENTNEXUS_CONFIG_HOME`` for test isolation, exactly like the rest
...
def load_providers(config: dict[str, object]) -> dict[str, ProviderEntry]:
    ...
    :param config: The parsed ``~/.agentnexus/config.yaml`` mapping, e.g.
        ``{"providers": {"anthropic": {"kind": "key", ...}}, "auth": {...}}``.
    :returns: Providers keyed by name, e.g.
        ``{"anthropic": ProviderEntry(...)}``. Empty when no ``providers:``
        block is present.
    :raises AgentNexusError: If any provider entry is malformed.
    """
...
def provider_credential_env_vars(config: dict[str, object]) -> frozenset[str]:
    ...
    *config* and collects the names of environment variables they reference for
    credentials, including the ``AGENTNEXUS_``-prefixed alias for each.  Two
    ...
    to list them in ``AGENTNEXUS_RUNNER_ENV_PASSTHROUGH`` by hand.
    ...
    :param config: The parsed ``~/.agentnexus/config.yaml`` mapping.
    :returns: Env var names (and their ``AGENTNEXUS_`` aliases) that provider
        credential fields reference, e.g.
        ``frozenset({"MY_TOKEN", "AGENTNEXUS_MY_TOKEN"})``.
    """
    names: set[str] = set()
...
def _cli_config_serves_pi(entry: ProviderEntry) -> bool:
    ...
    if entry.kind != CLI_CONFIG_KIND:
        return False
    from agentnexus.pi_native_credentials import cli_config_pi_provider_capable
...
def get_default_provider(config: dict[str, object], family: str) -> ProviderEntry | None:
    ...
    :raises AgentNexusError: If any provider is malformed, or more than one
...
def default_provider_for_harness(config: dict[str, object], harness: str) -> ProviderEntry | None:
    ...
    :raises AgentNexusError: If a provider is malformed, or more than one
...
def _source_descriptor(family: FamilyConfig) -> str:
    ...
    :raises AgentNexusError: If the family has no credential source (should
...
def harness_owns_its_credential(harness: str) -> bool:
    """Whether *harness* carries its own auth, so AgentNexus resolves none.

    True for ACP-backed harnesses whose spawn wires no AgentNexus provider
    (``acp``/``acp:<slug>``, goose, and unmapped community ACP plugins):
    the external agent authenticates itself, so describing an AgentNexus
...
def describe_active_credential(
    ...
    :raises AgentNexusError: If the resolved provider is malformed, or more
...
def set_default_provider(
    ...
    :raises AgentNexusError: If *name* is absent, an entry is malformed, or
...
def provider_entry_settings(
    ...
    ``~/.agentnexus/config.yaml`` under ``providers:``. When *make_default*
