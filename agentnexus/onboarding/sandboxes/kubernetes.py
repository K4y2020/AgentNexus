...
dependency (``pip install 'agentnexus[kubernetes]'``) imported lazily, so the
...
- **Writable HOME.** The host image's WORKDIR is ``/root`` (root-owned), but
  ...
  ``AGENTNEXUS_CONFIG_HOME``, the init container receives the same value so its
...
- **Least privilege.** ``automountServiceAccountToken: false`` keeps the runner
  ...
  filesystem stays writable (the host writes ``/tmp`` and ``~/.agentnexus``).
...
HOST_IMAGE_ENV_VAR: str = "AGENTNEXUS_KUBERNETES_HOST_IMAGE"
...
NAMESPACE_ENV_VAR: str = "AGENTNEXUS_KUBERNETES_NAMESPACE"
...
SANDBOX_SECRET_ENV_VAR: str = "AGENTNEXUS_KUBERNETES_SECRET"
...
SANDBOX_ENV_PASSTHROUGH_ENV_VAR: str = "AGENTNEXUS_KUBERNETES_SANDBOX_ENV"
...
SERVICE_ACCOUNT_ENV_VAR: str = "AGENTNEXUS_KUBERNETES_SERVICE_ACCOUNT"
...
KUBECONFIG_ENV_VAR: str = "AGENTNEXUS_KUBERNETES_KUBECONFIG"
...
_DEFAULT_NAMESPACE: str = "agentnexus-sandboxes"
_DEFAULT_SERVICE_ACCOUNT: str = "agentnexus-runner"
...
_MANAGED_BY_VALUE: str = "agentnexus"
_ROLE_LABEL: str = "agentnexus.ai/role"
...
_AGENT_LABEL: str = "agentnexus.ai/agent"
...
def _ensure_sdk() -> None:
    ...
    try:
        ...
        raise click.ClickException(
            ...
            "provider. Install it with `pip install 'agentnexus[kubernetes]'`."
        ) from exc
...
def _new_pod_name(label: str) -> str:
    ...
    :returns: A name like ``"agentnexus-managed-a1b2c3d4-1a2b3c"``.
    """
    base = re.sub(r"[^a-z0-9-]+", "-", label.lower()).strip("-")
    base = re.sub(r"-+", "-", base) or "host"
    return f"agentnexus-{base[:40]}-{uuid.uuid4().hex[:6]}"
...
def _token_secret_name(job_name: str) -> str:
    ...
    :returns: The Secret name, e.g. ``"agentnexus-managed-a1b2c3d4-1a2b3c-token"``.
    """
...
def _render_workspace_prep_command(
    ...
    ``$AGENTNEXUS_CONFIG_HOME`` or the default ``~/.agentnexus`` when set — all
...
def _render_host_command(server_url: str) -> list[str]:
    ...
    script = (
        ...
        f"agentnexus host --server {shlex.quote(server_url)}"
    )
    return ["bash", "-lc", script]
...
def build_job_manifest(
    *,
    ...
    - Pod + container ``securityContext`` satisfy Pod Security "restricted"
      ...
      root filesystem stays writable (the host writes ``/tmp`` + ``~/.agentnexus``).
    ...
    config_home = env_literals.get("AGENTNEXUS_CONFIG_HOME")
    if config_home is not None:
        ...
        # (~/.agentnexus), so only a non-empty override is checked. Resolve
        ...
        # lexical check, so an operator must not aim AGENTNEXUS_CONFIG_HOME inside
        ...
        if (
            ...
            raise ValueError(
                f"AGENTNEXUS_CONFIG_HOME ({config_home!r}) must resolve under {_HOME_DIR!r} "
        ...
        init_env.append({"name": "AGENTNEXUS_CONFIG_HOME", "value": config_home})
...
class KubernetesSandboxLauncher(SandboxHostLauncher):
    ...
    def start_host(
        ...
        *,
        ...
        :param host_config: Deployment-supplied ``~/.agentnexus/config.yaml``
