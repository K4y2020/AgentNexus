"""
Provider-agnostic sandbox bootstrap for AgentNexus hosts.

Composes a :class:`~omnigent.onboarding.sandboxes.base.SandboxLauncher`
into the full host-bootstrap flow: build the AgentNexus wheels locally,
...
sessions are reachable from the AgentNexus server's UI, TUI, and
``omnigent resume``.
...
if TYPE_CHECKING:
    ...
    from agentnexus.onboarding.sandboxes.base import RemoteProcess, SandboxLauncher
...
def build_wheels(
    ...
    *,
    ...
    """
    Build the AgentNexus wheels and pack them into a single tarball.
    ...
    click.echo(f"▸ Building AgentNexus wheels → {tgz_path}")
    env = os.environ.copy()
...
def ship_wheels(
    ...
    *,
    ...
    """
    Install AgentNexus wheels into a sandbox.
...
class DerivedWorkspace:
    ...
    probes of an AgentNexus server URL.
...
def derive_workspace(server_url: str) -> DerivedWorkspace | None:
    ...
    from agentnexus.cli import _databricks_workspace_login_target
...
def login_app_oauth_in_sandbox(
    ...
    **inside the sandbox**, driving the browser step from the local
    ...
    :param server_url: AgentNexus server URL to log in to, e.g.
    ...
    login = launcher.stream_exec(
        sandbox_id,
        f"agentnexus login {shlex.quote(server_url)}",
        pty=True,
    )
    try:
...
def set_sandbox_host_name(launcher: SandboxLauncher, sandbox_id: str, host_name: str) -> None:
    """
    Update the sandbox's ``~/.agentnexus/config.yaml`` to use a
    specific host name.
    ...
    """
    click.echo(f"  → setting host name to '{host_name}' in ~/.agentnexus/config.yaml")
    ...
    py = (
        "import os, uuid, yaml; "
        "p=os.path.expanduser('~/.agentnexus/config.yaml'); "
...
def connect_sandbox_host(
    ...
    *,
    ...
    ``~/.agentnexus/config.yaml`` is updated so the host registers
    ...
    :param launcher: The provider's launcher.
    :param sandbox_id: Target sandbox.
    :param server_url: AgentNexus App URL the runner registers with.
    ...
    returncode = launcher.exec_foreground(sandbox_id, f"agentnexus host --server {server_url}")
    if returncode != 0:
        raise click.ClickException(
...
def bootstrap_sandbox_host(
    ...
    *,
    ...
    :param sandbox_name: Label for a new sandbox (ignored when
        *sandbox_id* is set).
    :param server_url: AgentNexus server URL the sandbox logs in to.
