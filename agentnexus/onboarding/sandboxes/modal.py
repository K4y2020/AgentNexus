...
'agentnexus[modal]'``) imported lazily, so the provider can be listed and
...
- **24-hour lifetime.** Modal caps sandbox lifetime at 24 hours;
  ...
  Modal-hosted AgentNexus host must be re-created daily.
...
MODAL_APP_NAME: str = "agentnexus-sandboxes"
...
HOST_IMAGE_ENV_VAR: str = "AGENTNEXUS_MODAL_HOST_IMAGE"
...
REGISTRY_SECRET_ENV_VAR: str = "AGENTNEXUS_MODAL_REGISTRY_SECRET"
...
SANDBOX_SECRETS_ENV_VAR: str = "AGENTNEXUS_MODAL_SANDBOX_SECRETS"
...
# the AgentNexus host's runner + harness processes; 2 vCPU / 4 GiB is
...
def _ensure_sdk() -> None:
    ...
    try:
        ...
        raise click.ClickException(
            ...
            "Install it with `pip install 'agentnexus[modal]'`, then "
...
def _build_sandbox_image(image_ref: str | None = None) -> modal.Image:
    ...
    Pulls the prebaked AgentNexus host image — the full omnigent
...
class ModalSandboxLauncher(SandboxLauncher):
    ...
    def __init__(self, *, image: str | None = None, secrets: Sequence[str] | None = None) -> None:
        ...
        :param secrets: Optional Modal secret names whose env vars are
            injected into every sandbox, e.g. ``["agentnexus-llm"]`` —
    ...
    def provision(self, name: str) -> str:
        """
        Create a new Modal sandbox under the shared AgentNexus App.
        ...
        :param name: Human-readable label, e.g. ``"agentnexus-host"``.
        ...
        handle.set_tags({"agentnexus-name": name})
