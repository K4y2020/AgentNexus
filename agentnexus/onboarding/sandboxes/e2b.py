...
the SDK is an optional dependency (``pip install 'agentnexus[e2b]'``)
...
- **Templates, not registry images.** Unlike every other launcher, E2B
  ...
  a pre-built E2B *template*. The AgentNexus host image must therefore be
...
TEMPLATE_ENV_VAR: str = "AGENTNEXUS_E2B_TEMPLATE"
"""Environment variable overriding :data:`DEFAULT_E2B_TEMPLATE` — the
NAME (or id) of the pre-built E2B template the AgentNexus host image was
built into (``e2b template build``). NOT a registry image reference:
E2B boots from templates, not arbitrary images."""

SANDBOX_ENV_PASSTHROUGH_ENV_VAR: str = "AGENTNEXUS_E2B_SANDBOX_ENV"
...
DEFAULT_E2B_TEMPLATE: str = "agentnexus-host"
...
MAX_LIFETIME_ENV_VAR: str = "AGENTNEXUS_E2B_MAX_LIFETIME_S"
...
def _template_build_hint(template: str, exc: Exception) -> click.ClickException:
    ...
    return click.ClickException(
        ...
        "incompatible. Build the AgentNexus host image into an E2B template first "
...
def _ensure_sdk() -> None:
    ...
    try:
        ...
        raise click.ClickException(
            ...
            "Install it with `pip install 'agentnexus[e2b]'`, then set "
...
class E2BSandboxLauncher(SandboxLauncher):
    ...
    def __init__(self, *, template: str | None = None, env: Sequence[str] | None = None) -> None:
        ...
        :param template: Optional E2B template NAME (or id) to provision
            ...
            AgentNexus host image was built into (``e2b template build``),
