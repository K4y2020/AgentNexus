...
AgentNexus's exec-model host contract.

The standard ``blaxel/omnigent-host`` image combines the AgentNexus host runtime
...
"""Public Blaxel Hub image containing the AgentNexus host runtime."""

HOST_IMAGE_ENV_VAR: str = "AGENTNEXUS_BLAXEL_HOST_IMAGE"
"""Optional override for the Blaxel-compatible AgentNexus host image."""

SANDBOX_ENV_PASSTHROUGH_ENV_VAR: str = "AGENTNEXUS_BLAXEL_SANDBOX_ENV"
"""Comma-separated server environment variable names to inject."""

REGION_ENV_VAR: str = "AGENTNEXUS_BLAXEL_REGION"
...
_CREATE_ATTEMPT_LABEL = "agentnexus-create-attempt"
...
def _ensure_sdk() -> None:
    ...
    try:
        ...
        raise click.ClickException(
            ...
            "Install it with `pip install 'agentnexus[blaxel]'`, then set "
...
class BlaxelSandboxLauncher(ExecModelHostLauncher):
    """Run AgentNexus hosts in Blaxel sandboxes."""
        ...
        for name in names:
            if name in _CONTROL_CREDENTIAL_ENV_VARS:
                raise click.ClickException(
                    ...
                    "keep provider credentials in the AgentNexus server process only."
                )
    ...
    def keep_alive(self, sandbox_id: str) -> None:
        ...
        del sandbox_id
        click.echo("  → the AgentNexus host process keeps the Blaxel sandbox active")
    ...
    def run_background(
        ...
        *,
        ...
        name = f"agentnexus-host-{uuid.uuid4().hex[:12]}"
        ...
        if status in _TERMINAL_PROCESS_STATUSES:
            raise click.ClickException(
                f"AgentNexus host process failed to start on Blaxel sandbox '{sandbox_id}'."
            )
    ...
    def wheel_install_command(self, remote_tgz_path: str) -> str:
        """Overlay local wheels onto the prebuilt AgentNexus host image."""
