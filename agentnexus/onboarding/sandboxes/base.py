"""
Provider-agnostic interface for running AgentNexus hosts in remote sandboxes.
...
Injected host config uses the loader's ``AGENTNEXUS_CONFIG_HOME`` resolution,
...
from agentnexus.host import HOST_FATAL_EXIT_CODE, HOST_SIGTERM_EXIT_CODE
from agentnexus.host.identity import HOST_ID_ENV_VAR, HOST_NAME_ENV_VAR, HOST_TOKEN_ENV_VAR
from agentnexus.onboarding.sandboxes import types as _sandbox_types
...
config_dir = config_home if config_home else os.path.join(os.path.expanduser("~"), ".agentnexus")
...
def render_host_config_write_command(host_config: dict[str, object]) -> str:
    ...
    is ``$AGENTNEXUS_CONFIG_HOME`` when truthy, otherwise ``~/.agentnexus``, exactly
...
def supervise_host_command(command: str) -> str:
    ...
    islo's preserved-daemon stop matches ``"agentnexus host"`` against full argv,
    ...
    :param command: The host launch, e.g. ``"AGENTNEXUS_HOST_TOKEN=… omnigent
    ...
    return (
        ...
        "  attempt=$((attempt + 1))\n"
        '  echo "agentnexus host exited ($rc); attempt $attempt; '
...
class SandboxExecTransport(SandboxLifecycle):
    ...
    def exec_foreground(self, sandbox_id: str, command: str) -> int:
        ...
        :param sandbox_id: Target sandbox.
        :param command: Shell command to execute remotely, e.g.
            ``"agentnexus host --server https://… --profile oss"``.
...
class ExecModelHostLauncher(SandboxHostLauncher, SandboxExecTransport):
    ...
    def start_host(
        ...
        *host_config* into ``~/.agentnexus/config.yaml``, and start the host
