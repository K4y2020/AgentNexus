---
name: build-agentnexus
description: Patterns and templates for generating valid AgentNexus agent directories. Load when ready to create files.
---

# AgentNexus Agent Generation
...
that `agentnexus server` uses. If your environment exposes the
...
**`agentnexus-knowledge`** skill if you need the deeper field reference
...
- `guardrails` — runtime policy gates (see `agentnexus-knowledge`).

## Step 2a: Choose an executor

`executor.type` must be one of **`claude_sdk`**, **`agents_sdk`**, or
**`agentnexus`**. There is **no `llm` executor** — do not use it.
...
| A CLI/coding harness, shell + file tools, sub-agents | `agentnexus` + a `config.harness` |

When `executor.type: agentnexus`, **`config.harness` is required** and must
...
only when asked; see `agentnexus-knowledge` for `executor.model` / `auth`.
...
Use the `agentnexus` executor with a coding harness when the agent needs to
...
The **parent** needs the `agentnexus` executor — that's what provides the spawn
...
executor:            # any executor works here — only the parent needs agentnexus
  type: agentnexus
  config:
    harness: claude-sdk
instructions: |
...
- [ ] `executor.type` is one of `claude_sdk`, `agents_sdk`, `agentnexus`
- [ ] If `executor.type: agentnexus`, `executor.config.harness` is set to a
      valid harness
- [ ] `instructions` (or `prompt`) points to a file that exists or is
      inline text
- [ ] When declaring `tools.agents`, the parent uses `executor.type:
      agentnexus`, and each entry is the declared `name` of a sub-agent under
