---
name: agentnexus-knowledge
description: Deep reference on AgentNexus config format, executor types, skill/tool structure, and conventions. Load when you need to look up how the platform works.
---

# AgentNexus Knowledge Base

## What is AgentNexus?

AgentNexus is a server that hosts, manages, and executes agents via an
...
executor:
  # REQUIRED area. type must be one of: claude_sdk | agents_sdk | agentnexus.
  ...
  # type: agentnexus     — subprocess harness; requires config.harness below

  # Only for type: agentnexus — pick the harness that runs the loop.
...
| `agentnexus` | Coding/CLI harnesses, shell + file tools, sub-agents | Spawns a subprocess harness selected by `config.harness` |

**There is no `llm` executor type** — the only valid values are
`claude_sdk`, `agents_sdk`, and `agentnexus`. For **most new simple agents**,
use `claude_sdk` (or `agents_sdk`) — in-process, no extra config. Use
`agentnexus` when the agent needs a specific harness, shell/file access, or
...
prompt; AgentNexus may append framework-owned lifecycle or metadata instructions
...
from agentnexus.tools import tool
...
`executor.type: agentnexus` — that's what provides the spawn tools. Each
sub-agent is a full agent and may use any executor (`claude_sdk`,
`agents_sdk`, or `agentnexus`).
