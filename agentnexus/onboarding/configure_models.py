...
from agentnexus.onboarding.interactive import ACCENT, console
...
def key_providers() -> list[str]:
    ...
    :returns: Catalog provider names, e.g. ``["openai", "anthropic",
        "openrouter", ...]``.
    """
    from agentnexus.onboarding.providers import get_all_providers
...
def add_menu_options() -> list[AddOption]:
    ...
    return [
        ...
        _opt(
            "Pi — original auth",
            "Use Pi's own auth (~/.pi/agent) as-is, without AgentNexus managing the provider.",
            SUBSCRIPTION_KIND,
            cli="pi",
        ),
...
def render_provider_listing_by_harness(
    ...
    """
    from agentnexus.onboarding.provider_config import provider_families
...
def _subscription_auth_summary(entry: ProviderEntry) -> str:
    ...
    """
    from agentnexus.onboarding.ambient import codex_cli_effective_auth_mode
