...
from agentnexus.harness_install_spec import HarnessInstallSpec, SetupStep
from agentnexus.onboarding.provider_config import ANTHROPIC_FAMILY, GEMINI_FAMILY, OPENAI_FAMILY
from agentnexus.opencode_native_client import (
...
# ``~/.config/goose/config.yaml``) with no AgentNexus-managed credential, and ships
...
# AgentNexus-managed credentials). The ``hermes`` binary must be on PATH.
HERMES_KEY = "hermes"
...
_HARNESS_INSTALL: dict[str, HarnessInstallSpec] = {
    ...
    ANTHROPIC_FAMILY: HarnessInstallSpec(
        ...
        # The native bridge injects AgentNexus's MCP relay via `--mcp-config`;
...
def _all_harness_install() -> dict[str, HarnessInstallSpec]:
    from agentnexus.harness_plugins import install_specs
...
def _all_harness_name_to_key() -> dict[str, str]:
    from agentnexus.harness_plugins import harness_install_keys
