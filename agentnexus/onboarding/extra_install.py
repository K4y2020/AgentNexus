...
* **``uv tool``** — the package lives in an isolated tool environment that
  ...
  extra via ``uv tool install --with "agentnexus[extra]" omnigent --force``; a
  git-source install (``uv tool install git+…``) instead reinstalls from that
  same source (``uv tool install --force "agentnexus[extra] @ git+…"``) so the
  tool is not silently re-pulled from PyPI.
* **``uv`` (non-tool)** — ``uv pip install --python <sys.executable>
  "agentnexus[extra]"`` pins the install to the running interpreter.  The bare
  ...
  active.
* **``pip`` / fallback** — ``<sys.executable> -m pip install "agentnexus[extra]"``
...
def _installed_vcs_url() -> str | None:
    ...
    target.
    """
    from agentnexus.update_check import _read_installed_wheel_info
...
def extra_install_command(extra: str) -> list[str]:
    ...
    if _is_uv_tool_install():
        ...
        return ["uv", "tool", "install", "--with", target, "agentnexus", "--force"]
