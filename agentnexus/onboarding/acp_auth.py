...
commands in a dedicated top-level ``acp:`` block of ``~/.agentnexus/config.yaml``::
...
here. Auth is each agent's own — AgentNexus stores no credential, so unlike the
...
class AcpAgentEntry:
    ...
    :param omnigent_mcp: Lend AgentNexus's builtin MCP relay in ``session/new``.
...
def acp_agents(config: dict[str, object] | None = None) -> list[AcpAgentEntry]:
    ...
    :param config: A pre-loaded config mapping; ``None`` loads
        ``~/.agentnexus/config.yaml`` via :func:`load_config`.
...
def acp_agents_settings(entries: list[AcpAgentEntry]) -> dict[str, object]:
    ...
    for e in entries:
        ...
        if not e.agentnexus_mcp:
            item["agentnexus_mcp"] = False
