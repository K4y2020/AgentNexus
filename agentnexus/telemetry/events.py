...
class SessionCreatedEvent:
    ...
    :param session_id: AgentNexus conversation/session identifier (goes into
        ``params``).
...
class SessionStoppedEvent:
    ...
    :param session_id: AgentNexus conversation/session identifier.
    :param anon_user_id: First 16 hex chars of ``sha256("<installation_id>:<user_id>")``.
    """
...
class PolicyRegisteredEvent:
    ...
    :param handler: Registered handler path (e.g.
        ``"agentnexus.policies.blast_radius"``) or HTTPS endpoint URL for
        ``type="url"`` policies.
