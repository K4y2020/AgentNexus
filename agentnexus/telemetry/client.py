...
server run — it is NOT the AgentNexus conversation id (which goes in
...
On startup a daemon thread fetches a JSON config from a versioned URL::
    ...
    https://config.agentnexus-telemetry.io/{version}.json          (prod)
    https://config-staging.agentnexus-telemetry.io/{version}.json  (dev/pre-release)

The config shape::

    {
        "agentnexus_version": "0.5.0",
...
_CONFIG_URL_PROD = "https://config.agentnexus-telemetry.io"
_CONFIG_URL_STAGING = "https://config-staging.agentnexus-telemetry.io"
...
def _config_url() -> str:
    ...
    ``AGENTNEXUS_TELEMETRY_CONFIG_URL`` overrides for local testing.
    Dev/pre-release versions use the staging URL.
    """
    override = os.environ.get("AGENTNEXUS_TELEMETRY_CONFIG_URL", "").strip()
    if override:
...
def _fetch_remote_config() -> TelemetryConfig | None:
    ...
    try:
        ...
        if cfg.get("agentnexus_version") not in (VERSION, "default"):
...
def _config_telemetry_disabled() -> bool:
    ...
    Reads ``~/.agentnexus/config.yaml`` (honouring ``AGENTNEXUS_CONFIG_HOME``).
...
def is_disabled() -> bool:
    ...
    Checks (in order):
    1. ``AGENTNEXUS_ANALYTICS=0``
    2. ``DISABLE_TELEMETRY=true`` or ``AGENTNEXUS_DISABLE_TELEMETRY=true``
    3. ``DO_NOT_TRACK=1``
    4. Any CI environment variable from :data:`_CI_ENV_VARS`
    5. ``telemetry: false`` in ``~/.agentnexus/config.yaml``
...
def _compute_is_disabled() -> bool:
    ...
    if os.environ.get("AGENTNEXUS_ANALYTICS", "").strip() == "0":
        return True
    for var in ("DISABLE_TELEMETRY", "AGENTNEXUS_DISABLE_TELEMETRY"):
    ...
    def _ensure_started(self) -> None:
        ...
        with self._lock:
            ...
            self._config_thread = threading.Thread(
                ...
                name="AgentNexusTelemetryConfig",
                daemon=True,
            )
            ...
            self._thread = threading.Thread(
                ...
                name="AgentNexusTelemetryConsumer",
                daemon=True,
            )
            self._thread.start()
            self._started = True
...
def init_client(*, config: Mapping[str, object] | None = None) -> None:
    ...
    try:
        from agentnexus.telemetry.installation_id import get_installation_id
