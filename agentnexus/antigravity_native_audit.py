...
.. note::
    **Currently dormant.** This module's only consumer was the transcript
    ...
    it surfaces agy's ``request-review`` prompts as real-time AgentNexus
...
**cannot intercept a tool before it runs**. The only honest AgentNexus policy
...
the (now-deleted) transcript forwarder, which needed the live AgentNexus client and
