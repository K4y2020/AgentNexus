"""Shared process logging setup for AgentNexus entrypoints."""
...
DATA_DIR_ENV_VAR = "AGENTNEXUS_DATA_DIR"
LOG_LEVEL_ENV_VAR = "AGENTNEXUS_LOG_LEVEL"
LOG_TO_STDERR_ENV_VAR = "AGENTNEXUS_LOG_TO_STDERR"
LOG_FORCE_COLOR_ENV_VAR = "AGENTNEXUS_LOG_FORCE_COLOR"
PROCESS_LOG_FILE_ENV_VAR = "AGENTNEXUS_PROCESS_LOG_FILE"
LOG_TTY_FD_ENV_VAR = "AGENTNEXUS_LOG_TTY_FD"
...
def short_logger_name(name: str) -> str:
    ...
    for prefix in ("agentnexus.", "agentnexus_ui_sdk."):
...
def log_record_display_fields(
    ...
    *,
    ...
    """Temporarily add AgentNexus display columns to a log record."""
...
def data_dir() -> Path:
    ...
    return Path(value).expanduser() if value else Path.home() / ".agentnexus"
...
def display_log_path(path: Path) -> str:
    ...
    :param path: Absolute path, typically under the runtime data dir, e.g.
        ``Path("/Users/alice/.agentnexus/logs/runner/runner-ab12.log")``.
    ...
    try:
        ...
        # Not under $HOME (e.g. an AGENTNEXUS_DATA_DIR outside home), or no
...
def effective_log_level(default: str = "INFO") -> int:
    """Resolve the effective numeric logging level from ``AGENTNEXUS_LOG_LEVEL``."""
...
def current_process_log_path() -> Path | None:
    ...
    spawning parent published in ``AGENTNEXUS_PROCESS_LOG_FILE`` so callers
    work before logging is configured.

    :returns: Absolute log path, e.g.
        ``Path("/Users/alice/.agentnexus/logs/runner/runner-conv_ab12.log")``,
...
def process_log_dir_reference(destination: str) -> str:
    ...
    ``AGENTNEXUS_DATA_DIR`` instead of assuming the default tree. Use this
...
def process_log_reference(destination: str) -> str:
    ...
    :returns: A display path, e.g.
        ``"~/.agentnexus/logs/runner/runner-conv_ab12-20260806-101500.log"``,
        or ``"~/.agentnexus/logs/runner/"`` when no log file is configured.
    """
...
def terminal_supports_color() -> bool:
    ...
    # AgentNexus-owned mirrors (omnidev panes) may force ANSI; otherwise NO_COLOR wins.
...
def configure_process_logging(
    ...
    *,
    ...
    logger_names: Sequence[str] = ("agentnexus",),
    root: bool = True,
    force: bool = False,
) -> Path:
    ...
    from agentnexus.debug_logging import attach_debug_log_sink
