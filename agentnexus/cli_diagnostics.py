...
class CliLogContext:
    ...
    :param path: Absolute path to the current invocation's log file,
        e.g. ``~/.agentnexus/logs/cli-20260518-143012-12345-a1b2c3.log``.
...
def _log_dir() -> Path:
    ...
    Uses the shared AgentNexus runtime data dir so ``AGENTNEXUS_DATA_DIR``
...
def setup_cli_logging(argv: list[str]) -> CliLogContext:
    ...
    if env_truthy(os.environ.get("AGENTNEXUS_LOG_TO_STDERR")) and sys.stderr.isatty():
    ...
    # the file handler.
    for name in ("agentnexus", "agentnexus_ui_sdk"):
    ...
    root = logging.getLogger("agentnexus.cli_diagnostics")
...
def install_asyncio_exception_handler(loop: asyncio.AbstractEventLoop) -> None:
    ...
    :param loop: The running event loop (typically from
        ...
        context).
    """
    logger = logging.getLogger("agentnexus.asyncio")
...
def print_stale_host_hint() -> None:
    ...
    print(
        ...
        f"may be the cause. Run `{cli_invocation()} stop` to stop existing AgentNexus "
...
def log_cli_exception(exc: BaseException, *, prefix: str = "CLI error") -> None:
    ...
    if current_cli_log_path() is None:
        return
    logging.getLogger("agentnexus.cli_diagnostics").error(
        "%s: %s",
        prefix,
        exc,
