...
if TYPE_CHECKING:
    ...
    from agentnexus.onboarding.openclaw_config import OpenClawDiscovery, SourceKind
...
def _isolated_databricks_cfg() -> collections.abc.Generator[None, None, None]:
    ...
    import agentnexus.onboarding.internal_beta as internal_beta  # type: ignore[import-not-found]
    from agentnexus.onboarding.setup import CONFLICTING_ENV_VARS
    ...
    omnigent_dir = Path.home() / ".agentnexus"
...
def _run_configure_databricks() -> None:
    ...
    AgentNexus reads ``~/.ucode/state.json`` to pick per-harness model
    ...
    click.echo("ucode configuration complete. AgentNexus will use state.json for harness setup.")
...
def _print_credentials_by_harness() -> None:
    ...
    :returns: None. Side effect: writes the listing to the onboarding
        console.
    """
    from agentnexus.onboarding.configure_models import render_provider_listing_by_harness
...
def _existing_key_name_for_ref(  # type: ignore[explicit-any]  # config is a yaml-boundary mapping
    ...
    """
    from agentnexus.onboarding.provider_config import KEY_KIND, load_providers
...
def _configure_harness_add(family: str | None = None) -> str | None:
    ...
    :returns: A confirmation message for the caller to show as a transient
        status. Side effect: writes to ``~/.agentnexus/config.yaml`` and,
        for a pasted API key, the secret store.
    """
    from agentnexus.onboarding import secrets as secret_store
    from agentnexus.onboarding.ambient import detect_providers
    ...
    from agentnexus.onboarding.interactive import console, prompt_text, select
    from agentnexus.onboarding.provider_config import (
        ANTHROPIC_FAMILY,
        BEDROCK_KIND,
        CHAT_WIRE_API,
    ...
    if kind == CLI_CONFIG_KIND:
        ...
        from agentnexus.onboarding.configure_models import cli_display_name
        ...
        if cli_name in _CLI_LOGIN_BRAND:
            from agentnexus.onboarding.harness_install import harness_install_spec, harness_login
        ...
        # the one place AgentNexus triggers a Databricks CLI / ucode login;
        ...
        from agentnexus.onboarding.databricks_config import normalize_workspace_url
        from agentnexus.onboarding.interactive import clear_screen
        from agentnexus.onboarding.setup import login_databricks_workspace
        from agentnexus.onboarding.ucode_setup import (
...
def _promote_global_auth_to_provider() -> str | None:
    ...
    """
    from agentnexus.onboarding.configure_models import build_databricks_provider_entry
    from agentnexus.onboarding.provider_config import (
        load_providers,
        provider_entry_settings,
        provider_families,
...
def _claude_managed_gateway_label() -> str | None:
    ...
    """
    from agentnexus.onboarding.ambient import claude_managed_gateway_display_name
...
def _compact_credential_label(det: DetectedProvider) -> str:
    ...
    """
    from agentnexus.onboarding.ambient import SUBSCRIPTION_KIND
...
def _announce_auto_configured_credentials(adopted: list[str]) -> None:
    ...
    """
    from agentnexus.onboarding.ambient import detect_providers
    from agentnexus.onboarding.interactive import console
...
def _harness_credential_rows(config: dict[str, Any], family: str) -> list[_HarnessMenuRow]:  # type: ignore[explicit-any]
    ...
    """
    from agentnexus.onboarding.configure_models import kind_glyph
    from agentnexus.onboarding.provider_config import (
        load_providers,
        provider_families,
        surface_default_provider,
...
def _prompt_install_harness(family: str) -> bool:
    ...
    """
    from agentnexus.onboarding.configure_models import family_label
    from agentnexus.onboarding.harness_install import (
        harness_cli_version,
        harness_install_display,
        install_harness_cli,
    )
    from agentnexus.onboarding.interactive import console, select
...
def _manage_harness_providers(family: str) -> None:
    ...
    """
    from agentnexus.onboarding.configure_models import family_label
    from agentnexus.onboarding.harness_install import harness_cli_installed
    from agentnexus.onboarding.interactive import select
...
def _prompt_install_cursor() -> str | None:
    ...
    from agentnexus.onboarding.cursor_auth import CURSOR_EXTRA, install_cursor_sdk
    from agentnexus.onboarding.extra_install import extra_install_display
    from agentnexus.onboarding.interactive import console, select
...
def _manage_cursor_sdk_harness() -> None:
    ...
    ``keychain:``/``env:`` reference in ``~/.agentnexus/config.yaml``).
    ...
    :returns: None. Side effects: may install the ``cursor`` extra, and may
        write the ``cursor:`` block of ``~/.agentnexus/config.yaml`` and the
        secret store.
    """
    from agentnexus.onboarding import secrets as secret_store
    from agentnexus.onboarding.cursor_auth import (
...
def _set_cursor_api_key() -> str | None:
    ...
    from agentnexus.onboarding.interactive import prompt_text
...
def _manage_cursor_harness() -> None:
    ...
    from agentnexus.onboarding.cursor_auth import cursor_api_key_configured
    from agentnexus.onboarding.harness_install import (
        CURSOR_KEY,
        harness_cli_installed,
        harness_cli_logged_in,
    ...
    while True:
        from agentnexus._platform import resolve_cli_binary
...
def _prompt_install_antigravity() -> str | None:
    ...
    from agentnexus.onboarding.antigravity_auth import ANTIGRAVITY_EXTRA, install_antigravity_sdk
    from agentnexus.onboarding.extra_install import extra_install_display
    from agentnexus.onboarding.interactive import console, select
...
def _manage_antigravity_harness() -> None:
    ...
    from agentnexus.onboarding.interactive import select
    from agentnexus.onboarding.provider_config import GEMINI_FAMILY
...
def _manage_qwen_harness() -> None:
    ...
    AgentNexus* is deferred (see docs/QWEN_FOLLOWUPS.md, Provider Injection).
...
def _print_goose_auth_help() -> None:
    """Print Goose's configuration options (AgentNexus manages no Goose credential)."""
    from agentnexus.onboarding.interactive import console

    console.print(
        "\n  [bold]Configure Goose[/bold] (AgentNexus stores no Goose credential):\n"
...
def _launch_goose_configure() -> str | None:
    ...
    """
    from agentnexus.onboarding.goose_auth import goose_config_summary
    from agentnexus.onboarding.harness_install import (
        GOOSE_KEY,
        harness_cli_installed,
        harness_install_spec,
...
def _show_acp_cli_harness(name: str) -> None:
    ...
    so there is nothing for AgentNexus to store or run on the user's behalf — the
    ...
    launch = f"agentnexus run --harness {name}"
    if row.model_arg:
...
def _manage_goose_harness() -> None:
    ...
    Goose owns its own auth (keyring / ``~/.config/goose/config.yaml``) — AgentNexus
    ...
    does not store a key through AgentNexus. A missing CLI gates the drill-in
...
def _print_acp_examples() -> None:
    """Print example ACP-agent commands (AgentNexus stores no credential)."""
    from agentnexus.onboarding.interactive import console
    ...
    console.print(
        ...
        "  AgentNexus stores no credential — log into each agent via its own CLI first.\n\n"
...
def _add_acp_agent() -> None:
    ...
    from agentnexus.onboarding.interactive import console, prompt_text
...
def _choose_openclaw_source() -> OpenClawDiscovery | None:
    ...
    from agentnexus.onboarding.interactive import prompt_text, select
...
def _import_openclaw_agents() -> None:
    ...
    from agentnexus.onboarding.acp_auth import acp_agents_settings, command_binary_on_path
    from agentnexus.onboarding.interactive import console
    from agentnexus.onboarding.openclaw_config import (
...
def _manage_acp_agent(slug: str) -> None:
    ...
    A single-shot menu (Remove / Back) — AgentNexus stores no credential, so there
    is nothing else to manage per agent yet.

    :param slug: The agent's slug (see :func:`omnigent.onboarding.acp_auth.slugify`).
    """
    from agentnexus.onboarding.acp_auth import acp_agents, acp_agents_settings
    from agentnexus.onboarding.interactive import console, select
...
def _manage_hermes_harness() -> None:
    ...
    picker) and is installed via a curl script from Nous Research — AgentNexus
...
def _manage_kiro_harness() -> None:
    ...
    Identity Center) and is installed via Kiro's curl installer — AgentNexus stores
...
def _print_kimi_auth_help() -> None:
    ...
    Kimi authenticates against Moonshot AI's backend rather than an AgentNexus
    ...
    AgentNexus stores no kimi credential and cannot thread one per spawn, so all
    ...
    console.print(
        ...
        '            api_key = "sk-…"[/dim]\n'
        "    • AgentNexus stores no kimi credential and cannot thread one per "
...
def _manage_kimi_harness() -> None:
    ...
    from agentnexus.onboarding.interactive import console, select
    from agentnexus.onboarding.kimi_auth import kimi_auth_configured
...
def _prompt_install_copilot() -> str | None:
    ...
    from agentnexus.onboarding.copilot_auth import COPILOT_EXTRA, install_copilot_sdk
    from agentnexus.onboarding.extra_install import extra_install_display
    from agentnexus.onboarding.interactive import console, select
...
def _manage_copilot_harness() -> None:
    ...
    :returns: None. Side effects: may install the ``copilot`` extra, and may
        write the ``copilot:`` block of ``~/.agentnexus/config.yaml`` and the
        secret store.
    """
    from agentnexus.onboarding import secrets as secret_store
    ...
    while True:
        ...
        if action == "set_key":
            ...
            from agentnexus.onboarding.copilot_auth import copilot_github_host_settings
...
def _set_copilot_github_host() -> str | None:
    ...
    from agentnexus.onboarding.copilot_auth import copilot_github_host
...
def _manage_credential(provider: str, family: str) -> str | None:
    ...
    # harness configs outside ~/.agentnexus/config.yaml — so removing it
...
def _remove_subscription(provider: str, family: str) -> str | None:
    ...
    :returns: A confirmation message for the level-2 status line, or ``None``
        ...
        harness logout command and writes ``~/.agentnexus/config.yaml``.
    """
    from agentnexus.onboarding.harness_install import harness_install_spec, harness_logout
    from agentnexus.onboarding.interactive import select
...
def _remove_databricks_provider(provider: str) -> str:
    ...
    ``~/.agentnexus/config.yaml`` — most damagingly, for Codex < 0.134.0 it
    ...
    :returns: A confirmation message for the level-2 status line reporting
        ...
        ``claude mcp remove``, and write ``~/.agentnexus/config.yaml``.
    """
    from agentnexus.errors import AgentNexusError
    from agentnexus.onboarding.ucode_cleanup import remove_ucode_wiring

    cleanup_note = ""
    try:
        removal = remove_ucode_wiring()
    except (AgentNexusError, OSError) as exc:
...
def _set_harness_default(provider: str, family: str) -> str | None:
    ...
    :returns: A confirmation message for the caller to show as a transient
        ...
        writes ``~/.agentnexus/config.yaml``.
    """
    from agentnexus.onboarding.configure_models import family_label
    from agentnexus.onboarding.provider_config import load_providers, set_default_provider
...
def _clear_detection_dismissal(name: str) -> None:
    ...
    :returns: None. Side effect: writes ``~/.agentnexus/config.yaml`` when the
...
def _remove_credential(provider: str) -> str | None:
    ...
    :returns: A confirmation message for the caller to show as a transient
        ...
        writes ``~/.agentnexus/config.yaml`` (and, when the removed entry is
...
def _launch_opencode_auth_login() -> str | None:
    ...
    from agentnexus.onboarding.interactive import console
    from agentnexus.onboarding.opencode_auth import opencode_auth_summary
...
def _run_opencode_auth_list() -> None:
    ...
    from agentnexus.onboarding.harness_install import OPENCODE_KEY, harness_install_spec
...
def _set_opencode_default_model(current: str | None) -> str | None:
    ...
    """
    from agentnexus.onboarding.interactive import console, select
    from agentnexus.onboarding.opencode_auth import reachable_provider_ids
...
def _print_opencode_auth_help() -> None:
    ...
    console.print(
        ...
        "      AgentNexus synthesizes opencode's per-session provider config from it.\n"
        "  AgentNexus stores no OpenCode credential of its own.\n"
...
def _manage_opencode_harness() -> None:
    ...
    AgentNexus. (For the Databricks-gateway path the agent's ``profile`` is
    ...
    # ambient env keys; AgentNexus stores nothing. Report what's reachable and
...
def _run_configure_harnesses_interactive() -> None:
    ...
    :returns: None. Side effect: may write ``~/.agentnexus/config.yaml`` via
    ...
    # not an AgentNexus credential), so it dispatches to its own drill-in.
    _QWEN = "\x00qwen"
    # Sentinel marking the OpenCode row — native-server harness with no AgentNexus
    ...
    # AgentNexus credential), so it dispatches to its own drill-in.
    _GOOSE = "\x00goose"
    ...
    def _install_hint(command: str) -> str:
        ...
        # (e.g. ``pip install "agentnexus[cursor]"``) renders literally instead of
    ...
    def build_harness_rows() -> list[tuple[str, str, str, str, str]]:
        ...
        from agentnexus.onboarding.hermes_auth import hermes_config_summary
        from agentnexus.onboarding.opencode_auth import opencode_auth_summary
        ...
        from agentnexus._platform import resolve_cli_binary
        from agentnexus.acp_cli_harnesses import ACP_CLI_HARNESSES
        from agentnexus.onboarding.acp_auth import acp_agents, shadowed_builtin_acp_rows
        ...
        from agentnexus.onboarding.acp_auth import acp_config_summary
        from agentnexus.onboarding.openclaw_config import (
