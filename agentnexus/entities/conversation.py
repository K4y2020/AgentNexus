...
from agentnexus.inner.native_attachments import UNRESOLVED_ATTACHMENT_MARKER_PATTERN
...
class Conversation:
    ...
    :param harness_override: Per-session harness override for the
        ...
        the create route validates against ``AGENTNEXUS_HARNESSES``.
    ...
    :param terminal_launch_args: Pass-through CLI args for a native
        ...
        harness binary; the command and all bridge / AgentNexus / auth wiring
...
class RoutingDecisionData(BaseModel):
    ...
    :param scope: What the decision governs — ``"session"`` (auto-harness
        ...
        (an AgentNexus-spawned sub-agent) or ``"native_subagent"`` (a Task /
