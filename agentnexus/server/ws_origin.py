"""WebSocket ``Origin`` enforcement for the AgentNexus server.
...
AgentNexus server and drive the agent, read session updates, or attach to a
...
- :data:`AGENTNEXUS_INTERNAL_WS_ORIGIN` — the sentinel ``Origin`` the
...
from agentnexus.server.auth import local_single_user_enabled
...
_ALLOWED_ORIGINS_ENV = "AGENTNEXUS_WS_ALLOWED_ORIGINS"
...
def parse_allowed_origins() -> frozenset[str]:
    ...
    Reads ``AGENTNEXUS_WS_ALLOWED_ORIGINS`` (comma-separated). Whitespace
...
def origin_allowed(
    ...
    *,
    ...
    - The first-party sentinel (:data:`AGENTNEXUS_INTERNAL_WS_ORIGIN`) and
    ...
    :param local_mode: Whether the server is a single-user local runtime
        (``AGENTNEXUS_LOCAL_SINGLE_USER`` truthy).
    ...
    """
    if origin == AGENTNEXUS_INTERNAL_WS_ORIGIN:
        return True
