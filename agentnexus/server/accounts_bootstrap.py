...
Run once at server startup when ``AGENTNEXUS_AUTH_PROVIDER=accounts``.
...
1. **Password flag** — ``--admin-password`` /
   ``AGENTNEXUS_ACCOUNTS_INIT_ADMIN_PASSWORD``. Headless / CI. Bootstrap
...
from agentnexus.server.auth import _RESERVED_USERS
...
def resolve_admin_username() -> str:
    ...
    1. ``AGENTNEXUS_ACCOUNTS_INIT_ADMIN_USERNAME`` env var — explicit
    ...
    """
    explicit = os.environ.get("AGENTNEXUS_ACCOUNTS_INIT_ADMIN_USERNAME", "").strip()
    if explicit:
...
def _mint_loopback_cli_token(
    ...
    *,
    ...
    ``~/.agentnexus/auth_tokens.json`` keyed by the **exact** server
...
def bootstrap_admin(
    ...
    *,
    ...
    :param init_admin_password: Optional operator-provided
        password from ``AGENTNEXUS_ACCOUNTS_INIT_ADMIN_PASSWORD``.
    ...
    :param session_ttl_hours: TTL for the CLI session token
        written to ``~/.agentnexus/auth_tokens.json``.
    ...
    if any(u.has_password for u in account_store.list_users()):
        ...
        if init_admin_password:
            ...
            print(
                ...
                "    (--admin-password / AGENTNEXUS_ACCOUNTS_INIT_ADMIN_PASSWORD).\n"
    ...
    #      AGENTNEXUS_ACCOUNTS_INIT_ADMIN_PASSWORD — headless / CI;
