...
``AGENTNEXUS_FEATURES``. The set is resolved once when an application or route
...
FEATURES_ENV_VAR = "AGENTNEXUS_FEATURES"
_REMOVED_HARNESS_INSTALL_ENV_VAR = "AGENTNEXUS_HARNESS_INSTALL_ENABLED"


class Feature(StrEnum):
    """Canonical release-feature keys accepted by ``AGENTNEXUS_FEATURES``."""
...
def resolve_feature_flags(environ: Mapping[str, str] | None = None) -> FeatureFlags:
    """Resolve ``AGENTNEXUS_FEATURES`` into an immutable feature snapshot.
