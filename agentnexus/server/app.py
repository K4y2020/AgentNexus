...
from agentnexus._platform import resolve_repo_symlink
...
from agentnexus.coordination.workflow_auto_advance import WorkflowAutoAdvancer
...
from agentnexus.resources import examples as _examples_resources
...
from agentnexus.server.feature_flags import Feature, FeatureFlags, resolve_feature_flags
from agentnexus.server.managed_hosts import ManagedSandboxDeployment
from agentnexus.server.mcp_pool import ServerMcpPool
...
from agentnexus.server.routes.builtin_agents import create_builtin_agents_router
from agentnexus.server.routes.comments import create_comments_router
...
from agentnexus.server.routes.default_policies import create_default_policies_router
from agentnexus.server.routes.diagnostics import create_diagnostics_router
from agentnexus.server.routes.dictation import create_dictation_router
from agentnexus.server.routes.gateways import create_gateways_router
from agentnexus.server.routes.harnesses import create_harnesses_router
from agentnexus.server.routes.imports import create_imports_router
from agentnexus.server.routes.policy_registry import create_policy_registry_router
...
from agentnexus.server.routes.scheduled_tasks import create_scheduled_tasks_router
from agentnexus.server.routes.session_mcp_servers import create_session_mcp_servers_router
from agentnexus.server.routes.session_policies import create_session_policies_router
...
from agentnexus.server.routes.sharing import create_sharing_router
...
from agentnexus.server.routes.usage import create_usage_router
from agentnexus.server.runner_session_init import RunnerSessionInitializer
from agentnexus.server.scheduled import ScheduledTaskScheduler
from agentnexus.server.ws_origin import WebSocketOriginMiddleware
from agentnexus.stores import (
    AgentMemoryStore,
    AgentStore,
    ArtifactStore,
    BotStore,
...
from agentnexus.stores.comment_store import CommentStore
from agentnexus.stores.conversation_store import SessionConnectivity, runner_seen_is_fresh
from agentnexus.stores.host_store import HostStore
from agentnexus.stores.permission_store import PermissionStore
from agentnexus.stores.policy_store import PolicyStore
...
# AGENTNEXUS_WEB_UI_DIST, without rebuilding or repackaging.
_WEB_UI_DIST = Path(
    os.environ.get("AGENTNEXUS_WEB_UI_DIST") or (Path(__file__).parent / "static" / "web-ui")
)
...
_EXTRA_BUILTIN_AGENTS_ENV = "AGENTNEXUS_BUILTIN_AGENT_DIRS"
...
# name the role.
_ACP_AGENT_PROMPT = (
    "You are a coding agent running inside AgentNexus through the Agent Client "
...
def create_app(
    ...
    :param allowed_domains: Allowed email domains from the server
        ...
        Union'd with ``AGENTNEXUS_OIDC_ALLOWED_DOMAINS`` and the
    ...
    :param sharing_mode: Server policy for creating new session
        ...
        the ``AGENTNEXUS_SHARING_MODE`` env var
    ...
    :param feature_flags: Optional immutable release-feature snapshot.
        When omitted, resolves the comma-separated ``AGENTNEXUS_FEATURES``
    ...
    :param public_sharing: Whether public (anyone-with-the-link) read
        ...
        ``AGENTNEXUS_PUBLIC_SHARING`` env var (enabled unless explicitly
    ...
    if auth_provider is not None and account_store is not None:
        ...
        if isinstance(auth_provider, UnifiedAuthProvider) and auth_provider._source == "accounts":
            from agentnexus.server.accounts_bootstrap import bootstrap_admin
    ...
    async def _lifespan(
        ...
        ``designs/AGENTNEXUS_TERMINAL_BRIDGE.md`` §4.4.
        ...
        from agentnexus.telemetry import init_client as _init_telemetry

        _init_telemetry(config=server_config)

        # Apply AGENTNEXUS_LOG_LEVEL to the omnigent namespace after
        # uvicorn's dictConfig runs (dictConfig resets existing handlers,
        # making a pre-run basicConfig call ineffective).
        import os as _os

        _log_level_name = _os.environ.get("AGENTNEXUS_LOG_LEVEL", "INFO").upper()
        logging.getLogger("agentnexus").setLevel(getattr(logging, _log_level_name, logging.INFO))
        ...
        from agentnexus.runtime import set_resource_registry
        ...
        from agentnexus.server._runner_ws_tunnel import (
        ...
        from agentnexus.policies.registry import load_registry
        ...
        # passes AGENTNEXUS_ACCOUNTS_AUTO_OPEN=0 for --no-open). Broad
        # try so a missing display / browser never blocks startup.
        if _bootstrap_result is not None and _bootstrap_result.open_url:
            from agentnexus.server.auth import env_var_is_truthy

            if env_var_is_truthy("AGENTNEXUS_ACCOUNTS_AUTO_OPEN", default=True):
                import webbrowser
    ...
    app = FastAPI(title="AgentNexus Server", lifespan=_lifespan)
    ...
    # change (no restart).
    from agentnexus.server.admin_list import load_admin_list
    ...
    # endpoint.
    #
    # ``None`` (the OSS default): ``AGENTNEXUS_SHARING_MODE`` sets the boot
    ...
    if sharing_mode is None:
        from agentnexus.server.sharing_settings import read_sharing_mode_override

        _sharing_env_default = SharingMode.coerce(os.environ.get("AGENTNEXUS_SHARING_MODE"))
    ...
    # above and independent of it. ``None`` reads ``AGENTNEXUS_PUBLIC_SHARING``
    ...
    from agentnexus.server.managed_hosts import ManagedLaunchTracker
    ...
    async def _handle_omnigent_error(
        ...
        """
        _logger.warning(
            "AgentNexusError on %s %s: status=%s code=%s message=%s",
            request.method,
            request.url.path,
            exc.http_status,
            exc.code,
    ...
    async def info() -> ServerInfoResponse:
        ...
        """
        from agentnexus.server.auth import UnifiedAuthProvider, local_single_user_enabled
        ...
        # (AGENTNEXUS_LOCAL_SINGLE_USER=1, set by the managed local spawn paths).
        ...
        databricks_features = (
            importlib.util.find_spec("agentnexus.onboarding.sandboxes.lakebox") is not None
        )
        ...
        try:
            ...
            from agentnexus.server.routing_backend import routing_available, routing_sources
        ...
        from agentnexus.server.dictation import engine_availability
    ...
    async def _on_runner_exited(runner_id: str, error: str) -> None:
        ...
        """
        from agentnexus.server.routes.sessions import _mark_runner_sessions_offline
        from agentnexus.server.schemas import ErrorDetail
    ...
    if auth_provider is not None and getattr(auth_provider, "login_url", None):
        ...
        if (
            ...
            from agentnexus.server.routes.accounts_auth import (
            ...
            # OIDC invites are opt-in (AGENTNEXUS_OIDC_ALLOW_INVITES) and
        ...
        # AGENTNEXUS_DEVICE_GRANT_ENABLED, and accounts-mode only (the
        ...
        if (
            env_var_is_truthy("AGENTNEXUS_DEVICE_GRANT_ENABLED", default=False)
            ...
            # into AGENTNEXUS_DEVICE_CLIENT_SECRET (which closes initiation to
            # unauthorized callers) rather than leaving it off unknowingly. See
            # designs/DEVICE_AUTH.md § "Device-code phishing".
            if not os.environ.get("AGENTNEXUS_DEVICE_CLIENT_SECRET", "").strip():
                _logger.warning(
                    "device-grant: AGENTNEXUS_DEVICE_CLIENT_SECRET is not set — the "
                    "/oauth/device/authorize endpoint is PUBLIC (any caller may "
                    "initiate a login flow). Set AGENTNEXUS_DEVICE_CLIENT_SECRET on "
