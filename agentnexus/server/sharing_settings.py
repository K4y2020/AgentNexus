...
- the sharing *mode* — ``AGENTNEXUS_SHARING_MODE`` → ``<data_dir>/sharing_mode``
  (``on`` / ``read_only`` / ``restricted_read_only`` / ``off``);
- whether *public* (anyone-with-the-link) read access may be granted —
  ``AGENTNEXUS_PUBLIC_SHARING`` → ``<data_dir>/public_sharing`` (``on`` / ``off``).
...
from agentnexus.server.auth import SharingMode
