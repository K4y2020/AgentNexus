...
def _reject_uploaded_callable_tools(spec: AgentSpec) -> None:
    ...
    :raises AgentNexusError: If any (sub-)agent declares a server-runtime tool
...
def _cwd_escapes_workspace(spec_cwd: str) -> bool:
    ...
    ``AGENTNEXUS_RUNNER_WORKSPACE`` it becomes the agent environment root and
...
def validate_agent_bundle(
    ...
    *,
    ...
    :raises AgentNexusError: If the bundle is invalid, the spec is
    ...
    if enforce_handler_allowlist:
        ...
        # (GHSA-p8rw-8qj3-hf33): on a runner without AGENTNEXUS_RUNNER_WORKSPACE
