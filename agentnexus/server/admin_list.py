...
def resolve_data_dir() -> Path:
    ...
    1. The parent directory of ``AGENTNEXUS_ADMIN_CREDENTIALS_PATH`` if
...
def resolve_admin_list_path() -> Path:
    ...
    :returns: ``AGENTNEXUS_ADMIN_LIST_PATH`` if set, else
        ``<data_dir>/admins`` (see :func:`resolve_data_dir`).
    """
    explicit = os.environ.get("AGENTNEXUS_ADMIN_LIST_PATH", "").strip()
    if explicit:
