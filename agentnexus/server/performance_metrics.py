...
class MeterLike(Protocol):
    ...
    def create_counter(
        ...
        :param name: Metric name, e.g.
            ``"agentnexus.server.http.requests.started"``.
    ...
    def create_gauge(
        ...
        :param name: Metric name, e.g.
            ``"agentnexus.server.http.requests.in_flight"``.
        :param unit: UCUM unit string, e.g. ``"{request}"``.
        :param description: Human-readable metric description.
    ...
    def create_histogram(
        ...
        :param name: Metric name, e.g.
            ``"agentnexus.server.http.request.duration"``.
...
class RequestDurationAccessFormatter(AccessFormatter):
    """
    Uvicorn access formatter that appends AgentNexus request duration.
    ...
    def format(self, record: logging.LogRecord) -> str:
        """Apply AgentNexus's standard source and level display fields."""
...
def _create_request_otel_instruments(meter: MeterLike) -> _RequestOtelInstruments:
    ...
    return _RequestOtelInstruments(
        started=meter.create_counter(
            "agentnexus.server.http.requests.started",
            unit="{request}",
            description="HTTP requests started by the AgentNexus server.",
        ),
        completed=meter.create_counter(
            "agentnexus.server.http.requests.completed",
            unit="{request}",
            description="HTTP requests completed by the AgentNexus server.",
        ),
        failed=meter.create_counter(
            "agentnexus.server.http.requests.failed",
            unit="{request}",
...
def _create_processing_otel_instruments(
    ...
    return _ProcessingOtelInstruments(
        ...
        avg_30s_ms=meter.create_gauge(
            "agentnexus.server.http.request.processing.avg_30s",
            unit="ms",
            description=(
