...
class AccountsConfig:
    ...
    def from_env() -> AccountsConfig:
        """Read and validate every required ``AGENTNEXUS_ACCOUNTS_*`` env var.
        ...
        def _require(name: str) -> str:
            ...
            if not val:
                raise RuntimeError(
                    ...
                    f"(AGENTNEXUS_AUTH_PROVIDER=accounts requires it)"
                )
            return val

        cookie_secret_hex = _require("AGENTNEXUS_ACCOUNTS_COOKIE_SECRET")
        try:
            cookie_secret = bytes.fromhex(cookie_secret_hex)
        except ValueError as exc:
            raise RuntimeError(
                "AGENTNEXUS_ACCOUNTS_COOKIE_SECRET must be a valid hex string"
            ) from exc
        if len(cookie_secret) < 32:
            raise RuntimeError(
                "AGENTNEXUS_ACCOUNTS_COOKIE_SECRET must be at least 32 bytes "
        ...
        init_admin = os.environ.get("AGENTNEXUS_ACCOUNTS_INIT_ADMIN_PASSWORD") or None
