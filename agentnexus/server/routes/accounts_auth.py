...
Mounted at ``/auth`` when ``AGENTNEXUS_AUTH_PROVIDER=accounts``.
...
from agentnexus.server.auth import _RESERVED_USERS, RESERVED_USER_LOCAL, UnifiedAuthProvider
...
def create_accounts_auth_router(
    ...
    async def setup(body: SetupRequest) -> Response:
        ...
        that pre-seeds ``AGENTNEXUS_ACCOUNTS_INIT_ADMIN_PASSWORD`` also
