...
from agentnexus.runtime.policies.builder import invalidate_session_policy_specs_cache
from agentnexus.server.auth import LEVEL_EDIT, LEVEL_READ, AuthProvider
from agentnexus.server.routes._auth_helpers import get_user_id, require_access
...
from agentnexus.spec.types import FunctionPolicySpec, PolicySpec
...
from agentnexus.telemetry.events import PolicyDeletedEvent as _TelPolicyDeletedEvent
from agentnexus.telemetry.events import PolicyRegisteredEvent as _TelPolicyRegisteredEvent
...
def create_session_policies_router(
    ...
    def _require_session_exists(session_id: str) -> None:
        ...
        :param session_id: The session to check, e.g.
            ``"conv_abc123"``.
        :raises AgentNexusError: 404 if the session is not found.
        """
    ...
    async def create_policy(
        ...
        :raises AgentNexusError: 401/403 if the user lacks edit
        ...
        if body.type == "python":
            ...
            if validation_error:
                raise AgentNexusError(validation_error, code=ErrorCode.INVALID_INPUT)
    ...
    async def list_policies(
        ...
        :raises AgentNexusError: 401/403 if the user lacks read
    ...
    async def get_policy(
        ...
        if policy is None:
            raise AgentNexusError("Policy not found", code=ErrorCode.NOT_FOUND)
        return _entity_to_response(policy)
