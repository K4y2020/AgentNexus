...
from agentnexus.stores.host_store import host_is_live
    ...
    *,
    ...
    async def post_event(
        ...
        - ``"external_output_reasoning_delta"`` publishes a transient
          ...
          observed outside the AgentNexus task runtime, without persisting an
          item or starting / steering a task.
        - ``"external_session_interrupted"`` publishes a
          ``session.interrupted`` event observed outside the AgentNexus task
        ...
        async def _wake_bound_runner_for_control(
            ...
            :raises AgentNexusError: ``WRONG_REPLICA`` when the host tunnel is
        ...
        if runner_client is None:
            ...
            if body.type == "message" and _is_native_terminal_session(conv):
                ...
                return {"queued": True, "item_id": item_id}
            # Raise so the AgentNexus server doesn't persist an item the
    ...
    async def delete_session(
        ...
        :raises AgentNexusError: 404 if no session or no access,
