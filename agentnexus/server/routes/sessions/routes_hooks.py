    ...
    *,
    ...
    async def ask_user_question(
        ...
        if body.a2a_request_id is not None:
            from agentnexus.server.routes.coordination import _request_store
    ...
    async def claude_permission_request_hook(
        ...
        the same Bearer token used for every other AgentNexus request. For
        ...
        :param session_id: AgentNexus conversation id from the URL path.
        :returns: Claude PermissionRequest hookSpecificOutput JSON,
            or ``200`` with empty body on timeout (fail-ask).
        :raises AgentNexusError: 404 if the session doesn't exist,
    ...
    async def codex_elicitation_request_hook(
        ...
        :raises AgentNexusError: 404 if the session does not exist,
    ...
    async def antigravity_elicitation_request_hook(
        ...
        :raises AgentNexusError: 404 if the session does not exist, 400 if
    ...
    async def cursor_permission_request_hook(
        ...
        :returns: An ``ElicitationResult`` (``{"action": …}``) on a web
            ...
            disconnect.
        :raises AgentNexusError: 404 if the session does not exist, 400 if the
            body is malformed.
        """
        user_id = _get_user_id(request, auth_provider)
        await _require_access(
    ...
    async def _route_subagent_catalog(session_id: str) -> dict[str, list[str]] | None:
        ...
        """
        from agentnexus.server.smart_routing import fetch_runner_models
    ...
    async def route_subagent_hook(
        ...
        :raises AgentNexusError: 400 when the body is not a JSON object or
            omits ``harness``.
        """
    ...
    async def route_turn_hook(
        ...
        from agentnexus.server.routes._sessions.helpers import _resolve_harness
        from agentnexus.server.routes._sessions.orchestration import (
        ...
        from agentnexus.server.smart_routing import route_turn as _route_turn_seam
