    ...
    *,
    ...
    async def _schedule_managed_launch(
        ...
        *,
        ...
        :raises AgentNexusError: If managed hosts aren't configured or the
    ...
    async def create_session(
        ...
        :raises AgentNexusError: If metadata, bundle, or agent lookup
        ...
        if launch_host_id is not None and resp.runner_id is None:
            ...
            if host_registry is not None and host_store_inst is not None:
                ...
                from agentnexus.server.routes._host_launch import resolve_host_launch
    ...
    async def _create_bundled_session_from_multipart(
        ...
        :raises HTTPException: 422 when a required multipart part is
            absent.
        :raises AgentNexusError: If metadata or bundle validation
    ...
    async def put_read_state(
        ...
        :raises AgentNexusError: 403 if the caller lacks read access.
        """
        user_id = _require_user(request, auth_provider)
    ...
    async def get_session(
        ...
        :raises AgentNexusError: 404 if no session exists.
        """
    ...
    async def session_updates(websocket: WebSocket) -> None:
        ...
        async def _emit_deltas() -> None:
            ...
            from agentnexus.server.routes import sessions as _sf
    ...
    from agentnexus.server.routes.codex.sessions import register_codex_session_routes
    ...
    async def automatically_rename_session(
        ...
        conv = await asyncio.to_thread(conversation_store.get_conversation, session_id)
        if conv is None:
            raise AgentNexusError("Session not found", code=ErrorCode.NOT_FOUND)
    ...
    async def update_session(
        ...
        :raises AgentNexusError: 400 if the runner is not
        ...
        # dispatch — AgentNexus server stays harness-agnostic; the runner
        ...
        if live_forward and (model_override is not None or clear_model):
            ...
            # whose history AgentNexus writes. Gate on the wrapper label (NOT
        ...
        if set_project:
            ...
            if target_project_id == "":
                ...
                if owned is None:
                    raise AgentNexusError("Project not found", code=ErrorCode.NOT_FOUND)
    ...
    async def fork_session(
        ...
        :raises AgentNexusError: 404 if *source_id* does not exist
        ...
        # the copied AgentNexus transcript on its own — mark the fork so the
        ...
        # copied AgentNexus items. Cross-family: the source's native transcript
        # is the wrong format, so ALWAYS rebuild from the copied AgentNexus
        # items (the converters consume AgentNexus's normalized item shape, so
    ...
    async def switch_session_agent(
        ...
        :raises AgentNexusError: 404 if the session or target agent does
        ...
        # AgentNexus items (a format-agnostic conversion). SDK targets replay
