...
from agentnexus.server.routes._gzip_route import GZipFileContentRoute, skip_gzip
...
def register_resources_routes(
    ...
    *,
    ...
    async def _validate_session(
        ...
        :raises AgentNexusError: 401/403/404 on auth or access failure.
        """
    ...
    async def _fs_get_with_host_fallback(
        ...
        *,
        ...
        :raises AgentNexusError: Re-raised runner-offline error when the
    ...
    async def _environment_reach_for_session(
        ...
        """
        from agentnexus.inner.sandbox import is_unconfined, reachable_roots, resolve_sandbox
    ...
    async def _read_workspace_via_host(
        ...
        *,
        ...
        try:
            ...
            if exc.status == 404:
                raise AgentNexusError(exc.message, code=ErrorCode.NOT_FOUND) from exc
            if exc.status == 400:
    ...
    async def _synthesize_offline_environment(
        ...
        from agentnexus.inner.sandbox import reach_payload
    ...
    async def _annotate_direct_attach(
        ...
        :param request: The incoming request carrying auth context.
        """
        from agentnexus.runtime import get_runner_direct_attach_resolver
    ...
    async def create_session_terminal(
        ...
        :raises AgentNexusError: 400 when the requested terminal is not
        ...
        if status >= 400:
            ...
            # AgentNexusError derives http_status from code; pass the runner's code, not a status.
            raise AgentNexusError(
    ...
    async def _proxy_fs_response(
        ...
        *,
        ...
        if status >= 400:
            ...
            if status == 404:
                raise AgentNexusError(message, code=ErrorCode.NOT_FOUND)
