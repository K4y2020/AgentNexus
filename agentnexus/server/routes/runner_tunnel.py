...
def create_runner_tunnel_router(
    ...
    *,
    ...
    def _get_user_id_from_request(request: Request) -> str | None:
        ...
        :raises AgentNexusError: 401 when the provider rejects the
            request.
        """
    ...
    async def mint_runner_owner_token(request: Request, runner_id: str) -> dict[str, str | int]:
        ...
        (``X-AgentNexus-Runner-Tunnel-Token``) and the server returns a
        ...
        :raises AgentNexusError: 401 when the binding token is absent,
        ...
        if not token or token_bound_runner_id(token) != runner_id:
            raise AgentNexusError("unauthenticated", code=ErrorCode.UNAUTHORIZED)
        owner: str | None = None
