...
``AGENTNEXUS_DEVICE_GRANT_ENABLED`` is set). OIDC deployments delegate login
...
Optionally, setting ``AGENTNEXUS_DEVICE_CLIENT_SECRET`` on the server gates
...
# (authorize / token / revoke). When ``AGENTNEXUS_DEVICE_CLIENT_SECRET`` is
...
_CLIENT_SECRET_ENV = "AGENTNEXUS_DEVICE_CLIENT_SECRET"
...
LOGIN_GRANT_CLIENT_ID = "agentnexus-cli"
...
_GRANT_MAX_LIFETIME_ENV = "AGENTNEXUS_GRANT_MAX_LIFETIME_DAYS"
...
def mint_delegated_token(
    ...
    *,
    ...
    :param user_id: The AgentNexus identity the token acts as (``sub``).
...
def create_oauth_token_router(
    ...
    *,
    ...
    ``AGENTNEXUS_DEVICE_GRANT_ENABLED``).
    ...
    :param client_secret_ok: Optional client-secret gate (callable that validates
        ...
        the ``AGENTNEXUS_DEVICE_CLIENT_SECRET`` env var. When provided
...
def create_device_auth_router(
    ...
    async def device_consent_page(request: Request) -> Response:
        ...
        AgentNexus identity being delegated and the requesting ``client_id``
    ...
    async def device_approve(request: Request) -> Response:
        ...
        requires the browser identity; binds the approving AgentNexus
    ...
    *,
    ...
    if error:
        ...
        body = (
            ...
            f"<b>{esc(user_id)}</b> on this AgentNexus server.</p>"
    ...
    return (
        ...
        "<title>Authorize access — AgentNexus</title><style>"
