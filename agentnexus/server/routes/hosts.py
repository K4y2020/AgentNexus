...
from agentnexus.server.schemas import SessionGitOptions
...
def _host_absent_error(host: Host) -> AgentNexusError:
    ...
    :returns: The ``AgentNexusError`` to raise; the global handler maps its code
        to the HTTP status and the ``{"error": {"code": ...}}`` body the
        client's re-address matches on.
    """
    if host_is_live(host):
        return AgentNexusError("host is on another replica", code=ErrorCode.WRONG_REPLICA)
    return AgentNexusError("host is offline", code=ErrorCode.CONFLICT)
...
def create_hosts_router(
    ...
    *,
    ...
    :param feature_flags: Immutable deployment release-feature snapshot.
        When omitted, resolves ``AGENTNEXUS_FEATURES`` at router construction.
    ...
    async def launch_runner(
        ...
        # CreatedWorktree | None — set ONLY when AgentNexus creates a worktree
    ...
    async def install_host_harness(
        ...
        ``harness_install`` in ``AGENTNEXUS_FEATURES`` (default off): when
