...
from agentnexus.server.scheduled.rrule import RRuleValidationError, validate_rrule
from agentnexus.server.scheduled.run_reconciler import force_fail_stale_runs
from agentnexus.stores import AgentStore, ConversationStore, PermissionStore
...
def _validate_rrule_or_400(rrule: str) -> None:
    """Raise a 400 ``AgentNexusError`` if the RRULE is invalid."""
    try:
        validate_rrule(rrule)
    except RRuleValidationError as exc:
        raise AgentNexusError(f"invalid rrule: {exc}", code=ErrorCode.INVALID_INPUT) from exc


def _validate_timezone_or_400(timezone: str) -> None:
    """Raise a 400 ``AgentNexusError`` if *timezone* is not a valid IANA timezone."""
    try:
        ZoneInfo(timezone)
    ...
    *,
    ...
    def _require_owned(scheduled_task_id: str, owner: str | None) -> ScheduledTask:
        ...
        if task is None or task.user_id != owner:
            raise AgentNexusError("Scheduled task not found", code=ErrorCode.NOT_FOUND)
        return task
