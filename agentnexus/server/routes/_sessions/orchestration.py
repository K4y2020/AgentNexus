...
from agentnexus.coordination.channels import A2A_CHANNEL_SCOPE_LABEL
from agentnexus.db.utils import generate_agent_id, generate_task_id
from agentnexus.debug_logging import debug_event
...
from agentnexus.llms.context_window import resolve_effective_context_window
from agentnexus.model_override import validate_model_override
from agentnexus.native_coding_agents import (
    native_coding_agent_for_agent_name,
    native_coding_agent_for_harness,
)
from agentnexus.policies.types import (
    ElicitationRequest,
    EvaluationContext,
    PolicyResult,
)
from agentnexus.runner.routing import RunnerRouter
from agentnexus.runner.session_init_protocol import build_runner_session_init_payload
...
from agentnexus.server import session_live_state, shutdown_state
...
from agentnexus.server.bundles import bundle_location, validate_agent_bundle
...
from agentnexus.server.routes._errors import session_not_found as _session_not_found
...
from agentnexus.stores import AgentStore, BotStore, ConversationStore
...
from agentnexus.telemetry import emit as _tel_emit
from agentnexus.telemetry.events import NativeSessionUsageEvent as _TelNativeSessionUsageEvent
from agentnexus.telemetry.events import SessionCreatedEvent as _TelSessionCreatedEvent
from agentnexus.telemetry.events import TurnEndEvent as _TelTurnEndEvent
from agentnexus.telemetry.installation_id import get_installation_id as _get_installation_id
from agentnexus.telemetry.surface import classify_surface as _classify_surface
...
def _build_session_response(
    ...
    :raises AgentNexusError: If ``conv.agent_id`` is ``None``.
    """
...
def _persist_native_cumulative_usage(
    ...
    Unlike the AgentNexus relay path (:func:`_accumulate_session_usage`), which adds
    ...
    :raises AgentNexusError: When a cumulative field is the wrong type.
    """
...
async def _persist_external_session_usage(
    ...
    :raises AgentNexusError: On missing / malformed fields.
    """
    ...
    # from the AgentNexus relay's per-response accumulation. Persist this session's
...
async def _persist_external_antigravity_subagent_start(
    ...
    :raises AgentNexusError: 400 if ``cascade_id`` is missing, or the parent has
        no bound agent.
    """
...
async def _persist_external_codex_subagent_start(
    ...
    :param conversation_store: Store for reading/creating child rows.
    :returns: Child conversation id, e.g. ``"conv_child456"``.
    :raises AgentNexusError: If ``thread_id`` is missing or parent has
        no bound agent.
    """
...
async def _persist_external_conversation_item(
    ...
    without starting or steering the placeholder AgentNexus agent.
...
def _drive_terminal_resolved_elicitation(session_id: str, persisted: ConversationItem) -> None:
    ...
    :param session_id: AgentNexus conversation id the item was mirrored for,
        e.g. ``"conv_abc123"``.
...
async def _run_managed_launch(
    *,
    ...
    """
    from agentnexus.server.managed_hosts import resolve_managed_agent_label
...
async def _bind_and_launch_managed_runner(
    *,
    ...
    :param tunnel_registry: Runner-tunnel registry used to await the
        ...
        wirings (the rendezvous then settles at frame-send).
    """
    from agentnexus.server.managed_hosts import terminate_managed_host
...
async def _maybe_relaunch_managed_sandbox(
    *,
    ...
    :raises AgentNexusError: 503 when the relaunch failed or timed out.
    """
...
async def ensure_runner_connected(
    ...
    * host tunnel gone but managed/resumable → ``_maybe_relaunch_managed_sandbox``
    ...
    :raises AgentNexusError: 503 when a managed relaunch/wake failed or timed
...
async def _run_managed_wake(
    *,
    ...
    """
    from agentnexus.server.managed_hosts import resume_managed_host
    from agentnexus.server.routes import sessions as _facade
...
def _native_terminal_runtime(conv: Conversation) -> tuple[str, str, str]:
    ...
    :raises AgentNexusError: If the session is not a native terminal harness.
    """
...
async def _forward_native_subagent_terminal_failure(
    ...
    :returns: None.
    :raises AgentNexusError: If the parent's runner could not be reached
...
def _build_native_terminal_message_event(
    ...
    :raises AgentNexusError: If the event is not a user message.
    """
...
async def _forward_native_terminal_message(
    ...
    """
    Forward one AgentNexus web-chat message to the native terminal harness.
    ...
    if (
        ...
        from agentnexus.runtime.content_resolver import (
...
def _reject_out_of_family_child(
    ...
    :raises AgentNexusError: 400 ``INVALID_INPUT`` when the child's family
...
async def _forward_event_to_runner(
    ...
    try:
        ...
        if _routed_model is not None and _verdict is not None:
            ...
            from agentnexus.server.smart_routing import _bare_id as _bare_model_id
...
async def _record_create_route_prompt(
    ...
    """
    from agentnexus.runner.subagent_routing import CREATE_ROUTE_PROMPT_LABEL_KEY
    from agentnexus.runner.turn_routing import create_route_prompt_fingerprint
...
async def _dispatch_session_event_to_runner_impl(
    ...
    * **transcript-forwarded native + ``type == "message"``**: web-chat user
      ...
      server. The AgentNexus would otherwise persist an AP-side copy AND
        ...
        async with runner_client.stream(
            ...
            async for chunk in resp.aiter_text():
                ...
                while "\n\n" in buffer:
                    ...
                    # (the AgentNexus subscriber generates its own heartbeats).
...
async def _ensure_runner_relay_ready_impl(
    ...
    The runner stream has no replay buffer. For item events, AgentNexus must
    subscribe to runner output before it forwards the input event; a
    fast harness can otherwise complete before AgentNexus is listening, leaving
    ...
    :returns: The active relay handle, or ``None`` when no runner is
        bound.
    :raises AgentNexusError: If the relay cannot observe the
...
async def _evaluate_input_policy(
    ...
    *,
    ...
    if has_content_blocks and file_store is not None and artifact_store is not None:
        from agentnexus.runtime.content_resolver import extract_text_attachments
...
async def _wake_parent_for_blocked_child(
    ...
    *outcome* is reported back so the notifier can release its per-block
    ...
    except (httpx.HTTPError, AgentNexusError):
        _logger.warning(
...
def configure_subagent_block_notifier(
    ...
    """
    from agentnexus.runtime import pending_elicitations as _pending_elicitations
    from agentnexus.runtime.subagent_block_notifier import SubagentBlockNotifier
...
def _installed_native_harnesses(host: Host | None) -> list[str]:
    ...
    """
    from agentnexus.server.smart_routing import AUTO_NATIVE_ROUTING_HARNESSES
...
def _ungatewayed_native_harnesses(host: Host | None, harnesses: Sequence[str]) -> list[str]:
    ...
    """
    from agentnexus.gateway_inference import not_gateway_backed
    from agentnexus.server.routing_backend import reported_gateway_inference
...
def _oss_routing_available(caps: Any = None) -> bool:  # type: ignore[explicit-any]
    ...
    if caps is None:
        try:
            from agentnexus.runtime._globals import _caps
        except ImportError:
            return False
        caps = _caps
...
def _external_router_usable(caps: Any = None) -> bool:  # type: ignore[explicit-any]
    ...
    """
    from agentnexus.server.routing_backend import backends_from_caps, usable_external
...
async def _reject_ungatewayed_model_routing(
    ...
    :raises AgentNexusError: 400 when the harness is not AI-Gateway-backed AND
    ...
    if not _ungatewayed_native_harnesses(host, (harness,)):
        return
    raise AgentNexusError(_ungatewayed_model_routing_error(harness), code=ErrorCode.INVALID_INPUT)
...
async def _host_model_options(
    ...
    """
    from agentnexus.server.routes._host_model_options import request_host_model_options
...
def _spec_routes_its_own_harness(
    ...
    :raises AgentNexusError: ``invalid_input`` when the spec's opt-in value is
...
async def _resolve_fixed_native_model_routing(
    ...
    :raises HTTPException: 404 if ``host_id`` is unknown, 403 if it belongs
        to another user.
    """
    from agentnexus.server.smart_routing import models_in_family, route_session_harness
...
async def _create_session_from_existing_agent(
    ...
    :raises AgentNexusError: 404 if no agent matches ``body.agent_id``;
    ...
    if body.parent_session_id is None and _bot_store is not None:
        from agentnexus.bots import bot_owner_id, ensure_bot_home
        ...
        if body.bot_id is not None:
            ...
            if _bot is None or _bot.agent_id != agent.id:
                raise AgentNexusError("Bot not found for this agent", code=ErrorCode.NOT_FOUND)
        else:
        ...
        if _bot is not None:
            if _bot.status != "active":
                raise AgentNexusError("Bot is archived", code=ErrorCode.CONFLICT)
            _bot_id = _bot.id
            _session_purpose = body.purpose or (
                "a2a"
                if body.labels.get("agentnexus.teammate.channel") == "a2a"
                else "primary"
                if body.labels.get("agentnexus.teammate.primary") == "true"
                else "topic"
            )
            ...
            if _bot.behavior_mode != "off":
                labels.setdefault("agentnexus.behavior_mode", _bot.behavior_mode)
    ...
    if _bot_binding is not None and _bot_store is not None and _bot_id is not None:
        if body.project_id is not None:
            ...
            from agentnexus.bot_workspace import WORKSPACE_LAYOUT_LABEL, ensure_bot_task_workspace
    ...
    try:
        ...
        raise AgentNexusError(str(exc), code=ErrorCode.CONFLICT) from exc
    except Exception as exc:
        ...
        # NOT git_branch: only a worktree AgentNexus created here may be
    ...
    if native_agent is not None:
        ...
        if IS_WINDOWS and native_agent.harness in ("codex-native", "claude-native"):
            _native_labels.update(
                {"agentnexus.ui": "chat", "agentnexus.wrapper": native_agent.wrapper_label}
            )
        else:
...
def _create_session_from_bundle(
    ...
    :raises AgentNexusError: If bundle validation or agent insert
...
async def _handle_mcp_tools_call(
    ...
    *,
    ...
    # tunnel the runner already opened to the AgentNexus server at startup.
    ...
    try:
        from agentnexus.runner.tool_dispatch import MCP_PROXY_FORWARD_TIMEOUT_S
...
async def _fetch_model_options(
    ...
    * **claude-native** — the provider-neutral aliases from the exact launch
    ...
    if wrapper == _PI_NATIVE_WRAPPER_LABEL_VALUE:
        ...
        # actually loaded regardless of auth path (AgentNexus provider OR pi's
...
async def _get_session_snapshot(
    ...
    :raises AgentNexusError: 404 if no session exists, 500 if the
