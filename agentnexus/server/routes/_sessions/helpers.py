...
from agentnexus.cost_plan import (
    COST_CONTROL_LABEL_NAMESPACE,
    reserved_cost_control_keys,
)
from agentnexus.db.utils import generate_task_id
...
from agentnexus.server import presence, session_live_state
...
from agentnexus.server.host_registry import HostConnection, HostRegistry, RunnerExitReports
...
from agentnexus.server.routes._host_worktree import CreatedWorktree
...
from agentnexus.server.routes._sessions.common import (  # noqa: F401
...
from agentnexus.session_lifecycle import (
...
from agentnexus.stores import AgentStore, ConversationStore
...
from agentnexus.stores.host_store import Host, HostStore
...
def _native_ask_gate_lock(conversation_id: str, deciding_policy: str) -> asyncio.Lock:
    ...
    :param conversation_id: AgentNexus conversation id whose ASK gate is being
...
async def _poll_request_disconnect(*args: Any, **kwargs: Any) -> None:
    ...
    from agentnexus.server.routes import sessions as _facade
...
def _publish_and_persist_resource_event(
    ...
    """
    from agentnexus.entities.conversation import ResourceEventData
...
def _signal_terminal_resolved_harness_elicitation_impl(
    ...
    :param session_id: AgentNexus conversation id whose forwarder mirrored the
...
def _client_supplied_hook_elicitation_id(
    ...
    :raises AgentNexusError: 400 when the id is malformed or is
...
def _signal_harness_elicitation_resolved_by_id(
    ...
    :param elicitation_id: Harness elicitation id, e.g.
        ``"elicit_codex_abc123"``.
    :returns: None.
    :raises AgentNexusError: If the id is malformed or belongs to a
        different session.
    """
...
def _resolve_llm_model(
    ...
    *,
    ...
    try:
        ...
        from agentnexus.runtime import get_agent_cache

        if agent_store is None:
            from agentnexus.runtime._globals import _agent_store
...
def _validated_harness_override(value: str | None, agent: Agent) -> str | None:
    ...
    :raises AgentNexusError: ``invalid_input`` for an unknown harness, a
    ...
    from agentnexus.harness_aliases import canonicalize_harness
    from agentnexus.runtime import get_agent_cache
    from agentnexus.spec._omnigent_compat import (
        AGENTNEXUS_EXECUTOR_TYPE,
        AGENTNEXUS_HARNESSES,
    )

    canonical = canonicalize_harness(value) or value
    if canonical not in AGENTNEXUS_HARNESSES:
        raise AgentNexusError(
            f"invalid harness_override: must be one of "
            f"{sorted(AGENTNEXUS_HARNESSES)}, got {value!r}",
            code=ErrorCode.INVALID_INPUT,
        )
    try:
    ...
    if executor_type != AGENTNEXUS_EXECUTOR_TYPE:
        raise AgentNexusError(
            f"harness_override only applies to executor.type "
            f"{AGENTNEXUS_EXECUTOR_TYPE!r} agents; agent {agent.name!r} "
...
def _validated_harness_override_executor_type(agent: Agent) -> None:
    ...
    :raises AgentNexusError: ``invalid_input`` when the agent is not an
        omnigent executor type or the bundle cannot be loaded.
    """
    from agentnexus.runtime import get_agent_cache
    from agentnexus.spec._omnigent_compat import AGENTNEXUS_EXECUTOR_TYPE
...
def _validated_spec_smart_routing_harness(spec: AgentSpec) -> str | None:
    ...
    :raises AgentNexusError: ``invalid_input`` for any other value, or for the
    ...
    if spec.executor.type != AGENTNEXUS_EXECUTOR_TYPE:
        raise AgentNexusError(
            f"{key} only applies to executor.type {AGENTNEXUS_EXECUTOR_TYPE!r} "
...
def _coerce_cumulative_field(
    ...
    *,
    ...
    :raises AgentNexusError: When present but the wrong type / negative /
...
async def _persist_external_model_change(
    ...
    ``/model`` change made inside the pane — onto the AgentNexus session:
    ...
    :raises AgentNexusError: If ``data.model`` is missing or not a
        non-empty string.
    """
...
async def _persist_external_session_title(
    ...
    pane onto the AgentNexus session: writes ``title`` so the new name
    ...
    :raises AgentNexusError: If ``data.title`` is not a non-empty single line
...
def _persist_external_model_options(
    ...
    AgentNexus-configured provider OR the harness's own ``/login``. This is why
    ...
    :raises AgentNexusError: If the session is not pi-native, or ``data.models``
...
def _validate_external_reasoning_effort(body: SessionEventInput) -> str | None:
    ...
    :raises AgentNexusError: If the payload is missing or unsupported.
    """
...
async def _persist_external_reasoning_effort_change(
    ...
    Mirrors a native-terminal thinking-level change onto the AgentNexus session.
...
async def _persist_external_codex_collaboration_mode_change(
    ...
    :returns: None.
    :raises AgentNexusError: If ``data.mode`` is missing or unsupported.
    """
...
async def _persist_external_permission_mode_change(
    ...
    :param conversation_store: Store used to upsert the mode label.
    :returns: None.
    :raises AgentNexusError: If ``data.permission_mode`` is missing or unsupported.
    """
...
def _handle_external_session_todos(
    ...
    :raises AgentNexusError: When ``data.todos`` is missing or not a list.
    """
...
def _publish_external_output_text_delta(session_id: str, body: SessionEventInput) -> None:
    ...
    :returns: None.
    :raises AgentNexusError: If ``data.delta`` is not a string, or any
...
def _publish_external_tool_output_delta(session_id: str, body: SessionEventInput) -> None:
    ...
    :returns: None.
    :raises AgentNexusError: If either required value is missing or not a string.
    """
...
def _publish_external_output_reasoning_delta(session_id: str, body: SessionEventInput) -> None:
    ...
    :returns: None.
    :raises AgentNexusError: If ``data.delta`` is not a string, or ``data.started``
...
def _parse_external_assistant_message(
    ...
    :raises AgentNexusError: If required fields are missing or
        malformed.
    """
...
async def _persist_external_assistant_message(
    ...
    """
    Persist and broadcast assistant text produced outside AgentNexus tasks.

    The event is append-only conversation history. It intentionally
    bypasses the legacy persist path so mirroring a
    Claude terminal response does not create or steer an AgentNexus
    agent task.
...
async def _persist_external_acp_subagent_start(
    ...
    :raises AgentNexusError: 400 when a required key is missing or the parent has
        no ``agent_id``.
    """
...
async def _persist_external_subagent_start(
    ...
    never POSTs to AgentNexus to register them. The forwarder watches the
    ...
    :returns: The child conversation id, e.g. ``"conv_child456"``.
    :raises AgentNexusError: 400 if the payload is missing any of
...
def _is_kiro_native_session(conv: Conversation) -> bool:
    ...
    return conv.labels.get("agentnexus.wrapper") == "kiro-native-ui"
...
def _latest_assistant_text_from_store(
    ...
    helper lets AgentNexus forward the durable assistant output with the
...
def _require_external_status_forward(
    ...
    :returns: None.
    :raises AgentNexusError: If the runner was unavailable or
...
def _require_collaboration_mode_forward(
    ...
    app-server. Persisting the AgentNexus label without a successful runner
    ...
    :param runner_result: HTTP result returned by the runner, or ``None``
        when no runner could be reached.
    :returns: None.
    :raises AgentNexusError: If no runner was reachable or the runner rejected
...
def _require_permission_mode_forward(
    ...
    :raises AgentNexusError: If no runner was reachable or the runner could
...
async def _get_runner_client_impl(
    ...
    *,
    ...
    """
    from agentnexus.runtime import get_runner_client
    ...
    if runner_router is not None:
        ...
        except (LookupError, httpx.HTTPError, AgentNexusError):
            _logger.debug(
...
async def _query_host_runner_status(
    ...
    """
    from agentnexus.host.frames import HostRunnerStatusFrame, encode_host_frame
    from agentnexus.server.routes import sessions as _facade
...
async def _validate_session_workspace(
    *,
    ...
    raises :class:`AgentNexusError` on failure so the route layer
    ...
    :raises AgentNexusError: With ``ErrorCode.INVALID_INPUT`` on
...
async def _launch_runner_on_host_locked(
    ...
    from agentnexus.host.frames import HostLaunchRunnerFrame, encode_host_frame
...
async def _provision_managed_sandbox(
    *,
    ...
    """
    from agentnexus.server.managed_hosts import launch_managed_host, relaunch_managed_host
...
async def _await_settled_managed_launch(launch: ManagedLaunch) -> None:
    ...
    :raises AgentNexusError: 503 when the launch failed or is still
        running at the timeout.
    """
    from agentnexus.server.managed_hosts import MANAGED_LAUNCH_RENDEZVOUS_TIMEOUT_S
...
async def _get_runner_client_for_resource_access_impl(
    ...
    *,
    ...
    """
    from agentnexus.runtime import get_runner_client, get_runner_router
...
async def _reset_runner_resources_after_switch_impl(session_id: str) -> None:
    ...
    except (httpx.HTTPError, HTTPException, AgentNexusError, RuntimeError):
        # Best-effort: a runner hiccup must not break the (already-committed)
        # switch. AgentNexusError covers the session-not-runner-bound / runner-
...
def _native_terminal_name_for_harness(harness: str) -> str:
    ...
    :raises AgentNexusError: If *harness* is not a supported native
        terminal harness.
    """
...
def _extract_claude_native_runner_failure(resp: httpx.Response) -> str | None:
    ...
    ``response.failed``. Claude-native AgentNexus forwarding must treat that
...
async def _forward_session_change_to_runner_impl(
    ...
    * Explicit ``compact`` — the caller inspects the returned status
      ...
      200) or the AgentNexus server must run its own in-process compaction
    ...
    skipped — the persisted value on the AgentNexus side is the authoritative
    ...
    in the AgentNexus log — otherwise the POST succeeds at the httpx layer
...
async def _stop_session_via_runner_impl(
    ...
    :raises AgentNexusError: ``RUNNER_UNAVAILABLE`` (HTTP 503) if the
...
async def _stop_session_host_runner(
    ...
    *,
    ...
    if conn is None:
        ...
        return False
    from agentnexus.host.frames import HostStopRunnerFrame, encode_host_frame
...
def _parse_skill_slash_command(body: SessionEventInput) -> tuple[str, str]:
    ...
    :raises AgentNexusError: If the payload is not a skill command
...
def _build_skill_slash_command_policy_body(body: SessionEventInput) -> SessionEventInput:
    ...
    :raises AgentNexusError: If the slash-command payload is invalid.
    """
...
async def _resolve_skill_meta_text_via_runner(
    ...
    :returns: The hidden ``<skill>`` meta text for a single
        ``input_text`` block.
    :raises AgentNexusError: If the skill is not exposed for the session
...
async def _dispatch_skill_slash_command_to_runner(
    ...
    * a hidden ``message`` item with ``is_meta=True`` containing the
    ...
    :raises AgentNexusError: If the skill is not exposed for the
...
def _agent_provider_family(agent: Agent) -> str | None:
    ...
    """
    from agentnexus.onboarding.provider_config import provider_family_for_harness
...
def _same_provider_family_impl(a: Agent, b: Agent) -> bool:
    ...
    native transcript from AgentNexus items instead).
...
def _agent_carries_native_fork_history_impl(agent: Agent) -> bool:
    ...
    session file that the runner rebuilds from the copied AgentNexus items on
...
def _validate_terminal_launch_args(value: list[str] | None) -> list[str] | None:
    ...
    a caller to smuggle internal launch wiring (bridge dir, AgentNexus URL,
...
def _validated_cost_control_mode_override(value: str | None) -> str | None:
    ...
    :raises AgentNexusError: 400 (``invalid_input``) when *value* is
...
def _parse_session_create_metadata(metadata: str) -> SessionCreateMetadata:
    ...
    :raises AgentNexusError: If the JSON fails the request schema.
    """
    try:
...
def _require_host_conn_for_worktree(host_id: str | None, request: Request) -> HostConnection:
    ...
    :raises AgentNexusError: ``invalid_input`` when ``host_id`` is
...
async def _create_session_worktree(
    *,
    ...
    :raises AgentNexusError: ``invalid_input`` for a bad branch name,
        ...
        is configured.
    """
    from agentnexus.host.git_worktree import WorktreeError, validate_branch_name
    from agentnexus.server.routes._host_worktree import (
    ...
    try:
        ...
        raise AgentNexusError(exc.message, code=ErrorCode.INVALID_INPUT) from exc
    ...
    try:
        ...
        raise AgentNexusError(exc.message, code=ErrorCode.CONFLICT) from exc
...
def _require_declared_subagent(
    *,
    ...
    :raises AgentNexusError: 404 ``NOT_FOUND`` when the bundle loads and
...
def _reject_reserved_cost_control_label_seed(labels: dict[str, str]) -> None:
    ...
    :param labels: The client-supplied initial labels, e.g.
        ``{"team": "ml"}``.
    :raises AgentNexusError: 400 when any ``cost_control.*`` key is
        present.
    """
...
def _reject_server_reserved_label_seed(labels: dict[str, str] | None) -> None:
    ...
    :raises AgentNexusError: 400 when any reserved key is present.
    """
    if not labels:
        return
...
def _require_cost_control_label_authority(
    *,
    ...
    :param tunnel_token: Value of the ``X-AgentNexus-Runner-Tunnel-Token``
    ...
    :raises AgentNexusError: 403 when the caller presents no acceptable
...
def _persist_stored_session_bundle(
    ...
    *,
    ...
    :raises AgentNexusError: If the agent insert violates integrity
...
async def _authorize_bundled_parent_and_inherit_runner(
    ...
    *,
    ...
    :raises AgentNexusError: 403/404 when the caller may not access the
        parent session.
    """
...
def _registered_runner_id(
    ...
    *,
    ...
    :raises AgentNexusError: If the id is empty, the router is
...
async def _handle_advise_models_mcp(
    ...
    *,
    ...
    from agentnexus.server.routing_backend import backends_from_caps
    ...
    from agentnexus.model_catalog import spec_harness
    from agentnexus.server.smart_routing import _WORKER_NAME_TO_HARNESS, fetch_runner_models
...
async def _handle_mcp_tools_list(
    ...
    env, and tooling). The AgentNexus server's role here is routing only —
...
async def _load_model_options(
    ...
    import agentnexus.server.routes.sessions as _facade
...
async def _host_model_options_via_registry(host_id: str) -> list[dict[str, Any]] | None:
    ...
    from agentnexus.server.routes.hosts import _proxy_model_options
