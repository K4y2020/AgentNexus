...
def resolve_host_launch(
    *,
    ...
    :raises HTTPException: 404 if the host or session is missing (or the
        ...
        a different user.
    :raises AgentNexusError: 409 (CONFLICT) if the host is genuinely offline,
    ...
    if conn is None:
        ...
        if host_is_live(host):
            raise AgentNexusError("host is on another replica", code=ErrorCode.WRONG_REPLICA)
        raise AgentNexusError("host is offline", code=ErrorCode.CONFLICT)
