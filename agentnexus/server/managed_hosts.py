...
1. **Server YAML** (OSS / self-hosted): :func:`parse_sandbox_config`
   ...
   (``omnigent server -c`` / ``AGENTNEXUS_CONFIG`` /
   ``<data_dir>/config.yaml``)::
       ...
       sandbox:
         ...
         host_config:             # optional; provider-agnostic. Verbatim
                                  # in-sandbox ~/.agentnexus/config.yaml content,
...
from agentnexus.db.utils import builtin_agent_id, now_epoch
from agentnexus.stores.host_store import Host, HostStore

if TYPE_CHECKING:
    from agentnexus.onboarding.sandboxes import SandboxHostLauncher
...
# operator-overridable (AGENTNEXUS_CWSANDBOX_MAX_LIFETIME_S), so the TTL is
...
MANAGED_SANDBOX_LABEL_NAMESPACE = "agentnexus.sandbox."
...
MANAGED_REPO_LABEL_KEY = "agentnexus.sandbox.repo"
...
class ManagedSandboxConfig:
    ...
    :param host_config: Verbatim in-sandbox ``~/.agentnexus/config.yaml``
...
def _modal_launcher_factory(
    ...
    :param secrets: Modal secret names whose env vars (harness LLM
        ...
        e.g. ``["agentnexus-llm"]``, or ``None`` to resolve from the
    ...
    def _build() -> SandboxHostLauncher:
        ...
        from agentnexus.onboarding.sandboxes.modal import ModalSandboxLauncher
...
def _community_sandbox_providers() -> frozenset[str]:
    ...
    """
    from agentnexus.onboarding.sandboxes import registry as sandbox_registry
...
def _parse_host_config(raw: dict[str, object]) -> dict[str, object] | None:
    ...
    Verbatim in-sandbox ``~/.agentnexus/config.yaml`` content forwarded at
    ...
    if not isinstance(host_config, dict):
        raise ValueError(
            ...
            "in-sandbox ~/.agentnexus/config.yaml content merged in before "
            "'agentnexus host' starts"
        )
    ...
    if "providers" in host_config:
        ...
        from agentnexus.onboarding.provider_config import get_default_provider, load_providers
...
def _parse_single_provider_sandbox_config(raw: dict[str, object]) -> ManagedSandboxConfig:
    ...
    if provider == "modal":
        ...
        from agentnexus.onboarding.sandboxes.blaxel import managed_token_ttl_s
        ...
        from agentnexus.onboarding.sandboxes.cwsandbox import managed_token_ttl_s

        launcher_factory = _cwsandbox_launcher_factory(
            _parse_cwsandbox_image(raw), _parse_cwsandbox_env(raw)
        )
        # Derived from AGENTNEXUS_CWSANDBOX_MAX_LIFETIME_S so the token always
        ...
        from agentnexus.onboarding.sandboxes.e2b import managed_token_ttl_s

        launcher_factory = _e2b_launcher_factory(
            _parse_e2b_template(raw), _parse_provider_env(raw, "e2b")
        )
        # Derived from AGENTNEXUS_E2B_MAX_LIFETIME_S so the token always
...
def _parse_modal_secrets(raw: dict[str, object]) -> list[str] | None:
    ...
    :param raw: The raw ``sandbox`` mapping (provider already known to
        be ``"modal"``).
    :returns: The validated secret names, e.g. ``["agentnexus-llm"]``,
    ...
    if not isinstance(secrets, list) or not all(
        ...
        raise ValueError(
            ...
            "secret names, e.g. ['agentnexus-llm']"
        )
...
def _daytona_launcher_factory(
    ...
    def _build() -> SandboxHostLauncher:
        ...
        from agentnexus.onboarding.sandboxes.daytona import DaytonaSandboxLauncher
...
def _parse_blaxel_image(raw: dict[str, object]) -> str | None:
    ...
    if not isinstance(image, str) or not image.strip():
        raise ValueError(
            ...
            "public image or AGENTNEXUS_BLAXEL_HOST_IMAGE override)"
        )
    return image.strip()
...
def _boxlite_launcher_factory(
    ...
    def _build() -> SandboxHostLauncher:
        ...
        from agentnexus.onboarding.sandboxes.boxlite import BoxliteSandboxLauncher
...
def _cwsandbox_launcher_factory(
    ...
    def _build() -> SandboxHostLauncher:
        from agentnexus.onboarding.sandboxes.cwsandbox import CWSandboxLauncher
...
def _e2b_launcher_factory(
    ...
    :param template: E2B template NAME the AgentNexus host image was built
    ...
    def _build() -> SandboxHostLauncher:
        ...
        from agentnexus.onboarding.sandboxes.e2b import E2BSandboxLauncher
...
def _parse_e2b_template(raw: dict[str, object]) -> str | None:
    ...
    AgentNexus host image was built into — NOT a registry image reference
    ...
    if not isinstance(template, str) or not template.strip():
        raise ValueError(
            ...
            "'agentnexus-host'; see deploy/e2b/README.md) — NOT a registry image "
...
def _parse_islo_idle_pause_after_s(raw: dict[str, object]) -> int | None:
    ...
    lifecycle control.
    """
    from agentnexus.onboarding.sandboxes.islo import DEFAULT_IDLE_PAUSE_AFTER_S
...
def _islo_launcher_factory(
    *,
    ...
    def _build() -> SandboxHostLauncher:
        ...
        from agentnexus.onboarding.sandboxes.islo import IsloSandboxLauncher
...
def _openshell_launcher_factory(
    *,
    ...
    :param workspace: OpenShell workspace for sandbox lifecycle, or
        ``None`` to resolve from ``$AGENTNEXUS_OPENSHELL_WORKSPACE``
        then ``"default"``.
    ...
    def _build() -> SandboxHostLauncher:
        ...
        from agentnexus.onboarding.sandboxes.openshell import OpenShellSandboxLauncher
...
def _kubernetes_launcher_factory(
    *,
    ...
    def _build() -> SandboxHostLauncher:
        ...
        from agentnexus.onboarding.sandboxes.kubernetes import KubernetesSandboxLauncher
...
def host_sandbox_is_running(
    ...
    and fall back to AgentNexus liveness checks.
    """
    launcher = _launcher_for_teardown(host, config)
