...
- ``"header"`` (default): reads the ``X-Forwarded-Email`` header
  ...
  (``AGENTNEXUS_LOCAL_SINGLE_USER=1``), in which case they fall back
...
_AUTH_ENABLED_ENV = "AGENTNEXUS_AUTH_ENABLED"
...
_LOCAL_SINGLE_USER_ENV = "AGENTNEXUS_LOCAL_SINGLE_USER"
...
_AUTH_HEADER_ENV = "AGENTNEXUS_AUTH_HEADER"
...
_AUTH_HEADER_STRIP_PREFIX_ENV = "AGENTNEXUS_AUTH_HEADER_STRIP_PREFIX"
...
class SharingMode(str, Enum):
    ...
    ``AGENTNEXUS_SHARING_MODE`` env var round-trip it directly. Defaults to ``ON``.
    """
...
def local_single_user_enabled() -> bool:
    ...
    Reads ``AGENTNEXUS_LOCAL_SINGLE_USER``, the marker the managed
...
def resolve_auth_header() -> str:
    ...
    Reads ``AGENTNEXUS_AUTH_HEADER`` and falls back to
    ...
    e.g. ``AGENTNEXUS_AUTH_HEADER=Cf-Access-Authenticated-User-Email`` for
...
def resolve_auth_header_strip_prefix() -> str:
    ...
    Reads ``AGENTNEXUS_AUTH_HEADER_STRIP_PREFIX`` and returns it
    ...
    ``AGENTNEXUS_AUTH_HEADER=X-Goog-Authenticated-User-Email`` at IAP's
    identity header and set
    ``AGENTNEXUS_AUTH_HEADER_STRIP_PREFIX=accounts.google.com:`` so the
...
def _auth_enabled() -> bool:
    ...
    Reads ``AGENTNEXUS_AUTH_ENABLED``. The explicit-falsy kill-switch
    semantics mean ``AGENTNEXUS_AUTH_ENABLED=0`` disables auth even
...
def resolve_auth_source() -> str:
    ...
    - An explicit ``AGENTNEXUS_AUTH_PROVIDER`` (case-insensitive) always
      wins, e.g. ``"header"`` / ``"oidc"`` / ``"accounts"``. This is the
      low-level escape hatch.
    - Otherwise ``header`` is the default, unless the opt-in switch
      ``AGENTNEXUS_AUTH_ENABLED`` is truthy (see :func:`_auth_enabled`).
      When enabled, the mode depends on whether OIDC config was
      supplied:

      - ``AGENTNEXUS_OIDC_ISSUER`` is set → ``"oidc"`` (the operator
    ...
    :returns: The resolved source string, e.g. ``"accounts"``,
        ...
        ``AGENTNEXUS_AUTH_PROVIDER``). The caller is responsible for
        rejecting unknown values.
    """
    raw_source = os.environ.get("AGENTNEXUS_AUTH_PROVIDER")
    ...
    if _auth_enabled():
        ...
        if os.environ.get("AGENTNEXUS_OIDC_ISSUER", "").strip():
            return "oidc"
        return "accounts"
    return "header"
...
class UnifiedAuthProvider(AuthProvider):
    ...
    ``AGENTNEXUS_AUTH_PROVIDER``. OIDC and accounts modes share
    ...
    :param local_single_user: When ``True``, header mode falls back
        ...
        ``AGENTNEXUS_LOCAL_SINGLE_USER`` at construction (see
    ...
    :param header_name: The trusted identity header read in header
        ...
        ``AGENTNEXUS_AUTH_HEADER`` at construction, falling back to
    ...
    :param header_strip_prefix: A prefix stripped from the identity
        ...
        ``AGENTNEXUS_AUTH_HEADER_STRIP_PREFIX`` at construction, falling
    ...
    def mint_runner_token(self, user_id: str, ttl_seconds: int) -> str | None:
        ...
        if cookie_config is None:
            return None
        from agentnexus.server.oidc import mint_session_token
    ...
    def _check_cookie(self, request: HTTPConnection) -> str | None:
        ...
        from agentnexus.server.oidc import hmac_digest
    ...
    def _check_header(self, request: HTTPConnection) -> str | None:
        ...
        by default, overridable via ``AGENTNEXUS_AUTH_HEADER`` — e.g.
        ``Cf-Access-Authenticated-User-Email`` for Cloudflare Access).

        When :attr:`_header_strip_prefix` is set (from
        ``AGENTNEXUS_AUTH_HEADER_STRIP_PREFIX``), it is removed from the
        ...
        ``AGENTNEXUS_LOCAL_SINGLE_USER=1``): there the absent header
...
def create_auth_provider() -> AuthProvider:
    """Factory: read ``AGENTNEXUS_AUTH_PROVIDER`` and return a
    ...
    ``AGENTNEXUS_AUTH_HEADER``) — 401, fail closed; see
    :meth:`UnifiedAuthProvider._check_header` — unless the server
    is an explicit single-user local runtime
    (``AGENTNEXUS_LOCAL_SINGLE_USER=1``, set by the managed local
    ...
    -----------------------------------
    Set ``AGENTNEXUS_AUTH_ENABLED=1`` (or any truthy value) to turn on
    multi-user auth. With no OIDC config present this selects
    ``accounts`` mode — the built-in login flow with
    first-user-is-admin setup. Set the ``AGENTNEXUS_OIDC_*`` env vars
    (at minimum ``AGENTNEXUS_OIDC_ISSUER``) alongside it and the same
    ...
    leaves it off. An explicit ``AGENTNEXUS_AUTH_PROVIDER`` always wins
    over this switch — it only governs the env-unset default. Deploys
    behind an SSO proxy that injects ``X-Forwarded-Email`` set
    ``AGENTNEXUS_AUTH_PROVIDER=header`` (Databricks Apps, oauth2-proxy);
    ...
    (``AGENTNEXUS_AUTH_ENABLED`` is the opt-in gate: header is the
    ...
    if source not in ("header", "oidc", "accounts"):
        raise RuntimeError(
            f"Unknown AGENTNEXUS_AUTH_PROVIDER={source!r}. Valid: 'header', 'oidc', 'accounts'"
        )
    ...
    if source == "oidc":
        ...
        # — either AGENTNEXUS_AUTH_PROVIDER=accounts or the
        # AGENTNEXUS_AUTH_ENABLED=1 opt-in without OIDC config
