...
from agentnexus.server.admin_list import resolve_data_dir
...
def resolve_config_path() -> Path | None:
    ...
    :returns: ``AGENTNEXUS_CONFIG`` if set; else ``<data_dir>/config.yaml``
        when that file exists; else ``None``.
    """
    explicit = os.environ.get("AGENTNEXUS_CONFIG", "").strip()
    if explicit:
...
def copy_file_count_limit() -> int:
    ...
    """
    from agentnexus.runtime.content_resolver import MAX_COPY_FILES
...
def copy_total_bytes_limit() -> int:
    ...
    """
    from agentnexus.runtime.content_resolver import MAX_COPY_TOTAL_BYTES
...
def _branding_powered_by(section: Mapping[str, Any]) -> bool:
    """Whether to show the "Powered by AgentNexus" attribution (default True)."""
