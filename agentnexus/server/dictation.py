...
- ``remote`` — relays takes to a dictation worker on another machine
  (``AGENTNEXUS_DICTATION_REMOTE_URL``), so a small main server can borrow
...
Env var                                 Default
======================================  ==========================================
``AGENTNEXUS_DICTATION_MODEL_DIR``        ``~/.agentnexus/models/dictation/asr``
``AGENTNEXUS_DICTATION_PUNCT_DIR``        ``~/.agentnexus/models/dictation/punct``
======================================  ==========================================
...
ENGINE_ENV = "AGENTNEXUS_DICTATION_ENGINE"
MODEL_DIR_ENV = "AGENTNEXUS_DICTATION_MODEL_DIR"
PUNCT_DIR_ENV = "AGENTNEXUS_DICTATION_PUNCT_DIR"
MAX_STREAMS_ENV = "AGENTNEXUS_DICTATION_MAX_STREAMS"
#: Worker stream URL for the ``remote`` engine, e.g.
#: ``ws://venus:8100/v1/dictation/stream``.
REMOTE_URL_ENV = "AGENTNEXUS_DICTATION_REMOTE_URL"

#: Built-in engine names. The default (empty ``AGENTNEXUS_DICTATION_ENGINE``)
...
def register_engine(
    ...
    *,
    ...
    Selected via ``AGENTNEXUS_DICTATION_ENGINE=<name>``. This is the whole
...
def _asr_dir() -> Path:
    default = Path.home() / ".agentnexus" / "models" / "dictation" / "asr"
    return Path(os.environ.get(MODEL_DIR_ENV) or default).expanduser()


def _punct_dir() -> Path:
    default = Path.home() / ".agentnexus" / "models" / "dictation" / "punct"
