...
from agentnexus.error_layers import ErrorLayer, classify_error_layer
...
class ChildSessionSummary(BaseModel):
    ...
    :param tool: UI-facing sub-agent label. For AgentNexus-spawned
...
class ElicitationResult(BaseModel):
    ...
    AgentNexus clients deliver this shape inside the session-scoped
...
class SessionCreateRequest(BaseModel):
    ...
    :param harness_override: Optional per-session brain-harness
        ...
        ``AGENTNEXUS_HARNESSES`` and the bound agent must be an
    ...
    def _check_managed_host_fields(self) -> SessionCreateRequest:
        ...
        from agentnexus.server.managed_hosts import is_repo_workspace, parse_repo_workspace
...
class SessionResponse(BaseModel):
    ...
    :param pending_elicitations: Outstanding approval prompts on
        ...
        the AgentNexus server's in-memory
    ...
    :param todos: Current Claude Code todo list items for
        ...
        from the AgentNexus server's in-memory ``_session_todos_cache``.
    ...
    :param terminal_pending: ``True`` while the runner is auto-creating
        ...
        exists. Sourced from the AgentNexus server's in-memory
    ...
    :param sandbox_status: Managed-sandbox launch progress while the
        ...
        Sourced from the AgentNexus server's in-memory
...
class CodexGoalObject(BaseModel):
    ...
    Mirrors Codex app-server's ``ThreadGoal`` shape using AgentNexus's
...
class UpdateCodexGoalStatusRequest(BaseModel):
    ...
    updates. AgentNexus exposes the two user-driven transitions explicitly:
...
class SessionListItem(BaseModel):
    ...
    :param pending_elicitations_count: Number of approval prompts
        ...
        opening each chat. Sourced from the AgentNexus server's in-memory
        :mod:`omnigent.runtime.pending_elicitations` index,
...
class SessionPermissionModeEvent(_SSEEventBase):
    ...
    the TUI, which AgentNexus has no other way to see. Lets the composer's mode
...
class SessionTerminalPendingEvent(_SSEEventBase):
    ...
    1. The AgentNexus server at ``POST /v1/sessions`` for host-launched
    ...
    2. The AgentNexus relay when the runner's ``session.terminal_pending`` frame
...
class SessionSandboxStatusEvent(_SSEEventBase):
    ...
    A managed create returns before its sandbox exists; the AgentNexus
...
class ElicitationRequestParams(BaseModel):
    ...
    :param phase: AgentNexus policy-engine phase the elicitation
        belongs to, e.g. ``"pre_tool_use"``.
    :param policy_name: AgentNexus policy that triggered the
    ...
    # ``elicitation/create`` traversing harness → AgentNexus → client)
...
class ElicitationRequestEvent(_SSEEventBase):
    ...
    Emitted by AgentNexus (or, under the new contract, by a harness)
...
class BrowserActionRequestEvent(_SSEEventBase):
    ...
    AgentNexus desktop app's embedded browser to act. The event fans out
...
class ElicitationResolvedEvent(_SSEEventBase):
    ...
    Idempotent on the consumer side: the AgentNexus server's index
...
class PolicyDeniedEvent(_SSEEventBase):
    ...
    prompt through AgentNexus's policy engine via the vendor command-hook
...
class PolicyEvaluationRequestEvent(_SSEEventBase):
    ...
    policies on the AgentNexus server. The runner intercepts this event in
    ``proxy_stream``, calls the AgentNexus server's
    ...
    :param data: Event data dict passed to the AgentNexus server's
...
class SubagentStartedEvent(_SSEEventBase):
    ...
    ``external_subagent_start`` to the AgentNexus server, minting a child session so
