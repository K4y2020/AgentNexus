...
async def _presentation_labels(deps: FireDeps, task: ScheduledTask) -> dict[str, str]:
    ...
    * A non-native SDK agent whose runner auto-creates the ``omnigent`` REPL
    ...
    """
    from agentnexus.native_coding_agents import native_coding_agent_for_agent_name
    from agentnexus.server.routes.sessions import _repl_terminal_ui_labels
...
async def _create_session(deps: FireDeps, task: ScheduledTask) -> Conversation:
    ...
    if bot is not None:
        from agentnexus.bot_workspace import WORKSPACE_LAYOUT_LABEL

        labels[WORKSPACE_LAYOUT_LABEL] = "topic-v1"
    if bot is not None and bot.behavior_mode != "off":
        labels["agentnexus.behavior_mode"] = bot.behavior_mode
    if labels:
...
_COST_BUDGET_HANDLER = "agentnexus.policies.builtins.cost.cost_budget"
