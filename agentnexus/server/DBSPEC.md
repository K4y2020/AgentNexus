...
in `agentnexus/db/db_models.py`, which is the source of truth for the full table
list.
...
`agentnexus/runner/app.py`) and is never written to the DB. Only a coarse mirror
is persisted — `agentnexus_conversation_metadata.live_status` /
...
then cleans up the AgentNexus-side rows: comments, policies, session permissions,
...
AgentNexus rows survive a conversation that no longer exists. Do not remove any
...
in `agentnexus/runner/app.py`; none of it is a DB operation.
