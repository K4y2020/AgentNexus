...
1. **Domain allowlist.** The email's domain is in the effective
   allowlist — the union of ``AGENTNEXUS_OIDC_ALLOWED_DOMAINS`` (env,
...
3. **Individual invite.** The email was pre-authorized via an opt-in
   OIDC invite (``AGENTNEXUS_OIDC_ALLOW_INVITES`` — see
...
from agentnexus.server.admin_list import AdminList, MtimeCachedIdentitySet, resolve_data_dir


def resolve_allowed_domains_path() -> Path:
    """Resolve the path to the optional allowed-domains file.

    :returns: ``AGENTNEXUS_OIDC_ALLOWED_DOMAINS_PATH`` if set, else
        ``<data_dir>/allowed_domains`` (see
        :func:`omnigent.server.admin_list.resolve_data_dir`).
    """
    explicit = os.environ.get("AGENTNEXUS_OIDC_ALLOWED_DOMAINS_PATH", "").strip()
    if explicit:
...
class OidcAdmissionPolicy:
    ...
    :param env_allowed_domains: The frozen domain allowlist parsed from
        ``AGENTNEXUS_OIDC_ALLOWED_DOMAINS`` (``OIDCConfig.allowed_domains``).
