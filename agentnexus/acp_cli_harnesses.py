...
spawn-env builder. Rows own their auth (``OWN_AUTH``): no AgentNexus credential is
...
ACP_CLI_HARNESSES: dict[str, AcpCliHarness] = {
    ...
    # - the store the WorkBuddy desktop app writes too - so AgentNexus keeps no
    ...
    "codebuddy": AcpCliHarness(
        install=HarnessInstallSpec(
            ...
            auth_hint="run `codebuddy /login` (browser OAuth; AgentNexus stores no credential)",
        ),
        args=("--acp",),
    ...
    # spawn; AgentNexus stores nothing. The row runs Devin's account-default model:
    ...
    "devin": AcpCliHarness(
        install=HarnessInstallSpec(
            ...
            auth_hint="run `devin auth login` (AgentNexus stores no Devin credential)",
        ),
        args=("acp",),
    ),
    ...
    # (xAI OAuth, device-code capable) or ``XAI_API_KEY``; AgentNexus stores no
    # credential.
    "grok": AcpCliHarness(
        install=HarnessInstallSpec(
            "Grok Build",
