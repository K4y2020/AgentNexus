...
from agentnexus.model_override import is_codex_compatible_model, model_family_mismatch
from agentnexus.model_resolver import (
    ModelResolution,
...
from agentnexus.pi_model_compatibility import unsupported_in_pi
...
def is_direct_openai_provider(provider: ResolvedModelProvider) -> bool:
    ...
    if provider.family != OPENAI_FAMILY or not provider.base_url:
        return False
    from agentnexus.onboarding.configure_models import default_base_url_for_family
...
def resolve_model_provider(spec: object, harness: str | None) -> ResolvedModelProvider:
    ...
    try:
        ...
        # AgentNexusError text is this codebase's own (secret-free); anything
        # else is redacted to its type name — raw detail stays at DEBUG.
        reason = str(exc) if isinstance(exc, AgentNexusError) else type(exc).__name__
...
def _resolve_model_provider_unsafe(spec: object, harness: str | None) -> ResolvedModelProvider:
    ...
    from agentnexus.runtime.workflow import _resolve_provider_for_build
...
def _legacy_claude_sdk_provider(spec: AgentSpec) -> ResolvedModelProvider:
    ...
    from agentnexus.spec.types import ApiKeyAuth, DatabricksAuth
