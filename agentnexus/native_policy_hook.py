"""Shared conversion between native-harness hooks and AgentNexus policy events.
...
_EVAL_UNAVAILABLE_REASON = (
    "AgentNexus policy evaluation unavailable (could not reach or authenticate to the "
    "AgentNexus server); failing closed for this tool call."
)
_EVAL_UNAVAILABLE_REQUEST_REASON = (
    "AgentNexus policy evaluation unavailable (could not reach or authenticate to the "
    "AgentNexus server); failing closed for this request."
)
...
_AUTH_HEADERS_ENV = "_AGENTNEXUS_AUTH_HEADERS"

# Relay env vars for harnesses that deliver auth via env (hermes, cursor).
_RELAY_URL_ENV = "_AGENTNEXUS_RELAY_URL"
_RELAY_TOKEN_ENV = "_AGENTNEXUS_RELAY_TOKEN"
...
def policy_hook_wrapper_script(server_url: str, session_id: str, hook_script_path: str) -> str:
    ...
    Resolves a one-shot AgentNexus-server token and bakes the auth +
    ...
    :param server_url: AgentNexus server base URL the hook posts to.
    ...
    return (
        "#!/bin/sh\n"
        f"export _AGENTNEXUS_SERVER_URL={shlex.quote(server_url)}\n"
        f"export _AGENTNEXUS_SESSION_ID={shlex.quote(session_id)}\n"
...
class PolicyHookReauth:
    """Callable that re-mints the AgentNexus bearer for a policy hook subprocess.
...
def hook_payload_to_evaluation_request(
    ...
    ``prompt`` field becomes the request content). AgentNexus MCP tools
    (``mcp__agentnexus__*``) are skipped because they are already
    policy-checked by the relay path (``ProxyMcpManager`` → AgentNexus
    ...
    :returns: An ``EvaluationRequest`` dict suitable for POSTing to
        ...
        policy-relevant (unknown event or an ``mcp__agentnexus__*`` tool).
    """
    ...
    # AgentNexus MCP tools are already policy-checked by the relay path
...
def post_evaluate_with_retry(
    ...
    """
    POST to the AgentNexus policy evaluate endpoint, retrying on transient errors.
    ...
    :param headers: Auth headers for the AgentNexus server.
    ...
    while True:
        try:
            with httpx.Client(headers=headers, timeout=timeout) as client:
                ...
                if (
                    ...
                    if refreshed:
                        ...
                        reauthed = True
                        print(
                            f"agentnexus {hook_label}: AgentNexus auth expired "
            ...
            if status < 500:
                print(
                    f"agentnexus {hook_label}: AgentNexus returned {status}"
                    + (f": {body_preview}" if body_preview else ""),
                    file=sys.stderr,
                )
                return None, last_error
            print(
                f"agentnexus {hook_label}: AgentNexus returned {status}; retrying",
                file=sys.stderr,
            )
        except (httpx.ConnectError, httpx.ConnectTimeout) as exc:
            last_error = f"connection error: {exc}"
            print(
                f"agentnexus {hook_label}: AgentNexus request failed; retrying: {exc}",
                file=sys.stderr,
            )
            ...
            print(
                f"agentnexus {hook_label}: AgentNexus request failed: {exc}",
                file=sys.stderr,
            )
            return None, last_error
        if time.monotonic() + backoff_s >= deadline:
            print(
                f"agentnexus {hook_label}: retry budget exhausted",
                file=sys.stderr,
            )
