...
PI_NATIVE_CONFIG_ENV_VAR = "AGENTNEXUS_PI_NATIVE_CONFIG"

_BRIDGE_ROOT = Path.home() / ".agentnexus" / "pi-native"
_CONFIG_FILE = "config.json"
_EXTENSION_FILE = "agentnexus_pi_native_extension.js"
_EXTENSION_PACKAGE = "agentnexus.resources.pi_native"
_INBOX_DIR = "inbox"
...
def bridge_dir_for_session_id(session_id: str) -> Path:
    ...
    :returns: Absolute bridge directory under ``~/.agentnexus/pi-native``.
    """
...
def write_extension_files(
    ...
    *,
    ...
    :param auth_headers: HTTP headers the extension should use when posting
        terminal-originated events back to AgentNexus.
    :param tools: Flat AgentNexus tool schemas
        ...
        runner uses), so the Pi agent can invoke AgentNexus ``sys_*`` tools with
...
def refresh_config_auth_headers(bridge_dir: Path, auth_headers: dict[str, str]) -> bool:
    ...
    so launch-written headers such as ``X-AgentNexus-Runner-Tunnel-Token`` — set
