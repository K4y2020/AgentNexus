...
def state_dir() -> Path:
    if data_dir := os.environ.get("AGENTNEXUS_DATA_DIR"):
...
class LedgerEntries:
    ...
    state_paths: StatePathsEntry = field(
        default_factory=lambda: StatePathsEntry(
            omnigent_home=str(state_dir()), workspace=str(Path.home() / "agentnexus")
        )
    )
        ...
        entries = LedgerEntries(
            ...
            state_paths=StatePathsEntry(
                **cast(
                    ...
                    or {
                        "agentnexus_home": str(state_dir()),
                        "workspace": str(Path.home() / "agentnexus"),
                    },
                )
            ),
        )
...
def observed_external_configs(*, deep: bool) -> list[ExternalConfigEntry]:
    ...
    cwd = Path.cwd()
    candidates = [
        (cwd / ".cursor" / "mcp.json", "mcpServers.agentnexus", "json"),
        (cwd / ".kiro" / "settings" / "mcp.json", "mcpServers.agentnexus", "json"),
        (Path.home() / ".claude.json", "mcpServers.agentnexus", "json"),
        (Path.home() / ".codex" / "config.toml", "mcp_servers.agentnexus", "toml"),
    ]
...
def new_ledger(*, source: str, strategy: str, deep: bool) -> InstallLedger:
    ...
    return InstallLedger(
        ...
        generator={
            "name": "agentnexus",
            "version": _version_for(shutil.which("agentnexus")) or "unknown",
            "strategy": strategy,
            "os": platform_name(),
            "wrote_at": now,
        },
...
def write_install_ledger_from_env() -> InstallLedger:
    ...
    if existing and existing.ledger_source == "installer":
        ...
        for name, dep in existing.entries.deps.items():
            if name in ledger.entries.deps and dep.installed_by in {"agentnexus", "preexisting"}:
                current = ledger.entries.deps[name]
                if current.installed_by == "unknown" or dep.installed_by == "agentnexus":
                    current.installed_by = dep.installed_by
                    current.confidence = dep.confidence
    for name in list(ledger.entries.deps):
        env_name = f"AGENTNEXUS_LEDGER_DEP_{name.upper()}"
        installed_by = os.environ.get(env_name)
        if installed_by in {"agentnexus", "preexisting", "unknown"}:
