...
…) — AgentNexus does not re-execute them. The executor advertises
``handles_tools_internally=True`` and forwards ``tool_calls`` /
``role:"tool"`` events from kimi's transcript as informational
:class:`ToolCallRequest` / :class:`ToolCallComplete` so the AgentNexus
...
- ``AGENTNEXUS_KIMI_PATH``: explicit path to the ``kimi`` binary, e.g.
...
and is managed out-of-band via ``kimi provider add`` (AgentNexus-side
...
from agentnexus.harness_startup_config import resolve_harness_path
...
def _resolve_kimi_binary() -> str:
    ...
    ``AGENTNEXUS_KIMI_PATH`` wins (legacy ``HARNESS_KIMI_PATH`` still honored
...
class KimiExecutor(Executor):
    """Drive ``kimi -p`` per AgentNexus turn.
    ...
    def _translate_event(self, payload: Mapping[str, object]) -> list[ExecutorEvent]:
        """Translate one kimi stream-json line into AgentNexus events.
        ...
        - ``"tool"``: kimi's own tool execution result delivered back to
          its loop. Surfaced as a ``ToolCallComplete`` so the AgentNexus
        ...
        if role == "assistant":
            ...
            # so the AgentNexus UI can render the result. The Session layer
    ...
    async def run_turn(
        ...
        if tools and not self._warned_tools_without_bridge:
            _logger.warning(
                "kimi executor received %d declared tool(s) but AgentNexus has no "
        ...
        if shutil.which(self._binary_path) is None and not Path(self._binary_path).exists():
            yield ExecutorError(
                message=(
                    ...
                    "or set AGENTNEXUS_KIMI_PATH (legacy HARNESS_KIMI_PATH) to its"
                    " absolute location."
                ),
                retryable=False,
            )
