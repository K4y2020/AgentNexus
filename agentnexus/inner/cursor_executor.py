...
``AsyncAgent`` per AgentNexus conversation, created on a
...
evaluates PHASE_TOOL_CALL and cancels the run on DENY.  Bridged AgentNexus
...
def _normalize_cursor_usage(raw: Mapping[str, object], model: str) -> _JsonObject:
    """Map Cursor SDK usage fields to the standard AgentNexus usage dict."""
    ...
    # map to the AgentNexus-standard cache_read_input_tokens /
...
def _build_cursor_prompt(
    ...
    *,
    ...
    first turn the AgentNexus system prompt is prepended (the SDK has no separate
...
def _sdk_message_to_events(message: object) -> list[ExecutorEvent]:
    ...
    if mtype == "tool_call":
        ...
        # AgentNexus tool name + args so the observed events (and any name-keyed
...
def _write_cursor_hooks(cwd: str, hook_script_path: str, server_url: str, session_id: str) -> Path:
    ...
    :param server_url: AgentNexus server URL, e.g. ``"http://127.0.0.1:6767"``.
    ...
    wrapper = hooks_dir / "agentnexus-hook.sh"
...
class CursorExecutor(Executor):
    ...
    def __init__(
        ...
        *,
        ...
        :param permission_mode: AgentNexus permission stance. ``"auto"`` (default)
    ...
    def _make_execute(
        ...
        """Build a sync ``execute`` that bridges a cursor tool call to AgentNexus.
    ...
    async def _close_state(self, state: _CursorSessionState) -> None:
        ...
        if state.hooks_file is not None:
            try:
                ...
                wrapper = state.hooks_file.parent / "agentnexus-hook.sh"
