...
AgentNexus stores no credential. Tool approvals surface as web elicitation cards
...
- ``HARNESS_ACP_AGENTNEXUS_MCP``: ``"0"`` to disable AgentNexus's MCP relay;
...
- ``HARNESS_ACP_PERMISSION_MODE``: AgentNexus permission stance, ``auto`` (default) or
...
- ``HARNESS_ACP_INJECT_SYSTEM_PROMPT``: ``"0"`` to skip folding the AgentNexus system
  prompt into the first ACP turn. Recommended for Pi-fork agents (e.g. ``omp``) that
  fully own their own system prompt — prepending AgentNexus's text can cause the agent's
...
_ENV_AGENTNEXUS_MCP = "HARNESS_ACP_AGENTNEXUS_MCP"
