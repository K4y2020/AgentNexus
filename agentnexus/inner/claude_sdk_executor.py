...
underlying agent harness.  AgentNexus tools are bridged into the SDK session
...
This executor translates the SDK message stream into AgentNexus ExecutorEvents
...
from agentnexus._platform import resolve_cli_binary, stable_user_id
...
from agentnexus.inner.bundle_skills import ensure_bundle_plugin_manifest
from agentnexus.inner.hook_scripts import subagent_router
from agentnexus.json_types import JsonObject as _JsonObject
from agentnexus.llms._usage_observer import notify_from_dict as _notify_usage_from_dict
from agentnexus.llms.adapters._content import parse_data_uri as _parse_replay_data_uri
from agentnexus.reasoning_effort import CLAUDE_EFFORTS, validate_effort
...
# ---------------------------------------------------------------------------
# TypeAliases for AgentNexus JSON-shaped boundary values. The SDK exchanges
...
# with the AgentNexus tool executor and the SDK's MCP bridge.
...
_NO_SANDBOX_ENV = "AGENTNEXUS_CLAUDE_SDK_NO_SANDBOX"

# Env override for an explicit claude binary, mirroring codex's
# AGENTNEXUS_CODEX_PATH. Set this when claude lives on a PATH the host
# daemon doesn't inherit (e.g. an nvm-managed global bin dir).
_CLAUDE_PATH_ENV = "AGENTNEXUS_CLAUDE_PATH"
...
# path the AgentNexus producer resolves the model instead (see workflow.py).
_CLAUDE_API_KEY_HELPER_ENV_KEY = "AGENTNEXUS_CLAUDE_API_KEY_HELPER"
...
class PreparedClaudeCli:
    """Result of wrapping the Claude CLI in an AgentNexus sandbox.
...
def _build_mcp_tools(
    ...
    """Build SdkMcpTool objects from AgentNexus tool schemas.

    Each tool is backed by a handler that calls the AgentNexus tool_executor
    ...
    for schema in tool_schemas:
        ...
        # itself does not accept ``None``. AgentNexus tool schemas always
...
def _find_system_claude() -> str | None:
    ...
    Resolves via the ``AGENTNEXUS_CLAUDE_PATH`` override, then ``PATH``, then
...
def _resolve_databricks_claude_model(profile: str | None) -> str:
    ...
    """
    try:
        from agentnexus.databricks_model_discovery import discover_databricks_claude_catalog
...
def _resolve_sandbox_cwd(spec_cwd: str | None) -> pathlib.Path:
    ...
    ``AGENTNEXUS_RUNNER_WORKSPACE``. Prefer that workspace as the base so
    ...
    """
    base = os.environ.get("AGENTNEXUS_RUNNER_WORKSPACE") or os.getcwd()
    if spec_cwd:
...
def prepare_claude_cli_path(
    ...
    as ``AGENTNEXUS_CLAUDE_SDK_NO_SANDBOX``: file/shell access still goes
...
class ClaudeSDKExecutor(Executor):
    ...
    The SDK runs Claude Code's full agent loop internally.  AgentNexus tools
    ...
    is set.  Even without ``os_env``, AgentNexus tries to place the Claude CLI
    ...
    turns by keeping a live Claude SDK client per AgentNexus session.
        ...
        *,
        ...
        Args:
            ...
            os_env: If set, enable built-in OS tools (Bash, Read, Edit, …)
                ...
                spec's sandbox is enabled, AgentNexus wraps the Claude CLI
                in the same sandbox. If omitted, AgentNexus still tries to
        ...
        # the AgentNexus elicitation system rather than silently allowed.
        # ``None`` until the adapter installs it on first use.
        self._elicitation_handler: ElicitationHandler | None = None
        # Live Claude SDK clients keyed by AgentNexus session id.
    ...
    async def _can_use_tool_for_permission(
        ...
        """
        Route a Claude SDK permission request through the AgentNexus elicitation system.
        ...
        if allowed:
            return PermissionResultAllow()
        return PermissionResultDeny(message="Denied via AgentNexus elicitation")
    ...
    async def _evaluate_tool_call_policy(
        ...
        evaluated against AgentNexus TOOL_CALL-phase policies BEFORE they
        ...
        :returns: :class:`claude_agent_sdk.PermissionResultDeny` when a
            ...
            existing AgentNexus elicitation handler before returning
        ...
        if _action == "POLICY_ACTION_ASK":
            ...
            reason = _verdict.reason or "Approval required by AgentNexus TOOL_CALL policy"
            if self._elicitation_handler is None:
                logger.warning(
        ...
        if _action == "POLICY_ACTION_DENY":
            ...
            reason = _verdict.reason or "Denied by AgentNexus TOOL_CALL policy"
            logger.info("TOOL_CALL policy denied tool=%s reason=%s", tool_name, reason)
            return PermissionResultDeny(message=reason)
        from claude_agent_sdk import PermissionResultDeny

        reason = f"Unexpected AgentNexus TOOL_CALL policy verdict: {_action!r}"
    ...
    async def run_turn(
        ...
        # Build MCP tools from AgentNexus tool schemas
        mcp_tools = _build_mcp_tools(tools, self._tool_executor) if tools else []

        # Create MCP server config for AgentNexus tools. The SDK's
        ...
        if mcp_tools:
            mcp_servers["agentnexus"] = sdk.create_sdk_mcp_server(
                name="agentnexus",
                version="1.0.0",
                tools=mcp_tools,
            )
        ...
        # through AgentNexus ``sys_os_*`` MCP tools rather than the
        ...
        if self._permission_mode in ("auto", "bypassPermissions"):
            # Allow all AgentNexus MCP tools (no per-call human gate needed)
            for schema in tools:
        ...
        # to a ``databricks-*`` model: the AgentNexus producer always resolves a
        ...
        # OS-environment tools are provided via AgentNexus ``sys_os_*``
        ...
        # the CLI subprocess are evaluated against AgentNexus TOOL_CALL
        ...
        # callback, call it with the request data so the AgentNexus server
