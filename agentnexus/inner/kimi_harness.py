...
- ``HARNESS_KIMI_CWD`` — working directory the kimi subprocess runs in
  ...
  subprocess ``cwd=``).
- ``AGENTNEXUS_KIMI_PATH`` — path to the ``kimi`` binary. Default
...
``~/.kimi/config.toml`` (out-of-band from AgentNexus) — upstream kimi
has no per-spawn ``--config-file`` or env-var provider override.
AgentNexus-side provider injection remains a deferred follow-up.
"""
...
from agentnexus.inner.kimi_executor import KimiExecutor, _resolve_skills_dirs
...
def _resolve_os_env() -> OSEnvSpec:
    ...
    has its own internal sandbox / approval flow, so AgentNexus does not
...
def _build_kimi_executor() -> Executor:
    ...
    return KimiExecutor(
        ...
        # else the runner's AGENTNEXUS_RUNNER_WORKSPACE (the cwd the user launched
