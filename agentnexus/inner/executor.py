"""Executor adapter interface for AgentNexus.
...
# AgentNexus tool schema passed to the LLM:
...
class SubAgentStarted(ExecutorEvent):
    ...
    Surfaced so the runner can mint an AgentNexus child session (the web
...
class Executor:
    ...
    async def close_session(self, session_key: str) -> None:  # noqa: ARG002 — default no-op; subclasses with per-session state override
        """
        Release resources associated with one AgentNexus session.
