"""Hermes ``pre_tool_call`` shell hook for AgentNexus policy enforcement.
...
The hook evaluates ``PHASE_TOOL_CALL`` policy via the AgentNexus server.
...
def main() -> None:
    ...
    if action == "POLICY_ACTION_DENY":
        ...
        if reason:
            out["reason"] = f"Tool '{tool_name}' denied by AgentNexus policy: {reason}"
        else:
            out["reason"] = f"Tool '{tool_name}' denied by AgentNexus policy"
