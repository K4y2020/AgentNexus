...
- ``HARNESS_CODEX_CWD``: working directory the executor launches
  the Codex CLI in. ``None`` falls back to ``AGENTNEXUS_RUNNER_WORKSPACE``
  if set, then to the subprocess's inherited cwd.
- ``AGENTNEXUS_CODEX_PATH``: absolute path to a ``codex`` CLI binary.
...
_ENV_CODEX_PATH = "AGENTNEXUS_CODEX_PATH"
...
def _resolve_retry_policy() -> RetryPolicy:
    ...
    ``RetryPolicy()`` (defaults) when missing — AgentNexus omits the
...
def _build_codex_executor() -> Executor:
    ...
    :raises ImportError: If the ``codex`` CLI isn't on PATH and
        ``AGENTNEXUS_CODEX_PATH`` (legacy ``HARNESS_CODEX_PATH``) isn't set — the inner executor's
