...
surface as web elicitation cards (via AgentNexus's TOOL_CALL policy + ``ctx.elicit``
...
``~/.config/goose/config.yaml``); AgentNexus stores no Goose credential. A spec
...
- ``HARNESS_GOOSE_CWD``: working directory for the goose subprocess. ``None``
  falls back to ``AGENTNEXUS_RUNNER_WORKSPACE`` then the inherited cwd.
- ``AGENTNEXUS_GOOSE_PATH``: absolute path to a ``goose`` CLI binary.
...
_ENV_GOOSE_PATH = "AGENTNEXUS_GOOSE_PATH"
