...
AgentNexus session, persists the Codex thread across turns, and exposes
AgentNexus tools to Codex as App Server ``dynamicTools``.
"""
...
from agentnexus.inner.agent_env import clean_agent_env, declared_passthrough
from agentnexus.llms._usage_observer import notify_from_dict as _notify_usage_from_dict
from agentnexus.model_fallbacks import CODEX_CATALOG_CLONE_SOURCE_SLUG, CODEX_DEFAULT_MODEL
from agentnexus.reasoning_effort import CODEX_EFFORTS, EFFORT_ALIASES, validate_effort
...
# to running sessions without any action from AgentNexus. ``.credentials.json``
...
# AGENTNEXUS_GOOSE_PATH. Set this when codex lives on a PATH the host
# daemon doesn't inherit (e.g. an nvm-managed global bin dir).
_CODEX_PATH_ENV = "AGENTNEXUS_CODEX_PATH"
...
def _clean_codex_env(extra_allow: Iterable[str] = ()) -> dict[str, str]:
    ...
    the subprocess env: the app-server session reads AgentNexus's per-session
    codex signals back out of it, so those names have to survive the filter
    (see :data:`_CODEX_AGENTNEXUS_LAUNCH_ENV_VARS`).
    ...
    return clean_agent_env(
        ...
        allow_exact=(
            ...
            *_CODEX_AGENTNEXUS_LAUNCH_ENV_VARS,
        ),
...
def _is_omnigent_private_codex_home(path: Path) -> bool:
    """
    Return whether *path* is an AgentNexus-created private ``CODEX_HOME``.

    AgentNexus launches Codex with private homes for session state
    ...
    :param path: Candidate ``CODEX_HOME`` path, e.g.
        ``"/home/user/.agentnexus/codex-native/<hash>/codex-home"``.
    ...
    if (
        ...
        and parts[-4] == ".agentnexus"
    ):
        return True
    return expanded.name.startswith("agentnexus-codex-home-")
...
def _private_codex_home_config_source(path: Path) -> Path | None:
    ...
    A parent AgentNexus launch bridges ``auth.json`` and ``config.toml`` into
...
def _resolve_codex_home_config_source(source_dir: Path, home_codex_home: Path) -> Path:
    ...
    points at an AgentNexus private home from a parent session. In that
...
def _codex_home_config_source_from_env() -> Path:
    ...
    Codex stores subscription login state in ``CODEX_HOME``. AgentNexus
    launches Codex with private per-session homes for isolation, then bridges
    only auth/config from the user's source home. Nested AgentNexus processes
...
def _populate_codex_home_config(
    ...
    *,
    ...
    :param inject_hooks: Skip the ``hooks.json`` symlink because the caller
        generates a merged regular file (user hooks + AgentNexus hooks) at that
...
def materialize_codex_provider_config(
    ...
    *,
    ...
    :param retry_policy: AgentNexus retry policy to apply through Codex's native
...
CODEX_ROUTER_DIR_ENV_VAR = "AGENTNEXUS_CODEX_SUBAGENT_ROUTER_DIR"
# Session the spawns belong to, baked into the generated hook commands.
CODEX_ROUTER_SESSION_ID_ENV_VAR = "AGENTNEXUS_CODEX_SUBAGENT_ROUTER_SESSION_ID"
_CODEX_ROUTER_HOOK_MODULE = "agentnexus.inner.hook_scripts.codex_router_hook"
...
def _codex_router_hook_command(
    ...
    *,
    ...
    :param bridge_dir: Session bridge directory holding the router
        advertisement.
    :param session_id: AgentNexus session id, or ``None`` when the
...
def codex_router_hooks_settings(
    ...
    *,
    ...
    """
    Build the AgentNexus half of a routing ``hooks.json`` payload.
    ...
    :param session_id: AgentNexus session id baked into the commands.
...
def merge_codex_user_hooks(payload: dict[str, Any], user_hooks_path: Path) -> dict[str, Any]:
    ...
    AgentNexus's entries stay in first position per event so the routing
...
def write_codex_hooks_file(
    ...
    *,
    ...
    order (AgentNexus's stay in first position per event) and the user's
...
def write_codex_router_hooks_file(
    ...
    *,
    ...
    :param codex_home: Private per-session ``CODEX_HOME``.
    :param bridge_dir: Session bridge directory.
    :param session_id: AgentNexus session id baked into the hook commands.
...
CODEX_EXTENDED_CATALOG_ENV_VAR = "AGENTNEXUS_CODEX_EXTENDED_MODEL_CATALOG"
...
#: AgentNexus's own per-session signals for a codex launch: the subagent-router
...
_CODEX_AGENTNEXUS_LAUNCH_ENV_VARS: tuple[str, ...] = (
...
def _databricks_codex_config_overrides(
    *,
    ...
    """
    provider_name = "agentnexus_databricks"
    ...
    return [
        ...
        (
            "model_providers.agentnexus_databricks="
            '{name="AgentNexus Databricks",'
...
def _provider_codex_config_overrides(
    *,
    ...
    :returns: Codex TOML-fragment override strings.
    """
    provider_name = "agentnexus_provider"
    auth_executable, auth_args = _codex_auth_command_argv(auth_command)
...
class _PendingToolResult:
    ...
    :param name: The tool name Codex asked AgentNexus to run.
...
class _CodexAppServerSession:
    ...
    async def start(self) -> None:
        ...
        self._codex_home_dir = Path(
            tempfile.mkdtemp(prefix="agentnexus-codex-home-", dir=str(codex_home_root))
        )
        ...
        try:
            ...
            if router_bridge_dir is not None:
                ...
                from agentnexus.codex_native_app_server import trust_codex_router_hooks
    ...
    def __init__(
        ...
        *,
        ...
        :param gateway_host: Gateway workspace host origin, e.g.
            ...
            ``HARNESS_CODEX_GATEWAY_HOST`` (written by the AgentNexus workflow
        ...
        :param base_url_override: Override the Codex gateway base URL instead
            ...
            ``HARNESS_CODEX_GATEWAY_BASE_URL`` (written by the AgentNexus workflow
        ...
        if gateway:
            ...
            # gateway path the AgentNexus producer must have resolved a model, and the
            ...
            if host is None:
                ...
                if model is None:
                    # Directly-supplied neutral gateway: the AgentNexus producer always
                    ...
                    raise OSError(
                        ...
                        "model: the AgentNexus producer must resolve one before spawning."
                    )
        ...
        if not enable_web_search:
            ...
            # tools exposed by AgentNexus as dynamicTools. The top-level web_search
