...
_DEFAULT_ENV_PASSTHROUGH: tuple[str, ...] = (
    ...
    "PYTHONFAULTHANDLER",
    # AgentNexus session marker: always pass the "inside AgentNexus" marker
...
def create_os_environment(spec: OSEnvSpec | None) -> OSEnvironment | None:
    ...
    if spec.fork:
        fork_dir = Path(tempfile.mkdtemp(prefix="agentnexus-fork-"))
...
def _child_shell_env() -> dict[str, str]:
    ...
    omnigent.inner.os_env`` can import agentnexus. That entry has no business in
    ...
    if isinstance(token_value, str) and token_value:
        ...
        for env_key in ("HTTP_PROXY", "HTTPS_PROXY", "http_proxy", "https_proxy"):
            ...
            if not parts.hostname:
                continue
            netloc = f"agentnexus:{token_value}@{parts.hostname}"
