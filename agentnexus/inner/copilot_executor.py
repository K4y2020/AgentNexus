...
per AgentNexus conversation, created once and reused turn to turn. Each
...
Crucially, AgentNexus's spec-declared tools (``sys_session_send`` et al.) are
...
*inside* the SDK rather than through AgentNexus's bridged-tool dispatch. They are
...
from agentnexus.reasoning_effort import COPILOT_EFFORTS, validate_effort
...
# AgentNexus's bridged-tool callback: (tool_name, args) -> awaitable result.
...
def _build_copilot_prompt(messages: list[Message], *, is_first_turn: bool) -> str:
    ...
    AgentNexus system prompt is delivered separately (``system_message``), so on
...
class _CopilotSessionState:
    """Per-AgentNexus-conversation SDK session state."""
...
class CopilotExecutor(Executor):
    ...
    def __init__(
        ...
        *,
        ...
        # AgentNexus's session (policy gating, sub-agent dispatch, logging).
    ...
    def _make_tools(self, tools: list[ToolSpec]) -> list[Any]:  # type: ignore[explicit-any]
        """Build the SDK ``tools`` list from AgentNexus ToolSpecs.
        ...
        ``skip_permission=True`` because AgentNexus's policy layer (and the
    ...
    def _make_handler(self, tool_name: str) -> Callable[[Any], Awaitable[Any]]:  # type: ignore[explicit-any]
        """Build an async ``handler`` that bridges a Copilot tool call to AgentNexus.
    ...
    async def _on_permission_request(
        ...
        if evaluator is not None:
            ...
            if getattr(verdict, "action", "") == "POLICY_ACTION_DENY":
                ...
                return PermissionDecisionReject(feedback=f"Denied by AgentNexus policy: {reason}")
        ...
        if handler is not None:
            ...
            if not approved:
                return PermissionDecisionReject(feedback="Denied via AgentNexus approval UI")
    ...
    async def _ensure_session(
        ...
        try:
            ...
            # its built-in tools) and layers the AgentNexus agent's system prompt
...
def _ambient_github_token(host: str | None = None) -> str | None:
    ...
    for var in GITHUB_TOKEN_ENV_VARS:
        ...
        if value:
            return value
    from agentnexus.onboarding.copilot_auth import gh_cli_github_token
...
def _unwrap_tool_result(raw: Any) -> Any:  # type: ignore[explicit-any]
    ...
    AgentNexus-convention result (e.g. ``{"blocked": True}``) passes through
