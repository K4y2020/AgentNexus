...
turns to maintain conversational context across the AgentNexus session without
re-serialising the full history.

Each turn yields text output as ``TextChunk`` / ``TurnComplete`` events.
AgentNexus policies are enforced on Hermes' native tool calls via Hermes'
...
Requirements:
    ...
    ``AGENTNEXUS_HERMES_PATH``; legacy ``HARNESS_HERMES_PATH`` still honored).
...
- ``AGENTNEXUS_HERMES_PATH`` — absolute path to the ``hermes`` CLI binary.
...
class HermesExecutor(Executor):
    ...
    history is maintained without AgentNexus re-serializing the full
    message list.
    ...
    ``config.yaml`` that registers an AgentNexus policy hook as a
    ...
    def _setup_hermes_home(self) -> None:
        ...
        When the AgentNexus server URL and conversation ID are available,
        writes a ``config.yaml`` that registers the AgentNexus policy hook as a
        Hermes ``pre_tool_call`` shell hook and an ``mcp_servers.agentnexus``
        entry (``serve-mcp``) exposing AgentNexus builtin tools to the model.
    ...
    def _hermes_session_id(self, session_key: str) -> str | None:
        """Return the stored Hermes session ID for an AgentNexus session key."""
    ...
    def handles_tools_internally(self) -> bool:
        ...
        through AgentNexus's tool dispatch.  AgentNexus policies are enforced
        via Hermes' native ``pre_tool_call`` shell hook that evaluates
        ``PHASE_TOOL_CALL`` against the AgentNexus server before each tool
        execution.
        """
        return True
    ...
    async def run_turn(
        ...
        :param messages: Conversation history from AgentNexus.
    ...
    def _session_key(self, messages: list[Message]) -> str:
        """
        Derive a stable AgentNexus session key from the message list.
    ...
    async def close_session(self, session_key: str) -> None:
        ...
        via `hermes --resume` outside AgentNexus.
        """
