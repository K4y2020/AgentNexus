...
AgentNexus does not intercept. qwen *can* delegate approval externally
(``can_use_tool`` control requests on ``--json-file``, answered via
``confirmation_response`` on ``--input-file``) — wiring that through AgentNexus's
...
from agentnexus.inner.qwen_native_executor import QwenNativeExecutor
