"""Shared terminal-output styling for the AgentNexus CLI.
...
#: ``AGENTNEXUS_NO_SPINNER`` convention in :mod:`omnigent._runner_startup`.
NO_BANNER_ENV_VAR = "AGENTNEXUS_NO_BANNER"
...
console = Console(theme=AGENTNEXUS_THEME, highlight=False)
#: Console for stderr — status, warnings, errors, and the brand banner.
err_console = Console(stderr=True, theme=AGENTNEXUS_THEME, highlight=False)
...
def show_banner(*, isatty: bool | None = None, env: Mapping[str, str] | None = None) -> bool:
    ...
    and can be force-disabled with ``AGENTNEXUS_NO_BANNER``. Color *within*
...
def print_landing(
    *,
    ...
    banner is suppressed (non-TTY or ``AGENTNEXUS_NO_BANNER``).
