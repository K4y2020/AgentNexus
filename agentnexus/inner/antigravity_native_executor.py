"""Executor that delivers AgentNexus web/mobile turns into a native Antigravity agy.

``omnigent antigravity`` runs the Antigravity ``agy`` CLI in a runner-owned tmux
terminal and mirrors its transcript into the AgentNexus session via the RPC read
driver (the read path). This executor is the **write path**: when a turn is
submitted from the AgentNexus web/mobile UI it delivers the user's message by
...
so the agy TUI and the AgentNexus web mirror share ONE conversation in both
...
``GetAvailableModels``. The AgentNexus ``ExecutorConfig.model``/``reasoning_effort``
...
from agentnexus.reasoning_effort import ANTIGRAVITY_EFFORTS, validate_effort_or_llm_error
...
class AntigravityNativeExecutor(Executor):
    ...
    async def run_turn(
        ...
        :param tools: Tool schemas from AgentNexus. Ignored; native agy owns its
            own tool surface.
...
def _request_session_id_from_env() -> str | None:
    """
    Resolve the AgentNexus session id that requested this harness process.

    :returns: AgentNexus session id, e.g. ``"conv_abc123"``, or ``None``.
    """
