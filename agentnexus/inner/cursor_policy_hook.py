"""Cursor preToolUse hook script for AgentNexus policy enforcement.
...
PHASE_TOOL_CALL policy via the AgentNexus server, and returns the
verdict on stdout.

Environment variables (baked into the hooks.json command by the
CursorExecutor at session startup):

    _AGENTNEXUS_SERVER_URL  : Base URL of the AgentNexus server
                            (e.g. ``http://127.0.0.1:6767``).
    _AGENTNEXUS_SESSION_ID  : Session / conversation ID for policy
                            evaluation.
"""
...
def main() -> None:
    server_url = os.environ.get("_AGENTNEXUS_SERVER_URL", "")
    session_id = os.environ.get("_AGENTNEXUS_SESSION_ID", "")
    ...
    if resp is None:
        ...
        message = f"Tool '{tool_name}' blocked: AgentNexus policy evaluation unavailable"
        if detail:
            message += f" ({detail})"
    ...
    if action == "POLICY_ACTION_DENY":
        ...
        if reason:
            out["agent_message"] = f"Tool '{tool_name}' denied by AgentNexus policy: {reason}"
