...
- ``HARNESS_PI_CWD``: working directory the executor launches
  the Pi CLI in. ``None`` falls back to ``AGENTNEXUS_RUNNER_WORKSPACE`` if set,
  then to the subprocess's inherited cwd.
- ``AGENTNEXUS_PI_PATH``: absolute path to a ``pi`` CLI binary.
...
- ``HARNESS_PI_OS_ENV``: JSON-encoded :class:`OSEnvSpec`
  ...
  AgentNexus mode parity with the legacy non-AP path holds for
...
from agentnexus.inner.pi_executor import PiExecutor
...
_ENV_PI_PATH = "AGENTNEXUS_PI_PATH"
...
def _resolve_os_env() -> OSEnvSpec:
    ...
    AgentNexus serialized via :func:`dataclasses.asdict` on its
...
def _build_pi_executor() -> Executor:
    ...
    :raises ImportError: If the ``pi`` CLI isn't on PATH and
        ``AGENTNEXUS_PI_PATH`` (legacy ``HARNESS_PI_PATH``) isn't set — the inner executor's
    ...
    return PiExecutor(
        cwd=os.environ.get(_ENV_CWD) or os.environ.get("AGENTNEXUS_RUNNER_WORKSPACE"),
