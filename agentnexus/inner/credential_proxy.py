...
class DatabricksProfileTokenProvider:
    ...
    def __init__(
        ...
        *,
        ...
        :raises AgentNexusError: If the SDK is unavailable or the profile has
            no ``host``.
        """
    ...
    def resolve(self) -> str:
        ...
        :raises AgentNexusError: If authentication fails.
        """
        with self._lock:
            now = self._clock()
...
def _databricks_config(profile: str) -> object:
    ...
    :raises AgentNexusError: If the ``databricks`` extra is not installed or
...
def _databricks_authenticate(config: object) -> str:
    ...
    :raises AgentNexusError: If authentication fails or returns a non-Bearer
        scheme.
    """
    try:
...
def _prepare_databricks_runtime(
    ...
    *,
    ...
    :raises AgentNexusError: If a profile can't be resolved or two profiles
