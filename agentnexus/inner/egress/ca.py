...
def _generate_ca(cert_path: Path, key_path: Path) -> None:
    ...
    subject = issuer = x509.Name(
        [
            x509.NameAttribute(NameOID.COMMON_NAME, "AgentNexus Egress MITM CA"),
            x509.NameAttribute(NameOID.ORGANIZATION_NAME, "agentnexus"),
        ]
    )
...
def _system_ca_bundle() -> bytes:
    ...
    """
    from agentnexus.tls import resolve_ca_file
