"""Expose AgentNexus's builtin tools to an ACP agent via ``session/new.mcpServers``.
...
MCP server, which proxies each AgentNexus tool call back through ``tool_executor``
(→ :meth:`TurnContext.dispatch_tool` → the AgentNexus server, where TOOL_CALL /
TOOL_RESULT policy is enforced). The agent keeps its own filesystem/shell tools;
this only *adds* AgentNexus's builtin tools (``sys_session_*``, ``sys_agent_*``,
...
``AGENTNEXUS_ACP_MCP=0`` kill switch) yields an empty ``mcpServers`` — the agent
just runs without AgentNexus tools, exactly as before this feature.
"""
...
if TYPE_CHECKING:
    from agentnexus.claude_native_bridge import ClaudeNativeToolRelay, ToolExecutor
...
_ENV_KILL_SWITCH = "AGENTNEXUS_ACP_MCP"
...
class AgentNexusAcpMcp:
    """Lazily-started AgentNexus-tool relay + its ACP ``mcpServers`` entry.
    ...
    def session_new_servers(
        ...
        *,
        ...
        :param tools: AgentNexus tool schemas to advertise (each ``{"name", …}``).
        ...
        try:
            ...
            logger.info(
                "acp[%s] AgentNexus MCP relay ready (%d builtin tools bridged)",
                self._label,
                len(tools),
            )
            return self._acp_servers
