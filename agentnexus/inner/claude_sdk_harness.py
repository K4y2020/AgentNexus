...
than per-spec data on the request body. AgentNexus sets these env vars
...
# Set by the AgentNexus workflow layer when executor.auth: {type: api_key, …} is
...
def _resolve_retry_policy() -> RetryPolicy:
    ...
    missing — AgentNexus omits the env var when ``llm.retry`` matches
...
def _build_claude_sdk_executor() -> Executor:
    ...
    return ClaudeSDKExecutor(
        ...
        # AGENTNEXUS_RUNNER_WORKSPACE (the folder the user launched in, and
