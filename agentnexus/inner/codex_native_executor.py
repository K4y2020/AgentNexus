"""Executor that bridges AgentNexus messages into a native Codex TUI thread."""
...
from agentnexus.inner.codex_goal_command import goal_objective_from_content
...
class CodexNativeExecutor(Executor):
    ...
    async def interrupt_session(self, session_key: str) -> bool:
        ...
        try:
            if pending:
                ...
                # cancel above already updated what AgentNexus shows, and a
    ...
    async def run_turn(
        ...
        :param tools: Tool schemas from AgentNexus. Ignored here;
        ...
        :param config: Per-turn executor config. Its ``model`` and
            ``extra["reasoning_effort"]`` (carrying the AgentNexus web
...
def _model_effort_overrides(config: ExecutorConfig | None) -> dict[str, object]:
    ...
    A model or reasoning-effort change selected in the AgentNexus web UI is
