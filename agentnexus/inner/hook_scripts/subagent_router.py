...
# Per-turn relay advertisement listing the AgentNexus tools this session holds.
...
# MCP server the bridge registers the AgentNexus tools under. Must match
# ``claude_native_bridge._MCP_SERVER_NAME`` (and the codex-native
# ``[mcp_servers.agentnexus]`` table), which the same no-import rule forbids
# reading directly.
...
# AgentNexus tools a routed spawn needs. Named in the deny reason only when the
...
ROUTER_DIR_ENV_VAR = "AGENTNEXUS_SUBAGENT_ROUTER_DIR"
# Session the spawn belongs to, when the harness knows it out of band.
SESSION_ID_ENV_VAR = "AGENTNEXUS_SUBAGENT_ROUTER_SESSION_ID"
...
def _diagnose(message: str) -> None:
    ...
    """
    print(f"agentnexus subagent router: {message}", file=sys.stderr)
...
def _windows_pid_alive(pid: int) -> bool:
    """Return whether a Windows process is still active (dependency-free).

    ``os.kill(pid, 0)`` does not exist on Windows, so the probe opens the
    process with ``PROCESS_QUERY_LIMITED_INFORMATION`` and asks for its exit
    code. A nonexistent pid fails ``OpenProcess`` with
    ``ERROR_INVALID_PARAMETER`` and is rejected; an access-denied live process
    stays fail-open because the bearer token still gates the actual request.
    """
    import ctypes
    ...
    if not handle:
        ...
        if err == ERROR_INVALID_PARAMETER:
            return False
        # Any other failure is ambiguous; keep the old fail-open behavior.
        return True
    try:
...
def _advertiser_alive(pid: Any) -> bool:  # type: ignore[explicit-any]
    ...
    trusted. On Windows the liveness probe uses ``OpenProcess``; on POSIX it
    uses ``os.kill(pid, 0)``. The probe is also blind inside a PID-namespaced
    sandbox
...
def resolve_session_id(
    ...
    *,
    ...
    """
    Resolve the AgentNexus session the spawn belongs to.
...
def mcp_tool_name(bare: str, harness: str | None) -> str:
    """
    Spell an AgentNexus tool the way *harness* advertises it.

    :param bare: AgentNexus tool name, e.g. ``"sys_session_create"``.
...
def advertised_relay_tools(bridge_dir: str | Path | None) -> frozenset[str]:
    """
    Read the AgentNexus tool names this session's relay advertises.
...
def _no_spawn_tool_instruction(model: str, picked: str) -> str:
    ...
    return (
        ...
        "your built-in spawn tool cannot launch, and this session holds no AgentNexus "
...
def routed_spawn_instruction(
    ...
    *,
    ...
    :param available_tools: AgentNexus tools the session's relay advertises.
...
def decision_to_hook_output(
    ...
    *,
    ...
    :param requesting_harness: Harness whose model reads a deny reason,
        which decides how the AgentNexus tools are spelled.
    :param available_tools: AgentNexus tools the session's relay advertises,
...
def read_stdin_payload(
    ...
    :param label: Diagnostic prefix, e.g. ``"agentnexus codex router hook"``.
