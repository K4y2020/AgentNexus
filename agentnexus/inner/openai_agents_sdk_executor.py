...
AgentNexus as the system of record for tools, policies, and session history.
AgentNexus tools are exposed as SDK FunctionTools whose callbacks route back
through the Session's tool registry.

Conversation state is delegated to the SDK's session layer. Each AgentNexus
Session gets its own SDK SQLiteSession keyed by the AgentNexus session id.
"""
...
from agentnexus.llms.errors import is_context_length_exceeded as _is_context_length_exceeded
from agentnexus.reasoning_effort import OPENAI_AGENTS_EFFORTS, validate_effort
...
# to the AgentNexus tool executor.
...
def _normalize_content_blocks_for_chat(
    ...
    AgentNexus-only metadata (for example ``filename``) from known Responses
    ...
    ``filename``.  That metadata is useful in AgentNexus, but the OpenAI API
    ...
    for block in content:
        ...
        if block_type == "input_file":
            ...
            # Preserve inline images; strip AgentNexus-only metadata.
...
def _copy_known_keys(block: _JsonObject, keys: tuple[str, ...]) -> _JsonObject:
    ...
    AgentNexus content blocks can carry UI/history metadata such as
...
def _get_openai_async_client(
    ...
    try:
        ...
        raise ImportError(
            ...
            "the package (`pip install 'agentnexus[databricks]'`) or set "
    ...
    def __init__(
        ...
        *,
        ...
        :param base_url_override: Override the OpenAI-compatible base URL
            ...
            the AgentNexus workflow layer). Required whenever ``gateway_host`` is set.
