...
internally. This executor translates the ACP event stream into AgentNexus
...
tool calls as AgentNexus tool cards (``tool_call`` → ``ToolCallRequest``,
...
from agentnexus.inner._acp_omnigent_mcp import AgentNexusAcpMcp
from agentnexus.inner.acp_extension import NO_ACP_EXTENSION, AcpExtension
from agentnexus.inner.acp_subagents import SubAgentActivity, SubAgentStart, read_subagent_events
...
from agentnexus.inner.os_env import OSEnvironment, create_os_environment
from agentnexus.process_logging import current_process_log_path, display_log_path
...
class AcpAgentConfig:
    ...
    :param omnigent_mcp: Expose AgentNexus's builtin tools to the agent via
        ``session/new.mcpServers`` (the shared ``serve-mcp`` relay). On by
        default; the global ``AGENTNEXUS_ACP_MCP=0`` kill switch also disables it.
    ...
    :param permission_mode: AgentNexus permission stance, e.g. ``"auto"``
    ...
    :param inject_system_prompt: Fold the AgentNexus system prompt into the first
        ...
        system prompt: those agents prepend AgentNexus's text to the user message,
...
class AcpExecutor(Executor):
    ...
    def __init__(
        ...
        *,
        ...
        # reads/writes back to us (executed through the AgentNexus OSEnvironment,
        ...
        # mid-turn ``session/request_permission`` routes through AgentNexus's
        ...
        # attribute the SDK harnesses use); backs the AgentNexus MCP relay.
        self._tool_executor: _ToolExecutor | None = None

        # AgentNexus-tool MCP bridge — exposes builtin tools to the agent via
        ...
        self._mcp = AgentNexusAcpMcp(label=config.name)
        self._omnigent_tools: list[ToolSpec] = []
        # Tool names the agent can use to reach the AgentNexus MCP bridge, snapshotted
        ...
        try:
            resp = await self._rpc(
                _AGENT_METHOD_INITIALIZE,
                {
                    ...
                    "clientInfo": {"name": "agentnexus", "version": "1.0"},
                    "clientCapabilities": {
                        "fs": {
    ...
    async def _ensure_session(self) -> str:
        ...
        ``mcpServers`` carries AgentNexus's builtin tools (via the shared serve-mcp
        relay) unless disabled — see :class:`AgentNexusAcpMcp`.
        """
        ...
        params: _AcpJsonObject = {
            ...
            # helper returns [] when AgentNexus MCP is disabled.
    ...
    def _session_mcp_servers(self) -> list[_AcpJsonObject]:
        ...
        cannot drift apart. No entries means no aliases, so with AgentNexus MCP
        disabled every call classifies as agent-native.
        """
        mcp_servers: list[_AcpJsonObject] = []
        if self._config.agentnexus_mcp:
    ...
    async def _respond_to_agent_request(self, request: _AcpJsonObject) -> None:
        ...
        - ``session/request_permission`` — decide via AgentNexus's TOOL_CALL policy
        ...
        - ``fs/read_text_file`` / ``fs/write_text_file`` — when fs delegation is
          advertised, execute through the AgentNexus OSEnvironment so the spec's
        ...
        try:
            if method == _AGENT_REQUEST_REQUEST_PERMISSION:
                ...
                error = {
                    "code": -32601,
                    "message": f"agentnexus: unsupported ACP request method {method!r}",
                }
    ...
    async def _ensure_os_environment(self) -> OSEnvironment:
        ...
        if self._os_environment is None:
            ...
            if env is None:
                raise _AcpRequestError(-32603, "agentnexus: no os_env for fs delegation")
    ...
    def _usage_from_result(result: _AcpJsonObject) -> dict[str, int] | None:
        """Map an agent's final ``result.usage`` to AgentNexus's usage keys.
        ...
        ``cachedReadTokens``; AgentNexus's ``TurnComplete.usage`` uses
    ...
    def _is_bridge_tool_call(self, name: str, update: _AcpJsonObject) -> bool:
        """True when this tool call reaches AgentNexus through the MCP bridge.
    ...
    def _handle_session_update(self, update: _AcpJsonObject) -> list[ExecutorEvent]:
        ...
        if update_type == _UPDATE_AGENT_MESSAGE_CHUNK:
            ...
            if isinstance(call_id, str) and call_id:
                ...
                # round-trips AgentNexus dispatch. Leaving its id in the correlation
    ...
    async def run_turn(
        ...
        ``tools`` (AgentNexus's builtin tool schemas) are captured for the AgentNexus
        ...
        self._omnigent_tools = (tools or []) if self._config.agentnexus_mcp else []
        try:
        ...
        # (the default), fold the AgentNexus system prompt into the first turn so
    ...
    async def close(self) -> None:
        ...
        self._reset_session_state()
        # Tear down the AgentNexus MCP relay HTTP server + its bridge dir first.
