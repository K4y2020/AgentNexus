...
event stream into AgentNexus ExecutorEvents.

AgentNexus tools are bridged into Pi via a generated JavaScript extension
that registers each tool with ``pi.registerTool()``.  Tool execution is
proxied over a local TCP socket to the AgentNexus Python process, so
policies, history recording, sub-agents, runtime, and all other AgentNexus
...
from agentnexus.inner.agent_env import clean_agent_env
from agentnexus.inner.native_attachments import parse_data_uri
...
from agentnexus.model_metadata import ModelWireAPI
from agentnexus.onboarding.provider_config import CHAT_WIRE_API, RESPONSES_WIRE_API
from agentnexus.pi_model_compatibility import (
...
# ---------------------------------------------------------------------------
# TCP tool server — serves AgentNexus tools to the Pi extension
# ---------------------------------------------------------------------------
...
class _ToolServer:
    ...
    async def _evaluate_policy(  # type: ignore[explicit-any]
        ...
        AgentNexus outage can't wedge the agent mid-turn.
        """
...
# ---------------------------------------------------------------------------
# Pi extension generator — bridges AgentNexus tools into Pi
# ---------------------------------------------------------------------------
...
def _generate_extension_js(port: int, tool_schemas: list[ToolSpec], token: str) -> str:
    """Generate a JavaScript Pi extension that registers AgentNexus tools.

    Each tool is forwarded to the TCP tool server at ``127.0.0.1:<port>``.

    :param port: TCP port the AgentNexus tool server is listening on, e.g.
        ``54321``.
    :param tool_schemas: AgentNexus tool schemas to register with Pi.
    ...
    return f"""\
// Auto-generated AgentNexus tool bridge extension for Pi.
// Connects to the AgentNexus TCP tool server on port {port}.
...
/**
 * Ask the AgentNexus tool server for a TOOL_CALL policy verdict on a native
...
module.exports = function(pi) {{
  // Gate native (non-bridged) tool calls through AgentNexus policy. Pi's
  ...
  pi.on("tool_call", async (event) => {{
    ...
    if (verdict && verdict.block) {{
      return {{ block: true, reason: verdict.reason || "blocked by AgentNexus policy" }};
    }}
  }});

  for (const tool of TOOLS) {{
    // Pi passes tool.parameters directly to the LLM as JSON Schema, so we
    // can use the AgentNexus schema as-is without TypeBox conversion.
    pi.registerTool({{
      name: tool.name,
      label: tool.name,
...
def _build_pi_prompt(messages: list[Message], *, is_first_turn: bool) -> str | list[_JsonObject]:
    ...
    :param messages: AgentNexus conversation history for the
        turn.
...
def _aggregate_pi_turn_usage(
    ...
    A single AgentNexus turn drives Pi's full agent loop, which may make
...
class PiExecutor(Executor):
    ...
    def __init__(
        ...
        *,
        ...
        :param gateway_host: Gateway workspace host origin, e.g.
            ...
            ``HARNESS_PI_GATEWAY_HOST`` (written by the AgentNexus workflow layer).
        ...
        # off (they don't route through AgentNexus policies / history and
    ...
    async def _resolve_model(self, config: ExecutorConfig | None) -> str | None:
        ...
        if model is None and self._gateway_uses_databricks_profile:
            ...
            from agentnexus.inner.claude_sdk_executor import _resolve_databricks_claude_model
    ...
    async def _ensure_tool_server(self, tools: list[ToolSpec]) -> int | None:
        """Start the TCP tool server if there are AgentNexus tools to bridge."""
        if not tools:
            return None
    ...
    async def _gate_native_tool(  # type: ignore[explicit-any]
        ...
        """Evaluate a native Pi tool call against AgentNexus TOOL_CALL policy.
        ...
        PreToolUse hooks: the verdict is computed by the AgentNexus server
    ...
    def _build_env_and_dir(
        ...
        :param tools: AgentNexus tool schemas to bridge into Pi.
        :param tool_server_port: TCP port the AgentNexus tool server is
        ...
        """
        env = dict(self._env)
        tmp_dir = tempfile.mkdtemp(prefix="agentnexus_pi_")
        ...
        if self._gateway:
            ...
            from agentnexus.inner.pi_settings import prepare_managed_pi_agent_dir
        ...
        # Generate the AgentNexus tool bridge extension if tools are available.
        if tools and tool_server_port is not None:
            ...
            ext_path = os.path.join(tmp_dir, "agentnexus_tools.js")
            ...
            # an AgentNexus TOOL_CALL policy verdict; see
