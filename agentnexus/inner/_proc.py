...
from agentnexus._platform import IS_LINUX, IS_POSIX
...
def malloc_tuning_env() -> dict[str, str]:
    ...
    branch. Honors operator overrides ``AGENTNEXUS_RUNNER_MALLOC_ARENA_MAX``
    (default ``"2"``; ``"0"`` disables the cap) and
    ``AGENTNEXUS_RUNNER_MALLOC_TRIM_THRESHOLD`` (default 128 MiB).
    ...
    arena_max = os.environ.get(
        "AGENTNEXUS_RUNNER_MALLOC_ARENA_MAX", _DEFAULT_MALLOC_ARENA_MAX
    ).strip()
    if arena_max and arena_max != "0":
        out["MALLOC_ARENA_MAX"] = arena_max
    trim = os.environ.get(
        "AGENTNEXUS_RUNNER_MALLOC_TRIM_THRESHOLD", _DEFAULT_MALLOC_TRIM_THRESHOLD
    ).strip()
    if trim:
