...
class _AgentNexusYamlLoader(yaml.SafeLoader):
...
_AgentNexusYamlLoader.yaml_implicit_resolvers = {
    key: value[:] for key, value in yaml.SafeLoader.yaml_implicit_resolvers.items()
}
for key, resolvers in list(_AgentNexusYamlLoader.yaml_implicit_resolvers.items()):
    _AgentNexusYamlLoader.yaml_implicit_resolvers[key] = [
...
_AgentNexusYamlLoader.add_implicit_resolver(  # type: ignore[no-untyped-call]
...
def load_agent_def(
    ...
    *,
    ...
    if isinstance(path_or_dict, (str, Path)):
        ...
        with open(path) as f:
            data = yaml.load(f, Loader=_AgentNexusYamlLoader)
...
def _reject_unregistered_policy_handlers(data: YamlData) -> None:
    ...
    """
    from agentnexus.policies.registry import is_registered_handler
...
def _resolve_instructions(
    ...
    so omnigent-flavored YAMLs and native AgentNexus YAMLs treat the
    field identically:
    ...
    - File-path-shaped value that doesn't resolve: silently
      treated as inline text. Matches native AgentNexus behavior so users
...
def _parse_os_env_spec(data: YamlData | str | bool | None) -> OSEnvSpec | None:
    ...
    # Mirror the AgentNexus YAML parser's cross-field validation
    ...
    # accepts with weaker semantics than the AgentNexus parser would have
    # allowed).
...
def _parse_os_env_sandbox_spec(data: YamlData | str | bool | None) -> OSEnvSandboxSpec:
    ...
    # Mirror the AgentNexus parser's hard reject of ``egress_rules`` paired with
    ...
    from agentnexus.spec.parser import (
...
def load_agent_def_from_path(path_str: str) -> AgentDef:
    ...
    from agentnexus.spec import _find_omnigent_yaml_in_dir

    if path.is_dir():
        resolved_dir = path
        cleanup: tempfile.TemporaryDirectory[str] | None = None
    else:
        from agentnexus.spec.tar_utils import extract_safe

        cleanup = tempfile.TemporaryDirectory(prefix="agentnexus-bundle-")
