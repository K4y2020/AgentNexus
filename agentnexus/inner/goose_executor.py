...
output streams into the AgentNexus conversation as chat, and Goose's mid-turn tool
...
ExecutorEvents and routes Goose's permission requests through AgentNexus's
...
class GooseExecutor(Executor):
    ...
    def __init__(
        ...
        # Exposes AgentNexus builtin tools to goose via session/new.mcpServers
        # (the shared serve-mcp relay); goose keeps its own developer tools.
        self._tool_executor: _ToolExecutor | None = None
        self._mcp = AgentNexusAcpMcp(label="goose")
    ...
    async def _respond_to_agent_request(self, request: _AcpJsonObject) -> None:
        ...
        - ``fs/read_text_file`` / ``fs/write_text_file`` — when fs delegation is
          ...
          routes its file I/O here, executed through the AgentNexus OSEnvironment so
    ...
    def _usage_from_result(result: _AcpJsonObject) -> dict[str, int] | None:
        """Map Goose's final ``result.usage`` to AgentNexus's usage keys.

        Goose reports ``{totalTokens, inputTokens, outputTokens}``; AgentNexus's
