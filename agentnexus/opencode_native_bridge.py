...
Layout (per bridge id):
    ...
    ~/.agentnexus/opencode-native/<sha256(bridge_id)[:32]>/
...
OPENCODE_NATIVE_BRIDGE_ID_LABEL_KEY = "agentnexus.opencode_native.bridge_id"
...
# AgentNexus builtin tools (``sys_*``/``load_skill``/``web_fetch``) advertised in
...
# the elicitation against the AgentNexus server (mirrors codex-native's
...
# AgentNexus policy engine (REQUEST + TOOL_RESULT phases the reactive
# ``permission.asked`` path can't reach).
_POLICY_PLUGIN_FILE = "agentnexus-policy.js"
...
# function exports as plugins (legacy shape). It reads its AgentNexus coordinates
...
// AgentNexus policy bridge for opencode-native (generated; do not edit).
// Forwards opencode lifecycle hooks to the AgentNexus policy engine so
...
const BASE = (process.env.AGENTNEXUS_POLICY_URL || "").replace(/\/+$/, "");
const SESSION = process.env.AGENTNEXUS_SESSION_ID || "";
const RELAY_FILE = process.env.AGENTNEXUS_RELAY_FILE || "";
// Full routing header map baked by the runner for the direct-server fallback.
let POLICY_HEADERS = {};
try {
  POLICY_HEADERS = JSON.parse(process.env.AGENTNEXUS_POLICY_HEADERS || "{}") || {};
} catch (e) {
  POLICY_HEADERS = {};
}
...
export const AgentNexusPolicyPlugin = async () => ({
  ...
  "chat.message": async (_input, output) => {
    ...
    if (verdict.result === "POLICY_ACTION_DENY") {
      ...
      throw new Error(
        "AgentNexus policy blocked this prompt: " + (verdict.reason || "request denied"),
      );
    }
  },
  ...
  "tool.execute.after": async (input, output) => {
    ...
    if (verdict.result === "POLICY_ACTION_DENY") {
      output.output = "[AgentNexus policy withheld this tool result: " +
...
def write_opencode_policy_plugin(bridge_dir: Path) -> Path:
    """
    Write the AgentNexus policy-bridge plugin into *bridge_dir* and return its path.

    The runner registers the returned path in the synthesized ``opencode.json``
    ``plugin`` field and stamps ``AGENTNEXUS_POLICY_URL`` / ``AGENTNEXUS_SESSION_ID``
    / ``AGENTNEXUS_POLICY_HEADERS`` on the ``opencode serve`` process so the plugin
...
_STATE_VERSION = 1
_BRIDGE_ROOT = Path.home() / ".agentnexus" / "opencode-native"
_ID_HASH_CHARS = 32
...
def bridge_root() -> Path:
    ...
    :returns: Absolute root for OpenCode-native bridge directories, e.g.
        ``Path("~/.agentnexus/opencode-native")``.
    """
    return _BRIDGE_ROOT
...
def write_cost_popup_config(
    ...
    the opencode pane; it must POST the verdict to the AgentNexus server but cannot
    ...
    :param bridge_dir: OpenCode-native bridge directory.
    :param ap_server_url: AgentNexus server base URL, e.g. ``"http://127.0.0.1:6767"``.
...
def update_model_override(bridge_dir: Path, model_override: str | None) -> bool:
    """
    Persist a new per-session model override (AgentNexus→opencode model switch).
