...
* **Dev clone** (``git clone … && uv sync`` / ``pip install -e .``): we
  ...
  to ``~/.agentnexus/.update_check.json`` so the (potentially slow)
...
* **Installed wheel** (``uv tool install omnigent``,
  ...
  old. The index is resolved from ``AGENTNEXUS_INDEX_URL`` /
...
_ENV_SKIP = "AGENTNEXUS_NO_UPDATE_CHECK"
_CACHE_DIR = Path.home() / ".agentnexus"
_CACHE_FILE = _CACHE_DIR / ".update_check.json"
_STALENESS_SECONDS = 4 * 60 * 60  # 4 hours
_GIT_TIMEOUT_SECONDS = 5
_DIST_NAME = "agentnexus"
...
# explicit ``AGENTNEXUS_INDEX_URL`` wins; then uv's and pip's. When none is
...
def _resolve_index_url() -> str:
    ...
    1. index env vars (:data:`_INDEX_ENV_VARS`) — explicit
       ``AGENTNEXUS_INDEX_URL`` first, then uv's and pip's;
...
def _versions_from_filenames(filenames: Iterable[str]) -> list[Version]:
    ...
    :param filenames: Distribution filenames, e.g.
        ``["agentnexus-0.2.0-py3-none-any.whl", "agentnexus-0.2.0.tar.gz"]``.
    ...
    with contextlib.suppress(OSError, ValueError):
        subprocess.Popen(
            [
                sys.executable,
                "-c",
                "from agentnexus.update_check import refresh_update_cache as r; r()",
            ],
...
def _read_build_info() -> tuple[float, str] | None:
    ...
    """
    try:
        from agentnexus import _build_info
    except ImportError:
        return None
    try:
...
def _parse_extras_from_spec(spec: str) -> list[str]:
    ...
    Examples: ``"agentnexus[all,server]"`` → ``["all", "server"]``.
...
def _package_spec(*, version: str | None = None, extras: Collection[str] = ()) -> str:
    ...
    :returns: ``"agentnexus"``, ``"agentnexus[all]"``, ``"agentnexus==0.2.0[all]"``,
...
def _probe_installed_distribution() -> tuple[str | None, str | None]:
    ...
    *pre-upgrade* version. A fresh ``sys.executable`` subprocess loads the
    ...
    probe = (
        ...
        "try:\n"
        "    print(m.version('agentnexus'))\n"
        "except Exception:\n"
        "    print('')\n"
        "try:\n"
        "    from agentnexus.update_check import _read_installed_wheel_info as r\n"
        "    i = r()\n"
