...
Why under ``~/.agentnexus/`` and not the existing bridge dir at
...
the day after creating it still gets the chdir prompt. ``~/.agentnexus/``
is where the persistent AgentNexus server SQLite db and other durable
single-user state already live.

Layout (per conversation):

    ~/.agentnexus/claude-native/<sha256(conv_id)[:32]>/launch.json

The directory is hashed (not the raw conv id) so a malicious server
returning an attacker-chosen conversation id like ``"../../../etc"``
cannot escape ``~/.agentnexus/claude-native/``. A short hash is enough
...
from agentnexus.process_logging import data_dir
...
# ``~/.agentnexus/claude-native``. Tests should set this to a per-
# test ``tmp_path`` via monkeypatch.setenv so they never touch the
# user's real home directory.
_STATE_ROOT_ENV_VAR = "AGENTNEXUS_CLAUDE_NATIVE_STATE_DIR"
...
def _claude_native_state_root() -> Path:
    ...
    ``AGENTNEXUS_DATA_DIR``, falling back to
    ``~/.agentnexus/claude-native``.
    ...
    directories on disk). Lives under ``~/.agentnexus/claude-native/``
...
def _state_dir_for_conversation_id(conversation_id: str) -> Path:
    ...
    :param conversation_id: AgentNexus conversation id, bare 32-char hex
...
def read_launch_state(conversation_id: str) -> ClaudeNativeLaunchState | None:
    ...
    * the picker, to render the Workspace column for each row.
    ...
    user who wiped ``~/.agentnexus/claude-native/``). The callers
