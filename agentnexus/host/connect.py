...
from agentnexus._platform import IS_POSIX, WINDOWS_ENV_PASSTHROUGH
from agentnexus.cli_invocation import cli_invocation
from agentnexus.debug_logging import (
    ORIGIN_WORKSPACE_ID_ENV_VAR,
    PRIMARY_SESSION_ID_ENV_VAR,
    USER_ID_ENV_VAR,
)
from agentnexus.env_credentials import env_names_with_omnigent_prefix
from agentnexus.gateway_inference import gateway_inference_map
from agentnexus.harness_aliases import canonicalize_harness, is_claude_sdk_harness_name
from agentnexus.harness_availability import HARNESS_BINARY_MISSING, HarnessAvailability
from agentnexus.host import HOST_FATAL_EXIT_CODE
from agentnexus.host.daemon_lifecycle import DaemonLifecycleLock
...
from agentnexus.host.identity import HostIdentity, load_or_create_host_identity
from agentnexus.host.runner_zygote import ZygoteManager, ZygoteRunnerProc, ZygoteUnavailable
from agentnexus.inner import _proc
from agentnexus.onboarding.harness_auth import (
...
from agentnexus.onboarding.provider_config import ANTHROPIC_FAMILY, OPENAI_FAMILY
...
from agentnexus.suspend_watch import watch_for_resume
...
def _read_log_tail(path: Path, max_bytes: int = _LOG_TAIL_MAX_BYTES) -> str:
    ...
    :param path: The runner's captured stdout/stderr log file, e.g.
        ``Path("~/.agentnexus/logs/runner/runner-ab12.log")``.
...
# ``AGENTNEXUS_HOST_LIFECYCLE_POLL_S`` overrides it (e2e tests set it low so the
# self-terminate path is observable without a minute-long wait).
try:
    _LIFECYCLE_POLL_INTERVAL_S = float(os.environ.get("AGENTNEXUS_HOST_LIFECYCLE_POLL_S", "60"))
except ValueError:
...
_RUNNER_ENV_ALLOWLIST: frozenset[str] = frozenset(
    {
        ...
        #   - AGENTNEXUS_CONFIG_HOME: where config.yaml / provider config live,
        #     so the runner resolves the same providers the CLI configured.
        #   - AGENTNEXUS_DATA_DIR: where the sqlite db + pidfile live, so the
        ...
        # AGENTNEXUS_DATABASE_URI is intentionally NOT here — it may embed a
        ...
        # AGENTNEXUS_AUTH_PROVIDER and the daemon-spawned local server
        ...
        "AGENTNEXUS_LOG_LEVEL",
        "AGENTNEXUS_LOG_TO_STDERR",
        LOG_TTY_FD_ENV_VAR,
        ...
        "AGENTNEXUS_DEBUG_LOG_CLIENT_ID",
        "AGENTNEXUS_DEBUG_LOG_CLIENT_SECRET",
        "AGENTNEXUS_DEBUG_LOG_WORKSPACE_URL",
        "AGENTNEXUS_DEBUG_LOG_ENDPOINT",
        ...
        # ``AGENTNEXUS_CLAUDE_SDK_NO_SANDBOX=1 omnigent run …`` had no
        ...
        "AGENTNEXUS_CLAUDE_SDK_NO_SANDBOX",
        ...
        # (and the harness it spawns) never see AGENTNEXUS_TELEMETRY_ENABLED, so
        ...
        # telemetry is opt-in. Not a secret (a boolean). The AGENTNEXUS_OTEL_*
        # knobs (capture-content, FastAPI toggle) ride the prefix allowlist below.
        "AGENTNEXUS_TELEMETRY_ENABLED",
        ...
        # remote daemon prefixes are DATABRICKS_ + LC_/MLFLOW_/OTEL_/AGENTNEXUS_OTEL_,
        # not plain AGENTNEXUS_), so _build_runner_env never sees the names it lists
...
# both the standard ``OTEL_*`` vars and AgentNexus's ``AGENTNEXUS_OTEL_*`` knobs
# (capture-content, FastAPI toggle) so they reach the runner/harness too.
_RUNNER_ENV_ALLOWLIST_PREFIXES: tuple[str, ...] = ("LC_", "MLFLOW_", "OTEL_", "AGENTNEXUS_OTEL_")
...
RUNNER_ENV_PASSTHROUGH_ENV_VAR: str = "AGENTNEXUS_RUNNER_ENV_PASSTHROUGH"
...
def _build_runner_env(
    ...
    *,
    ...
    :param base_env: Host process environment to filter, e.g.
        ``os.environ``.
    :param server_url: AgentNexus server URL the runner connects back to, e.g.
    ...
    # AGENTNEXUS_RUNNER_ENV_PASSTHROUGH — their credential resolves fine in
    # the CLI/daemon but silently drops before reaching the runner subprocess.
    from agentnexus.errors import AgentNexusError as _AgentNexusError
    ...
    except (OSError, _AgentNexusError):
...
class HostProcess:
    ...
    :param server_url: AgentNexus server URL, e.g.
    ...
    def __init__(
        ...
        # to AGENTNEXUS_USER_ID so host/runner debug-log rows carry it. None until
        ...
        # AGENTNEXUS_RUNNER_ZYGOTE=0 (or false/no/off) to opt out onto the direct
    ...
    def _login_fix_hint(self) -> str:
        ...
        the server runs AgentNexus accounts or OIDC auth, a Databricks
    ...
    def _classify_http_status(self, status: int, body: str = "") -> HostConnectError | None:
        ...
        if status == 403:
            ...
            from agentnexus.cli_auth import stored_token_status
    ...
    async def _handle_import_local(
        ...
        def _targets() -> tuple[list[tuple[str, str]], str | None]:
            ...
            from agentnexus.session_import.models import ImportSource, SessionImportNotFoundError
        ...
        def _load(source: str, session_id: str) -> HostImportedLocalSession | None:
            from agentnexus.session_import.local import load_local_session
            from agentnexus.session_import.models import ImportSource, SessionImportNotFoundError
    ...
    async def _probed_codex_model_options(self) -> ModelOptionsResult | None:
        ...
        """
        from agentnexus.codex_native_app_server import codex_launch_catalog
    ...
    async def _probed_claude_model_options(self) -> ModelOptionsResult | None:
        ...
        :returns: The catalog listing, or ``None`` when unavailable.
        """
        from agentnexus.claude_native import claude_launch_catalog, resolve_native_claude_config
    ...
    async def _ensure_owner_user_id(self) -> None:
        ...
        this host spawns) and in ``AGENTNEXUS_USER_ID`` (so the host's own
        ...
        if self._owner_user_id is not None:
            return
        try:
            from agentnexus.resume_dispatch import _resolve_current_user_id
    ...
    def _build_connect_headers(self) -> dict[str, str]:
        ...
        """
        from agentnexus.host.identity import HOST_TOKEN_ENV_VAR, MANAGED_HOST_TOKEN_HEADER
        ...
        headers: dict[str, str] = {"Origin": AGENTNEXUS_INTERNAL_WS_ORIGIN}
    ...
    def _current_auth_token(self, *, initialize: bool = True) -> str | None:
        ...
        """
        from agentnexus.host.identity import HOST_TOKEN_ENV_VAR
    ...
    async def _serve_frames(self, ws: websockets.asyncio.client.ClientConnection) -> None:
        ...
        _tel_opt_out = False
        try:
            from agentnexus.telemetry.client import is_disabled as _tel_disabled
        ...
        try:
            from agentnexus.telemetry.installation_id import get_installation_id as _get_install_id
