"""Persistent background local AgentNexus server lifecycle.
...
from agentnexus.config import global_config_path
...
def _local_data_dir() -> Path:
    ...
    Honors ``AGENTNEXUS_DATA_DIR`` (the purpose-built data-isolation knob),
    else ``~/.agentnexus``. This lets a checkout/worktree isolate its local
    runtime DB: two worktrees otherwise share ``~/.agentnexus/chat.db``, and
    ...
    inheriting a shared ``AGENTNEXUS_CONFIG_HOME``).
...
# config change (e.g. flipping AGENTNEXUS_AUTH_ENABLED) respawns the
...
def server_config_signature(*, include_features: bool = True) -> str:
    ...
    The daemon (in local mode) spawns the AgentNexus server once and never
    ...
    the user flips ``AGENTNEXUS_AUTH_ENABLED``, or upgrades the package) and
    ...
    * the installed package version — a running server holds its code in
    ...
    from agentnexus.server.feature_flags import resolve_feature_flags
    from agentnexus.server.server_config import session_title_instructions

    title_instructions = None
    if include_features:
        from agentnexus.config import load_global_config

        title_instructions = session_title_instructions(load_global_config())

    try:
        version = importlib.metadata.version("agentnexus")
...
def _write_local_server_record(
    ...
    :param log_path: Absolute path of the spawned server's captured log file,
        e.g. ``Path("/Users/alice/.agentnexus/logs/server/server-ab12cd.log")``.
...
def _read_local_server_log_path() -> Path | None:
    ...
    :returns: The absolute log path the background server writes to, e.g.
        ``Path("/Users/alice/.agentnexus/logs/server/server-ab12cd.log")``, or
...
def _atomic_write(path: Path, text: str) -> None:
    ...
    :param path: Destination file, e.g. ``~/.agentnexus/local_server.pid``.
...
class LocalServerInfo:
    ...
    :param log_path: Absolute path of the background server's captured log
        file, e.g. ``Path("/Users/alice/.agentnexus/logs/server/server-ab12cd.log")``.
...
def local_server_status() -> LocalServerInfo:
    ...
    Reads ``~/.agentnexus/local_server.pid`` for the recorded pid/port and
...
def ensure_local_omnigent_server() -> LocalServerStartup:
    """Ensure a persistent background local AgentNexus server is running.
    ...
    persistent ``~/.agentnexus`` data store so conversations survive across
    invocations (designs/RUN_AGENTNEXUS_SESSION_RESUMPTION.md). The server runs
...
class _SpawnedLocalServer:
    ...
    :param log_path: File capturing the child's stdout/stderr, e.g.
        ``Path("~/.agentnexus/logs/server/server-ab12cd.log")``.
...
def _spawn_local_server(port: int) -> _SpawnedLocalServer:
    ...
    # An explicit AGENTNEXUS_DATABASE_URI wins over the per-data-dir sqlite
    # file (e.g. to point a worktree at its own Postgres). Defaults to the
    # isolated sqlite db under the runtime data dir.
    db_uri = os.environ.get("AGENTNEXUS_DATABASE_URI") or f"sqlite:///{db_path}"
    ...
    # the env-unset default; AGENTNEXUS_AUTH_ENABLED=1 opts into accounts (or
    # oidc when AGENTNEXUS_OIDC_* is set). In header/oidc mode we must NOT mint
    ...
    child_env["AGENTNEXUS_LOCAL_SINGLE_USER"] = "1"
    _accounts_mode = resolve_auth_source() == "accounts"
    if _accounts_mode:
        if "AGENTNEXUS_ACCOUNTS_COOKIE_SECRET" not in os.environ:
            from agentnexus.server.accounts_secret import (
                load_or_generate_cookie_secret,
            )

            child_env["AGENTNEXUS_ACCOUNTS_COOKIE_SECRET"] = load_or_generate_cookie_secret(data_dir)
...
def _local_server_health_ok(base_url: str) -> bool:
    """Return ``True`` if *base_url* answers ``/health`` as an AgentNexus server.

    Confirms a listener is actually an AgentNexus server (``GET /health`` →
...
def stop_untracked_local_server(port: int = _DEFAULT_LOCAL_PORT) -> int | None:
    ...
    running. This sweep covers that hole: if a live AgentNexus server answers
