...
- ``--server <url>``: connect to an existing (remote or local) AgentNexus server.
- ``--local``: this daemon owns a local AgentNexus server — start (or reuse) a
...
def main() -> None:
    ...
    parser.add_argument(
        "--local",
        action="store_true",
        help="Start (or reuse) a local AgentNexus server and connect to it.",
    )
    ...
    from agentnexus.host.daemon_lifecycle import normalize_daemon_target
