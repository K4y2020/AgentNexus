...
(``run`` / ``claude`` / ``codex``) asks the AgentNexus server to launch
...
from agentnexus.process_logging import display_log_path, process_log_dir
...
def open_daemon_client(
    ...
    *,
    ...
    :param base_url: AgentNexus server base URL, e.g. the workspace API mount.
