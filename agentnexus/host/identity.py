...
Reads or creates the ``host`` section in ``~/.agentnexus/config.yaml``.
...
HOST_TOKEN_ENV_VAR = "AGENTNEXUS_HOST_TOKEN"
HOST_ID_ENV_VAR = "AGENTNEXUS_HOST_ID"
HOST_NAME_ENV_VAR = "AGENTNEXUS_HOST_NAME"

# WebSocket upgrade header carrying a managed host's launch token.
# Mirrors the runner tunnel's X-AgentNexus-Runner-Tunnel-Token pattern:
...
# auth provider.
MANAGED_HOST_TOKEN_HEADER = "X-AgentNexus-Host-Token"
...
def load_or_create_host_identity(
    ...
    :param path: Path to the config YAML file, e.g.
        ``Path("~/.agentnexus/config.yaml")``. Defaults to
        :data:`CONFIG_PATH`.
