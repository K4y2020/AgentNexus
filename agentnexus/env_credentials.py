"""Helpers for AgentNexus-prefixed credential environment variables."""
...
AGENTNEXUS_ENV_PREFIX = "AGENTNEXUS_"

_ENV_REF_RE = re.compile(r"(?<!\$)\$(?:\{([A-Za-z_][A-Za-z0-9_]*)\}|([A-Za-z_][A-Za-z0-9_]*))")


def omnigent_prefixed_env_name(name: str) -> str:
    """Return the AgentNexus-prefixed alias for *name*.

    :param name: Environment variable name, e.g. ``"ANTHROPIC_API_KEY"``.
    :returns: ``"AGENTNEXUS_ANTHROPIC_API_KEY"`` unless *name* is already
        AgentNexus-prefixed.
    """
    return name if name.startswith(AGENTNEXUS_ENV_PREFIX) else f"{AGENTNEXUS_ENV_PREFIX}{name}"


def env_names_with_omnigent_prefix(name: str) -> tuple[str, ...]:
    """Return the canonical env var name plus its AgentNexus-prefixed alias.
...
def getenv_with_omnigent_prefix(
    ...
    """Read *name*, falling back to ``AGENTNEXUS_<name>`` when unset.
...
def getenv_nonempty_with_omnigent_prefix(
    ...
    """Read a non-empty env var with ``AGENTNEXUS_`` fallback.
...
def expand_envvars_with_omnigent_prefix(value: str) -> str:
    """Expand ``$VAR`` references with ``AGENTNEXUS_VAR`` fallback.
    ...
    ``AGENTNEXUS_VAR`` is set, the prefixed value is used. Unresolved references
