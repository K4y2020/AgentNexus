...
_BRIDGE_ROOT = Path(tempfile.gettempdir()) / f"agentnexus-{stable_user_id()}" / "kiro-native"
_TMUX_FILE = "tmux.json"
_FORWARDER_READY_FILE = "kiro_session_forwarder_ready.json"
_ACP_RECORD_FILE = "kiro_acp_record.jsonl"
# Shared AgentNexus MCP relay (serve-mcp) registration for kiro.
...
_PASTE_BUFFER = "agentnexus-kiro-paste"
...
def write_mcp_bridge_config(bridge_dir: Path) -> None:
    """Write the token config the shared AgentNexus MCP bridge requires at boot.
...
def build_kiro_mcp_config(
    ...
    """Build the kiro ``mcpServers`` entry for the AgentNexus relay MCP server.
    ...
    per-server auto-approve field, so AgentNexus MCP tool calls surface through
...
def write_kiro_workspace_mcp_config(
    ...
    *,
    ...
    """Write the workspace-scoped kiro MCP config declaring the AgentNexus server.
    ...
    ``.cursor/mcp.json``). The AgentNexus entry is *merged* into any existing
