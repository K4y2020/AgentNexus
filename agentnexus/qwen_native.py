"""Native qwen TUI wrapper for the AgentNexus CLI.

``omnigent qwen`` launches Qwen Code's interactive TUI (``qwen``) inside an
AgentNexus-runner-owned tmux terminal and attaches the local TTY — the qwen analog
...
``/auth`` command persisted under ``~/.qwen``); no AgentNexus-managed key is
...
from agentnexus._wrapper_labels import QWEN_NATIVE_WRAPPER_VALUE as _WRAPPER_LABEL_VALUE
...
_QWEN_PATH_ENV = "AGENTNEXUS_QWEN_PATH"
...
def run_qwen_native(
    *,
    ...
    """
    Launch the qwen TUI in an AgentNexus terminal.
    ...
    if server is None:
        raise click.ClickException(
            "qwen requires a resolved AgentNexus server URL. The CLI should call "
            "_ensure_backend before run_qwen_native."
        )
    with TemporaryDirectory(prefix="agentnexus-qwen-native-") as tmpdir:
...
def _run_with_remote_server(
    ...
    *,
    ...
    """
    Launch qwen on an AgentNexus server via a daemon-spawned runner.
