"""Claude Code hook recorder for the native AgentNexus wrapper."""
...
def _env_int(name: str, default: int) -> int:
    ...
    try:
        ...
        print(
            f"agentnexus hook: ignoring non-integer {name}={raw!r}; using {default}",
            file=sys.stderr,
        )
        return default
...
def _env_float(name: str, default: float) -> float:
    ...
    *default* rather than crashing the hook at import time.
    ...
    if not math.isfinite(value):
        print(
            f"agentnexus hook: ignoring non-finite {name}={raw!r}; using {default}",
            file=sys.stderr,
        )
        return default
    return value
...
_PERMISSION_MAX_CONSECUTIVE_FAILURES = max(1, _env_int("AGENTNEXUS_HOOK_MAX_RETRIES", 8))
...
# this via AGENTNEXUS_HOOK_HELD_POLL_FLOOR_S so their legitimate slow-human severs
...
_PERMISSION_HELD_POLL_FLOOR_S = max(0.0, _env_float("AGENTNEXUS_HOOK_HELD_POLL_FLOOR_S", 10.0))
...
# budget — an unresponsive AgentNexus server would otherwise hang the banner.
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        ...
        print(f"agentnexus claude hook: malformed JSON: {exc}", file=sys.stderr)
        return 0
    if not isinstance(payload, dict):
        print("agentnexus claude hook: expected JSON object", file=sys.stderr)
        return 0
    ...
    if payload.get("hook_event_name") == "SessionStart" and payload.get("source") == "clear":
        ...
        if rotated_session_id:
            payload["agentnexus_clear_rotated_to"] = rotated_session_id
    elif _is_claude_branch_session_start(payload):
        payload["agentnexus_fork_detected"] = True
        rotated_session_id = _rotate_session_on_fork(bridge_dir)
        if rotated_session_id:
            payload["agentnexus_fork_rotated_to"] = rotated_session_id
    try:
        record_hook_event(bridge_dir, payload)
    except Exception as exc:  # noqa: BLE001 - hook must not break Claude Code.
        print(f"agentnexus claude hook: failed to record hook: {exc}", file=sys.stderr)
    ...
    if conversation_url and payload.get("hook_event_name") == "SessionStart":
        print(
            json.dumps({"systemMessage": (f"Open this session in AgentNexus: {conversation_url}")})
        )
    return 0
...
def _annotate_resume_session_context(bridge_dir: Path, payload: dict[str, object]) -> None:
    ...
    if (
        ...
        payload["agentnexus_previous_claude_session_id"] = current_claude_session_id
    payload["agentnexus_claude_session_was_seen"] = new_claude_session_id in seen
...
def _is_claude_branch_session_start(payload: dict[str, object]) -> bool:
    ...
    :param payload: Hook payload read from Claude Code stdin, e.g.
        ``{"hook_event_name": "SessionStart", "source": "resume"}``.
    :returns: ``True`` when the event should fork the active AgentNexus session.
    """
    if payload.get("hook_event_name") != "SessionStart" or payload.get("source") != "resume":
        return False
    new_claude_session_id = payload.get("session_id")
    if not isinstance(new_claude_session_id, str) or not new_claude_session_id:
        return False
    current_claude_session_id = payload.get("agentnexus_previous_claude_session_id")
    if not isinstance(current_claude_session_id, str) or not current_claude_session_id:
        return False
    if payload.get("agentnexus_claude_session_was_seen") is True:
        return False
...
def _rotate_session_on_clear(bridge_dir: Path) -> str | None:
    """
    Rotate AgentNexus sessions synchronously for a Claude ``/clear`` SessionStart.

    The SessionStart hook output is what Claude renders as the welcome
    banner. Rotating here lets the banner point at the new AgentNexus session
    ...
    :param bridge_dir: Native Claude bridge directory.
    :returns: New AgentNexus session id, e.g. ``"conv_new"``, or ``None`` when
    ...
    # host_id (databricks_request_headers reads AGENTNEXUS_RUNNER_SLICE_KEY when no
    ...
    try:
        ...
        print(f"agentnexus claude clear hook: AgentNexus rotation failed: {exc}", file=sys.stderr)
        return None
    except RuntimeError as exc:
        print(f"agentnexus claude clear hook: rotation failed: {exc}", file=sys.stderr)
        return None
...
def _rotate_session_on_fork(bridge_dir: Path) -> str | None:
    """
    Fork AgentNexus sessions synchronously for a Claude ``/fork``/``/branch``.
    ...
    forked AgentNexus session before Claude prints it. Failures return
    ``None`` so the background forwarder can perform the fork from the
    annotated hook record.

    :param bridge_dir: Native Claude bridge directory.
    :returns: New AgentNexus session id, e.g. ``"conv_fork"``, or ``None``
    ...
    try:
        ...
        print(f"agentnexus claude fork hook: AgentNexus fork failed: {exc}", file=sys.stderr)
        return None
    except RuntimeError as exc:
        print(f"agentnexus claude fork hook: fork failed: {exc}", file=sys.stderr)
        return None
    return new_session_id
...
def _create_clear_replacement_session(
    ...
    """
    Create and activate the fresh AgentNexus session for ``/clear``.

    :param client: Sync AgentNexus HTTP client.
    :param ap_server_url: AgentNexus server base URL without a trailing slash,
    ...
    :raises httpx.HTTPError: If AgentNexus rejects session creation,
        new-session binding, or terminal transfer.
    :raises RuntimeError: If AgentNexus returns malformed session data.
    """
    ...
    if clear_resp.status_code >= 400:
        print(
            (
                "agentnexus claude clear hook: failed to clear old runner binding: "
...
def _create_fork_replacement_session(
    ...
    """
    Create and activate the forked AgentNexus session for Claude ``/fork``.
    ...
    :param old_session_id: Session being forked away from, e.g.
        ``"conv_old"``.
    :param bridge_dir: Native Claude bridge directory.
    :returns: New AgentNexus session id, e.g. ``"conv_fork"``.
    :raises httpx.HTTPError: If AgentNexus rejects session fetch, fork,
        new-session binding, or terminal transfer.
    :raises RuntimeError: If AgentNexus returns malformed session data.
    """
    old_resp = client.get(f"{ap_server_url}/v1/sessions/{url_component(old_session_id)}")
    ...
    if clear_resp.status_code >= 400:
        print(
            (
                "agentnexus claude fork hook: failed to clear old runner binding: "
                f"{clear_resp.status_code} {clear_resp.text}"
            ),
            file=sys.stderr,
        )
...
def _conversation_url_for_active_session(
    ...
    """
    Build the web URL for the bridge's current active AgentNexus session.
...
def _post_hook_with_reattach(
    ...
    * **Held-poll sever → reset the counter.** An established connection that
    ...
    :param headers: Outbound auth headers for the AgentNexus server.
    ...
    from agentnexus.native_policy_hook import _is_login_redirect_or_unauthorized
    ...
    while True:
        ...
        try:
            with httpx.Client(headers=headers, timeout=timeout) as client:
                ...
                if (
                    ...
                    if refreshed:
                        ...
                        reauthed = True
                        print(
                            f"agentnexus {hook_label} hook: AgentNexus auth expired "
            ...
            if exc.response.status_code < 500:
                print(
                    f"agentnexus {hook_label} hook: AgentNexus request rejected: {exc}",
                    file=sys.stderr,
                )
                return None
            ...
            print(
                f"agentnexus {hook_label} hook: AgentNexus request failed; retrying: {exc}",
                file=sys.stderr,
            )
        except httpx.HTTPError as exc:
        ...
        if consecutive_hard_failures >= _PERMISSION_MAX_CONSECUTIVE_FAILURES:
            print(
                f"agentnexus {hook_label} hook: giving up after "
        ...
        if time.monotonic() >= deadline:
            print(
                f"agentnexus {hook_label} hook: retry budget exhausted "
...
def _main_permission_request(argv: list[str]) -> int:
    """
    Forward one Claude ``PermissionRequest`` hook to the active AgentNexus session.
    ...
    """
    from agentnexus.native_policy_hook import policy_hook_reauth
    ...
    try:
        ...
        print(f"agentnexus claude permission hook: malformed JSON: {exc}", file=sys.stderr)
        return 0
    if not isinstance(payload, dict):
        print("agentnexus claude permission hook: expected JSON object", file=sys.stderr)
        return 0
    bridge_dir = Path(args.bridge_dir)
    session_id = read_active_session_id(bridge_dir)
    if not session_id:
        print("agentnexus claude permission hook: active session missing", file=sys.stderr)
        return 0
    config = read_permission_hook_config(bridge_dir)
    ap_server_url = args.agentnexus_server_url or config.get("ap_server_url")
    if not isinstance(ap_server_url, str) or not ap_server_url:
        print("agentnexus claude permission hook: AgentNexus server URL missing", file=sys.stderr)
        return 0
    headers = _parse_headers(args.agentnexus_auth_headers_json)
    if not headers:
        raw_headers = config.get("ap_auth_headers")
        if isinstance(raw_headers, dict):
    ...
    # runner-env host_id (reads AGENTNEXUS_RUNNER_SLICE_KEY; workspace mount only).
    from agentnexus.cli_auth import databricks_request_headers
...
def _main_ask_user_question(argv: list[str]) -> int:
    ...
    in all permission modes), POSTs the same payload to the AgentNexus server's
    ...
    try:
        ...
        print(f"agentnexus ask-user-question hook: malformed JSON: {exc}", file=sys.stderr)
        return 0
    if not isinstance(payload, dict):
        print("agentnexus ask-user-question hook: expected JSON object", file=sys.stderr)
        return 0
    ...
    session_id = read_active_session_id(bridge_dir)
    if not session_id:
        print("agentnexus ask-user-question hook: active session missing", file=sys.stderr)
        return 0
    config = read_permission_hook_config(bridge_dir)
    ap_server_url = args.agentnexus_server_url or config.get("ap_server_url")
    if not isinstance(ap_server_url, str) or not ap_server_url:
        print("agentnexus ask-user-question hook: AgentNexus server URL missing", file=sys.stderr)
        return 0
    headers = _parse_headers(args.agentnexus_auth_headers_json)
    if not headers:
        raw_headers = config.get("ap_auth_headers")
    ...
    if resp is None or not resp.content:
        return 0
    # The AgentNexus server returns a PermissionRequest-shaped response:
    ...
    try:
        ...
        print(f"agentnexus ask-user-question hook: invalid JSON from AP: {exc}", file=sys.stderr)
        return 0
    decision = (
...
def _main_evaluate_policy(argv: list[str]) -> int:
    ...
    ``UserPromptSubmit`` hook against AgentNexus policies.
    ...
    try:
        ...
        print(f"agentnexus evaluate-policy hook: malformed JSON: {exc}", file=sys.stderr)
        return 0
    if not isinstance(payload, dict):
        print("agentnexus evaluate-policy hook: expected JSON object", file=sys.stderr)
        return 0
    bridge_dir = Path(args.bridge_dir)
    session_id = read_active_session_id(bridge_dir)
    if not session_id:
    ...
    if not resp.content:
        print("agentnexus evaluate-policy hook: empty AgentNexus response", file=sys.stderr)
        return _fail_closed()

    try:
        eval_response = resp.json()
    except json.JSONDecodeError:
        print("agentnexus evaluate-policy hook: malformed AgentNexus response", file=sys.stderr)
        return _fail_closed()
...
def _parse_headers(raw: str | None) -> dict[str, str]:
    """
    Parse serialized AgentNexus auth headers for the permission hook.
    ...
    try:
        ...
        print(f"agentnexus claude permission hook: bad headers JSON: {exc}", file=sys.stderr)
        return {}
...
def _write_turn_routing_marker(
    ...
    if write_turn_routing_marker(
        ...
        return True
    print(
        "agentnexus claude route-turn hook: could not write the turn marker",
        file=sys.stderr,
    )
    return False
