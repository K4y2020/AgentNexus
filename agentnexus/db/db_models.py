...
from agentnexus.db.compression import CompressedText
...
class AgentNexusBase(DeclarativeBase):
    """Declarative base for the AgentNexus operational tables.
    ...
    autogenerate can target the AgentNexus side independently of the
    conversation tables.
    """
...
_current_workspace_id: ContextVar[int] = ContextVar(
    "agentnexus_workspace_id", default=DEFAULT_WORKSPACE_ID
)
...
class SqlBotComputerBinding(AgentNexusBase):
...
class SqlComputerExecutionLease(AgentNexusBase):
...
class SqlAccountToken(AgentNexusBase):
    """
...
class SqlDeviceGrant(AgentNexusBase):
    """
    ...
    :param user_id: The AgentNexus identity that approved the grant, set at
...
class SqlSessionPermission(AgentNexusBase):
    """
...
class SqlConversationMetadata(AgentNexusBase):
    ...
    __tablename__ = "agentnexus_conversation_metadata"
    ...
    # Bot ownership is operational metadata: Bot is an AgentNexus entity and the
...
class SqlConversation(ConversationBase):
    ...
    (``agent_id`` + the ``session_overrides`` JSON blob). AgentNexus
...
class SqlComment(AgentNexusBase):
...
class SqlPolicy(AgentNexusBase):
    """
...
class SqlHost(AgentNexusBase):
    ...
    :param host_id: Stable host identifier from the host's local
        ``~/.agentnexus/config.yaml``, e.g. ``"host_a1b2c3d4e5f6..."``.
...
class SqlUserDailyCost(AgentNexusBase):
    """
...
class SqlScheduledTask(AgentNexusBase):
    """
...
class SqlScheduledTaskRun(AgentNexusBase):
    """
...
class SqlCoordinationRun(AgentNexusBase):
    """SQLAlchemy model for ``coordination_runs``.

    A multi-agent collaboration run bound to a root conversation. Kept in
    the operational AgentNexus DB with the rest of the control-plane tables so
...
class SqlCoordinationTask(AgentNexusBase):
...
class SqlAgentMessage(AgentNexusBase):
...
class SqlCoordinationOutbox(AgentNexusBase):
...
class SqlDeliveryAttempt(AgentNexusBase):
...
class SqlCoordinationEvent(AgentNexusBase):
...
class SqlWorkspaceLease(AgentNexusBase):
...
class SqlWorkspaceMergeOperation(AgentNexusBase):
...
class SqlCoordinationArtifact(AgentNexusBase):
...
class SqlAgentMemory(AgentNexusBase):
