...
# resolvable (``AGENTNEXUS_LAKEBASE_INSTANCE`` is set, or a provider was injected
...
# token-refresh mode.
_LAKEBASE_INSTANCE_ENV = "AGENTNEXUS_LAKEBASE_INSTANCE"
...
def set_lakebase_token_provider(provider: LakebaseTokenProvider | None) -> None:
    ...
    *provider* once per new DBAPI connection, and uses the shorter
    ...
    fall back to the ``AGENTNEXUS_LAKEBASE_INSTANCE`` env-var path.
...
def _databricks_lakebase_token_provider(instance_name: str) -> str:
    ...
    :param instance_name: The Lakebase database instance name, e.g.
        ``"agentnexus-db"`` (the value of ``AGENTNEXUS_LAKEBASE_INSTANCE``).
...
def _create_engine(db_uri: str) -> Engine:
    ...
    multi-process workloads — REPL + AgentNexus server + runner subprocess
    ...
    # ``None`` unless ``AGENTNEXUS_LAKEBASE_INSTANCE`` is set or a provider was
    ...
    if db_uri not in _engine_cache:
        with _engine_lock:
            if db_uri not in _engine_cache:
                ...
                from agentnexus.runtime.telemetry import instrument_sqlalchemy_engine
...
def get_or_create_conversation_engine(conv_uri: str) -> Engine:
    ...
    For the common case where AP DB == AgentNexus DB, callers should
...
def _ensure_conversation_tables(engine: Engine) -> None:
    ...
    from agentnexus.db.db_models import ConversationBase

    with query_name_scope("agentnexus.database.ensure_conversation_schema"):
...
def _run_migrations(engine: Engine, db_uri: str) -> None:
    ...
    with query_name_scope("agentnexus.database.run_migrations"):
        ...
        for base in (AgentNexusBase, ConversationBase):
...
def _get_current_db_revision(engine: Engine) -> str | None:
    ...
    with query_name_scope("agentnexus.database.select_current_revision"):
...
def _verify_db_revision_is_supported(
    ...
    try:
        ...
        raise RuntimeError(
            "AgentNexus database schema is newer than this version of AgentNexus "
            f"(found revision {current!r}, latest supported revision {head!r}). "
            "Upgrade AgentNexus before using this database."
        ) from exc
...
def _initialize_or_verify_schema(engine: Engine, db_uri: str) -> None:
    ...
    - **Newer than this build** — stop without attempting a migration
      and tell the operator to upgrade AgentNexus.
    ...
    if current != head:
        _logger.warning(
            "AgentNexus database schema is out of date "
        ...
        try:
            ...
            raise RuntimeError(
                f"AgentNexus database schema is out of date "
        ...
        if migrated != head:
            raise RuntimeError(
                f"AgentNexus automatic database migration did not reach head "
...
_shared_read_sessions: ContextVar[dict[int, Session] | None] = ContextVar(
    "agentnexus_shared_read_sessions", default=None
)
...
def ensure_fts_table(engine: Engine) -> None:
    ...
    if _supports_fts5(engine.dialect.name):
        with query_name_scope("agentnexus.database.ensure_fts_table"), engine.connect() as conn:
...
def extract_search_text(item: NewConversationItem) -> str:
    ...
    """
    from agentnexus.entities.conversation import CompactionData
