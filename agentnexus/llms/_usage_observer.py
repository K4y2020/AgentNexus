...
2. **Auto-recorder** that activates whenever
   ``AGENTNEXUS_TOKEN_USAGE_JSON=<path>`` is set in the environment.
...
runner, harness) inherit both ``AGENTNEXUS_TOKEN_USAGE_JSON`` and
...
_ENV_VAR = "AGENTNEXUS_TOKEN_USAGE_JSON"
...
def _current_test_path() -> Path | None:
    ...
    Derived from the ``AGENTNEXUS_TOKEN_USAGE_JSON`` base path plus the
