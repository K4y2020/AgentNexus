...
def parse_model_string(model: str) -> RoutedModel:
    ...
    :raises AgentNexusError: If the provider prefix is not recognized.
    """
    if "/" in model:
...
def web_search_native_passthrough_provider(
    ...
    :param harness: The spec's harness kind (``AgentSpec.harness_kind``);
        empty/``"agentnexus"`` falls back to :func:`infer_harness_from_model`.
    ...
    from agentnexus.onboarding.provider_config import _EXECUTOR_TYPE_HARNESS_ALIASES

    if not harness or harness == "agentnexus":
