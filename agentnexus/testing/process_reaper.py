"""Reap AgentNexus processes leaked by a test session.

Tests spawn real AgentNexus subprocesses — detached host daemons
...
The session fixture points ``AGENTNEXUS_DATA_DIR`` at a throwaway
...
the directory doubles as an attribution key: a live AgentNexus process whose
...
def _is_pytest_process(argv: list[str]) -> bool:
    """Whether *argv* is a pytest run rather than a spawned AgentNexus process.
...
def find_leaked_omnigent_processes(data_dir: Path | str) -> list[psutil.Process]:
    """Find live AgentNexus processes attributable to *data_dir*.

    A process is attributed when its exec-time ``AGENTNEXUS_DATA_DIR`` equals
    *data_dir* (host daemons and zygotes inherit it via the spawn-env
    ...
    carry this session's ``AGENTNEXUS_DATA_DIR`` at exec time without being
    a leak.

    :param data_dir: The session's throwaway ``AGENTNEXUS_DATA_DIR``.
    ...
    for proc in psutil.process_iter():
        ...
        cmdline = " ".join(argv)
        if "agentnexus" not in cmdline or _is_pytest_process(argv):
            continue
        env = _proc_environ(proc)
        if env.get("AGENTNEXUS_DATA_DIR", "").rstrip(os.sep) != needle and not (
...
def reap_leaked_omnigent_processes(
    ...
    """Terminate every AgentNexus process attributable to *data_dir*.
