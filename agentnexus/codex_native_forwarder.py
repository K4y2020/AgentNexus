"""Forward Codex app-server notifications into AgentNexus sessions."""
...
from agentnexus.codex_native_elicitation import (
...
# in the bridge dir and posted to AgentNexus as ``external_mcp_startup``; the
...
# AgentNexus control event type sent when a Codex child thread is discovered.
...
class _CodexToolCall:
    ...
    :param call_id: Codex item id reused as the AgentNexus call id, e.g.
        ``"call_abc"``.
    :param name: AgentNexus function-call name, e.g. ``"shell"``.
...
class _CodexForwarderState:
    ...
    :param model: Latest known Codex model for this thread, e.g.
        ``"gpt-5.2-codex"``.
    :param posted_model: Last model already mirrored to AgentNexus via an
    ...
    :param posted_effort: Last reasoning effort already mirrored to AgentNexus
    ...
    :param parent_session_id: AgentNexus parent session id, e.g.
    ...
    :param subagents_by_thread: Maps Codex child thread ids to AgentNexus child
    ...
    :param synced_item_keys: Stable item keys already posted to AgentNexus this
    ...
    :param posted_user_turns: Turn ids whose ``userMessage`` has been
        posted to AgentNexus this connection, e.g. ``{"turn_123"}``. Used to
    ...
    :param prompted_plan_turns: Turn ids that already exposed the
        implementation prompt, either natively or through the AgentNexus bridge.
    ...
    # Last context-compaction status mirrored to AgentNexus
    ...
    def note_resume_response(self, response: CodexMessage) -> None:
        ...
        # Do NOT seed ``posted_model`` here. AgentNexus must learn the session's
    ...
    def session_for_child_thread(self, thread_id: str) -> str | None:
        """
        Return the AgentNexus child session id for a known Codex child thread.

        :param thread_id: Codex child thread id, e.g. ``"thread_child"``.
        :returns: AgentNexus child session id, e.g. ``"conv_child"``, or ``None``
    ...
    def note_child_thread(self, thread_id: str, session_id: str) -> None:
        """
        Record the AgentNexus child session id for a Codex child thread.

        :param thread_id: Codex child thread id, e.g. ``"thread_child"``.
        :param session_id: AgentNexus child session id, e.g. ``"conv_child"``.
        :returns: None.
        """
    ...
    def claim_item_key(self, item_key: str) -> bool:
        """
        Claim a transcript item key for AgentNexus posting.
...
class _CodexTerminalError:
    ...
    ``error`` ThreadItem. Forces the turn's AgentNexus status to ``failed`` and
...
class _CodexTurnStatusEdge:
    """
    AgentNexus session-status edge derived from Codex turn lifecycle state.

    :param status: AgentNexus session status, e.g. ``"running"`` or ``"idle"``.
...
class _OutputTextDeltaCoalescer:
    ...
    Posting each one through AgentNexus as an awaited HTTP request makes the
    ...
    :param client: HTTP client for AgentNexus event posts.
...
class _PendingCodexElicitation:
    """
    Background AgentNexus hook wait for one Codex server-to-client request.
    ...
    :param elicitation_id: AgentNexus elicitation id, e.g.
        ``"elicit_codex_abc123"``.
    """
...
class _CodexElicitationTaskTracker:
    ...
    AgentNexus web/REPL hook does. If the forwarder awaits the AgentNexus hook inline,
    ...
    def start(
        ...
        *,
        ...
        """
        Start one AgentNexus hook bridge in the background.

        :param client: HTTP client for AgentNexus hook posts.
    ...
    async def _post_resolved_once(
        ...
        """
        Post one AgentNexus resolution signal, suppressing duplicates.
...
async def supervise_forwarder(
    *,
    ...
    """
    Mirror Codex app-server notifications into an AgentNexus session.

    :param base_url: AgentNexus server base URL, e.g.
        ``"http://127.0.0.1:6767"``.
    :param headers: Static HTTP headers for AgentNexus requests.
    :param session_id: AgentNexus conversation id, e.g. ``"conv_abc123"``.
    ...
    :param ap_transport: Optional HTTP transport for the AgentNexus client,
    ...
    if client is None:
        client = client_for_transport(app_server_url, client_name="agentnexus-codex-forwarder")
...
async def _maybe_rotate_session_on_thread_started(
    *,
    ...
    """
    Rotate AgentNexus ownership when Codex starts a new native thread.

    Native Codex ``/clear`` starts a fresh app-server thread in the
    existing terminal. The forwarder must move the AgentNexus session binding
    ...
    :param ap_client: AgentNexus HTTP client used for session rotation.
    ...
    # begins. That event must not rotate the parent AgentNexus session — the child
    # is discovered later via a ``collabAgentToolCall`` item and routed to
    # its own AgentNexus child session by ``_handle_event``.
    ...
    _logger.info(
        "Codex forwarder rotated AgentNexus session after native thread switch: "
...
async def _create_thread_replacement_session(
    *,
    ...
    """
    Create and activate the AgentNexus session for a new native Codex thread.

    :param client: AgentNexus HTTP client.
    ...
    :param new_thread_id: Newly started Codex thread id, e.g.
        ``"thread_new"``.
    :returns: New AgentNexus session id, e.g. ``"conv_new"``.
    :raises httpx.HTTPStatusError: If AgentNexus rejects the create, bind,
...
async def _fetch_session_snapshot(client: httpx.AsyncClient, session_id: str) -> _JsonObject:
    """
    Fetch an AgentNexus session snapshot for Codex session rotation.

    :param client: AgentNexus HTTP client.
    :param session_id: AgentNexus session id, e.g. ``"conv_abc123"``.
    :returns: Decoded JSON session snapshot.
    :raises httpx.HTTPStatusError: If AgentNexus rejects the request.
...
async def _subscribe_until_ready(
    ...
    *ready_signal* and only retries once the caller observes the thread
    ...
    :param ap_client: AgentNexus HTTP client used for replayed items.
...
async def _post_resume_terminal_status(
    ...
    *,
    ...
    terminal, the forwarder can close the AgentNexus session status even though no
...
def _resume_terminal_status_edge_for_latest_turn(
    ...
    """
    Return the AgentNexus terminal status represented by the latest resume turn.
...
def _omnigent_status_from_resume_turn(turn: _JsonObject) -> str | None:
    """
    Convert an explicit Codex resume turn status to AgentNexus session status.
    ...
    :returns: AgentNexus status literal for terminal turns, or ``None`` for active
...
def _resolve_event_session(
    ...
    *,
    ...
    """
    Resolve which AgentNexus session should receive a Codex event.
...
def _refresh_model_from_config(bridge_dir: Path, forwarder_state: _CodexForwarderState) -> None:
    ...
    ``_sync_model_change`` mirrors it to AgentNexus as ``model_override``. This
    ...
    AgentNexus-initiated ``thread/settings/update`` switches the running
...
async def _sync_model_change(
    ...
    *,
    ...
    """
    Mirror a Codex TUI ``/model`` switch to AgentNexus (web picker + cost gate).
    ...
    baseline, POST an
    ``external_model_change`` event so the AgentNexus server persists
...
async def _sync_reasoning_effort_change(
    ...
    *,
    ...
    """
    Mirror Codex's active reasoning effort to AgentNexus session metadata.
...
async def _sync_codex_collaboration_mode_change(
    ...
    *,
    ...
    """
    Mirror Codex's active collaboration mode kind to AgentNexus labels.
    ...
    *,
    ...
    if method == "turn/started":
        ...
        if forwarder_state is not None:
            ...
            # on AgentNexus before this turn's first tool call reaches the cost gate.
            _refresh_model_from_config(bridge_dir, forwarder_state)
            _refresh_developer_instructions_from_config(bridge_dir, forwarder_state)
            await _sync_model_change(
...
async def _handle_mcp_startup_status(
    ...
    *,
    ...
    text read it) and republishes the full per-server map to AgentNexus so the
...
def _expected_mcp_servers_from_config(bridge_dir: Path) -> list[str]:
    ...
    :param bridge_dir: Native Codex bridge directory.
    :returns: Sorted enabled server names, e.g. ``["agentnexus", "safe"]``.
...
async def _post_mcp_startup(
    ...
    """
    Post the current per-MCP-server startup map to AgentNexus.
...
async def _handle_codex_elicitation_request(
    ...
    *,
    ...
    """
    Forward one Codex input request to AgentNexus and reply to app-server.

    The AgentNexus hook publishes the web elicitation and blocks until the
    ...
    AgentNexus timed out or saw the upstream disconnect, so the forwarder
...
async def _codex_elicitation_hook_result(
    ...
    *,
    ...
    Empty 2xx responses mean AgentNexus timed out or saw the upstream
...
async def _post_codex_elicitation_request(
    ...
    *,
    ...
    """
    POST a Codex server-to-client request to the AgentNexus hook endpoint,
...
def _note_native_plan_implementation_prompt(
    ...
    the AgentNexus bridge should relay that native request and skip its
...
async def _maybe_handle_plan_implementation_prompt(
    ...
    *,
    ...
    """
    Publish and resolve the Plan-mode implementation prompt in AgentNexus Web.
...
def _turn_started_status_edge(
    ...
    """
    Record a Codex turn start and return the AgentNexus running edge.
...
def _terminal_turn_status_edge(
    ...
    """
    Return the terminal AgentNexus edge for a Codex terminal turn event.
...
def _claim_completed_item(
    ...
    """
    Claim one completed Codex transcript item for AgentNexus posting.
...
async def _handle_completed_item(
    ...
    *,
    ...
    """
    Forward one Codex completed item event when it maps to AgentNexus history.
    ...
    if item_type == "agentMessage":
        ...
        # AgentNexus assigns each mirrored item a position by POST arrival order
...
async def _handle_collab_item(
    ...
    Registers newly discovered child threads and posts AgentNexus status updates
...
def _parent_session_id_from_forwarder_state(
    ...
    """
    Return the parent AgentNexus session id stored on the forwarder state.
...
async def _ensure_child_session(
    ...
    *,
    ...
    """
    Ensure a Codex child thread has an AgentNexus child session row.

    Registers the child via ``_register_child_session`` when unknown,
    then backfills its history at most once per connection.

    :param client: HTTP client for AgentNexus event posts.
    :param parent_session_id: Parent AgentNexus session id, e.g. ``"conv_parent"``.
...
async def _register_child_session(
    ...
    *,
    ...
    :param child_thread_id: Codex child thread id, e.g. ``"thread_child"``.
    :param item: Codex child-spawn item.
    :returns: AgentNexus child session id, or ``None`` on failure.
    """
...
def _extract_child_session_id(
    ...
    :param response: AgentNexus HTTP response.
    :param child_thread_id: Codex child thread id for error logging.
    :returns: AgentNexus child session id, or ``None`` when absent or malformed.
    """
...
async def _backfill_child_thread(
    ...
    *,
    ...
    :param client: HTTP client for AgentNexus event posts.
    :param codex_client: Connected Codex app-server client.
    :param parent_session_id: Parent AgentNexus session id, e.g.
        ``"conv_parent"``.
    :param child_session_id: AgentNexus child session id, e.g.
        ``"conv_child"``.
...
async def _resume_child_thread_or_log(
    ...
    *,
    ...
    :param client: HTTP client for AgentNexus status posts on failure.
    :param codex_client: Connected Codex app-server client.
    :param child_session_id: AgentNexus child session id, e.g. ``"conv_child"``.
    :param child_thread_id: Codex child thread id, e.g.
        ``"thread_child"``.
...
async def _post_collab_agent_statuses(
    ...
    *,
    ...
    """
    Publish AgentNexus status updates from a Codex collab-agent state snapshot.
...
def _omnigent_status_from_collab_state(state: _JsonObject) -> str | None:
    """
    Convert a Codex collab-agent state dict to an AgentNexus session status.

    :param state: Codex ``CollabAgentState`` dict, e.g.
        ``{"status": "running"}``.
    :returns: AgentNexus status literal, e.g. ``"running"``, or ``None`` when
...
async def _handle_agent_message_delta(
    ...
    running. AgentNexus normally persists only the completed ``agentMessage`` item,
...
async def _handle_plan_delta(
    ...
    AgentNexus uses the same transient output-text delta channel for both,
...
async def _ensure_user_message_posted(
    ...
    normal claim/post path so it takes an earlier AgentNexus position than the
...
async def _post_tool_item(
    ...
    *,
    ...
    """
    Mirror one completed Codex built-in tool call into AgentNexus history.
...
async def _post_review_mode_marker(
    ...
    """
    Mirror a Codex review-mode enter/exit transition into AgentNexus history.
...
_CODEX_SANDBOX_BYPASS_GUIDANCE = (
    "AgentNexus: Codex's command sandbox could not start because this container "
...
# Codex built-in tool item types this forwarder mirrors into AgentNexus history.
...
async def _post_external_elicitation_resolved(
    ...
    *,
    ...
    :param elicitation_id: AgentNexus elicitation id, e.g.
        ``"elicit_codex_abc123"``.
    :returns: ``True`` when AgentNexus accepted the event.
    """
    response = await _post_session_event(
        client,
        session_id,
...
async def _post_compaction_status(
    ...
    *,
    ...
    """
    Mirror a Codex context-compaction edge to AgentNexus (#1255).
...
async def _handle_reasoning_delta(
    ...
    ``item/reasoning/summaryTextDelta`` while it thinks. AgentNexus has no
...
def _session_usage_data_from_params(params: _JsonObject) -> dict[str, int] | None:
    """
    Extract AgentNexus session-usage fields from a Codex usage notification.
    ...
    if isinstance(cumulative_input_tokens, int) and cumulative_input_tokens >= 0:
        ...
        # ``response.completed``, so the AgentNexus relay never accounts its cost.
...
class _ForwardHealth:
    """
    Process-level health of AgentNexus session-event forwarding (#1120).
...
def _note_forward_failure(event_type: str) -> None:
    ...
    if (
        ...
        _logger.error(
            "codex-native forward sync degraded: %d consecutive AgentNexus "
...
async def _replay_dead_letters_on_startup(
    ...
    :param ap_client: HTTP client for AgentNexus event posts.
...
async def _post_session_event(
    ...
    *,
    ...
    """
    Post one AgentNexus session event, tracking forward-sync health (#1120).
...
async def _post_session_event_inner(
    ...
    *,
    ...
    """
    Post one AgentNexus session event with bounded transient retries.
...
def _is_final_post_attempt(attempt: int, max_attempts: int) -> bool:
    """
    Return whether an AgentNexus event POST attempt is the final try.
...
def _log_post_transport_failure(event_type: str, exc: httpx.HTTPError, max_attempts: int) -> None:
    """
    Log an exhausted AgentNexus session-event transport failure.
...
def _log_failed_session_event_post(
    ...
    :param response: Final AgentNexus response, or ``None`` after transport
...
def _should_retry_post_status(status_code: int) -> bool:
    """
    Return whether an AgentNexus event POST status is transient.
...
def _post_retry_delay(attempt: int) -> float:
    """
    Return the retry delay for a failed AgentNexus event POST attempt.
...
def _params_with_turn_id(params: _JsonObject, turn_id: str) -> _JsonObject:
    """
    Return params with a top-level ``turnId`` for AgentNexus response ids.
...
def _streaming_message_id(params: _JsonObject, item_type: str) -> str | None:
    """
    Build a stable AgentNexus live-delta stream id for a Codex item.

    AgentNexus Web uses this id to keep terminal-observed live text in a
...
def _response_id(params: _JsonObject) -> str:
    """
    Build a stable AgentNexus response id for a Codex notification.
