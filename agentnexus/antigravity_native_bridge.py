...
ANTIGRAVITY_NATIVE_BRIDGE_ID_LABEL_KEY = "agentnexus.antigravity_native.bridge_id"
...
_BRIDGE_ROOT = Path.home() / ".agentnexus" / "antigravity-native"
...
# onboarding is a distinct flow AgentNexus does not drive, so it stays ``False``.
...
def bridge_root() -> Path:
    ...
    :returns: Absolute root for Antigravity-native bridge directories, e.g.
        ``Path("~/.agentnexus/antigravity-native")``.
    """
    return _BRIDGE_ROOT
...
# ── AgentNexus MCP relay wiring (sys_* tools) ──────────────────────────────────
#
# agy is otherwise tool-isolated from AgentNexus: unlike claude/codex/cursor it
# wires no MCP relay, so the wrapped agy cannot reach any ``sys_*`` tool (spawn
# sub-agent sessions, drive AgentNexus terminals, list agents/models, ``sys_os_*``).
...
# AgentNexus's own TOOL_CALL policy + elicitation gate still applies on the server
# side, so auto-approving the agy-side MCP gate only avoids a hidden in-terminal
# prompt blocking the call before AgentNexus ever sees it.
_AGY_ENABLED_TOOLS = [
    "list_comments",
    "sys_add_policy",
    "sys_agent_download",
...
def agy_gemini_dir(bridge_dir: Path) -> Path:
    ...
    :returns: Absolute ``.gemini`` directory that should receive agy's session
        config/state and the AgentNexus MCP config.
    """
...
def build_mcp_config(
    ...
    *,
    ...
    """Build agy's ``mcp_config.json`` payload for the AgentNexus relay server.
...
def write_mcp_bridge_config(bridge_dir: Path) -> None:
    """Write the token config the shared AgentNexus MCP relay requires.
...
def write_mcp_config(
    ...
    *,
    ...
    ``mcp_config.json`` for the AgentNexus relay into the per-session isolated agy
...
def update_conversation_id(
    ...
    AgentNexus session stays the same (e.g. the runner cold-start replacing the
...
_PASTE_BUFFER = "agentnexus-agy-paste"
...
def send_interaction_keys_via_tui(
    ...
    **in-process permission / question prompt**. When agy runs attended (the
    ...
    AgentNexus web UI AND as agy's own numbered TUI prompt ("Do you want to
