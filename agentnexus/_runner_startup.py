...
# log collectors.
_NO_SPINNER_ENV_VAR = "AGENTNEXUS_NO_SPINNER"
...
def _spinner_enabled(stream_isatty: bool, env: dict[str, str]) -> bool:
    ...
    :param env: Environment snapshot, e.g. ``dict(os.environ)``.
        ``AGENTNEXUS_NO_SPINNER`` (any truthy value) force-disables
        the spinner.
...
def runner_startup_progress(
    *,
    ...
    On a TTY (and absent ``AGENTNEXUS_NO_SPINNER``) a rich spinner
    ...
    :param enabled: Force the renderer choice. ``None`` (default)
        ...
        ``AGENTNEXUS_NO_SPINNER``. ``True`` always renders the
    ...
    click.echo(f"agentnexus: {initial_message}", err=True)
    ...
    def _update_plain(msg: str) -> None:
        ...
        :returns: None.
        """
        click.echo(f"agentnexus: {msg}", err=True)
...
def format_runner_log_tail(log_path: Path | None) -> str:
    ...
    :param log_path: Path to the captured runner log, e.g.
        ``Path("/home/u/.agentnexus/logs/runner/runner-abcd.log")``.
    ...
    :returns: A formatted block ready to append to an exception
        message, e.g.::
            ...
            \nRunner log: /home/u/.agentnexus/logs/runner/runner-abcd.log
