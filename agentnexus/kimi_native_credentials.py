"""Per-session ``KIMI_CODE_HOME`` builder that injects AgentNexus hooks.
...
- carries a ``config.toml`` that is the user's config text with two AgentNexus
...
def render_kimi_hooks_toml(*, bridge_dir: Path, python_executable: str | None = None) -> str:
    """Render the two AgentNexus ``[[hooks]]`` entries as TOML text.
    ...
    return (
        "\n"
        "# --- AgentNexus native hooks (auto-generated; do not edit) ---\n"
        "[[hooks]]\n"
        'event = "PreToolUse"\n'
        f'command = "{pre}"\n'
        "timeout = 600\n"
...
def build_kimi_session_home(
    ...
    *,
    ...
    """Materialize a session-scoped ``KIMI_CODE_HOME`` with AgentNexus hooks.
    ...
    the user's config plus the AgentNexus hooks. Best-effort and idempotent:
