...
OpenCode's permission surface and AgentNexus's policy/approval model:

1. Normalize a raw permission request into a flat policy-evaluation input.
2. Map an AgentNexus policy verdict (allow / allow-always / deny / ask) onto
   an OpenCode reply.
...
# AgentNexus-side normalized decisions used by the forwarder.
...
def normalize_for_policy(
    ...
    *,
    ...
    """
    Build an AgentNexus policy-evaluation input from a permission request.
    ...
    :param omnigent_session_id: Owning AgentNexus conversation id.
    ...
    return {
        ...
        "agentnexus_session_id": omnigent_session_id,
...
def map_verdict_to_decision(verdict: _JsonMapping | None) -> PolicyDecision:
    """
    Map an AgentNexus policy verdict onto a normalized decision.
...
def decision_to_reply(decision: PolicyDecision) -> OpenCodeReply | None:
    ...
    ``permission/index.ts``), which bypasses the AgentNexus policy engine and
