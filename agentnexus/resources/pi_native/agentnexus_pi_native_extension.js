...
// (agentnexus.native_policy_hook.post_evaluate_with_retry, ~1-day client
...
// agentnexus.policies.types.FAIL_CLOSED_PHASES and the Python native hook's
 ...
 * (agentnexus/inner/pi_executor.py): Pi (``@earendil-works/pi-coding-agent``)
...
function setOmnigentStatus(config, ctx, state) {
  ...
  try {
    ...
    ctx.ui.setStatus("agentnexus", label);
    ctx.ui.setStatus("agentnexus_state", undefined);
  } catch (_err) {}
}
...
function startInboxPoller(
  ...
  if (!config || !config.inboxDir || pi.__agentnexusInboxPoller) return;
  ...
  pi.__agentnexusInboxPoller = setInterval(() => {
    let files = [];
    try {
      files = fs
