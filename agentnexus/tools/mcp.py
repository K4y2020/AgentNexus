...
class McpServerConnection:
    ...
    async def _invoke_tool(
        ...
        ``/mcp/execute`` → AgentNexus server) can surface the elicitation
    ...
    async def call_tool_with_elicitation(
        ...
        Called by the AgentNexus server after the user has responded to an
        ...
        # InputRequiredResult on the retry. Raise so the AgentNexus server
    ...
    async def _open_stdio_transport(
        ...
        (``omnigent/inner/mcp_tools.py``); AgentNexus now matches that
...
async def _call_tool_with_reconnect(
    ...
    :param session_id: AgentNexus session id forwarded to
