...
with ``language == "agentnexus-python-callable"`` and ``path``
holding a dotted-import string (e.g.
``"examples._shared.tool_functions.calculate"``). These are
distinct from native AgentNexus local tools, which live as
...
# translator (omnigent/spec/omnigent.py:AGENTNEXUS_TOOL_LANGUAGE).
...
def load_local_callable_tools(
    ...
    ``"agentnexus-python-callable"``. Other entries (native AP
    ...
    :returns: One :class:`LocalCallableTool` per omnigent-style
        ...
        case for native AgentNexus specs).
    """
    result: list[LocalCallableTool] = []
    for info in local_tools:
        if info.language != _AGENTNEXUS_CALLABLE_LANGUAGE:
            continue
