...
from agentnexus.spec.types import SharePolicy, ToolRuntime
from agentnexus.tools._srt import is_srt_available
...
from agentnexus.tools.local import load_local_python_tools
...
class ToolManager:
    ...
    def _register_policy_tools(self) -> None:
        ...
        the AgentNexus server's REST endpoints.
        """
    ...
    def _register_scheduled_task_tools(self) -> None:
        ...
        runner dispatches all four via the AgentNexus server's
    ...
    def _create_builtin(
        ...
        if name == "upload_file":
            from agentnexus.tools.builtins.upload_file import UploadFileTool
    ...
    def _create_web_fetch(self) -> Tool:
        ...
        """
        from agentnexus.tools.builtins.web_fetch import WebFetchTool
    ...
    def _register_sub_agent_tools(self) -> None:
        ...
        from agentnexus.server.routing_backend import routing_available
    ...
    def _register_task_lifecycle_tools(self) -> None:
        ...
        """
        from agentnexus.tools.builtins.async_inbox import SysCancelTaskTool
    ...
    def _register_browser_tools(self) -> None:
        ...
        not carry.
        """
        from agentnexus.tools.builtins.browser import (
            BrowserClickTool,
            BrowserNavigateTool,
    ...
    def _register_terminal_tools(self) -> None:
        ...
        Per ``designs/AGENTNEXUS_TERMINAL_BRIDGE.md`` §4.3, the five
    ...
    def _register_local_tools(self, workdir: Path | None) -> None:
        ...
        # Native AgentNexus local tools (``language == "python"``) need a
        # workdir on disk so the subprocess loader can locate the
        # ``tools/python/*.py`` files. AgentNexus-style tools
        # (``language == "agentnexus-python-callable"``) come
        ...
        if workdir is not None:
            for tool in load_local_python_tools(
                ...
                if not is_valid_tool_name(tool.name()):
                    ...
                    continue
                self._tools[tool.name()] = tool
        # AgentNexus-style callable tools — sibling loader for the
        ...
        from agentnexus.tools.local_callable import load_local_callable_tools

        for callable_tool in load_local_callable_tools(server_local_tools):
            if not is_valid_tool_name(callable_tool.name()):
                _logger.warning(
                    "AgentNexus callable tool %r has invalid name — skipping",
                    callable_tool.name(),
                )
                continue
            if callable_tool.name() in self._tools:
                ...
                raise ValueError(
                    f"agentnexus callable tool {callable_tool.name()!r} "
                    f"collides with an already-registered tool"
                )
    ...
    def _register_client_tools(
        ...
        Raises :class:`AgentNexusError` if a tool name violates the
        ...
        :param specs: List of :class:`ClientSideToolSpec` objects to
            register, e.g.
            ``[ClientSideToolSpec(name="get_weather", ...)]``.
        :raises AgentNexusError: If any tool name is invalid.
        """
        for spec in specs:
