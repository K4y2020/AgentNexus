...
Per ``designs/AGENTNEXUS_TERMINAL_BRIDGE.md`` §4.3, these tools register
...
class _ResolvedInstance:
    ...
    :param lock: The per-instance ``threading.Lock`` from
        ...
        thread's send. See ``designs/AGENTNEXUS_TERMINAL_BRIDGE.md`` §9.1.
    """
...
# agents in the wild see an unchanged tool surface under AgentNexus mode.
...
class SysTerminalLaunchTool(Tool):
    ...
    def _resolve_cwd(
        ...
        3. ``spec.os_env.cwd`` if it's set to a meaningful path
           (not ``None``, ``""``, ``"."``, or ``"./"``).
        4. ``ctx.workspace`` — the per-task workspace AgentNexus creates
