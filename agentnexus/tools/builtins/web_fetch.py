...
# spawned — the runner aborts with ``unknown harness 'agentnexus'``.
_UNBOOTABLE_DEFAULT_HARNESS: str = "agentnexus"
...
def _ensure_default_sandbox_runnable() -> None:
    ...
    :raises AgentNexusError: On Linux when ``bwrap`` is not on ``PATH``,
...
def build_researcher_spec(parent_spec: AgentSpec) -> AgentSpec:
    ...
    - The parent executor's harness (``config``), ``auth``, ``model``, and
      ...
      parent's provider. Without this the child defaults to ``type="agentnexus"``
      with no harness, which the runner rejects as ``unknown harness
      'agentnexus'`` before any model routing (Layer 1), and even past that
    ...
    - An ``os_env`` block — registers ``sys_os_shell`` for one-shot
      ...
      per ``designs/AGENTNEXUS_TERMINAL_BRIDGE.md`` §3a in favor of
    ...
    :raises AgentNexusError: If the parent declares no bootable harness, so the
        researcher would spawn the unspawnable literal harness ``"agentnexus"``;
    ...
    # ``type="agentnexus"`` harness and strips the parent's provider. Drop
    ...
    if child_executor.harness_kind == _UNBOOTABLE_DEFAULT_HARNESS:
        raise AgentNexusError(
            ...
            f"unknown harness 'agentnexus'. Set executor.config.harness on the parent "
