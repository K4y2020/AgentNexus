...
# so traffic from the AgentNexus provider is attributable.
_CLIENT_TITLE = "AgentNexus"


def _keenable_base_url() -> str:
    """Resolve the Keenable base URL; ``AGENTNEXUS_KEENABLE_BASE_URL`` overrides for tests."""
    return os.environ.get("AGENTNEXUS_KEENABLE_BASE_URL", _DEFAULT_KEENABLE_URL).rstrip("/")
