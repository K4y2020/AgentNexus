...
is then the only surface that reflects the agent's work — the AgentNexus
...
from agentnexus import cursor_native_status
from agentnexus._native_post_delivery import post_may_have_been_delivered
from agentnexus.cursor_native_bridge import FORK_HISTORY_CLOSE_TAG, FORK_HISTORY_OPEN_TAG
from agentnexus.inner.native_attachments import ATTACHMENT_MARKER_STRIP_PATTERN
...
# real text — the copied history already lives in the AgentNexus timeline, so
...
async def _patch_external_session_id(
    ...
    """PATCH the AgentNexus session with the cursor chat id for cold-resume.
