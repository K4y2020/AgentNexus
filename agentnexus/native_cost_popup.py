...
verdict to the AgentNexus server's elicitation-resolve endpoint — the **same**
...
def _tmux_window_activity_at(socket_path: str, tmux_target: str) -> float | None:
    ...
    independent of any AgentNexus-side status pipeline. Harness-agnostic.
...
def launch_blocked_notice(
    ...
    *,
    ...
    if not clients:
        return
    python = python_executable or sys.executable
    argv = [python, "-I", "-m", "agentnexus.native_cost_popup", "--notice", "--message", message]
    if policy_name:
        argv += ["--policy-name", policy_name]
    inner_cmd = shlex.join(argv)
    for client in clients:
...
def _read_omnigent_routing(config_file: Path) -> dict[str, object]:
    """
    Read AgentNexus base URL + auth headers from a harness routing-config file.
    ...
    :raises SystemExit: If the file is missing, unreadable, not JSON, or
        ...
        for "where is the AgentNexus server", so this fails loud (the popup just
    ...
    try:
        ...
        raise SystemExit(
            f"cost-approval popup: cannot read AgentNexus config {config_file}: {exc}"
        ) from exc
    if not isinstance(raw, dict) or not isinstance(raw.get("ap_server_url"), str):
        raise SystemExit(
            f"cost-approval popup: AgentNexus config {config_file} has no 'ap_server_url' string"
        )
    return raw
...
def _post_verdict(
    *,
    ...
    """
    POST the verdict to the AgentNexus elicitation-resolve endpoint.
    ...
    :param ap_server_url: AgentNexus server base URL, e.g.
        ``"http://127.0.0.1:8787"``.
...
def main(argv: list[str] | None = None) -> int:
    ...
    Reads AgentNexus routing up front (so a missing/invalid config fails loud
    ...
    """
    parser = argparse.ArgumentParser(prog="agentnexus.native_cost_popup")
    parser.add_argument(
        "--notice",
        action="store_true",
