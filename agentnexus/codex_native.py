"""Native Codex TUI wrapper for the AgentNexus CLI."""
...
from agentnexus._wrapper_labels import WRAPPER_LABEL_KEY as _WRAPPER_LABEL_KEY
...
from agentnexus.codex_native_state import read_launch_state, write_launch_state
...
def _resolve_codex_auth_source() -> _CodexAuthSource:
    ...
    This is the seam for future managed credentials: once AgentNexus can provide
    ...
    inherited private AgentNexus homes map back to the user's real Codex home.

    :returns: Local Codex auth source to inspect synchronously.
    """
    from agentnexus.inner.codex_executor import _codex_home_config_source_from_env
...
def _find_codex_cli() -> str | None:
    ...
    from agentnexus.onboarding.harness_install import harness_install_spec
...
def _codex_auth_unavailable_reason() -> HarnessUnavailableReason | None:
    ...
    try:
        ...
        if defers_to_codex_config:
            ...
            from agentnexus.onboarding.codex_auth_readiness import codex_config_effective_auth
...
class LaunchedCodexTerminal:
    """
    Terminal resource returned by the AgentNexus runner launch path.
...
def run_codex_native(
    *,
    ...
    """
    Launch Codex TUI in an AgentNexus terminal.

    :param server: Resolved AgentNexus server URL, e.g.
        ``"http://127.0.0.1:8123"``.
    :param session_id: Optional existing AgentNexus conversation id,
        e.g. ``"conv_abc123"``.
    ...
    if server is None:
        raise click.ClickException(
            "Codex requires a resolved AgentNexus server URL. The CLI should call "
            "_ensure_backend before run_codex_native."
        )
    with TemporaryDirectory(prefix="agentnexus-codex-native-") as tmpdir:
...
def _run_with_local_server(
    ...
    *,
    ...
    """
    Start a local AgentNexus server, launch Codex, and attach to it.

    :param spec_path: Generated Codex wrapper agent spec.
    :param session_id: Optional existing AgentNexus session id.
...
def _run_with_remote_server(
    ...
    *,
    ...
    """
    Launch Codex on an AgentNexus server via a daemon-spawned runner.

    :param base_url: Remote AgentNexus server base URL, e.g.
    ...
    from agentnexus.cli import _ensure_host_daemon
...
async def _prepare_codex_terminal_via_daemon(
    *,
    ...
    :param base_url: AgentNexus server base URL, e.g.
        ``"https://example.databricks.com"``.
    :param headers: HTTP auth headers for AgentNexus requests.
...
async def _post_initial_prompt(
    *,
    ...
    """
    Send the first Codex prompt through AgentNexus instead of the app-server.
    ...
    :returns: None.
    :raises click.ClickException: If AgentNexus rejects the prompt.
    """
        ...
        try:
            ...
            if thread_id is None:
                event_client = client_for_transport(
                    codex_ws_url,
                    client_name="agentnexus-codex-native",
                )
            ...
            launched_terminal = await _launch_codex_terminal(
                ...
                # the app-server so it resolves the AgentNexus provider
...
async def _attach_terminal_resource(
    *,
    ...
    """
    Attach the current terminal to the prepared AgentNexus terminal resource.
...
def _active_codex_session_id(bridge_dir: Path) -> str | None:
    """
    Return the active AgentNexus session id for a native Codex bridge.

    :param bridge_dir: Native Codex bridge directory.
    :returns: AgentNexus session id, e.g. ``"conv_abc123"``, or ``None`` when
...
async def _attach_direct_tmux(socket_path: Path, tmux_target: str) -> None:
    ...
    tmux session can still attach to AgentNexus' private tmux server.
...
async def _create_codex_session(
    ...
    *,
    ...
    :param terminal_launch_args: Pass-through Codex CLI args to persist
        ...
        ``["--config", "approval_policy=on-request"]``.
    :returns: New AgentNexus session id.
    """
...
async def _fetch_codex_session(client: httpx.AsyncClient, session_id: str) -> _JsonObject:
    """
    Fetch an existing AgentNexus session.
...
def _mint_codex_thread_id() -> str:
    ...
    so the clone gets its own AgentNexus ``external_session_id`` — mirroring how
...
def _find_codex_rollout(codex_home: Path, thread_id: str) -> Path | None:
    ...
    :param codex_home: A per-session private ``CODEX_HOME``, e.g.
        ``Path("~/.agentnexus/codex-native/<hash>/codex-home")``.
...
def _clone_codex_rollout(
    ...
    *clone_workspace* (see :func:`_copy_rollout_with_cwd`), preserving the
    ...
    :param clone_codex_home: The clone's per-session private ``CODEX_HOME``,
        e.g. ``Path("~/.agentnexus/codex-native/<hash>/codex-home")``.
...
async def _ensure_local_codex_resume_rollout(
    ...
    *,
    ...
    Cross-machine resume has the AgentNexus conversation and Codex thread id on
    ...
    state, not a cache that AgentNexus should rewrite.
    ...
    :raises click.ClickException: If AgentNexus history cannot be fetched or the
    ...
    if codex_path is not None:
        from agentnexus.inner.codex_executor import _codex_cli_version
...
def _codex_resume_rollout_path(codex_home: Path, external_session_id: str) -> Path:
    ...
    :param codex_home: Per-session private ``CODEX_HOME``, e.g.
        ``Path("~/.agentnexus/codex-native/x/codex-home")``.
...
async def _fetch_all_session_items_for_codex_resume(
    ...
    """
    Fetch committed AgentNexus session items in chronological order.
...
def _codex_rollout_records_from_session_items(
    ...
    *,
    ...
    """
    Convert AgentNexus session items into Codex rollout JSONL records.

    The generated records follow Codex's rollout shape: one
    ``session_meta`` record, a ``turn_context`` before each AgentNexus response
...
def _interrupted_response_ids_from_session_items(items: list[_JsonObject]) -> set[str]:
    """
    Return response ids for AgentNexus turns that ended interrupted.

    A Codex interrupted turn is persisted in AgentNexus as visible transcript text
...
def _session_item_response_id(item: _JsonObject) -> str | None:
    """
    Extract a non-empty AgentNexus response id from a flat item.
...
def _codex_response_item_from_session_item(item: _JsonObject) -> _JsonObject | None:
    """
    Convert one AgentNexus item into one Codex ``response_item`` payload.

    :param item: Flat AgentNexus item dict, e.g.
        ``{"type": "function_call", "name": "shell", ...}``.
    :returns: Responses-style item payload, or ``None`` for unsupported
        or empty AgentNexus items.
    """
...
def _is_interrupted_assistant_session_item(item: _JsonObject) -> bool:
    """
    Return whether an AgentNexus item is an interrupted assistant partial.

    AgentNexus persists these messages so the web transcript can show the text and
...
def _codex_response_item_payload(item: _JsonObject) -> _JsonObject | None:
    """
    Convert one supported AgentNexus item into a Codex response payload body.

    :param item: Flat AgentNexus item dict.
    :returns: Payload without the optional item id, or ``None`` for
        unsupported / empty AgentNexus items.
    """
...
def _codex_message_payload_from_session_item(item: _JsonObject) -> _JsonObject | None:
    """
    Convert an AgentNexus message item into a Codex message payload.

    :param item: AgentNexus message item.
...
def _codex_function_call_payload_from_session_item(
    ...
    """
    Convert an AgentNexus function call item into a Codex function call payload.

    :param item: AgentNexus function call item.
    :returns: Codex function call payload, or ``None`` when optional
        routing fields are absent.
    :raises click.ClickException: If the AgentNexus item violates required tool
        history fields.
    """
    name = item.get("name")
    ...
    if not isinstance(arguments, str):
        item_id = item.get("id")
        raise click.ClickException(
            "Cannot synthesize Codex resume rollout: AgentNexus function_call "
...
def _codex_function_call_output_payload_from_session_item(
    ...
    """
    Convert an AgentNexus function output item into a Codex function output payload.

    :param item: AgentNexus function output item.
    ...
    if not isinstance(output, str):
        item_id = item.get("id")
        raise click.ClickException(
            "Cannot synthesize Codex resume rollout: AgentNexus function_call_output "
...
def _codex_content_blocks_from_api_content(
    ...
    *,
    ...
    """
    Extract text blocks from an AgentNexus content array for Codex rollout items.
...
def _codex_turn_id_for_session_item(
    *,
    ...
    """
    Return a Codex turn id for an AgentNexus item.

    Codex-native forwarder stores AgentNexus ``response_id`` as
    ...
    return (
        ...
        + uuid.uuid5(
            uuid.NAMESPACE_URL,
            f"agentnexus-codex-resume:{session_id}:{external_session_id}:{stable}",
        ).hex
    )
...
async def _patch_external_session_id(
    ...
    """
    Persist the native Codex thread id on the AgentNexus session.
...
async def _wait_for_thread_started(client: CodexAppServerClient) -> str:
    ...
    """
    from agentnexus.codex_native_forwarder import wait_for_thread_started
...
async def _start_initial_turn(app_server_url: str, thread_id: str, prompt: str) -> None:
    ...
    :returns: None.
    """
    client = client_for_transport(app_server_url, client_name="agentnexus-codex-native")
...
async def _launch_codex_terminal(
    ...
    *,
    ...
    :param client: HTTP client pointed at AP.
    :param session_id: AgentNexus session id.
    :param codex_args: Raw Codex CLI args.
    :param command: Codex executable.
...
def _response_error_code(resp: httpx.Response) -> str | None:
    """
    Extract a structured AgentNexus error code from *resp* if present.
...
def _runner_offline_message(message: str) -> bool:
    """
    Return whether *message* is the AgentNexus stale-runner error shape.
...
def _resolve_session_id_for_resume(
    *,
    ...
    async def _drive() -> str | None:
        ...
        :returns: Selected AgentNexus session id, or ``None``.
        """
