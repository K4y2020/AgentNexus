...
#: Fixed namespace for deriving a stable qwen ``--session-id`` from an AgentNexus
...
_BRIDGE_ROOT = Path(tempfile.gettempdir()) / f"agentnexus-{stable_user_id()}" / "qwen-native"
...
#: Token config the shared AgentNexus MCP relay (``serve-mcp``) reads from the
#: bridge dir. Mirrors cursor-/claude-native (``cursor_native_bridge.py``).
_BRIDGE_CONFIG_FILE = "bridge.json"
#: Name qwen lists the AgentNexus MCP server under (shows in ``/mcp``).
...
def qwen_session_id_for_conversation(conversation_id: str) -> str:
    """Return the deterministic qwen ``--session-id`` for an AgentNexus conversation.
...
def _qwen_text_from_api_content(content: object, api_type: str) -> str:
    """Concatenate the text of an AgentNexus content array's blocks of *api_type*.

    :param content: AgentNexus content array, e.g. ``[{"type":"input_text","text":"hi"}]``.
...
def qwen_session_records_from_session_items(
    ...
    *,
    ...
    """Convert AgentNexus session items into qwen chat-recording JSONL records.
...
def write_qwen_session_recording(
    ...
    *,
    ...
    try:
        hostname = socket.gethostname() or "agentnexus"
    except OSError:
        hostname = "agentnexus"
...
def _ensure_secure_bridge_dir(bridge_dir: Path) -> None:
    ...
    """
    from agentnexus.claude_native_bridge import _ensure_secure_dir
...
# ---------------------------------------------------------------------------
# AgentNexus MCP server config — expose AgentNexus's builtin tools (sys_*,
...
def write_mcp_bridge_config(bridge_dir: Path) -> None:
    """Write the token config the shared AgentNexus MCP relay reads at startup.
...
def build_mcp_server_entry(
    ...
    *,
    ...
    """Build qwen's ``mcpServers.agentnexus`` entry for the AgentNexus relay.

    ``trust: true`` auto-approves the qwen-side MCP tool gate so the TUI doesn't
    add a second in-terminal prompt: AgentNexus already gates these calls through
...
def write_mcp_config(
    ...
    *,
    ...
    """Write the per-session AgentNexus MCP config for qwen's ``--mcp-config`` flag.

    Writes ``{"mcpServers": {"agentnexus": ...}}`` to a file *inside the bridge dir*
