...
class Elicitation(str, Enum):
    """How a policy ASK / tool-approval is surfaced to the AgentNexus web UI."""

    NONE = "none"
    HOOK = "hook"  # vendor PreToolUse hook posts to AgentNexus
...
class Resume(str, Enum):
    ...
    COLD_ONLY = "cold-only"  # rebuild from AgentNexus transcript / history replay
...
class AuthModel(str, Enum):
    ...
    AGENTNEXUS_CREDENTIAL = "agentnexus-credential"  # AgentNexus gateway / provider config
    OWN_AUTH = "own-auth"  # vendor login / API key, not AgentNexus-managed
...
class HarnessCapabilities:
    ...
    :param subagents: Whether the harness can spawn AgentNexus sub-agents.
