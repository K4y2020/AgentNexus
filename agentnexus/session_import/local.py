...
from agentnexus.claude_native_bridge import read_transcript_items_from_offset
from agentnexus.codex_native import _CODEX_THREAD_ID_RE, _find_codex_rollout
from agentnexus.entities import NewConversationItem, parse_item_data
from agentnexus.kimi_native_credentials import resolve_user_kimi_home
from agentnexus.kimi_native_forwarder import (
    read_kimi_wire_items,
    workdirs_for_kimi_sessions,
)
from agentnexus.kiro_native_session_forwarder import (
...
from agentnexus.opencode_native_forwarder import opencode_tool_output_text
...
def _codex_message_data(payload: dict[str, object]) -> dict[str, object] | None:
    """Convert a visible Codex message payload to AgentNexus message data."""
...
def _codex_response_item(
    ...
    *,
    ...
    """Convert one supported Codex response item to an AgentNexus item."""
...
def _qwen_message_data(record: dict[str, object]) -> dict[str, object] | None:
    """Convert one visible Qwen recording row to AgentNexus message data."""
...
def _pi_message_items(record: dict[str, object]) -> tuple[NewConversationItem, ...]:
    """Convert one Pi message entry to visible AgentNexus items."""
