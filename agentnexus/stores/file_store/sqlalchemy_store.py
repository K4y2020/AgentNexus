...
from agentnexus.db.db_models import SqlFile, current_workspace_id, normalize_uuid
...
from agentnexus.entities import PagedList, StoredFile
...
class SqlAlchemyFileStore(FileStore):
    ...
    def __init__(self, storage_location: str) -> None:
        ...
        self._session = make_named_managed_session_maker(
            self._engine,
            query_name_prefix="agentnexus.file_store",
        )
    ...
    def delete(
        ...
        with self._session("delete_file") as session:
            with query_name_scope("agentnexus.file_store.select_file_by_id"):
                row = session.get(SqlFile, (current_workspace_id(), file_id))
            if not row:
                return False
            if session_id is not None and row.session_id != normalize_uuid(session_id):
    ...
    def delete_all_for_session(self, session_id: str) -> builtins.list[str]:
        ...
        with self._session("delete_session_files") as session:
            ...
            with query_name_scope("agentnexus.file_store.list_session_files_for_delete"):
                rows = list(session.execute(stmt).scalars().all())
