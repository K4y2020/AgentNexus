...
from agentnexus._wrapper_labels import UI_MODE_LABEL_KEY, WRAPPER_LABEL_KEY
from agentnexus.db.converters import sql_agent_to_entity
...
from agentnexus.db.query_context import query_name_scope
...
from agentnexus.session_import.models import (
...
def _to_conversation(
    ...
    :param meta: Optional metadata row from
        ``agentnexus_conversation_metadata``. When ``None``, all
        AgentNexus-operational fields default (``kind="default"``,
...
def _new_session_conversation_row(
    ...
    (``session_overrides``) live on this row; AgentNexus operational fields
...
def _new_session_metadata_row(
    ...
    """
    Build the AgentNexus metadata row paired with a new session conversation.
...
def _fetch_labels(
    ...
    """
    with query_name_scope("agentnexus.conversation_store.select_conversation_labels"):
        rows = (
            session.execute(
...
class SqlAlchemyConversationStore(ConversationStore):
    ...
    def __init__(
        ...
        :param storage_location: SQLAlchemy database URI for the AgentNexus DB,
        ...
        # AgentNexus DB: agents, hosts, policies, files, user_daily_costs,
        # session_permissions, comments, agentnexus_conversation_metadata.
        self._engine = get_or_create_engine(storage_location)
        self._session = make_named_managed_session_maker(
            self._engine,
            query_name_prefix="agentnexus.conversation_store",
        )
        ...
        # Defaults to the AgentNexus DB when not separately configured. Always creates
        # a separate session factory so AP and AgentNexus writes run in independent
        ...
        # AgentNexus) gets the correct lock strategy for each table group.
    ...
    def _get_meta(
        ...
        """
        Fetch the metadata row for a conversation from the AgentNexus DB.
        """
    ...
    def create_conversation(
        ...
        ap_created = False
        try:
            # Get parent's root from AP, then write AP row and AgentNexus meta separately.
            root_id = new_id
    ...
    def set_bot_ownership(
        ...
        *,
        ...
        """Persist Bot ownership on the AgentNexus metadata row."""
    ...
    def get_session_connectivity(
        ...
        if not conversation_ids:
            return {}
        unique_ids = list(set(conversation_ids))
        # runner_id and host_id are in the AgentNexus DB (metadata).
    ...
    def set_conversation_project(
        ...
        Sets ``agentnexus_conversation_metadata.project_id``. ``None`` unfiles the
    ...
    def list_projects(
        ...
        *non-archived* ``conversation_labels`` row with ``key="omni_project"``
        ...
        # the AgentNexus DB, so it still needs a pre-fetch; archived now lives on
    ...
    def list_conversations(
        ...
        # still require an AgentNexus-side prefetch are the permission scopes.
        needs_meta_filter = (accessible_by is not None) or (owned_by is not None)

        qualifying_ids: list[str] | None = None
        if needs_meta_filter:
            # Pre-fetch permission-qualifying IDs from the AgentNexus DB
        ...
        with self._conv_session("list_conversations") as session:
            ...
            if agent_name is not None:
                # Agents live in the AgentNexus DB — resolve to IDs first, then
            ...
            if project is not None:
                ...
                # projects + metadata are on the AgentNexus DB, so member ids are
        ...
        # Fetch metadata from AgentNexus DB and merge.
    ...
    def update_conversation(
        ...
        # binding + per-session override blob) and AgentNexus (metadata).
    ...
    def touch_runner_liveness(self, runner_ids: list[str], now: int) -> None:
        ...
        One bulk ``UPDATE`` on ``agentnexus_conversation_metadata``, so
    ...
    def clear_runner_liveness(self, runner_id: str) -> None:
        ...
        Lives on ``agentnexus_conversation_metadata``, so ``conversations.updated_at``
    ...
    def _create_session_with_agent_with_id(
        ...
        *,
        ...
        # Conversation + labels go to AP; agent + metadata go to AgentNexus.
    ...
    def fork_conversation(
        ...
        *,
        ...
        :param resume_source_native_session: When ``True`` (default), a
            ...
            copied AgentNexus items instead.
    ...
    def _fork_conversation_with_id(
        ...
        *,
        ...
        # from the AgentNexus DB before opening the AP session.
        ...
        with self._conv_session("fork_conversation") as session:
            ...
            # Cloned agent: the row itself is written to the AgentNexus DB after
            ...
            # AgentNexus items) rather than launching fresh (see
            # FORK_CARRY_HISTORY_LABEL_KEY).
        ...
        # Write fork metadata (and cloned agent if any) to the AgentNexus DB.
        with self._session("fork_conversation") as meta_sess:
    ...
    def switch_conversation_agent(
        ...
        *,
        ...
        # AgentNexus holds agent+metadata. Read old_agent_id before overwriting it.
        ...
        # Update agent + metadata on the AgentNexus side.
    ...
    def has_other_live_session_in_workspace(
        ...
        *,
        ...
        table (AgentNexus DB) while ``archived`` lives on ``conversations`` (AP
    ...
    async def delete_conversation(self, conversation_id: str) -> bool:
        ...
        # AgentNexus-side rows (metadata/comments/policies/permissions) are cleaned up
        # second. A failure of the second transaction leaves orphaned AgentNexus rows
        ...
        with self._conv_session("delete_conversation") as ap_sess:
            ...
            # the AgentNexus transaction below can delete the session-scoped
