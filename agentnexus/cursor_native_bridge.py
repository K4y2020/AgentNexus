...
executor then delivers AgentNexus web-UI messages into the *same* pane via
...
if TYPE_CHECKING:
    from agentnexus.cursor_native import CursorModelOption


#: Env var carrying the bridge dir into the harness executor process.
BRIDGE_DIR_ENV_VAR = "HARNESS_CURSOR_NATIVE_BRIDGE_DIR"

_BRIDGE_ROOT = Path(tempfile.gettempdir()) / f"agentnexus-{stable_user_id()}" / "cursor-native"
...
_PASTE_SETTLE_S = 0.3
_PASTE_BUFFER = "agentnexus-cursor-paste"
...
#: when mirroring the user turn back to AgentNexus so the copied history isn't
...
def wrap_fork_preamble(preamble: str, user_text: str) -> str:
    ...
    duplicated in the AgentNexus timeline.
...
def build_mcp_config(
    ...
    *,
    ...
    """Build Cursor's ``.cursor/mcp.json`` for the AgentNexus relay server.

    Cursor prompts for MCP tool approval before it sends ``tools/call`` to the
    server. AgentNexus tools already route through the AgentNexus ``/mcp`` proxy,
    ...
    hidden in-terminal approval prompt blocking the call before AgentNexus ever
    sees it, while preserving AgentNexus's own policy/elicitation gate.
    """
...
def write_mcp_bridge_config(bridge_dir: Path) -> None:
    """Write the token config required by the shared AgentNexus MCP bridge."""
    _ensure_dir(bridge_dir)
...
def write_mcp_config(
    ...
    *,
    ...
    """Write the workspace-scoped Cursor MCP config for AgentNexus tools.

    Merges the AgentNexus bridge server into an existing ``mcp.json`` so that
    ...
    if not isinstance(omnigent_servers, dict):  # pragma: no cover - build_mcp_config invariant
        raise ValueError("AgentNexus MCP config is missing mcpServers")
...
def build_hooks_config(bridge_dir: Path, *, python_executable: str | None = None) -> _JsonObject:
    ...
    command = " ".join(
        ...
        for part in (
            ...
            "-I",
            "-m",
            "agentnexus.cursor_native_usage",
            "record-usage",
            "--bridge-dir",
            str(bridge_dir),
        )
...
def approve_mcp_server_for_workspace(workspace: Path) -> None:
    """Approve the workspace-scoped AgentNexus MCP server in Cursor's state.
...
def enable_mcp_for_workspace(workspace: Path) -> None:
    """Ensure Cursor does not keep the AgentNexus MCP disabled for this workspace."""
    disabled_path = (
...
def allow_mcp_tools_in_cli_config() -> None:
    """Allow AgentNexus MCP tool calls in Cursor's CLI permission config."""
