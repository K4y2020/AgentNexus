"""SSE consumer that mirrors OpenCode events into an AgentNexus session.
...
OpenCode events into AgentNexus session-stream events posted to
...
Design references: the SSE-event → AgentNexus-event translation table in
...
from agentnexus.opencode_native_bridge import update_active_message_id
from agentnexus.opencode_native_client import OpenCodeClient, OpenCodeEvent
from agentnexus.opencode_native_permissions import (
...
# AgentNexus session-event types (must match the server's ingestion route;
...
# back to AgentNexus so the web model pill stays in sync (claude-native contract).
...
class OpenCodeNativeForwarder:
    """
    Translate one OpenCode session's SSE stream into AgentNexus events.
    ...
    :param server_client: HTTP client for the AgentNexus server (event posts).
    ...
    def __init__(
        ...
        *,
        ...
        # Last model mirrored to AgentNexus (provider/id), to dedupe switches.
    ...
    async def handle_event(self, event: OpenCodeEvent) -> None:
        """
        Translate one OpenCode event into AgentNexus session events.
    ...
    async def _post_event(self, event_type: str, data: _JsonObject) -> httpx.Response | None:
        """
        POST one AgentNexus session event with a single retry.

        :param event_type: AgentNexus event type, e.g.
    ...
    async def _on_compaction_started(self, event: OpenCodeEvent) -> None:
        ...
        server-side. The AgentNexus server maps ``external_compaction_status``
    ...
    async def _on_model_switched(self, event: OpenCodeEvent) -> None:
        ...
        When the user switches model in the opencode TUI, reflect it to AgentNexus
...
_HANDLERS: dict[str, Callable[[OpenCodeNativeForwarder, OpenCodeEvent], Awaitable[None]]] = {
    ...
    # Mirror a TUI model switch back to AgentNexus (in-harness session-cmd sync).
