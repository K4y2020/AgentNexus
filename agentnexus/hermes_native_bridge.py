...
then delivers AgentNexus web-UI messages into the *same* pane via
...
When AgentNexus policies are configured the runner writes a per-session
``HERMES_HOME`` with a ``pre_tool_call`` hook (via :func:`write_policy_hook_config`)
that evaluates tool calls against the AgentNexus policy engine — the same hook used
...
_BRIDGE_ROOT = Path(tempfile.gettempdir()) / f"agentnexus-{stable_user_id()}" / "hermes-native"
_TMUX_FILE = "tmux.json"
_TMUX_READY_TIMEOUT_S = 30.0
_TMUX_SEND_TIMEOUT_S = 10.0
_POLL_INTERVAL_S = 0.2
_PASTE_SETTLE_S = 0.3
_PASTE_BUFFER = "agentnexus-hermes-paste"
...
# starts the AgentNexus MCP server (a heavyweight ``python -m`` subprocess). A
...
def clone_hermes_session(
    ...
    *,
    ...
    try:
        ...
        # messages (AgentNexus already has them from the fork item copy).
...
def build_hermes_native_spawn_env(session_id: str) -> dict[str, str]:
    ...
    :param session_id: The AgentNexus session id (keys the bridge dir).
...
def write_policy_hook_config(
    ...
    """Write per-session ``HERMES_HOME`` with AgentNexus policy hook and MCP server.

    Creates a ``config.yaml`` registering:

    1. A ``pre_tool_call`` shell hook that evaluates tool calls against the
       AgentNexus policy engine (same hook the headless ``hermes`` harness uses).
    2. An ``mcp_servers.agentnexus`` entry that launches the AgentNexus MCP stdio
       server (``serve-mcp --bridge-dir <bridge_dir>``), exposing AgentNexus
    ...
    from agentnexus.native_policy_hook import policy_hook_wrapper_script
    ...
    # Register the AgentNexus MCP stdio server so Hermes can call
    # AgentNexus builtin tools (sys_session_*, sys_agent_*, load_skill, etc.).
...
def inject_relay_into_policy_hook(
    ...
    hook_script_path = str(Path(__file__).resolve().parent / "inner" / "hermes_policy_hook.py")
    from agentnexus.native_policy_hook import _RELAY_TOKEN_ENV, _RELAY_URL_ENV
    ...
    fd, tmp_name = tempfile.mkstemp(prefix="agentnexus-policy-hook.", dir=str(hermes_home))
    try:
