...
- The AgentNexus harness turn, running when the web UI submits a
...
- An MCP stdio server (``serve-mcp`` subcommand) that Claude Code
  launches as a child process. It advertises AgentNexus tools to
  Claude (workspace ``sys_os_*`` tools outside an active turn,
  active-turn AgentNexus tools via a per-turn relay).
...
from agentnexus._platform import stable_user_id
from agentnexus.claude_model_vocabulary import MODEL_VOCABULARY_ENV_VARS
from agentnexus.claude_native_message_display_hook import MESSAGE_DELTAS_FILE
from agentnexus.claude_native_status import CONTEXT_RAW_FILE
from agentnexus.json_types import JsonObject as _JsonObject
from agentnexus.kiro_native_bridge import bridge_root as kiro_bridge_root
...
BRIDGE_ID_LABEL_KEY = "agentnexus.claude_native.bridge_id"
...
_BRIDGE_ROOT_PARENT = _TRUSTED_PARENT / f"agentnexus-{stable_user_id()}"
...
# to the AgentNexus policy server and back. It MUST exceed ``_TOOL_CALL_TIMEOUT_S``
...
# AgentNexus never drives that picker — it switches with ``/model <id>`` — but a
...
def _absolute_syntactic_path(path: Path) -> Path:
    ...
    :param path: Path to normalize, e.g. ``Path("~/.agentnexus/x")``.
...
def _trusted_parent_for_bridge_dir(target: Path) -> Path:
    ...
    from agentnexus.codex_native_bridge import bridge_root
    ...
    if target.is_relative_to(pi_root):
        ...
        if pi_root.name == "pi-native" and pi_root.parent.name == ".agentnexus":
            trusted_parent = pi_root.parent.parent
        return _absolute_syntactic_path(trusted_parent)

    from agentnexus.cursor_native_bridge import bridge_root as cursor_bridge_root
    ...
    from agentnexus.antigravity_native_bridge import bridge_root as antigravity_bridge_root

    # antigravity-native keeps its bridge files below ``~/.agentnexus/antigravity-native``,
    # the same ``$HOME/.agentnexus/<harness>-native`` shape codex uses, so apply the
    # identical anchor logic: in production trust ``$HOME`` and validate/chmod the
    # two bridge-owned dirs below it (``.agentnexus`` and ``antigravity-native``); in
    ...
    if target.is_relative_to(antigravity_root):
        ...
        if (
            ...
            and antigravity_root.parent.name == ".agentnexus"
        ):
            trusted_parent = antigravity_root.parent.parent
        return _absolute_syntactic_path(trusted_parent)

    from agentnexus.qwen_native_bridge import bridge_root as qwen_bridge_root
    ...
    from agentnexus.hermes_native_bridge import bridge_root as hermes_bridge_root
    ...
    from agentnexus.opencode_native_bridge import bridge_root as opencode_bridge_root

    # opencode-native keeps its bridge files below ``~/.agentnexus/opencode-native``
    # (the same ``$HOME/.agentnexus/<harness>-native`` shape codex/antigravity use),
    # so apply the identical anchor logic: in production trust ``$HOME`` and
    # validate/chmod the two bridge-owned dirs below it (``.agentnexus`` and
    ...
    if target.is_relative_to(opencode_root):
        ...
        if opencode_root.name == "opencode-native" and opencode_root.parent.name == ".agentnexus":
    ...
    # Headless ACP harnesses (acp / goose / qwen) put their AgentNexus-MCP relay
...
class ClaudeTranscriptItem:
    """
    One AgentNexus conversation item parsed from Claude's JSONL log.
    ...
    :param item_type: AgentNexus conversation item type, e.g.
    ...
    :param is_compact_summary: ``True`` when this item was parsed from a
        ...
        AgentNexus compaction boundary (see
...
class TranscriptReadResult:
    ...
    :param items: Parsed AgentNexus conversation items from the
    ...
    :param latest_custom_title: ``customTitle`` from the most recent
        ...
        not surfaced here; AgentNexus titles unnamed sessions itself.
    """
...
class ClaudeHookRecord:
    ...
    :param claude_session_was_seen: Whether the incoming Claude
        ...
        context.
    :param clear_rotated_to: AgentNexus session id created synchronously by the
    ...
    :param fork_detected: Whether the hook identified this record as a
        ...
        hooks.jsonl.
    :param fork_rotated_to: AgentNexus session id created synchronously by the
...
class ClaudeNativeToolRelay:
    ...
    Code invokes a relayed AgentNexus tool; the relay forwards the call
    into the ``tool_executor`` callback supplied at start, which dispatches
    it on the runner event loop (e.g. through the AgentNexus REST API).
...
def _ensure_secure_dir(target: Path) -> None:
    ...
    ``/tmp/omnigent-<UID>`` (Claude-native), ``~/.agentnexus``
...
def acp_mcp_bridge_root() -> Path:
    """Bridge root for the headless ACP harnesses' AgentNexus-MCP relay.

    Shares the uid-scoped temp parent with claude-native
    (``$TMPDIR/omnigent-<uid>/acp-mcp``). Used by the acp / goose / qwen
    executors' ``AgentNexusAcpMcp`` relay so ``serve-mcp``'s bridge dir passes the
...
def bridge_dir_for_conversation_id(conversation_id: str) -> Path:
    ...
    :param conversation_id: AgentNexus conversation id used as bridge id, e.g.
        ``"conv_abc123"``.
    :returns: Absolute bridge directory under
        ``/tmp/omnigent-<UID>/claude-native``.
    """
...
def build_claude_native_spawn_env(
    ...
    *,
    ...
    :param conversation_id: AgentNexus conversation id, e.g.
        ``"conv_abc123"``.
...
def prepare_bridge_dir(
    ...
    *,
    ...
    # reads the AgentNexus server URL from it at runtime, so wiping it on re-prep
...
def read_active_session_id(bridge_dir: Path) -> str | None:
    """
    Read the AgentNexus session currently receiving bridge-originated events.

    :param bridge_dir: Bridge directory path.
    :returns: Active AgentNexus session id, e.g. ``"conv_abc123"``, or
...
def write_active_session_id(bridge_dir: Path, session_id: str) -> None:
    """
    Atomically update the bridge's active AgentNexus session.

    :param bridge_dir: Bridge directory path.
    :param session_id: New active AgentNexus session id, e.g.
        ``"conv_abc123"``.
    :returns: None.
...
def read_permission_hook_config(bridge_dir: Path) -> _JsonObject:
    """
    Read AgentNexus routing details for the permission command hook.
...
def build_mcp_config(bridge_dir: Path, *, python_executable: str | None = None) -> _JsonObject:
    """
    Build the Claude Code MCP config for the AgentNexus bridge server.
...
def build_hook_settings(
    ...
    *,
    ...
    :param python_executable: Python executable to run, e.g.
        ``"/path/to/.venv/bin/python"``. ``None`` uses
        :data:`sys.executable`.
    :param ap_server_url: AgentNexus server base URL the ``PermissionRequest``
    ...
    if ap_server_url:
        ...
        evaluate_policy_command = (
            ...
            '-H "Authorization: Bearer $AGENTNEXUS_RELAY_TOKEN" '
            "-H 'Content-Type: application/json' --data-binary @- "
            '"$AGENTNEXUS_RELAY_URL/hook/claude/evaluate-policy" 2>/dev/null) '
    ...
    if subagent_router_dir is not None:
        ...
        router_command_parts = [
            ...
            "-I",
            "-m",
            "agentnexus.inner.hook_scripts.claude_router_hook",
            "--bridge-dir",
            str(bridge_dir),
            "--router-dir",
...
_AGENTNEXUS_DISALLOWED_TOOLS: tuple[str, ...] = ()
...
def augment_claude_args(
    ...
    *,
    ...
    """
    Return Claude CLI args with AgentNexus MCP/hook/skill injection.
    ...
    :param ap_server_url: AgentNexus server base URL passed through to
    ...
    args = _merge_disallowed_tools(list(claude_args), _AGENTNEXUS_DISALLOWED_TOOLS)
    ...
    from agentnexus.inner.bundle_skills import claude_native_skill_args
...
def _merge_allowed_tools(args: list[str], extra: tuple[str, ...]) -> list[str]:
    ...
    :param extra: Tool names AgentNexus may call without an interactive prompt.
...
def _merge_disallowed_tools(args: list[str], extra: tuple[str, ...]) -> list[str]:
    ...
    :param args: Claude CLI argument list to mutate-and-return.
    :param extra: Tool names AgentNexus wants disabled.
...
def read_transcript_items_since(
    ...
    *,
    ...
    """
    Read Claude transcript records as AgentNexus conversation items.
    ...
    while translating the user-visible semantic records into AgentNexus
...
def read_transcript_items_from_offset(
    ...
    *,
    ...
    :param include_sidechains: Pass ``True`` when reading a
        ...
        leave the sub-agent's child AgentNexus conversation empty. The
...
def compute_transcript_cumulative_cost(
    ...
    **Deduplicated by ``requestId``.** Claude writes more than one
    ...
    from agentnexus.llms.context_window import compute_llm_cost
    from agentnexus.onboarding.provider_config import load_config
...
def read_hook_events_since(
    ...
    events to AgentNexus when Claude Code's ``Stop`` / ``StopFailure`` hooks
...
def _hook_record_from_jsonl_record(record: _JsonlRecord) -> ClaudeHookRecord:
    ...
    raw_previous_claude_session_id = (
        payload.get("agentnexus_previous_claude_session_id") if isinstance(payload, dict) else None
    )
    raw_claude_session_was_seen = (
        payload.get("agentnexus_claude_session_was_seen") if isinstance(payload, dict) else None
    )
    raw_clear_rotated_to = (
        payload.get("agentnexus_clear_rotated_to") if isinstance(payload, dict) else None
    )
    raw_fork_detected = (
        payload.get("agentnexus_fork_detected") if isinstance(payload, dict) else None
    )
    raw_fork_rotated_to = (
        payload.get("agentnexus_fork_rotated_to") if isinstance(payload, dict) else None
    )
...
def inject_user_message(
    ...
    *,
    ...
    :param bridge_dir: Bridge directory path.
    :param content: User text from the AgentNexus web UI. Must be non-empty.
    ...
    # that AgentNexus cannot drive. Allowed commands (``/clear``,
    ...
    try:
        _run_tmux(info["socket_path"], "load-buffer", "-b", "agentnexus-paste", paste_path)
        _run_tmux(
            info["socket_path"],
            "paste-buffer",
...
def kill_session(
    ...
    *,
    ...
    of that manual exit for the AgentNexus web UI's "Stop session" affordance:
...
def display_cost_approval_popup(
    ...
    *,
    ...
    the popup at *config_file* for AgentNexus routing (base URL + auth
    ...
    """
    from agentnexus.native_cost_popup import launch_cost_popup
...
def start_tool_relay(
    *,
    ...
    """
    Start a relay for AgentNexus tool calls from Claude.
    ...
    AgentNexus server using the runner's refresh-capable client — so hook
    subprocesses never need a server bearer token of their own.

    :param bridge_dir: Bridge directory path.
    :param tools: AgentNexus tool schemas to advertise.
    ...
    env_path.write_text(
        f"AGENTNEXUS_RELAY_URL='http://{host}:{port}'\nAGENTNEXUS_RELAY_TOKEN='{token}'\n",
        encoding="utf-8",
    )
...
def _run_relay_tool(
    ...
    try:
        ...
        return _mcp_error(f"AgentNexus tool dispatch failed: {exc}")
    if name == "sys_os_view_image" and isinstance(result, dict):
        from agentnexus.runtime.image_tool import image_mcp_response
...
def _stdio_jsonrpc_loop(
    ...
    :param tools: AgentNexus tools exposed over MCP.
    :param stdout_lock: Lock protecting JSON-RPC writes to stdout.
...
def _handle_mcp_request(
    ...
    if method == "initialize":
        return {
            ...
            "instructions": (
                "AgentNexus tools are available as MCP tools when the "
                "active AgentNexus turn advertises them; local sys_os_* "
...
def _mcp_tool_schema(tool: Tool) -> _JsonObject:
    """
    Convert an AgentNexus tool schema into MCP tool-list shape.
...
def _combined_mcp_tool_schemas(
    ...
    :returns: MCP tool descriptors. Active relay tools override
        local tools with the same name so calls flow through AgentNexus and
        appear in the AgentNexus event stream during web turns.
    """
...
def _mcp_tool_schema_from_spec(tool_spec: _JsonObject) -> _JsonObject:
    """
    Convert an AgentNexus tool schema dict into MCP tool-list shape.
...
def _call_relay_tool(
    ...
    url = relay.get("url")
    if not isinstance(token, str) or not isinstance(url, str):
        return _mcp_error("active AgentNexus tool relay is missing url/token")
    ...
    # server.
        ...
        return _mcp_error(f"failed to call AgentNexus tool relay: {exc}")
    try:
        decoded = json.loads(raw)
    except json.JSONDecodeError:
        return _mcp_error("AgentNexus tool relay returned malformed JSON")
    if not isinstance(decoded, dict):
        return _mcp_error("AgentNexus tool relay returned non-object JSON")
    return decoded
...
def _build_tools(config: _JsonObject) -> tuple[dict[str, Tool], Callable[[], None]]:
    """
    Build AgentNexus MCP tools served by the bridge.
    ...
    from agentnexus.tools.builtins.os_env import build_os_env_tools
...
def _transcript_items_from_entry(
    ...
    *,
    ...
    """
    Convert one Claude transcript entry into AgentNexus conversation items.
    ...
    :param include_sidechains: When ``False`` (the default) any record
        ...
        appear in the parent's AgentNexus conversation. When ``True`` the
...
def _attachment_transcript_items_from_entry(
    ...
    *,
    ...
    AgentNexus transcript and reset the active assistant response.
...
# Slash commands that AgentNexus lets a user type directly into the native
...
# (menu, login prompt, exit, etc.) that AgentNexus cannot drive.
...
def _escape_unsupported_slash_command(content: str) -> str:
    """
    Escape built-in UI slash commands that AgentNexus cannot drive.
...
def _response_id_from_source(source: str) -> str:
    """
    Derive a deterministic AgentNexus response id from a Claude source key.
...
def _source_id(source_key: str, item_index: int, item_type: str) -> str:
    ...
    :param item_index: Content block index inside the record.
    :param item_type: AgentNexus item type.
