...
_BRIDGE_ROOT = Path(tempfile.gettempdir()) / f"agentnexus-{stable_user_id()}" / "kimi-native"
_TMUX_FILE = "tmux.json"
# AgentNexus routing details the kimi hook subprocess reads to reach the server.
# Mirrors claude-native's ``permission_hook.json`` (server URL + auth headers +
# the active AgentNexus session). Written by the runner at terminal-create time;
...
_PASTE_BUFFER = "agentnexus-kimi-paste"
...
def write_hook_config(
    ...
    *,
    ...
    """Record the AgentNexus routing details the kimi hook subprocess reads.
    ...
    :param server_url: AgentNexus server base URL, e.g. ``"http://127.0.0.1:8787"``.
    :param headers: Auth headers to replay on the hook's POSTs (may be empty).
    :param session_id: The AgentNexus session the hook events belong to.
    """
    _ensure_dir(bridge_dir)
    payload = {
...
def read_hook_config(bridge_dir: Path) -> _JsonObject:
    """Read AgentNexus routing details for the kimi hook subprocess.
...
def read_active_session_id(bridge_dir: Path) -> str | None:
    """Return the AgentNexus session id recorded for the hook subprocess.
...
#: "Approve once" re-prompts each call so AgentNexus governs every one.
APPROVE_KEY = "1"
DENY_KEY = "3"
