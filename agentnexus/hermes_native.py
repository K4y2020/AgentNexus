"""Native Hermes TUI wrapper for the AgentNexus CLI.

``omnigent hermes`` launches Nous Research's Hermes Agent interactive TUI (the bare
``hermes`` command) inside an AgentNexus-runner-owned tmux terminal and attaches the
...
``~/.hermes/config.yaml``); no AgentNexus-managed key is required. Like goose there
...
from agentnexus._native_resume_hint import echo_native_cold_resume_hint, echo_native_resume_hint
...
from agentnexus._wrapper_labels import HERMES_NATIVE_WRAPPER_VALUE as _WRAPPER_LABEL_VALUE
...
_HERMES_PATH_ENV = "AGENTNEXUS_HERMES_PATH"
...
class PreparedHermesTerminal:
    ...
    :param cold_resumed: ``True`` when resuming an existing AgentNexus session whose
...
def run_hermes_native(
    *,
    ...
    """
    Launch the Hermes TUI in an AgentNexus terminal.
    ...
    if server is None:
        raise click.ClickException(
            "Hermes requires a resolved AgentNexus server URL. The CLI should call "
            "_ensure_backend before run_hermes_native."
        )
    with TemporaryDirectory(prefix="agentnexus-hermes-native-") as tmpdir:
...
def _run_with_remote_server(
    ...
    *,
    ...
    """
    Launch Hermes on an AgentNexus server via a daemon-spawned runner.
