...
``command`` —
  ...
  2. ambient env var (``AGENTNEXUS_<NAME>_PATH``)
...
# Legacy ``HARNESS_*_PATH`` env vars and their canonical ``AGENTNEXUS_<NAME>_PATH``
...
_LEGACY_PATH_VARS: dict[str, str] = {
    "HARNESS_CODEX_PATH": "AGENTNEXUS_CODEX_PATH",
    "HARNESS_PI_PATH": "AGENTNEXUS_PI_PATH",
    "HARNESS_KIMI_PATH": "AGENTNEXUS_KIMI_PATH",
    "HARNESS_GOOSE_PATH": "AGENTNEXUS_GOOSE_PATH",
    "HARNESS_QWEN_PATH": "AGENTNEXUS_QWEN_PATH",
    "HARNESS_HERMES_PATH": "AGENTNEXUS_HERMES_PATH",
}
...
# ``AGENTNEXUS_CLAUDE_PATH`` with ``claude-native``. Add entries here only when a
...
def resolve_harness_command(
    ...
    *,
    ...
    # Check both the canonical AGENTNEXUS_* and the deprecated HARNESS_* env var
...
def config_harness_path_override(
    ...
    ``AGENTNEXUS_<NAME>_PATH`` env var — but only when the user hasn't already
    ...
    ``if v: env["AGENTNEXUS_X_PATH"] = v``.

    :param harness: A harness id (canonical or alias), e.g. ``"codex"``.
    :param cfg: Effective config dict.
    :returns: The config command string to set as ``AGENTNEXUS_<NAME>_PATH``,
        or ``None`` when config has no override or the ambient env var is set.
    """
    canonical = _canonicalize(harness)
    # Ambient env wins over config — check BOTH the canonical AGENTNEXUS_* and
