"""Native Goose TUI wrapper for the AgentNexus CLI.

``omnigent goose`` launches Block's Goose CLI interactive TUI (``goose session``)
inside an AgentNexus-runner-owned tmux terminal and attaches the local TTY — the
...
``~/.config/goose/config.yaml`` + keyring); no AgentNexus-managed key is required.
...
from agentnexus._wrapper_labels import GOOSE_NATIVE_WRAPPER_VALUE as _WRAPPER_LABEL_VALUE
...
_GOOSE_PATH_ENV = "AGENTNEXUS_GOOSE_PATH"
...
def run_goose_native(
    *,
    ...
    """
    Launch the Goose TUI in an AgentNexus terminal.
    ...
    if server is None:
        raise click.ClickException(
            "Goose requires a resolved AgentNexus server URL. The CLI should call "
            "_ensure_backend before run_goose_native."
        )
    with TemporaryDirectory(prefix="agentnexus-goose-native-") as tmpdir:
...
def _run_with_remote_server(
    ...
    *,
    ...
    """
    Launch Goose on an AgentNexus server via a daemon-spawned runner.
