"""Codex Code hook entrypoint for native AgentNexus policy enforcement.
...
the AgentNexus policy schema is shared with the Claude-native hook via
...
if TYPE_CHECKING:
    from agentnexus.codex_native_app_server import CodexAppServerClient
...
def main(argv: list[str] | None = None) -> int:
    ...
    print(
        f"agentnexus codex hook: unknown subcommand {raw_argv[:1]!r}",
        file=sys.stderr,
    )
    return 0
...
def _main_evaluate_policy(argv: list[str]) -> int:
    ...
    try:
        ...
        print(f"agentnexus codex evaluate-policy hook: malformed JSON: {exc}", file=sys.stderr)
        return 0
    if not isinstance(payload, dict):
        print("agentnexus codex evaluate-policy hook: expected JSON object", file=sys.stderr)
        return 0
    ...
    if not resp.content:
        print("agentnexus codex evaluate-policy hook: empty AgentNexus response", file=sys.stderr)
        return _fail_closed()
    ...
    try:
        ...
        print(
            "agentnexus codex evaluate-policy hook: malformed AgentNexus response",
            file=sys.stderr,
        )
        return _fail_closed()
...
def _main_route_turn(argv: list[str]) -> int:
    ...
    if declined is not None:
        ...
        print(
            f"agentnexus codex route-turn hook: {declined}; "
...
def _write_marker(bridge_dir: Path, session_id: str, decision: dict[str, object]) -> bool:
    ...
    if write_turn_routing_marker(
        ...
        return True
    print(
        f"agentnexus codex route-turn hook: could not write the marker in {bridge_dir}",
        file=sys.stderr,
    )
    return False
...
def _apply_thread_model(bridge_dir: Path, model: str) -> str | None:
    ...
    from agentnexus.codex_model_vocabulary import codex_reachable_model_slug
    ...
    from agentnexus.runner.turn_routing import SETTINGS_UPDATE_TIMEOUT_S
    ...
    async def _switch() -> None:
        ...
        client = client_for_transport(state.socket_path, client_name="agentnexus-route-turn-hook")
    ...
    if not write_codex_config_model(bridge_dir, applied):
        print(
            f"agentnexus codex route-turn hook: could not mirror {applied} into config.toml",
            file=sys.stderr,
        )
    return None
...
async def _list_codex_models(client: CodexAppServerClient) -> list[dict[str, object]] | None:
    ...
    try:
        ...
        print(
            f"agentnexus codex route-turn hook: model/list failed: {exc}",
            file=sys.stderr,
        )
        return None
    return rows
