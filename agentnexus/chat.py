...
The CLI always ends by connecting an AgentNexus client to a server URL. For
...
from agentnexus_client import (
    AgentNexusError as ClientAgentNexusError,
)
...
from agentnexus.conversation_browser import (
...
from agentnexus.inner.databricks_executor import _DatabricksBearerAuth, _read_databrickscfg
from agentnexus.model_catalog import resolve_catalog_model
from agentnexus.model_resolver import ModelResolutionError
from agentnexus.native_coding_agents import native_coding_agent_for_wrapper_label
from agentnexus.native_dispatch import resolve_hook_for_key
...
from agentnexus.spec.types import AgentSpec, SkillSpec
...
_REMOTE_AUTH_TOKEN_ENV = "AGENTNEXUS_REMOTE_AUTH_TOKEN"

# Env-var override name. ``AGENTNEXUS_MODEL=foo`` lets a user
...
_AGENTNEXUS_MODEL_ENV_VAR = "AGENTNEXUS_MODEL"
...
def _default_cli_model() -> str:
    ...
    Reads ``AGENTNEXUS_MODEL`` first, then resolves the Databricks OpenAI-family
    ...
    """
    configured = os.environ.get(_AGENTNEXUS_MODEL_ENV_VAR)
    ...
    try:
        ...
        raise click.ClickException(
            ...
            "set AGENTNEXUS_MODEL, or retry when Databricks catalog discovery is available."
        ) from exc
...
class LocalServer:
    ...
    :param log_path: Path to the file that captures the
        ...
        e.g. ``Path("~/.agentnexus/logs/server/server-abc123.log")``.
...
def _on_session_known(
    *,
    ...
    the stable ``AgentNexus session: <url>`` line; additionally opens the
...
def run_chat(
    ...
    *,
    ...
    :param ephemeral: When ``True``, place the local server's
        ...
        the persistent ``~/.agentnexus`` location. Maps to
    ...
    :param log: When ``True``, write a JSON dump of the active
        conversation to ``~/.agentnexus/logs/`` on REPL exit.
        Maps to ``--log`` on the CLI (default-on for the legacy
        path, default-off here so it stays explicit on
        AgentNexus mode). See ``omnigent.repl._session_log`` for the
...
def run_attach(
    *,
    ...
    :param base_url: AgentNexus server hosting the session, e.g.
        ``"http://127.0.0.1:6767"``.
...
def _remote_headers(
    ...
    *,
    ...
    :returns: Headers to pass to httpx / AgentNexusClient.
    """
...
def _server_headers(
    *,
    ...
    """
    Build non-auth HTTP headers for an AgentNexus server client.
    ...
    :param runner_id: Optional runner UUID, e.g.
        ...
        request header.
    :returns: Static headers for ``httpx`` / ``AgentNexusClient``.
    """
    del runner_id
    return {}
...
def _chat_with_server(
    ...
    *agent_name* is supplied by an upstream setup step such as
    ...
    :param server_log_path: Path to the local server's
        ...
        ``Path("~/.agentnexus/logs/server/server-abc123.log")``. Shown in the
        Ctrl+O debug overview. ``None`` for remote servers.
    :param runner_log_path: Path to the local runner's
        stdout/stderr log file, e.g.
        ``Path("~/.agentnexus/logs/runner/runner-abc123.log")``. Shown in the
...
def _redirect_native_resume_if_needed(
    *,
    ...
    """
    Redirect a terminal-native resume before AgentNexus attach liveness runs.

    :param base_url: AgentNexus server base URL, e.g. ``"https://example.com"``.
    :param conversation_id: AgentNexus conversation id, e.g. ``"conv_abc123"``.
    ...
    if run_native is None:
        return False
    # The native TUI owns the turns; resuming through the AgentNexus REPL would run
    # an AgentNexus turn per message *and* let the transcript forwarder mirror the
...
def _finish_native_redirect_progress(
    *,
    ...
    """
    Finish any AgentNexus startup progress and print the native redirect notice.
...
def _wrapper_label_for_conversation(
    *,
    ...
    the response's ``labels.agentnexus.wrapper`` field. ``None`` on any
    transport / parse error so a flaky server doesn't silently misroute
    the resume — the caller falls back to the normal AgentNexus REPL path and
...
def _await_accounts_first_run_setup(
    ...
    *,
    ...
    the browser auto-opened (e.g. ``AGENTNEXUS_ACCOUNTS_AUTO_OPEN=0``) — and
    ...
    :param base_url: Resolved local AgentNexus server URL, e.g.
        ``"http://127.0.0.1:6767"``.
...
def _unreachable_server_message(base_url: str) -> str:
    ...
    if is_loopback_url(base_url):
        return (
            f"Could not connect to the local AgentNexus server at {base_url}. "
    ...
    return (
        f"Could not connect to the AgentNexus server at {display_server_url(base_url)}. "
...
async def _prepare_chat_session_via_daemon(
    *,
    ...
    :param base_url: AgentNexus server base URL, e.g. ``"http://127.0.0.1:8123"``.
    ...
    async def resolve_session() -> tuple[str, bool]:
        ...
        """
        async with AgentNexusClient(base_url=base_url, headers=headers, auth=auth) as sdk:
            try:
            ...
            except ClientAgentNexusError as exc:
...
def _chat_via_daemon(
    ...
    *,
    ...
    :param base_url: Resolved AgentNexus server base URL (the daemon is already
...
def _wait_for_remote_runner(
    ...
    *,
    ...
    :param log_path: Optional path to the captured runner log
        ...
        e.g. ``Path("~/.agentnexus/logs/runner/runner-abcd.log")``.
...
def _bundle_agent(agent_path: Path) -> bytes:
    ...
    :raises AgentNexusError: If bundling fails, for example due to
...
def _chat_local(
    ...
    *,
    ...
    :param ephemeral: When ``True``, point the local server at a
        ...
        (default) uses the persistent ``~/.agentnexus``
        location so prior conversations remain reachable —
        see designs/RUN_AGENTNEXUS_SESSION_RESUMPTION.md.
    ...
    try:
        ...
        try:
            agent_spec = load_spec(spec_path)
        except (AgentNexusError, FileNotFoundError) as exc:
...
async def _query_sessions_once(
    *,
    ...
    """
    from agentnexus_client import SessionsChat
    ...
    # raises ``AgentNexusError`` on that ``failed`` status, and the
    ...
    # ``AgentNexusError`` (session flipped to ``failed``) and an empty
    ...
    # ClientAgentNexusError`` and print the message to stderr + exit non-zero.
    turn_error = await _persisted_turn_error(client, bound.id)
    if turn_error is not None:
        raise ClientAgentNexusError(turn_error)
    return None
...
def _response_output_text(output: _ResponseOutput) -> str | None:
    """Extract assistant text from an AgentNexus response ``output`` list."""
    parts: list[str] = []
    for item in output:
...
async def _persisted_turn_text(
    ...
    * A transport-level runner disconnect publishes
      ...
      :meth:`SessionsChat.send` raises ``AgentNexusError`` on it.
...
def _run_picker(
    *,
    ...
    :raises click.ClickException: When the agent has no prior
        ...
        out of.
    """
    from agentnexus.repl._resume_picker import pick_conversation_from_sdk
...
async def _resolve_latest_conversation_id_async(
    *,
    ...
    ``AgentNexusClient`` connected to a real URL — the path
    used in production by ``_chat_local``.

    :param client: A connected :class:`AgentNexusClient`.
...
def _materialize_override_bundle(source: Path, overrides: ChatOverrides) -> Path:
    ...
    tmpdir = Path(tempfile.mkdtemp(prefix="agentnexus-override-"))
    try:
        if source.is_file():
...
def _effective_openai_auth_harness(
    ...
    if model is None:
        return None
    from agentnexus.llms.routing import infer_harness_from_model
...
def _should_inject_openai_env_auth_for_executor(
    ...
    # AGENTNEXUS_CONFIG_HOME, so skipping injection loses nothing. The env
...
def _apply_overrides_to_raw(raw: _YamlMapping, overrides: ChatOverrides) -> None:
    ...
    if overrides.harness is not None:
        ...
        if overrides.model is None:
            ...
            env_model = os.environ.get(_AGENTNEXUS_MODEL_ENV_VAR)
...
def _apply_harness_override_to_executor(
    ...
    if "spec_version" not in raw:
        ...
        return
    etype = str(executor_block.get("type", AGENTNEXUS_EXECUTOR_TYPE))
    if etype != AGENTNEXUS_EXECUTOR_TYPE:
        raise click.ClickException(
            f"--harness only applies to specs with executor.type "
            f"{AGENTNEXUS_EXECUTOR_TYPE!r}; this spec declares executor.type {etype!r}."
        )
    config = executor_block.get("config")
...
def _validate_agent_spec(agent_path: Path) -> None:
    ...
    ``_start_local_server``). Both ``AgentNexusError`` (parse/
...
def _extract_agent_name(agent_path: Path) -> str:
    ...
    except (AgentNexusError, FileNotFoundError):
...
def _omnigent_log_dir() -> Path:
    """
    Resolve the shared AgentNexus process log directory.
    ...
    :returns: ``~/.agentnexus/logs``, created if needed.
    """
    log_dir = logs_root()
...
def _omnigent_persistent_dir() -> Path:
    ...
    Honors ``AGENTNEXUS_DATA_DIR`` (the data-isolation knob a worktree sets
    to avoid sharing ``~/.agentnexus/chat.db``), else lives at
    ``~/.agentnexus`` alongside the native paths ``sessions/`` and ``logs/``
    (see designs/RUN_AGENTNEXUS_SESSION_RESUMPTION.md). Created on first access;
    ...
    :returns: The absolute path to the persistent dir,
        ...
        subdir.
    """
    override = os.environ.get("AGENTNEXUS_DATA_DIR")
    ap_dir = Path(override).expanduser() if override else Path.home() / ".agentnexus"
...
def _start_local_server(
    ...
    *,
    ...
    """
    Launch a local AgentNexus server.

    Server stdout/stderr are routed to ``server.log`` in a
    per-run directory under ``~/.agentnexus/logs`` so concurrent AgentNexus sessions don't
    ...
    ``~/.agentnexus/{chat.db,artifacts/}`` by default —
    persistent across runs so ``--continue`` / ``--resume``
    can resume prior conversations
    (designs/RUN_AGENTNEXUS_SESSION_RESUMPTION.md). Pass
    ...
    :param ephemeral: When ``True``, place the SQLite DB and
        ...
        ``~/.agentnexus`` location. Used for ``--no-session``
    ...
    from agentnexus.cli import _start_cli_runner_process
    ...
    # we spawn below (read by server() via AGENTNEXUS_RUNNER_TUNNEL_TOKEN).
    #
    # Accounts opt-in: when the parent shell selects accounts mode
    # (AGENTNEXUS_AUTH_PROVIDER=accounts), inject the per-spawn
    ...
    child_env = {
        **os.environ,
        ...
        "AGENTNEXUS_LOCAL_SINGLE_USER": "1",
    }
    ...
    # header is the env-unset default; AGENTNEXUS_AUTH_ENABLED=1 opts into
    # accounts (or oidc when AGENTNEXUS_OIDC_* is set). In header/oidc mode we
    ...
    if _accounts_mode:
        if "AGENTNEXUS_ACCOUNTS_COOKIE_SECRET" not in os.environ:
            child_env["AGENTNEXUS_ACCOUNTS_COOKIE_SECRET"] = secrets.token_hex(32)
        ...
        child_env["AGENTNEXUS_ACCOUNTS_BASE_URL"] = f"http://127.0.0.1:{port}"
    ...
    # This ensures the AgentNexus server and its runner subprocess resolve credentials
...
def _stop_local_server(server: LocalServer) -> None:
    ...
    """
    from agentnexus.cli import _stop_cli_runner_process
...
def _spec_used_families(agent_yaml: Path | None) -> list[str]:
    ...
    if agent_yaml.is_dir():
        ...
        return []
    try:
        from agentnexus.onboarding.provider_config import PI_SURFACE, harness_family
...
def _run_repl(
    ...
    *,
    ...
    :param auto_open_conversation: When ``True``, open the
        browser conversation URL when the session id becomes known.
    """
    from agentnexus.repl import run_repl
    from agentnexus.repl._session_log import DEFAULT_LOG_DIR
    from agentnexus.repl._tmux_pane import register_pane
    ...
    async def _main() -> None:
        ...
        from agentnexus.cli_diagnostics import install_asyncio_exception_handler
...
def _run_one_shot(
    *,
    ...
    try:
        ...
        # raises as an AgentNexusError. Surface its message as a clean
...
def _load_tool_handler(name: str) -> ToolHandler:
    ...
    """
    try:
        from agentnexus.client_tools import get_tool_set
    ...
    if hasattr(tool_set, "_TOOL_FNS"):
        ...
        from agentnexus_client.tools import build_tool_handler
