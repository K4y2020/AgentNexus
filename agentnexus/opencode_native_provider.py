...
def build_opencode_mcp_block(
    ...
    """
    Translate AgentNexus MCP server declarations into opencode.json's ``mcp`` block.
...
def build_opencode_omnigent_mcp_server(
    ...
    """
    Build the opencode ``mcp`` entry that connects opencode to AgentNexus's MCP.

    This is what makes opencode's model call the AgentNexus builtin tools
    ...
    runner's comment relay) and proxies each tool call back through the AgentNexus
    ...
    :returns: A one-entry ``mcp`` block ``{"agentnexus": {type:"local", …}}``.
    """
    from agentnexus.claude_native_bridge import build_mcp_config
...
def maybe_merge_user_provider_config(config: dict[str, object]) -> dict[str, object]:
    ...
    URLs). When running under AgentNexus, the per-session ``XDG_CONFIG_HOME``
    ...
    any AgentNexus-synthesized providers (e.g. Databricks gateway).
    ...
    """
    from agentnexus.opencode_native_bridge import user_opencode_config_path
    ...
    if isinstance(existing, dict):
        ...
        # synthesized providers (AgentNexus's keys like "databricks-gateway"
        # take priority).
