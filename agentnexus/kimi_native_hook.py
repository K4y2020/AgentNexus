"""Kimi Code hook commands for the native AgentNexus wrapper.
...
- ``evaluate-policy`` — the ``PreToolUse`` deny-gate. Mirrors
  ...
  Kimi hook payload into an AgentNexus ``EvaluationRequest`` (the snake-cased
  ...
  runs — AgentNexus enforces its deny-policy without silencing the user's
...
def _read_stdin_payload() -> dict[str, object] | None:
    ...
    try:
        ...
        print(f"agentnexus kimi hook: malformed JSON: {exc}", file=sys.stderr)
        return None
    if not isinstance(payload, dict):
        print("agentnexus kimi hook: expected JSON object", file=sys.stderr)
        return None
    return payload


def _main_evaluate_policy(argv: list[str]) -> int:
    """Evaluate a kimi ``PreToolUse`` hook against AgentNexus policies.
    ...
    CLOSED with a ``deny`` (this hook is the sole AgentNexus enforcement point
    for kimi tool calls).
...
def _main_permission_request(argv: list[str]) -> int:
    ...
    try:
        ...
        print(
            f"agentnexus kimi permission-request hook: keystroke inject failed: {exc}",
            file=sys.stderr,
        )
    return 0
...
def _request_web_approval(
    ...
    try:
        ...
        print(
            f"agentnexus kimi permission-request hook: approval request failed: {exc}",
            file=sys.stderr,
        )
        return None
    if not resp.content:
...
def _parse_bridge_dir_args(argv: list[str], prog: str) -> argparse.Namespace:
    ...
    parser = argparse.ArgumentParser(prog=f"agentnexus.kimi_native_hook {prog}")
...
def main(argv: list[str] | None = None) -> int:
    ...
    print(f"agentnexus kimi hook: unknown subcommand {subcommand!r}", file=sys.stderr)
    return 2
