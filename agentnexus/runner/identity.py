...
RUNNER_ID_ENV_VAR = "AGENTNEXUS_RUNNER_ID"
RUNNER_PARENT_PID_ENV_VAR = "AGENTNEXUS_RUNNER_PARENT_PID"
...
RUNNER_WORKSPACE_ENV_VAR = "AGENTNEXUS_RUNNER_WORKSPACE"
RUNNER_TUNNEL_BINDING_TOKEN_ENV_VAR = "AGENTNEXUS_RUNNER_TUNNEL_BINDING_TOKEN"
...
RUNNER_INITIAL_AUTH_TOKEN_ENV_VAR = "AGENTNEXUS_RUNNER_INITIAL_AUTH_TOKEN"
...
RUNNER_DELEGATED_AUTH_ENV_VAR = "AGENTNEXUS_RUNNER_DELEGATED_AUTH"
...
RUNNER_SLICE_KEY_ENV_VAR = "AGENTNEXUS_RUNNER_SLICE_KEY"
...
RUNNER_LAUNCH_HARNESS_ENV_VAR = "AGENTNEXUS_RUNNER_LAUNCH_HARNESS"
RUNNER_TUNNEL_TOKEN_HEADER = "X-AgentNexus-Runner-Tunnel-Token"
...
AGENTNEXUS_INTERNAL_WS_ORIGIN = "agentnexus://internal"
...
RUNNER_ISOLATE_SESSION_ENV_VAR = "AGENTNEXUS_RUNNER_ISOLATE_SESSION"

# Marker env var stamped into every agent-facing environment so any
# process launched inside an AgentNexus agent session can detect it is
# running under AgentNexus. This is the analog of Claude Code's
...
def get_stable_runner_id() -> str:
    ...
    the id is loaded from ``~/.agentnexus/runners/runner_id`` or
...
def token_bound_runner_id(token: str) -> str:
    ...
    digest = hashlib.sha256(f"agentnexus-runner:{stripped}".encode()).hexdigest()
...
def load_or_create_runner_id(path: Path) -> str:
    ...
    :param path: Path to the runner id cache file, e.g.
        ``Path.home() / ".agentnexus" / "runners" / "runner_id"``.
...
def _default_runner_id_path() -> Path:
    ...
    :returns: ``~/.agentnexus/runners/runner_id``.
    """
    return Path.home() / ".agentnexus" / "runners" / "runner_id"
