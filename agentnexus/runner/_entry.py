...
- ``RUNNER_SERVER_URL``: AgentNexus server base URL for outbound calls
...
if TYPE_CHECKING:
    ...
    from agentnexus.runner.native import ResolvedSpec
    from agentnexus.runner.transports.ws_tunnel.serve import _ASGIApp
...
_RUNNER_CONFIG_HOME_ENV_VAR = "AGENTNEXUS_CONFIG_HOME"
...
_RUNNER_THREADPOOL_MAX_WORKERS_ENV_VAR = "AGENTNEXUS_RUNNER_THREADPOOL_MAX_WORKERS"
...
def _server_url_from_env() -> str:
    """Return the required AgentNexus server URL from the runner environment.
...
def _runner_config_path() -> Path:
    """Return the global AgentNexus config path visible to the runner.

    Respects :envvar:`AGENTNEXUS_CONFIG_HOME` for test isolation and
    subprocess consistency with the CLI/onboarding layer.

    :returns: Config path, e.g. ``Path("~/.agentnexus/config.yaml")``.
    """
    config_home = os.environ.get(_RUNNER_CONFIG_HOME_ENV_VAR)
    if config_home:
        return Path(config_home).expanduser() / "config.yaml"
    return Path.home() / ".agentnexus" / "config.yaml"
...
def _runner_threadpool_max_workers() -> int:
    ...
    overridable by ``AGENTNEXUS_RUNNER_THREADPOOL_MAX_WORKERS``. Missing config
...
class _RunnerDatabricksAuth(httpx.Auth):
    ...
    Used by the runner's HTTP client for callbacks to the AgentNexus server
    ...
    def __init__(
        ...
        :param server_url: AgentNexus server URL used to look up the ``?o=``
...
class _InitialAuthTokenFactory:
    ...
    def __init__(self, token: str, server_url: str) -> None:
        ...
        :param server_url: AgentNexus server URL used by the fallback resolver.
        """
...
def _make_auth_token_factory(
    ...
    *,
    ...
    - :class:`_RunnerDatabricksAuth` for the httpx client
      (refreshed on each HTTP callback to the AgentNexus server).
...
def _make_managed_mint_factory(
    ...
    *,
    ...
    :param server_url: AgentNexus server base URL, e.g.
    ...
    # an Apps OAuth redirect that happens before the request reaches AgentNexus.
...
class _ManagedMintTokenFactory:
    ...
    def __init__(
        ...
        *,
        ...
        :param server_url: AgentNexus server base URL.
...
def _mint_managed_owner_token(
    ...
    *,
    ...
    :param binding_token: The runner's tunnel binding token, sent as the
        ``X-AgentNexus-Runner-Tunnel-Token`` header to authenticate the mint.
    :param proxy_bearer: Optional bearer for a Databricks Apps proxy sitting
        in front of the AgentNexus server. The proxy requires a valid
        ``Authorization`` header even on unauthenticated-to-AgentNexus
...
def _runner_parent_pid_from_env() -> int | None:
    ...
    """
    from agentnexus.runner.identity import RUNNER_PARENT_PID_ENV_VAR
...
def _runner_workspace_from_env() -> Path | None:
    ...
    """
    from agentnexus.runner.identity import RUNNER_WORKSPACE_ENV_VAR
...
def _runner_isolate_session_from_env() -> bool:
    """Return ``True`` when ``AGENTNEXUS_RUNNER_ISOLATE_SESSION`` is ``"1"``.

    See :data:`RUNNER_ISOLATE_SESSION_ENV_VAR` for the contract.
    """
    from agentnexus.runner.identity import RUNNER_ISOLATE_SESSION_ENV_VAR
...
async def _resolve_agent_spec_from_server(
    ...
    """
    Fetch, cache, and parse one agent spec bundle from the AgentNexus server.
...
def create_app(
    ...
    # Stamp the AgentNexus session marker into the runner's environment so
    # every process this runner spawns can detect it is running inside an
    # AgentNexus agent session, the way Claude Code sets CLAUDE_CODE and
    ...
    # The AgentNexus server's POST /v1/sessions/{id}/mcp handles policy
    # evaluation and delegates execution here via
    # POST /v1/sessions/{id}/mcp/execute (tunneled through the WS
    # tunnel the runner opened to the AgentNexus server at startup).
    ...
    async def spec_resolver(agent_id: str, session_id: str | None = None) -> ResolvedSpec | None:
        """
        Fetch agent spec from the AgentNexus server, extract under the
    ...
    from agentnexus.inner.terminal import reap_orphaned_terminals
    ...
    async def _start_pm() -> None:
        ...
        if prewarm_path and mcp_manager is not None:
            try:
                from agentnexus.spec import load as _load_spec
    ...
    async def _stop_pm() -> None:
        ...
        from agentnexus.runner.native import teardown_all_codex_native_app_servers
...
async def _run_tunnel_from_env() -> None:
    ...
    from agentnexus.runner.identity import get_stable_runner_id
    from agentnexus.runner.transports.ws_tunnel.serve import serve_tunnel
    ...
    # separate module object from agentnexus.runner._entry). Importing the
    # canonical name here ensures sys.modules registers it before any
    # harness code tries to read it.
    import agentnexus.runner._entry as _canonical_entry
...
def _maybe_prewarm_ambient_detection() -> None:
    ...
    if os.environ.get(RUNNER_LAUNCH_HARNESS_ENV_VAR) != CLAUDE_NATIVE_CODING_AGENT.harness:
        return
    from agentnexus.onboarding.ambient import prewarm_detect_providers
