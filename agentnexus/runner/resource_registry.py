...
from agentnexus.inner.sandbox import contained_realpath, containment_prefix

if TYPE_CHECKING:
    from agentnexus.claude_native_status_file import SessionStatusPoller
...
# Role marker for the embedded AgentNexus REPL terminal auto-created for
...
AGENTNEXUS_REPL_TERMINAL_ROLE = "agentnexus-repl"
...
class SessionResourceRegistry:
    ...
    def set_session_status_publisher(
        ...
        event onto the session's SSE queue (which the AgentNexus server relays
    ...
    def compute_default_env_root(
        ...
        1. ``self._runner_workspace`` (sourced from
           ``AGENTNEXUS_RUNNER_WORKSPACE``) — when set, ALWAYS
