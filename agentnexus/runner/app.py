...
if TYPE_CHECKING:
    ...
    from agentnexus.claude_native_bridge import ClaudeNativeToolRelay
    from agentnexus.codex_native_bridge import CodexNativeBridgeState
    from agentnexus.llms.client import Client as LLMClient
    from agentnexus.runner.mcp_manager import RunnerMcpManager
    from agentnexus.runner.policy import PolicyVerdict
...
from agentnexus.coordination.behavior import framework_instructions_for_session
...
from agentnexus.harness_capabilities import InstructionDelivery
...
from agentnexus.llms.summarize import (
...
from agentnexus.runner import native as _native
from agentnexus.runner import pending_approvals
from agentnexus.runner.background_titles import (
...
from agentnexus.runner.background_titles.service import BACKGROUND_TITLE_MAX_PROMPT_CHARS
from agentnexus.runner.codex.goal import CodexGoalRunner
from agentnexus.runner.launch_failure import FailureDiagnosis, classify_terminal_failure
from agentnexus.runner.native import (
...
from agentnexus.runner.native import orchestration as _native_runtime
...
from agentnexus.runtime.harnesses.process_manager import HarnessProcessManager, NoLiveHarnessError
...
from agentnexus.spec.skill_sources import SkillSourceContext, resolve_harness_skills
from agentnexus.spec.types import AgentSpec, LocalToolInfo, SkillSpec
from agentnexus.terminals.control_bridge import bridge_tmux_control_to_websocket
from agentnexus.terminals.ws_common import WS_CLOSE_TERMINAL_NOT_FOUND
...
def _client_safe_error_detail(exc: BaseException, *, context: str) -> str:
    ...
    :returns: A non-sensitive string safe to return to clients, e.g.
        ...
        ~/.agentnexus/logs/runner/runner-conv_ab12.log"``.
    """
    _logger.warning(
        "%s failed: %s",
        context,
...
# delivery signal for the last child of a fan-out, and AgentNexus routinely
...
def _get_runner_llm_client() -> LLMClient:
    ...
    (or more) credentials than the AgentNexus server.
...
# marker.
_RUNNER_DISPATCHED_FIELD = "agentnexus_runner_dispatched"
...
async def _evaluate_policy_via_omnigent(
    *,
    ...
    """
    Proxy a policy evaluation request from the harness to the AgentNexus server.

    Called by the runner's ``proxy_stream`` when it intercepts a
    ``policy_evaluation.requested`` SSE event from the harness. Posts
    the evaluation request to the AgentNexus server's
    ...
    - ``PHASE_LLM_REQUEST`` / ``PHASE_LLM_RESPONSE`` fail OPEN
      (``POLICY_ACTION_ALLOW``) so a transient AgentNexus outage does not
    ...
    verdict_reason: str | None = (
        f"AgentNexus policy evaluation unavailable; failing closed for {phase}."
        if _fail_closed
        else None
    )
...
async def _mint_acp_subagent_child(
    ...
    *,
    ...
    :param client: AgentNexus HTTP client for the runner subprocess.
...
async def _complete_acp_subagent_child(
    ...
    *,
    ...
    """
    from agentnexus._native_post_delivery import post_external_session_status
...
# tools (omnigent/spec/omnigent.py:AGENTNEXUS_TOOL_LANGUAGE). Duplicated rather
...
_AGENTNEXUS_CALLABLE_LANGUAGE = "agentnexus-python-callable"
...
def _spec_with_workdir_paths(
    ...
    for info in local_tools:
        ...
        if (
            path
            and getattr(info, "language", None) != _AGENTNEXUS_CALLABLE_LANGUAGE
...
async def _resolve_forwarded_message_content(
    ...
    *,
    ...
    Remote AgentNexus servers can forward session messages with raw file IDs
...
def _wake_post_is_retryable(exc: httpx.HTTPError) -> bool:
    ...
    AgentNexus returns while the parent's runner tunnel is reconnecting), as
    ...
    if not isinstance(exc, httpx.HTTPStatusError):
        # Transport failure — the POST may never have reached AgentNexus.
        return True
...
async def _deliver_subagent_wake_post(
    ...
    *,
    ...
    :param server_client: AgentNexus HTTP client for the runner subprocess.
...
def _subagent_delivery_not_confirmed_response(
    ...
    *,
    ...
    was created as a sub-agent. For known sub-agents, AgentNexus must not receive a
...
def create_runner_app(
    *,
    ...
    :param server_client: httpx.AsyncClient pointed at the AP
        ...
        In-process: pointed at the AgentNexus ASGI app.
    ...
    app = FastAPI(title="agentnexus-runner")
    ...
    if terminal_registry is not None:
        from agentnexus.runtime import _globals as _rt_globals
    ...
    async def _resolve_session_claude_launch_config(
        ...
        if task is None:
            from agentnexus.claude_native import resolve_native_claude_config
        ...
        if event.last_output:
            ...
            parts.extend(
                [
                    ...
                    "AgentNexus captured a pane snapshot.",
                ]
            )
        return "\n".join(parts)
    ...
    async def _session_behavior_instructions(session_id: str) -> tuple[str, ...]:
        ...
        from agentnexus.bot_workspace import WORKSPACE_INSTRUCTIONS, WORKSPACE_LAYOUT_LABEL
    ...
    from agentnexus.entities.environment_filesystem import (
        FilesystemEntry,
        ResourceError,
    )
            ...
            if harness_name == "claude-native":
                ...
                async def _claude_build_context(ctx: NativeLaunchContext) -> NativeLaunchContext:
                    ...
                    if bundle_dir is None:
                        bundle_dir = Path(tempfile.mkdtemp(prefix="agentnexus-skill-bundle-"))
                    _logger.info(
    ...
    async def delete_session(session_id: str) -> JSONResponse:
        ...
        from agentnexus.runner.tool_dispatch import forget_spawn_family
    ...
    async def _handle_codex_native_settings_update(
        ...
        from agentnexus.codex_native_app_server import client_for_transport
    ...
    async def _codex_native_model_options(conv_id: str) -> list[_JsonObject]:
        ...
        from agentnexus.codex_native_bridge import read_codex_home_config_model
    ...
    async def _handle_pi_native_effort_change(
        ...
        from agentnexus.reasoning_effort import to_pi_thinking_level
    ...
    async def _handle_pi_native_model_change(
        ...
        from agentnexus.pi_native_bridge import bridge_dir_for_session_id, enqueue_model_change
    ...
    async def _teardown_session_terminals(conv_id: str) -> None:
        ...
        from agentnexus.runner.tool_dispatch import _publish_terminal_deleted_event
    ...
    async def _handle_claude_native_permission_mode_change(
        ...
        rendered, which the AgentNexus server persists as the session's
        current mode.
        """
    ...
    async def _handle_opencode_native_compact(conv_id: str) -> Response:
        from agentnexus.opencode_native_bridge import bridge_dir_for_bridge_id, read_bridge_state
    ...
    async def _opencode_native_model_options(conv_id: str) -> list[_JsonObject]:
        ...
        from agentnexus.opencode_native_bridge import bridge_dir_for_bridge_id, read_bridge_state
        from agentnexus.opencode_native_client import OpenCodeClient
    ...
    async def _handle_cursor_native_compact(conv_id: str) -> Response:
        from agentnexus.cursor_native_bridge import bridge_dir_for_session_id, inject_user_message
    ...
    async def _handle_pi_native_compact(conv_id: str) -> Response:
        from agentnexus.pi_native_bridge import bridge_dir_for_session_id, enqueue_compact
    ...
    def _inject_codex_compact(socket_path: str, target: str) -> None:
        from agentnexus.claude_native_bridge import _run_tmux
    ...
    async def _handle_qwen_native_compact(conv_id: str) -> Response:
        from agentnexus.qwen_native_bridge import bridge_dir_for_session_id, submit_user_message
    ...
    async def _handle_opencode_native_blocked_notice(
        ...
        from agentnexus.native_cost_popup import launch_blocked_notice
    ...
    async def _native_cost_popup_config_file(conv_id: str, harness: str) -> Path:
        ...
        from agentnexus.opencode_native_bridge import write_cost_popup_config
        from agentnexus.runner._entry import _make_auth_token_factory

        if harness == "claude-native":
            from agentnexus import claude_native_bridge as _cnb
            ...
            from agentnexus import codex_native_bridge as _cxb
    ...
    async def _repop_pending_cost_popup_on_attach(
        ...
        if harness not in ("claude-native", "codex-native", "opencode-native"):
            return
        from agentnexus.native_cost_popup import launch_cost_popup, wait_for_tmux_client
    ...
    async def _forward_harness_interrupt(conv_id: str) -> None:
        ...
        try:
            ...
            await harness_client.post(
                f"/v1/sessions/{conv_id}/events",
                json={"type": "interrupt"},
                # Bounded under the AgentNexus server's 5s stop deadline.
                timeout=3.0,
            )
    ...
    async def _run_turn_bg_setup_and_stream(
        ...
        if cached_spec is None and spec_resolver is not None:
            ...
            if _aid:
                ...
                except (AgentNexusError, httpx.HTTPError, RuntimeError):
                    _logger.warning(
        ...
        if _reasoning is not None:
            from agentnexus.reasoning_effort import efforts_for_harness, format_supported
        ...
        if conv not in _session_tool_schemas:
            ...
            if cached_spec is not None:
                try:
                    from agentnexus.tools.manager import (
                        ToolManager,
                    )
        ...
        if cached_spec and cached_spec.mcp_servers:
            from agentnexus.runner.mcp_manager import compute_spec_hash
    ...
    async def list_session_resources(
        ...
        from agentnexus.entities.pagination import paginate_in_memory
    ...
    async def terminal_resource_attach_ws(
        ...
        if entry is None or not entry.instance.running or not await entry.instance.is_alive():
            if terminal_role == AGENTNEXUS_REPL_TERMINAL_ROLE:
    ...
    async def _fs_search(
        ...
        *,
        ...
        from agentnexus.runner.environment_filesystem import (
    ...
    async def list_filesystem_changes(
        ...
        from agentnexus.runtime.filesystem_registry import GitStatusUnavailable
    ...
    async def read_environment_file_diff(
        ...
        from agentnexus.entities.environment_filesystem import InvalidPath
        from agentnexus.runner.environment_filesystem import _validate_path
    ...
    async def _resolve_session_agent_spec_or_none(session_id: str) -> AgentSpec | None:
        ...
        The cursor/opencode/kimi launch arms swallow ``AgentNexusError`` and
    ...
    async def get_session_models(session_id: str) -> JSONResponse:
        ...
        from agentnexus.model_catalog import catalog_for_spec
    ...
    async def get_session_kiro_model_options(session_id: str) -> JSONResponse:
        if _session_harness_name(session_id) != "kiro-native":
            return JSONResponse(status_code=200, content={"models": []})
        from agentnexus.kiro_native import list_kiro_cli_model_options
    ...
    async def get_session_claude_model_options(session_id: str) -> JSONResponse:
        ...
        from agentnexus.claude_native import claude_launch_catalog
    ...
    async def reset_session_agent_cache(session_id: str, request: Request) -> JSONResponse:
        ...
        if not isinstance(agent_id, str) or not agent_id:
            agent_id = _session_agent_ids.get(session_id)
        if not agent_id:
            with contextlib.suppress(AgentNexusError, httpx.HTTPError, RuntimeError):
                snapshot = await _session_snapshot(session_id)
    ...
    async def mcp_execute(session_id: str, request: Request) -> JSONResponse:
        ...
        if method == "tools/call":
            ...
            if "__" in tool_name:
                ...
                from agentnexus.tools.mcp import McpElicitationRequired
    ...
    def _resolve_summarize_connection(
        ...
        from agentnexus.spec.types import ApiKeyAuth, DatabricksAuth, ProviderAuth
        ...
        if auth is None and not _spec_has_legacy_profile:
            from agentnexus.runtime.workflow import _load_global_auth
    ...
    if (
        ...
        from agentnexus.native_cost_popup import _list_tmux_clients, _tmux_window_activity_at
        from agentnexus.runner.tool_dispatch import _publish_terminal_deleted_event
        from agentnexus.terminals.pane_reaper import (
...
def create_runner_app_from_env() -> FastAPI:
    ...
    minimal :class:`httpx.AsyncClient` for the AgentNexus server, then delegates
...
async def _ensure_session_subagent_router(
    ...
    *,
    ...
    """
    from agentnexus.runner.subagent_routing import ensure_session_router_quietly
...
def _build_spawn_env_from_spec(
    ...
    *,
    ...
    if env is not None and session_id:
        from agentnexus.runner.subagent_routing import session_router_env
