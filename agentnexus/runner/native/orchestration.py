...
if TYPE_CHECKING:
    ...
    from agentnexus.codex_native_app_server import CodexAppServerClient, CodexNativeAppServer
    from agentnexus.inner.datamodel import OSEnvSpec
    from agentnexus.opencode_native_app_server import OpenCodeNativeServer
    from agentnexus.opencode_native_client import OpenCodeClient, OpenCodeSession
    from agentnexus.opencode_native_forwarder import OpenCodeNativeForwarder
    from agentnexus.runner.subagent_routing import SubagentRouter
    from agentnexus.runner.turn_routing import TurnRouter
    from agentnexus.spec.types import MCPServerConfig
...
from agentnexus.debug_logging import runner_primary_session_id
...
_logger = logging.getLogger("agentnexus.runner.app")
...
_AGENTNEXUS_PACKAGE_DIR = Path(__file__).resolve().parent.parent.parent
...
def _publish_tmux_target_for_bridge(
    *,
    ...
    from agentnexus.claude_native_bridge import bridge_dir_for_bridge_id, write_tmux_target
...
class _CodexNativeLaunchConfig:
    ...
    :param policy_server_url: AgentNexus server URL for the Codex policy hook and
    ...
    :param fork_carry_history: ``True`` on a forked clone bound to a
        ...
        builds the clone's rollout from the copied AgentNexus items instead (see
...
class _PiNativeLaunchConfig:
    ...
    :param server_url: AgentNexus server URL for the extension/forwarder.
    ...
    :param fork_carry_history: ``True`` on a forked clone bound to a native
        ...
        AgentNexus items (see :func:`_auto_create_pi_terminal`). Also consumed by
...
def _start_turn_router_for_native_session(
    ...
    *,
    ...
    """
    from agentnexus.runner.turn_routing import ensure_session_turn_router
...
def _recover_pending_turn_replay(
    ...
    *,
    ...
    :returns: None.
    """
    from agentnexus.runner.turn_routing import schedule_pending_replay_recovery
...
async def _shutdown_session_turn_router_async(
    ...
    :returns: None.
    """
    from agentnexus.runner.turn_routing import shutdown_session_turn_router
...
async def _shutdown_session_router_async(
    ...
    :returns: None.
    """
    from agentnexus.runner.subagent_routing import shutdown_session_router
...
def _codex_session_workspace(session_workspace: str | None) -> Path:
    ...
    runner's ``AGENTNEXUS_RUNNER_WORKSPACE``.
    ...
    :raises RuntimeError: If no workspace is available (neither the
        session snapshot nor ``AGENTNEXUS_RUNNER_WORKSPACE``).
    """
    raw = session_workspace or _required_runner_env("AGENTNEXUS_RUNNER_WORKSPACE")
...
async def _pi_native_launch_config(
    *,
    ...
    :param server_client: Runner AgentNexus server client.
...
class _OpenCodeNativeLaunchConfig:
    ...
    :param policy_server_url: AgentNexus server URL for the forwarder.
    ...
    :param fork_carry_history: ``True`` on a forked clone whose prior
        ...
        clone, so the runner rehydrates from the copied AgentNexus transcript.
    """
...
async def _auto_create_opencode_terminal(
    ...
    *,
    ...
    :param server_client: Runner AgentNexus server HTTP client.
    :param ensure_comment_relay: Callback that starts the AgentNexus builtin-tool
        relay for this session's bridge dir (the nested
        ``_ensure_comment_relay_started``). ``None`` skips wiring the AgentNexus
        MCP relay (tests / no server).
    :returns: The created terminal resource view.
    """
    from agentnexus.inner.datamodel import OSEnvSpec, TerminalEnvSpec
    from agentnexus.opencode_native_app_server import (
    ...
    # AgentNexus builtin-tool relay (wired below) can start. Safe to call before
    ...
    from agentnexus.opencode_native_bridge import xdg_config_home_for_bridge_dir
    ...
    # Build opencode's ``mcp`` block: the AgentNexus builtin-tool relay (so the
    # model can call sys_*/load_skill/web_fetch — the real "connects to AgentNexus
    ...
    # through AgentNexus's policy engine via the forwarder's permission gate —
    ...
    # Load the AgentNexus policy-bridge plugin so opencode's lifecycle hooks reach
    ...
    # PostToolUse hooks); coordinates come from the AGENTNEXUS_* env stamped on
    ...
    if server_client is not None and runner_server_url:
        ...
        policy_env["AGENTNEXUS_POLICY_URL"] = runner_server_url
        policy_env["AGENTNEXUS_SESSION_ID"] = session_id
        ...
        policy_env["AGENTNEXUS_RELAY_FILE"] = str(bridge_dir / _TOOL_RELAY_FILE)
        # Bake fallback headers for the first calls before relay starts.
        from agentnexus.runner._entry import _make_auth_token_factory

        _policy_factory = _make_auth_token_factory()
        _policy_token = _policy_factory() if _policy_factory is not None else None
        if _policy_token:
            from agentnexus.cli_auth import databricks_request_headers

            policy_env["AGENTNEXUS_POLICY_HEADERS"] = json.dumps(
    ...
    # Start the AgentNexus builtin-tool relay BEFORE opencode boots, so
    ...
    # relay POSTs each call back through the AgentNexus server (policy enforced).
    if server_client is not None and ensure_comment_relay is not None:
        ...
        try:
            ...
            if launch_config.external_session_id is not None:
                ...
                if existing is not None:
                    ...
                    # XDG store) — we'll rehydrate from the AgentNexus transcript
                    # below instead of silently starting empty.
                    resume_lost_history = True
            if opencode_session_id is None:
                created = await client.create_session({"title": f"agentnexus:{session_id}"})
                ...
                # seed the copied AgentNexus transcript as a noReply preamble.
    ...
    # lifetime; ``server_client`` is the runner's AgentNexus client. The
...
def _build_opencode_policy_evaluator(
    *,
    ...
    :param server_client: Runner's AgentNexus server HTTP client.
    :param conversation_id: Owning AgentNexus session id, e.g. ``"conv_abc"``.
    :returns: An async evaluator returning a verdict mapping, or a deny
        verdict on failure.
    """
    from agentnexus.opencode_native_permissions import OPENCODE_NATIVE_HARNESS
...
def _opencode_native_model_from_spec(
    ...
    if agent_spec is None:
        return None
    try:
        from agentnexus.runtime.workflow import _resolve_spec_model
...
def _resolve_opencode_compact_model(
    ...
    opencode's ``/summarize`` requires an explicit model, but AgentNexus
...
def _render_opencode_transcript_text(items: list[object]) -> str:
    """
    Render committed AgentNexus message items into a plain-text transcript.
...
async def _rehydrate_opencode_session_from_transcript(
    *,
    ...
    persisted opencode session is gone) inject the AgentNexus transcript as a
...
def _pi_args_have_session_control(args: list[str]) -> bool:
    ...
    :returns: ``True`` when AgentNexus should not add resume/session flags.
    """
    session_flags = {
        "--session-dir",
        "--session",
...
def _pi_args_have_provider(args: list[str]) -> bool:
    ...
    :param args: User pass-through Pi CLI args.
    :returns: ``True`` when AgentNexus should not add provider/model args.
    """
...
def _build_pi_native_args(
    *,
    ...
    :param extension_path: Generated AgentNexus Pi extension path.
    :param session_dir: Per-AgentNexus-session Pi session directory.
...
async def _resolve_pi_resume_session(
    *,
    ...
    1. **Cold resume** — the session already carries a captured Pi
       ...
       file from committed AgentNexus items so ``pi --session <id>`` opens with
    ...
    2. **Fork rebuild** — a forked clone bound to a pi-native target with NO
       ...
       session id, build its file from the clone's OWN copied AgentNexus items,
       and patch the server so AgentNexus reflects the clone's session id and a
    ...
    :param session_id: AgentNexus conversation id, e.g. ``"conv_abc123"``.
    ...
    from agentnexus.pi_native_resume import ensure_local_pi_resume_session, mint_pi_session_id
    ...
    model = ""
    try:
        from agentnexus.pi_native_credentials import resolve_pi_native_provider
    ...
    # yet. Build the clone's session from its OWN copied AgentNexus items under a
    ...
    if launch_config.fork_carry_history:
        ...
        if built is not None:
            # Record the minted id so AgentNexus reflects the clone's own Pi
...
async def _auto_create_pi_terminal(
    ...
    *,
    ...
    from agentnexus.inner.datamodel import OSEnvSpec, TerminalEnvSpec
    from agentnexus.pi_native import resolve_pi_executable
    ...
    from agentnexus.pi_native_bridge import extension_path as pi_extension_path
    from agentnexus.runner._entry import _make_auth_token_factory
    ...
    from agentnexus.runner._entry import _runner_tunnel_binding_token_from_env
    from agentnexus.runner.identity import RUNNER_TUNNEL_TOKEN_HEADER

    binding_token = _runner_tunnel_binding_token_from_env()
    if binding_token:
        auth_headers[RUNNER_TUNNEL_TOKEN_HEADER] = binding_token
    # Build the AgentNexus tool surface (sys_* tools) the Pi extension registers
    ...
    # can call AgentNexus tools with centralized server-side policy enforcement
    ...
    # Rebuild the local Pi session JSONL from committed AgentNexus items so a
    ...
    from agentnexus.pi_native import pi_supports_approve
    ...
    if not _pi_args_have_provider(launch_config.terminal_launch_args or []):
        from agentnexus.pi_native_credentials import (
    ...
    if server_client is not None and ensure_comment_relay is not None:
        ...
        from agentnexus.claude_native_bridge import _TOOL_RELAY_FILE, _read_json_file
        from agentnexus.pi_native_bridge import inject_relay_into_config
...
async def _post_pi_native_credential_warning(
    *,
    ...
    :param session_id: Session/conversation identifier.
    :param server_client: Runner AgentNexus server client (``None`` in tests).
...
async def _auto_create_cursor_terminal(
    ...
    *,
    ...
    :returns: Created terminal resource view.
    """
    from agentnexus.cursor_native import resolve_cursor_executable
    from agentnexus.inner.datamodel import OSEnvSpec, TerminalEnvSpec
    ...
    await _cancel_auto_forwarder_task(session_id)
    from agentnexus.cursor_native import is_valid_cursor_chat_id
    ...
    from agentnexus.cursor_native_forwarder import clear_cursor_bridge_state, preseed_resume_state
    from agentnexus.cursor_native_status import clear_cursor_status_state
    from agentnexus.cursor_native_usage import clear_cursor_usage_state
    ...
    # Render the copied AgentNexus items once and stash them; the executor prepends
    ...
    if launch_config.fork_carry_history and not preseeded and server_client is not None:
        try:
            from agentnexus.claude_native import _fetch_all_session_items_for_claude_resume
    ...
    # Mirror the Cursor TUI's conversation back into the AgentNexus session so the
    ...
    from agentnexus.runner._entry import _make_auth_token_factory, _RunnerDatabricksAuth
    ...
    from agentnexus.cursor_native_forwarder import supervise_cursor_forwarder
    from agentnexus.cursor_native_permissions import (
        cursor_launch_args_enable_yolo,
        supervise_cursor_transcript_elicitations,
    )
    from agentnexus.cursor_native_usage import supervise_cursor_usage_forwarder
...
async def _auto_create_goose_terminal(
    ...
    *,
    ...
    ``~/.config/goose/config.yaml``), so HOME is inherited and AgentNexus writes no
    ...
    :returns: Created terminal resource view.
    """
    from agentnexus.goose_native import resolve_goose_executable
    from agentnexus.inner.datamodel import OSEnvSpec, TerminalEnvSpec
    ...
    await _cancel_auto_forwarder_task(session_id)
    from agentnexus.goose_native_bridge import bridge_dir_for_session_id, write_tmux_target
    from agentnexus.goose_native_forwarder import clear_goose_bridge_state
    ...
    # Mirror the Goose TUI's conversation back into the AgentNexus session so the
    ...
    from agentnexus.goose_native_forwarder import supervise_goose_forwarder
    from agentnexus.goose_native_permissions import supervise_goose_approval_mirror
...
async def _auto_create_hermes_terminal(
    ...
    *,
    ...
    ``~/.hermes/config.yaml``), so HOME is inherited and AgentNexus writes no vendor
    ...
    :returns: Created terminal resource view.
    """
    from agentnexus.hermes_native import resolve_hermes_executable
    from agentnexus.inner.datamodel import OSEnvSpec, TerminalEnvSpec
    ...
    from agentnexus.hermes_native_forwarder import clear_hermes_bridge_state
    from agentnexus.hermes_native_status import clear_hermes_status_state
    ...
    # Write a per-session HERMES_HOME with the AgentNexus policy hook so the
    # native TUI evaluates tool calls against AgentNexus policies.
        ...
        if _source_db is not None and _source_db.is_file():
            ...
            if _target_db is not None:
                try:
                    ...
                    # the forwarder only mirrors NEW messages (AgentNexus already
                    # has the cloned ones from the fork item copy).
                    if _clone_max_id > 0:
                        from agentnexus.hermes_native_forwarder import (
                            _ForwardState,
                            _write_state,
                        )
    ...
    # Mirror the Hermes TUI's conversation back into the AgentNexus session so the
    # chat view tracks the embedded terminal. Host-spawned sessions have no CLI
    # client to start this, so the runner owns it — reusing the runner's own
    # server URL + refresh-capable auth.
    from agentnexus.runner._entry import _make_auth_token_factory, _RunnerDatabricksAuth
    ...
    from agentnexus.hermes_native_bridge import read_hermes_home
    from agentnexus.hermes_native_forwarder import supervise_hermes_forwarder
    from agentnexus.hermes_native_permissions import supervise_hermes_approval_mirror
...
async def _auto_create_kiro_terminal(
    ...
    *,
    ...
    from agentnexus.inner.datamodel import OSEnvSpec, TerminalEnvSpec
    from agentnexus.kiro_native import build_kiro_launch
    ...
    bridge_dir = prepare_bridge_dir(session_id)
    # Declare the AgentNexus MCP server in the workspace-scoped kiro config so
    # kiro-cli can call AgentNexus tools. Only when the tool relay will actually
    ...
    if terminal_registry is not None:
        ...
        if instance is not None and instance.running:
            from agentnexus.kiro_native_bridge import write_tmux_target
    ...
    # Start the AgentNexus builtin-tool relay (writes tool_relay.json into the kiro
    # bridge dir) so the serve-mcp server declared in the workspace mcp.json can
    # route AgentNexus tool calls back through the session's policy/elicitation
    ...
    from agentnexus.kiro_native_permissions import supervise_kiro_permission_mirror
    from agentnexus.kiro_native_session_forwarder import supervise_kiro_session_forwarder
...
async def _persist_qwen_external_session_id(
    ...
    """Record the qwen session id on the AgentNexus session as ``external_session_id``.
    ...
    :param server_client: Runner AgentNexus server client (``None`` skips the write).
    :param session_id: AgentNexus session/conversation id.
...
async def _build_qwen_fork_recording(
    ...
    *,
    ...
    """Synthesize a qwen chat recording for a forked clone from its AgentNexus items.

    A forked clone has its OWN copied AgentNexus items but no qwen recording yet
    ...
    :param server_client: Runner AgentNexus server client.
    :param session_id: The forked clone's AgentNexus conversation id.
    ...
    """
    from agentnexus.pi_native_resume import fetch_all_session_items_for_pi_resume
...
async def _auto_create_qwen_terminal(
    ...
    *,
    ...
    env vars or ``~/.qwen`` from ``/auth``), so HOME is inherited and AgentNexus
    ...
    """
    from agentnexus.inner.datamodel import OSEnvSpec, TerminalEnvSpec
    from agentnexus.qwen_native import resolve_qwen_executable
    ...
    from agentnexus.qwen_native_forwarder import clear_qwen_bridge_state
    ...
    # is persisted on the AgentNexus session and read back from the snapshot
    ...
    # clone's copied AgentNexus items and force ``--resume``. Gated on a NULL
    ...
    # Expose AgentNexus's builtin tools (sys_*, load_skill, web_fetch, …) to qwen
    ...
    if mcp_enabled:
        try:
            ...
            _logger.warning(
                ...
                "AgentNexus MCP wiring for session %s.",
                session_id,
                exc_info=True,
            )
        else:
    ...
    # the default in-terminal prompt (the embedded pane shows it) — AgentNexus-side
    ...
    # Mirror the qwen TUI's conversation back into the AgentNexus session so the
    # chat view tracks the embedded terminal. Host-spawned sessions have no CLI
    # client to start this, so the runner owns it — reusing the runner's own
    # server URL + refresh-capable auth.
    from agentnexus.runner._entry import _make_auth_token_factory, _RunnerDatabricksAuth
    ...
    from agentnexus.qwen_native_bridge import qwen_session_recording_path
    from agentnexus.qwen_native_forwarder import (
        supervise_qwen_compaction_mirror,
        supervise_qwen_forwarder,
    )
    from agentnexus.qwen_native_permissions import supervise_qwen_approval_mirror
...
async def _auto_create_kimi_terminal(
    ...
    *,
    ...
    and adds the AgentNexus tool-policy hooks — a ``PreToolUse`` deny-gate and a
    ...
    and mirrors each user prompt + assistant reply into the AgentNexus chat, so the
    ...
    :param resource_registry: Session resource registry for launching the
        terminal.
    :param publish_event: Runner session event publisher.
    :param server_client: Runner AgentNexus server client (used only for the
    ...
    from agentnexus.inner.datamodel import OSEnvSpec, TerminalEnvSpec
    from agentnexus.kimi_native import resolve_kimi_executable
    from agentnexus.kimi_native_bridge import (
        bridge_dir_for_session_id,
        write_hook_config,
        write_tmux_target,
    )
    from agentnexus.kimi_native_credentials import build_kimi_session_home
    from agentnexus.kimi_native_forwarder import clear_kimi_bridge_state, supervise_kimi_forwarder
    from agentnexus.runner._entry import _make_auth_token_factory
    ...
    # Wire the AgentNexus tool-policy hooks: kimi reads a single
    ...
    # Mirror the kimi TUI transcript into the AgentNexus chat: tail the per-session
...
async def _auto_create_codex_terminal(
    ...
    *,
    ...
    :param publish_event: The runner's per-session SSE emitter, used to
        surface the new terminal on the live stream (the AgentNexus relay
    ...
    :param server_client: Runner's AgentNexus server HTTP client. Used to read
    ...
    from agentnexus.inner.codex_executor import codex_extended_catalog_env
    from agentnexus.inner.datamodel import OSEnvSpec, TerminalEnvSpec
    ...
    from agentnexus.inner.codex_executor import _find_codex_cli
    ...
    if launch_config.model_override or (
        ...
        from agentnexus.model_catalog_store import catalog_contains, default_row
    ...
    from agentnexus.codex_native_process_registry import (
    ...
    if (
        ...
        from agentnexus.codex_native import _clone_codex_rollout, _mint_codex_thread_id
        ...
        if cloned_rollout is not None:
            ...
            # Record the assigned thread id now so AgentNexus reflects the clone's
            ...
            if server_client is not None:
                try:
                    ...
                    # unset on the AgentNexus session and a future relaunch of this
    ...
    if (
        ...
        # available: build the clone's rollout from its own copied AgentNexus
        ...
        from agentnexus.codex_native import (
            _ensure_local_codex_resume_rollout,
    ...
    if launch_config.external_session_id is not None and original_external_session_id is not None:
        from agentnexus.codex_native import _ensure_local_codex_resume_rollout
    ...
    # the terminal launch.
    from agentnexus.inner.codex_executor import populate_codex_skills_from_bundle
    ...
    # AgentNexus coordinates for the codex-native policy hook. The hook runs as a
    ...
    if _codex_router_dir is not None:
        from agentnexus.runner.subagent_routing import router_env
    ...
    event_client = CodexAppServerClient(
        ws_url=codex_ws_url,
        client_name="agentnexus-codex-native-auto",
    )
    if launch_config.external_session_id is None:
        ...
        from agentnexus.codex_native_bridge import CodexNativeBridgeState, write_bridge_state
        ...
        terminal_view = await resource_registry.launch_auxiliary_terminal(
            ...
            spec=TerminalEnvSpec(
                ...
                args=build_codex_remote_args(
                    ...
                    # AgentNexus provider instead of falling back to the
                    ...
                    # AgentNexus provisions the private CODEX_HOME and vets
                    ...
                    # gate: AgentNexus's supported Codex floor is newer than the
...
async def _codex_discover_thread_and_forward(
    *,
    ...
    """
    Adopt the fresh Codex TUI's thread, then mirror it into the AgentNexus session.
    ...
    :param session_id: AgentNexus session/conversation id, e.g. ``"conv_abc123"``.
    ...
    from agentnexus.codex_native_forwarder import (
        supervise_forwarder,
    ...
    if login_required:
        ...
        write_bridge_startup_error(
            bridge_dir,
            "Codex is not signed in and no AgentNexus provider routes the codex "
    ...
    try:
        ...
        # Mirror the discovered Codex thread id onto the AgentNexus session as its
        ...
        # would resume fresh. Best-effort: a transient AgentNexus failure here still
        # leaves chat streaming working — only fork-history carry-over
        # degrades.
        from agentnexus.cli_auth import open_server_client
...
async def _codex_forward_known_thread(
    *,
    ...
    :returns: None. Runs until cancelled or the app-server connection
        closes.
    """
    from agentnexus.codex_native_forwarder import supervise_forwarder
    from agentnexus.runner._entry import (
        _make_auth_token_factory,
        _RunnerDatabricksAuth,
    )
...
async def _auto_create_antigravity_terminal(
    ...
    *attended* (``headless=False``). Auto-bypass comes only from the user's
    ...
    :param ensure_comment_relay: The runner's relay starter
        (``_ensure_comment_relay_started``). When provided, the AgentNexus MCP
    ...
    from agentnexus.antigravity_native_launch import build_agy_launch
    ...
    # Wire the AgentNexus MCP relay so the wrapped agy gets the sys_* tools
    # (spawn sub-agent sessions, drive AgentNexus terminals, list agents/models,
    ...
    # elicitations via the Task 9 hook. The reader owns its own AgentNexus client
...
async def _agy_cascade_is_locally_owned(bridge_dir: Path, cascade_id: str) -> bool:
    ...
    """
    from agentnexus.antigravity_native_bridge import agy_gemini_dir
...
async def _cold_start_agy_conversation(
    ...
    *,
    ...
    The cold-started id is also PATCHed onto the AgentNexus session as
    ...
    *timeout_s*, no ready model catalog within *timeout_s*, or ``StartCascade``
    ...
    :param server_client: Runner AgentNexus server client used for the
    ...
    from agentnexus.antigravity_native_rpc import (
        AntigravityRpcError,
...
async def _session_payload_for_host_spawn_check(
    ...
    :param server_client: The runner's AgentNexus server HTTP client, or
...
async def _codex_session_needs_runner_terminal(
    ...
    :param server_client: The runner's AgentNexus server HTTP client, or ``None`` in
...
def _native_startup_raw_instructions_from_spec(
    ...
    """
    from agentnexus.runtime.prompt import raw_author_instructions
...
def _cursor_fork_history_preamble(items: list[_JsonObject]) -> str:
    ...
    :param items: Committed AgentNexus items (``GET /v1/sessions/{id}/items``),
        chronological.
...
def _build_claude_native_base_args(
    *,
    ...
    bridge / MCP / hook / AgentNexus wiring. The order is: ``--resume`` for a
    ...
    """
    from agentnexus.reasoning_effort import CLAUDE_EFFORTS
...
def _publish_terminal_pending(
    ...
    fails. The AgentNexus relay caches the latest value and republishes it, and
...
def _publish_native_terminal_start_error(
    ...
    outside a transcript turn; AgentNexus writes and publishes the turn-scoped
...
def _codex_ensure_response_with_policy_notice(
    ...
    ``policy_hook_disabled_reason`` exactly once so AgentNexus can post a single
...
def _ensure_orchestrator_skills_in_bundle(
    ...
    source = _AGENTNEXUS_PACKAGE_DIR / "onboarding" / "agent" / "skills" / skill_name
...
#: AgentNexus MCP tools an auto-harness Claude session must be able to call
...
async def _auto_create_claude_terminal(
    ...
    *,
    ...
    :param publish_event: The runner's per-session SSE emitter, used to
        ...
        republishes it to the web UI) so the Terminal toggle enables
        without a refresh.
    :param server_client: AgentNexus server client used to fetch the session
    ...
    from agentnexus.claude_native_forwarder import reset_transcript_forward_state
    from agentnexus.inner.datamodel import OSEnvSpec, TerminalEnvSpec

    workspace = (
        session_init.snapshot.workspace
        if session_init is not None and session_init.snapshot.workspace
        else os.environ.get("AGENTNEXUS_RUNNER_WORKSPACE", str(Path.cwd()))
    )
    ...
    # The AgentNexus server URL + auth are needed in two places below: the
    ...
    from agentnexus.claude_launcher import resolve_claude_launch
    ...
    if server_client is not None and session_external_id is not None:
        from agentnexus.claude_native import _ensure_local_claude_resume_transcript
        ...
        from agentnexus.claude_native import _clone_claude_transcript
        ...
        if _cloned is not None:
            ...
            # Record the assigned id now so AgentNexus reflects the clone's own
        ...
        # native transcript from its OWN copied AgentNexus items under a uuid we
        ...
        if _built is not None:
            ...
            # Record the assigned id so AgentNexus reflects the clone's own Claude
    ...
    if session_model_override or launch_model is None:
        ...
        from agentnexus.model_catalog_store import default_row
    ...
    from agentnexus.claude_native_bridge import record_model_vocabulary
    ...
    # the three AgentNexus tools that carry out the re-route. A pinned session's
    ...
    # back to the AgentNexus server. Normally the CLI client runs this,
    ...
    # offset 0 already holds every item AgentNexus has. Starting the forwarder at
    ...
    # ``claude_native.py``.
    from agentnexus.claude_native_forwarder import supervise_forwarder
...
async def _auto_create_repl_terminal(
    ...
    *,
    ...
    """
    Auto-create an AgentNexus REPL terminal for a runner-hosted SDK session.
    ...
    like the claude-/codex-native terminals — with the AgentNexus REPL as
    the TUI.
    ...
    :param publish_event: The runner's per-session SSE emitter,
        ...
        without a refresh.
    :param server_client: AgentNexus server client used to stamp the
    ...
    started_at = time.monotonic()
    workspace = os.environ.get("AGENTNEXUS_RUNNER_WORKSPACE", str(Path.cwd()))
    server_url = os.environ.get("RUNNER_SERVER_URL", "http://localhost:6767")
    ...
    terminal_view = await resource_registry.launch_auxiliary_terminal(
        ...
        resource_role=AGENTNEXUS_REPL_TERMINAL_ROLE,
    )
...
async def _delete_native_bridge_dirs(
    *,
    ...
    ``/tmp/omnigent-<uid>/<harness>-native/<digest>`` (some use ``~/.agentnexus``)
    ...
    :param server_client: AgentNexus server client used to resolve rotated bridge
...
async def _claude_native_bridge_id_for_session(
    *,
    ...
    :param server_client: AgentNexus server client used to fetch the session
        snapshot.
    :param session_id: AgentNexus session/conversation id, e.g.
        ``"conv_abc123"``.
...
async def _resolve_native_spawn_env(
    ...
    *,
    ...
    :param session_id: AgentNexus conversation id.
    :param server_client: Runner's client to the AgentNexus server, for label reads.
...
async def _launch_native_terminal(
    ...
    *,
    ...
    :param resolve_agent_spec: Optional async callback that resolves the session
        ...
        resolver that tolerates ``AgentNexusError`` must swallow it and return
...
async def _claude_native_terminal_arrives_via_transfer(
    *,
    ...
    :param server_client: AgentNexus client to resolve the bridge id label;
...
def _is_spec_local_native_python_tool(
    ...
    return any(
        ...
        and getattr(info, "language", None) in ("python", "agentnexus-python-callable")
