...
ROUTING_DECISION_LABEL_KEY = "agentnexus.routing.decision_id"
...
#: belong there.
CREATE_ROUTE_PROMPT_LABEL_KEY = "agentnexus.routing.create_prompt"
...
AUTO_HARNESS_LABEL_KEY = "agentnexus.routing.auto_harness"
...
def _harness_family(harness: str) -> str | None:
    from agentnexus.server.smart_routing import _HARNESS_FAMILY
...
def model_in_family(family: str | None, model: str) -> bool:
    ...
    if family is None or family == "pi":
        return True
    from agentnexus.model_catalog import model_family_token
...
def _requested_match(req: SubagentRouteRequest, model: str | None) -> bool:
    ...
    if not req.requested_model or not model:
        return False
    from agentnexus.server.smart_routing import _bare_id
...
async def persist_subagent_decision(
    ...
    """
    from agentnexus.entities.conversation import NewConversationItem
...
def start_subagent_router(
    *,
    ...
    threading.Thread(
        ...
        name="agentnexus-subagent-router",
        daemon=True,
    ).start()
    return router
...
def _prune_router_dirs(router: SubagentRouter) -> None:
    ...
    """
    from agentnexus.claude_native_bridge import subagent_router_bridge_root
...
def router_dir_for_session(session_id: str) -> Path:
    ...
    """
    from agentnexus.claude_native_bridge import ensure_secure_dir, subagent_router_bridge_root
