...
Routes all MCP calls through the AgentNexus server's
...
The AgentNexus server holds the live connections in its
...
- **AP mode** (out-of-process runner paired with AgentNexus server):
  ``ProxyMcpManager(session_id, server_client)`` — all tool calls flow
  through the AgentNexus server, policy enforced centrally.
- **No-AP / test mode**:
...
from agentnexus.runner.tool_dispatch import MCP_PROXY_CALL_TIMEOUT_S
...
class ProxyMcpManager:
    """Routes runner-side MCP calls through the AgentNexus server MCP proxy endpoint.

    Drop-in substitute for :class:`omnigent.runner.mcp_manager.RunnerMcpManager`
    used in AgentNexus mode.  The runner creates one instance per session on first MCP use
    ...
    :param ap_client: An :class:`httpx.AsyncClient` pointed at the AgentNexus server.
        Must already carry the runner's service auth (e.g. Databricks bearer
        token) so requests are accepted by the AgentNexus server's auth middleware.
    """
    ...
    def __init__(
        ...
        :param ap_client: HTTP client pointed at the AgentNexus server.
        ...
        self._agentnexus_client = ap_client
    ...
    async def schemas_for(self, spec: AgentSpec) -> McpSchemasResult:
        """Fetch tool schemas from the AgentNexus server MCP proxy (``tools/list``).

        Sends a ``tools/list`` JSON-RPC 2.0 request to the AgentNexus server's MCP
        ...
        the AgentNexus server (e.g. ``github__search``).  This ensures the harness
        ...
        try:
            resp = await self._agentnexus_client.post(
                self._mcp_url,
                json=payload,
                timeout=30.0,
            )
        ...
        for tool in tools_list:
            ...
            if not isinstance(name, str) or not name:
                continue
            # The AgentNexus server returns ``inputSchema`` (JSON Schema from MCP).
    ...
    async def call_tool(
        ...
        """Dispatch a tool call via the AgentNexus server MCP proxy (``tools/call``).

        Sends a ``tools/call`` JSON-RPC 2.0 request.  The AgentNexus server enforces
        TOOL_CALL and TOOL_RESULT policies before forwarding to the real MCP
        server.

        **ASK policy / approval flow**: when the AgentNexus server returns an
        ...
        :returns: Tool output string.  On denial the AgentNexus server returns an
        ...
        """
        del spec  # AgentNexus server resolves spec from session context
        ...
        for _attempt in range(2):
            ...
            # ── ASK: AgentNexus server returned InputRequiredResult ───────────────
            ...
            if result.get("resultType") == "input_required":
                ...
                # The AgentNexus server uses the elicitation_id as the key in
    ...
    async def prewarm(self, spec: AgentSpec) -> None:
        """No-op — the AgentNexus server warms connections lazily via ServerMcpPool.
    ...
    async def shutdown(self) -> None:
        """No-op — the AgentNexus server owns and manages MCP connections."""
