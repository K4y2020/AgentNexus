...
from agentnexus.spec.types import AgentSpec, MCPServerConfig, RetryPolicy
from agentnexus.tools.base import is_valid_tool_name
from agentnexus.tools.mcp import McpServerConnection
...
def _mcp_tool_schema(
    ...
    """
    from agentnexus.tools.mcp import _normalize_input_schema
...
class RunnerMcpManager:
    ...
    def __init__(
        ...
        :param server_client: ``httpx.AsyncClient`` pointed at the
            AgentNexus server. When provided, inline MCP elicitations are
            surfaced to the user via the AgentNexus server's session events
    ...
    def _build_elicitation_callback(
        ...
        elicitation to the user via the AgentNexus server's session events
        API (approval card in web UI, y/a/n prompt in REPL) and
        parks until the user responds. Falls back to decline
        when no AgentNexus server is available.
        ...
        async def _elicit(
            ...
            When an AgentNexus server client is available, POSTs a
            ...
            if server_client is None:
                _logger.warning(
                    "MCP elicitation callback: no AgentNexus server client available — declining",
            ...
            try:
                ...
                _logger.warning(
                    "MCP elicitation callback: AgentNexus server POST failed (%s) — declining",
                    exc,
                    extra={"session_id": session_id},
                )
                return ElicitResult(action="decline")
            ...
            if not isinstance(elicitation_id, str) or not elicitation_id:
                _logger.warning(
                    "MCP elicitation callback: AgentNexus server returned no "
            ...
            # won't fire on timeout/cancellation, so the AgentNexus server's
            # sidebar badge may stay stale. Same pattern as
            # proxy_mcp_manager. A future enhancement could POST
            # the resolved event back to the AgentNexus server here.
