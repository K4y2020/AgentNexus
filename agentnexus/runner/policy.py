...
Pre-refactor the AgentNexus server's :class:`PolicyEngine` enforced
...
``evaluate_policy=True`` to the AgentNexus server, which independently
...
from agentnexus.policies import FunctionPolicy, resolve_function_policy
...
class PolicyVerdict:
    ...
    on ASK (and escalate to the AgentNexus server) rather than treating
    ...
    :param action: ``"allow"`` (tool may proceed), ``"deny"`` (tool
        ...
        the AgentNexus server for a user verdict; ``policy_name`` and
