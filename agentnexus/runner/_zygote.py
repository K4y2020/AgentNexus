...
``AGENTNEXUS_RUNNER_ZYGOTE=0`` to opt out. The daemon falls back to direct
...
from agentnexus.process_logging import LOG_TTY_FD_ENV_VAR, env_truthy

# Env var the daemon sets to the inherited control-socket fd number.
ZYGOTE_CONTROL_FD_ENV_VAR = "AGENTNEXUS_RUNNER_ZYGOTE_CONTROL_FD"
...
ZYGOTE_ENABLED_ENV_VAR = "AGENTNEXUS_RUNNER_ZYGOTE"
...
ZYGOTE_HARNESS_FD_ENV_VAR = "AGENTNEXUS_RUNNER_ZYGOTE_HARNESS_FD"
...
ZYGOTE_HARNESS_FORKED_ENV_VAR = "AGENTNEXUS_HARNESS_ZYGOTE_FORKED"
...
_ZYGOTE_TEST_CHILD_EXIT_ENV_VAR = "AGENTNEXUS_RUNNER_ZYGOTE_TEST_CHILD_EXIT"
...
_ZYGOTE_TEST_CHILD_RAISE_ENV_VAR = "AGENTNEXUS_RUNNER_ZYGOTE_TEST_CHILD_RAISE"
...
_ZYGOTE_TEST_CHILD_SLEEP_ENV_VAR = "AGENTNEXUS_RUNNER_ZYGOTE_TEST_CHILD_SLEEP"
...
def _import_runner_graph() -> None:
    ...
    """
    from agentnexus.runner import _entry, app, native  # noqa: F401
    from agentnexus.runtime.harnesses import _runner as _harness_runner  # noqa: F401
...
def _run_child(request: dict[str, Any], harness_fd: int) -> None:
    ...
    from agentnexus.runner._entry import main
...
def _maybe_run_test_seam() -> None:
    ...
    if test_exit is not None:
        sys.stdout.write(f"marker={os.environ.get('AGENTNEXUS_ZYGOTE_MARKER', '')}\n")
...
def _run_harness_child(request: dict[str, Any]) -> None:
    ...
    from agentnexus.runtime.harnesses._runner import main
...
class _ZygoteServer:
    ...
    def _refuse_if_upgraded(self, conn: socket.socket, kind: str) -> bool:
        ...
        _send(
            ...
            {
                "error": (
                    "agentnexus was upgraded on disk after the zygote imported its "
...
def main() -> None:
    ...
    Reads the control-socket fd from ``AGENTNEXUS_RUNNER_ZYGOTE_CONTROL_FD``.
