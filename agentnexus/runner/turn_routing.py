...
from agentnexus.runner.subagent_routing import routing_enabled, write_advertisement
...
def trace_turn_routing(bridge_dir: Path, outcome: str, detail: str = "") -> None:
    ...
    print(f"agentnexus route-turn hook: {outcome}: {detail}", file=sys.stderr)
...
def _bridge_thread_probe(bridge_dir: Path) -> Callable[[], bool]:
    ...
    def _ready() -> bool:
        from agentnexus.codex_native_bridge import read_bridge_state
...
async def _apply_routed_model(
    ...
    *,
    ...
    if harness != "claude-native" or not model:
        return True
    from agentnexus.claude_model_vocabulary import claude_model_command_arg, normalized_model_id
...
def _settle_probe(bridge_dir: Path, harness: str | None) -> Callable[[str | None], bool]:
    ...
    if harness == "claude-native":
        ...
        def _pane_ready(blocked_turn_id: str | None) -> bool:
            del blocked_turn_id
            from agentnexus.claude_native_bridge import claude_pane_ready
