...
if TYPE_CHECKING:
    ...
    from agentnexus.runtime.filesystem_registry import FilesystemRegistry
...
from agentnexus.coordination.behavior import BEHAVIOR_MODE_LABEL_KEY
...
from agentnexus.harness_aliases import canonicalize_harness, is_native_harness
...
from agentnexus.native_coding_agents import public_agent_name
...
from agentnexus.tools.builtins.download_file import DownloadFileTool
...
from agentnexus.tools.builtins.timer import (
...
from agentnexus.tools.builtins.update_comment import UpdateCommentTool
from agentnexus.tools.builtins.upload_file import UploadFileTool, safe_resolve
...
_SESSION_WRAPPER_LABEL_KEY = "agentnexus.wrapper"
...
# runner (runner → AgentNexus server → runner). ``sys_os_shell`` accepts caller-provided
...
_TURN_ACTOR_LABEL = "agentnexus.turn_actor"
...
# state via the AgentNexus server's existing REST endpoints (GET /items, GET
...
# these proxy the AgentNexus server's REST endpoints (GET /v1/sessions/{id}/agent,
...
# The runner proxies the AgentNexus server's session policy REST endpoint.
_POLICY_TOOLS = frozenset({"sys_add_policy", "sys_policy_registry"})

# Priority 5l.1: Scheduled-task management — the runner proxies the AgentNexus
...
# AgentNexus desktop renderer claims and executes the action, then POSTs the
...
_BROWSER_TIMEOUT_ERROR = (
    '{"error": "browser action timed out — is the session open in the AgentNexus desktop app?"}'
)
...
# rides through the AgentNexus ``/mcp`` endpoint (comment, session read/write,
...
_NATIVE_RELAY_BUILTIN_TOOLS = (
    ...
    | _TERMINAL_TOOLS
    # ``browser_*`` must ride the native relay: the AgentNexus desktop app
...
def build_native_relay_tool_schemas(spec: AgentSpec | None) -> list[_JsonObject]:
    """Build the flat AgentNexus tool surface for native harness bridges.
    ...
    centralized policy enforcement on the AgentNexus server.
    ...
    if spec is not None:
        ...
        from agentnexus.tools.builtins.ask_user import SysAskUserTool
        from agentnexus.tools.builtins.policy import SysAddPolicyTool, SysPolicyRegistryTool
...
# tools stay below the same AgentNexus-owned budget instead of guessing a
...
async def _execute_uc_function_tool(
    ...
    *,
    ...
    :returns: Tool output as a string, or an error message.
    """
    from agentnexus.runner.uc_function import execute_uc_function
...
async def _list_child_sessions(
    *,
    ...
    :param server_client: AgentNexus server client.
...
def _validate_subagent_reasoning_effort(effort: str, harness: str | None) -> str:
    ...
    """
    from agentnexus.reasoning_effort import efforts_for_harness, validate_effort
...
async def _teardown_failed_child(
    ...
    *,
    ...
    """
    from agentnexus.runner import app as _runner_app
...
async def _build_subagent_message_content(
    ...
    *,
    ...
    :param server_client: Authenticated AgentNexus server client.
...
def _normalize_subagent_model(
    ...
    *,
    ...
    """
    from agentnexus.model_catalog import resolve_model_provider
...
async def _execute_subagent_tool(
    ...
    *,
    ...
    :param server_client: httpx client pointed at the AgentNexus server.
    ...
    if existing is not None:
        ...
        # The parent gear stores a per-partner preference. Continuing an SDK
        # child must apply that preference to the existing session row before
        # posting the next turn; otherwise the UI changes only a parent label
        # while this child silently keeps its launch-time/default model. SDK
        ...
        # and the requested harness must canonicalize into AGENTNEXUS_HARNESSES.
        ...
        # override against the GLOBAL ``AGENTNEXUS_HARNESSES`` (plus the omnigent
        ...
        if harness_override is not None:
            ...
            if canonical not in AGENTNEXUS_HARNESSES:
                return (
                    f"Error: sys_session_send 'harness' {harness_override!r} is "
                    f"not a known harness; must be one of {sorted(AGENTNEXUS_HARNESSES)}."
                )
        ...
        from agentnexus.onboarding.harness_install import missing_harness_cli
        ...
        if cost_budget is not None:
            policy_body = {
                ...
                "type": "python",
                "handler": "agentnexus.policies.builtins.cost.subagent_cost_budget",
    ...
    from agentnexus.server.schemas import SessionCreatedEvent
    ...
    if created_child:
        ...
        # has no subscribers (they live in the AgentNexus server), so a direct
        # publish here is silently dropped. ``publish_event`` enqueues onto
        # the parent's queue, which the AgentNexus server's relay republishes onto
...
async def _send_to_existing_session(
    ...
    *,
    ...
    if question_id is not None:
        from agentnexus.runtime.subagent_questions import validate_answers
...
async def _execute_session_create(
    ...
    *,
    ...
    :param server_client: HTTP client pointed at the AgentNexus server; ``None``
...
def _bundle_local_agent_source(source: Path) -> bytes:
    ...
    :param source: Local agent config YAML, agent directory, or
        bundle file, e.g.
        ``Path("/work/.agentnexus/agent-configs/helper.yaml")``.
...
async def _upload_config_bundle(
    ...
    *,
    ...
    :param config_path: Caller-supplied path to the agent config YAML,
        ...
        cwd, e.g. ``".agentnexus/agent-configs/helper.yaml"``.
...
async def _execute_web_search_tool(
    ...
    *,
    ...
    from agentnexus.llms.routing import web_search_native_passthrough_provider
...
async def _execute_nimble_research_tool(
    ...
    *,
    ...
    """
    from agentnexus.tools.base import ToolContext
    from agentnexus.tools.builtins.nimble_research import NimbleResearchTool
...
async def _execute_nimble_extract_tool(
    ...
    *,
    ...
    """
    from agentnexus.tools.base import ToolContext
    from agentnexus.tools.builtins.nimble_extract import NimbleExtractTool
...
def _has_subagent(
    ...
    if _find_subagent_spec(sub_agent_name, agent_spec) is not None:
        return True
    # AgentNexus inner loader: tools dict with AgentTool entries
    tools = getattr(agent_spec, "tools", None)
...
async def _execute_timer_set(
    ...
    *,
    ...
    """
    from agentnexus.runner import app as _app
...
async def _execute_comment_tool(
    ...
    *,
    ...
    The runner is a separate subprocess from the AgentNexus server and has no
    in-process ``CommentStore``. This handler uses ``server_client`` to
    call the AgentNexus server's REST API (``GET/PATCH
...
async def _execute_browser_tool(
    ...
    *,
    ...
    Does the blocking round-trip that drives the AgentNexus desktop app's
...
async def _execute_scheduled_task_tool(
    ...
    *,
    ...
    AgentNexus server's ``/v1/scheduled-tasks`` REST endpoints over
...
async def _execute_session_query_tool(
    ...
    *,
    ...
    AgentNexus server's existing REST endpoints over ``server_client``:
...
def _omnigent_error_message(resp: httpx.Response) -> str | None:
    """
    Extract the human-readable message from an AgentNexus error response.

    The server renders :class:`omnigent.errors.AgentNexusError` as
...
async def _execute_agent_tool(
    ...
    *,
    ...
    these proxy the AgentNexus server's REST endpoints over ``server_client``:
...
def _in_spawn_family(builtins: list[_JsonObject], family: str | None) -> list[_JsonObject]:
    ...
    if family is None:
        return builtins
    from agentnexus.runner.subagent_routing import harness_family
...
async def _session_get_history_via_rest(
    ...
    # lives only in the AgentNexus server's pending-elicitations index, replayed
...
class _PeekMeta:
    ...
    :param pending_elicitations: Outstanding
        ...
        parked on, replayed on the snapshot from the AgentNexus server's
        ...
        try:
            ...
            if sess_resp.status_code == 200:
                for s in sess_resp.json().get("data", []):
                    ...
                    is_a2a_chan = (
                        s.get("purpose") == "a2a"
                        or s_labels.get("agentnexus.teammate.channel") == "a2a"
                    )
                    if is_a2a_chan:
...
async def _execute_seedance_tool(
    ...
    *,
    ...
    if tool_name == "seedance_read_canvas":
        ...
        try:
            from agentnexus.seedance.bridge import read_seedance_canvas_snapshot
    ...
    *,
    ...
    try:
        ...
        if post_resp.status_code in (200, 201):
            try:
                from agentnexus.runner import app as actual_runner_app
    ...
    try:
        if mcp_manager is not None:
            ...
            if server_client is None or conversation_id is None:
                ...
                from agentnexus.tools.builtins.ask_user import AskUserRequest
...
def _runner_default_os_env_cwd(conversation_id: str | None) -> str:
    ...
    root = Path(
        os.environ.get(
            "AGENTNEXUS_RUNNER_OS_ENV_ROOT",
            str(Path(tempfile.gettempdir()) / "agentnexus-runner-os-envs"),
        )
    )
...
def _effective_runner_os_env_spec(
    ...
    - When ``runner_workspace`` is set, it ALWAYS wins — whether
      ...
      passes it via ``AGENTNEXUS_RUNNER_WORKSPACE``) and
    ...
    :param runner_workspace: Authoritative runtime cwd for the
        runner, sourced from ``AGENTNEXUS_RUNNER_WORKSPACE``.
...
async def _execute_os_env_tool(
    ...
    *,
    ...
    """
    from agentnexus.inner.os_env import _DEFAULT_READ_LIMIT, create_os_environment
...
async def _execute_file_tool(
    ...
    *,
    ...
    :param server_client: HTTP client for the AgentNexus server.
    ...
    :param runner_workspace: Authoritative runtime cwd for the runner,
        sourced from ``AGENTNEXUS_RUNNER_WORKSPACE``. Combined with
...
async def _execute_terminal_tool(
    ...
    *,
    ...
    # the AgentNexus server's relay republishes onto ``session_stream``.
...
async def _publish_terminal_created_event(
    *,
    ...
    if resource_registry is not None:
        ...
        from agentnexus.entities.session_resources import terminal_resource_view
        from agentnexus.terminals.registry import TerminalListEntry
...
def _subagent_tool_result_policy_request(
    ...
    """
    Build the AgentNexus policy-evaluation request for delayed child output.
...
async def _post_subagent_policy_verdict(
    *,
    ...
    """
    POST delayed sub-agent output to AgentNexus policy evaluation.

    :param server_client: HTTP client pointed at AgentNexus server.
...
def _apply_subagent_policy_verdict(
    ...
    """
    Apply an AgentNexus policy verdict to a sub-agent inbox payload.

    :param payload: Original completed sub-agent payload.
    :param verdict: Parsed AgentNexus policy response, e.g.
...
_CONSECUTIVE_EMPTY_INBOX_DRAINS: dict[str, int] = {}
...
async def _drain_inbox(
    ...
    *,
    ...
    if inbox is None or inbox.empty():
        if conversation_id:
            count = _CONSECUTIVE_EMPTY_INBOX_DRAINS.get(conversation_id, 0) + 1
            _CONSECUTIVE_EMPTY_INBOX_DRAINS[conversation_id] = count
            if count >= 2:
                return (
                    "Inbox is empty — no completed tasks yet. "
                    "Background tasks and sub-agents are still actively running. "
                    "You MUST END YOUR TURN now and stop polling. "
                    "The system will automatically notify and wake you when results are ready."
                )
    ...
    if conversation_id and items:
        _CONSECUTIVE_EMPTY_INBOX_DRAINS[conversation_id] = 0
...
def _execute_skill_tool(
    ...
    *,
    ...
    :returns: Tool output string.
    """
    from agentnexus.tools.builtins.load_skill import LoadSkillTool
