...
from agentnexus.policies.schema import (
    PolicyCallable,
    PolicyEvent,
    PolicyResponse,
    request_attachments,
...
# These bypass AgentNexus' ``sys_os_*`` MCP tools and execute directly
...
# as the AgentNexus ``sys_os_*`` tools (``path`` / ``command``), so the
...
def ask_on_os_tools(event: PolicyEvent) -> PolicyResponse:
    ...
    - **AgentNexus built-in OS tools** (``sys_os_read``,
    ...
    if tool in _all_os_tools:
        ...
        if tool in (
            ...
            # AgentNexus tools use ``path``; Claude native tools use ``file_path``.
            preview = (
...
# AgentNexus runner tools that load skills in non-native (SDK) harnesses.
...
def block_skills(blocked: list[str]) -> PolicyCallable:
    ...
    2. **Native ``Skill`` tool** — Claude Code's built-in ``Skill`` tool,
       ...
       the AgentNexus server's ``/policies/evaluate`` endpoint. This is how
    ...
    3. **Slash commands** — ``/skill-name`` commands submitted by the
       user (or UI). The AgentNexus server evaluates these at the ``request``
    ...
    def evaluate(event: PolicyEvent) -> PolicyResponse:
        ...
        if event_type == "tool_call":
            ...
            # Path 1: AgentNexus runner tools (load_skill / read_skill_file).
            ...
            # Fired via PreToolUse hook → AgentNexus /policies/evaluate.
        ...
        # The AgentNexus server converts ``/skill-name args`` into a synthetic
...
def deny_pii_in_llm_request(
    ...
    def evaluate(event: PolicyEvent) -> PolicyResponse:
        ...
        - ``request``: user message text, enforced universally on
          the AgentNexus server for every harness (including supervisor,
          native).
...
POLICY_REGISTRY: list[dict[str, object]] = [
    ...
    {
        "handler": "agentnexus.policies.builtins.safety.detect_loop",
        "kind": "factory",
    ...
    },
    {
        ...
        "covers AgentNexus sys_os_* tools, Claude Code native tools "
    ...
    {
        "handler": "agentnexus.policies.builtins.safety.block_skills",
        "kind": "factory",
    ...
    {
        "handler": "agentnexus.policies.builtins.safety.enforce_sandbox",
        "kind": "factory",
    ...
    {
        "handler": "agentnexus.policies.builtins.safety.deny_pii_in_llm_request",
        "kind": "factory",
