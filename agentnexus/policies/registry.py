...
Usage::
    ...
    from agentnexus.policies.registry import load_registry, get_registry
    ...
    schema = get_params_schema("agentnexus.policies.builtins.safety.max_tool_calls_per_turn")
"""
...
class PolicyRegistryEntry:
    ...
    :param handler: Full dotted import path to the callable,
        e.g. ``"agentnexus.policies.builtins.safety.ask_on_os_tools"``.
...
def load_registry(
    ...
    """
    from agentnexus.policies.builtins import BUILTIN_POLICY_MODULES
...
def is_registered_handler(handler: str) -> bool:
    ...
    called). Trusted spec loading (local ``agentnexus run``, operator
...
def get_params_schema(handler: str) -> dict[str, object] | None:
    ...
    :param handler: Full dotted import path, e.g.
        ``"agentnexus.policies.builtins.safety.max_tool_calls_per_turn"``.
