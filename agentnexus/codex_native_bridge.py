...
CODEX_NATIVE_BRIDGE_ID_LABEL_KEY = "agentnexus.codex_native.bridge_id"
...
# reads to reach the AgentNexus server. Mirrors Claude-native's
...
# prep time (the AgentNexus URL + auth do not change across thread rotations),
# whereas ``state.json`` mutates on every turn/thread change.
_POLICY_HOOK_FILE = "policy_hook.json"
_BRIDGE_ROOT = Path.home() / ".agentnexus" / "codex-native"
...
def write_mcp_bridge_config(bridge_dir: Path) -> None:
    ...
    :param bridge_dir: Codex bridge directory, e.g.
        ``Path("~/.agentnexus/codex-native/<hash>")``.
    """
...
def codex_mcp_config_overrides(
    ...
    *,
    ...
    """
    Return ``-c`` config overrides that register the AgentNexus MCP server.
    ...
    :returns: Codex ``-c`` config override strings, e.g.
        ``['mcp_servers.agentnexus.command="python"', ...]``.
    """
    ...
    return [
        ...
        f"mcp_servers.agentnexus.args={args_toml}",
    ]
...
def write_policy_hook_config(
    ...
    *,
    ...
    """
    Write the AgentNexus coordinates the codex-native policy hook needs.

    The ``PreToolUse`` / ``PostToolUse`` command hook runs as a short
    subprocess that must POST to ``/v1/sessions/{id}/policies/evaluate``
    on the AgentNexus server. It cannot inherit the long-lived forwarder's
    in-memory client, so the AgentNexus base URL and auth headers are persisted
    here and read by :func:`read_policy_hook_config` at hook time.

    :param bridge_dir: Native Codex bridge directory, e.g.
        ``Path("~/.agentnexus/codex-native/<hash>")``.
    :param ap_server_url: AgentNexus server base URL the hook POSTs to, e.g.
        ``"http://127.0.0.1:8787"``.
    :param ap_auth_headers: Outbound auth headers for AgentNexus requests, e.g.
...
def read_policy_hook_config(bridge_dir: Path) -> dict[str, object] | None:
    """
    Read the AgentNexus coordinates for the codex-native policy hook.

    :param bridge_dir: Native Codex bridge directory.
    :returns: Parsed config, e.g.
        ``{"ap_server_url": "http://127.0.0.1:8787",
        "ap_auth_headers": {"Authorization": "Bearer <token>"}}``, or
        ``None`` when no config has been written (no AgentNexus server
...
def write_codex_config_model(bridge_dir: Path, model: str) -> bool:
    ...
    Companion writer to :func:`read_codex_config_model`, used when AgentNexus
    ...
    mirrors it back to AgentNexus as an ``external_model_change``, silently
...
def settle_pending_mcp_startup(bridge_dir: Path) -> tuple[dict[str, dict[str, str | None]], bool]:
    ...
    connection that owns the thread — never to AgentNexus's observer
...
def update_thread_id(bridge_dir: Path, thread_id: str, active_turn_id: str | None = None) -> None:
    ...
    AgentNexus session stays the same.
