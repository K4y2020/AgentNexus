"""AgentNexus compatibility surface — bundled for surgical removal.

🚨 **TECH DEBT — REMOVE WHEN OMNIGENT COMPAT WORKSTREAM ENDS.**
This entire module exists *only* to support the AgentNexus
integration (see ``designs/AGENTNEXUS_INTEGRATION.md``). It
consolidates every omnigent-specific addition that would otherwise
be scattered across ``validator.py``, ``spec/__init__.py``, and
``runtime/workflow.py``.

When AgentNexus is consolidated (phase 6 of the integration design),
deleting AgentNexus support means:
...
3. Delete ``omnigent/spec/omnigent.py`` (the bidirectional
   translator).
4. The AgentNexus executor module is already gone (it held an
...
if TYPE_CHECKING:
    ...
    from agentnexus.spec.validator import ValidationResult
...
# selects ``AgentNexusExecutor``. Single source of truth — every
...
# ``executor.type == "agentnexus"``. Matches the set of internal-loop
# harnesses ``AgentNexusExecutor`` wraps. ``databricks`` is
...
# designs/AGENTNEXUS_INTEGRATION.md §1.
#
...
AGENTNEXUS_HARNESSES = frozenset(
    {
        "antigravity",
        "antigravity-native",
        "claude-native",
...
AGENTNEXUS_HARNESS_ALIASES = frozenset(
    {
        "claude",
        "native-kiro",
        "native-pi",
...
_AGENTNEXUS_ACCEPTED_HARNESSES = AGENTNEXUS_HARNESSES | AGENTNEXUS_HARNESS_ALIASES
...
AGENTNEXUS_HARNESSES = valid_harnesses()
AGENTNEXUS_HARNESS_ALIASES = frozenset(registry_harness_aliases())
_AGENTNEXUS_ACCEPTED_HARNESSES = accepted_harnesses()
...
_AGENTNEXUS_NAME_KEY = "name"
_AGENTNEXUS_SYSTEM_PROMPT_KEYS = frozenset({"prompt", "instructions"})
_AGENTNEXUS_DISCRIMINATOR_KEY = "spec_version"
...
def validate_omnigent_executor(
    ...
    of :data:`AGENTNEXUS_HARNESSES`. ``executor.config.profile`` is
    ...
    if spec.compaction is not None:
        result.add(
            "compaction",
            f"not supported when executor.type is {AGENTNEXUS_EXECUTOR_TYPE!r}"
    ...
    if not harness:
        result.add(
            ...
            f"required when executor.type is {AGENTNEXUS_EXECUTOR_TYPE!r} — "
            f"must be one of {sorted(_AGENTNEXUS_ACCEPTED_HARNESSES)}",
        )
    elif canonicalize_harness(harness) not in AGENTNEXUS_HARNESSES:
        ...
        result.add(
            "executor.config.harness",
            f"must be one of {sorted(_AGENTNEXUS_ACCEPTED_HARNESSES)}, got {harness!r}"
            f"{install_hint}",
        )
...
def is_omnigent_yaml(path: Path) -> bool:
    ...
    Detection rule (from AGENTNEXUS_INTEGRATION design):
    ...
    if not isinstance(raw, dict):
        return False
    if _AGENTNEXUS_DISCRIMINATOR_KEY in raw:
        return False
    if _AGENTNEXUS_NAME_KEY not in raw:
        return False
    # At least one system-prompt key must be present.
    return bool(_AGENTNEXUS_SYSTEM_PROMPT_KEYS.intersection(raw.keys()))
...
def diagnose_yaml_rejection(path: Path) -> str:
    ...
    suitable for embedding in an :class:`AgentNexusError` message.
...
def load_omnigent_yaml(
    ...
    *,
    ...
    :func:`omnigent.spec.agentnexus.agent_def_to_agent_spec` →
    :func:`omnigent.spec.validator.validate`. Validation failure
    raises :class:`AgentNexusError` so the caller sees the specific
    ...
    :returns: A validated :class:`AgentSpec` with
        ``executor.type == AGENTNEXUS_EXECUTOR_TYPE``.
    :raises AgentNexusError: If the synthesized spec fails
    ...
    from agentnexus.inner.loader import _AgentNexusYamlLoader
    ...
    # Use _AgentNexusYamlLoader (not yaml.safe_load) so this raw
    ...
    raw = _yaml.load(path.read_text(), Loader=_AgentNexusYamlLoader) or {}
    if not isinstance(raw, dict):
        raw = {}
    ...
    if prune_invalid_sub_agents:
        ...
        from agentnexus.spec import _prune_invalid_sub_agents

        _prune_invalid_sub_agents(spec)
    result = validate(spec)
    if not result.valid:
        errors = "; ".join(f"{e.path}: {e.message}" for e in result.errors)
        message = f"invalid agent spec synthesized from agentnexus YAML: {errors}"
        ...
        # sibling "required when executor.type is 'agentnexus' — must be
        ...
        if any(
            ...
            message += (
                "\n\nNote: if this harness is valid on a newer AgentNexus server, "
                "this client (runner) may be older than the server that produced "
                "the spec — upgrade the runner to pick up newer harnesses."
            )
        raise AgentNexusError(message, code=ErrorCode.INVALID_INPUT)
    return spec
