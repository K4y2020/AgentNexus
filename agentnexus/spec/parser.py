...
def parse(root: Path, *, expand_env: bool = True) -> AgentSpec:
    ...
    :returns: A fully populated :class:`AgentSpec` (not yet
        validated).
    :raises AgentNexusError: If ``config.yaml`` is not valid YAML,
    ...
    # encoding: agent YAML is UTF-8 by spec; without this Windows reads it
    # with the ANSI codepage (GBK/cp936) and any non-ASCII prompt crashes.
    ...
    # same YAML must produce the same tool surface under AgentNexus mode and
    ...
    # ``_AGENTNEXUS_SYSTEM_PROMPT_KEYS``); ``instructions:`` wins if both set.
...
def _parse_llm(
    ...
    *,
    ...
    :raises AgentNexusError: If the ``llm:`` block is present but
...
def _parse_sandbox_config(
    ...
    runtime decision, not an agent config decision::
        ...
        sandbox:
          ...
          container_runtime: podman  # optional, defaults to AGENTNEXUS_CONTAINER_RUNTIME or docker
...
def _parse_builtin_tools(
    ...
    *,
    ...
    :raises AgentNexusError: If a dict entry is missing ``name``.
    """
...
def _parse_executor(
    ...
    *,
    ...
    :param raw: The raw ``executor:`` mapping, or ``None`` if
        absent. Example: ``{"type": "agentnexus"}``.
    :returns: A populated :class:`ExecutorSpec`.
    """
    if raw is None:
        return ExecutorSpec()
    etype = str(raw.get("type", "agentnexus"))
    ...
    if etype == "agentnexus" and profile is not None and "profile" not in config:
...
def _parse_executor_auth(
    ...
    *,
    ...
    - ``type: provider`` — requires ``name`` (a provider declared in
      the ``providers:`` block of ``~/.agentnexus/config.yaml``).
    ...
    :raises AgentNexusError: If the ``auth:`` block is present but
...
def _parse_os_env(
    ...
    Native AgentNexus YAML mirrors the omnigent YAML shape so users
    ...
    :raises AgentNexusError: If *raw* is not a mapping, or
...
def _parse_terminals(
    ...
    Native AgentNexus YAML mirrors the omnigent-compat ``terminals:`` shape — a
    ...
    ``designs/AGENTNEXUS_TERMINAL_BRIDGE.md`` §3 (``_parse_terminals`` parallel to
    ``_parse_os_env``).
    ...
    :raises AgentNexusError: If ``terminals`` (or any entry) is not a mapping,
...
def _parse_os_env_sandbox(
    ...
    from agentnexus.inner.sandbox import _default_sandbox_for_platform, _resolve_sandbox_type
    ...
    if macos_reason is not None:
        raise AgentNexusError(macos_reason, code=ErrorCode.INVALID_INPUT)
...
def _parse_cwd_allow_hidden(raw: object) -> list[str] | None:
    ...
    :raises AgentNexusError: If ``raw`` isn't a list, contains a
...
def _parse_cwd_hidden_scan_max_entries(raw: object) -> int:
    ...
    :returns: Validated positive integer, or the dataclass default
        when ``raw`` is ``None``.
    :raises AgentNexusError: If ``raw`` is not an int or is not
        strictly positive.
    """
    if raw is None:
        return cast(
...
def _parse_cwd_hidden_scan_overflow(raw: object) -> str:
    ...
    :raises AgentNexusError: If ``raw`` is not one of the supported
        modes.
    """
    if raw is None:
        return cast(
...
def _parse_cwd_hidden_scan_recursive(raw: object) -> bool:
    ...
    :raises AgentNexusError: If ``raw`` is not a boolean.
    """
    if raw is None:
        return cast(
            bool,
...
def _parse_mask_paths(raw: object) -> list[str] | None:
    ...
    :raises AgentNexusError: If ``raw`` is not a list, or any entry is
...
def _parse_egress_rules(raw: object) -> list[str] | None:
    ...
    :raises AgentNexusError: If the value isn't a list or any
        rule fails to parse.
    """
    if raw is None:
        return None
    ...
    if not raw:
        return None
    from agentnexus.inner.egress.rules import parse_rule
...
def _format_validation_error(exc: ValidationError) -> str:
    ...
    :class:`AgentNexusError` so the CLI / loader surface a single error
...
def _parse_credential_proxy(raw: object) -> CredentialProxySpec | None:
    ...
    is re-raised as an :class:`AgentNexusError` so callers see one error
    type.
    ...
    :raises AgentNexusError: If the value isn't a list, an entry fails
...
def _merge_databricks_cli(
    ...
    *,
    ...
    :raises AgentNexusError: On a duplicate profile across entries or a
...
def _normalize_https_bearer(
    ...
    *,
    ...
    :raises AgentNexusError: If a host fails DNS-safety validation.
    """
...
def _normalize_gh_basic(
    ...
    *,
    ...
    :raises AgentNexusError: If an explicit host fails DNS-safety validation.
    """
...
def _parse_credential_proxy_host(raw: str, *, field_path: str) -> str:
    ...
    :raises AgentNexusError: If the value is empty or the host contains
        characters outside the DNS grammar ``[A-Za-z0-9.-]`` (wildcards
        included — credentials bind to an exact host).
    """
    from agentnexus.inner.egress.rules import is_dns_safe_host
...
def _parse_share_policy(raw: object) -> SharePolicy:
    ...
    :raises AgentNexusError: When the value is neither ``None`` nor one
...
def _discover_skills(
    ...
    *,
    ...
    for skill_dir in entries:
        ...
        except (AgentNexusError, yaml.YAMLError) as exc:
            if skipped is None:
                raise
            msg = f"{skill_md}: {exc}"
...
def _parse_skill(skill_md: Path) -> SkillSpec:
    ...
    :raises AgentNexusError: If the file cannot be read, or the
    ...
    try:
        ...
        # OSError — funnel it through AgentNexusError too so the lenient
...
def expand_env_vars(
    ...
    Raises :class:`AgentNexusError` if any value still contains an
    ...
    :raises AgentNexusError: If a value contains an unresolved
...
def check_unresolved_env_vars(key: str, value: str) -> None:
    ...
    :param value: The expanded value to check, e.g.
        ``"Bearer ${MISSING}"``.
    :raises AgentNexusError: If *value* contains an unresolved
...
def _discover_mcp_servers(
    ...
    *,
    ...
    :returns: A sorted list of parsed :class:`MCPServerConfig`
        ...
        exist.
    :raises AgentNexusError: If any YAML file is malformed or
...
def _parse_http_mcp_server(
    ...
    *,
    ...
    :returns: A fully populated :class:`MCPServerConfig` with
        ``transport == "http"``.
    :raises AgentNexusError: If ``url`` is missing or a stdio-only
        field was supplied.
    """
...
def _parse_stdio_mcp_server(
    ...
    *,
    ...
    :returns: A fully populated :class:`MCPServerConfig` with
        ``transport == "stdio"``.
    :raises AgentNexusError: If ``command`` is missing, ``args`` is
...
def _reject_wrong_transport_keys(
    ...
    *,
    ...
    :raises AgentNexusError: When any *disallowed* key is present
        in *raw*.
    """
...
# spec load — these helpers raise ``AgentNexusError`` on
...
def _parse_guardrails(
    ...
    *,
    ...
    :returns: A populated :class:`GuardrailsSpec`, or ``None``
        when *raw* is ``None``.
    :raises AgentNexusError: On any spec-load validation
...
def _parse_guardrails_ask_timeout(raw: object) -> int:
    ...
    :raises AgentNexusError: On non-integer or non-positive
        values.
    """
...
def _parse_label_defs(
    ...
    :raises AgentNexusError: On malformed entries — empty
...
def _parse_single_label_def(key: str, entry: object) -> LabelDef:
    ...
    :raises AgentNexusError: On any malformed value.
    """
...
def _coerce_label_values(key: str, raw: object) -> list[str] | None:
    ...
    :returns: Every element str-coerced; ``None`` when
        *raw* is ``None``.
    :raises AgentNexusError: When *raw* is a non-list
        non-None value.
    """
    if raw is None:
        return None
...
def _validate_label_def_cross_fields(
    ...
    :raises AgentNexusError: On any cross-field violation.
    """
...
def _parse_policies(
    ...
    *,
    ...
    :returns: List of policy specs, or ``None`` when *raw*
        is ``None``.
    :raises AgentNexusError: On any malformed policy entry.
    """
    if raw is None:
        return None
    if not isinstance(raw, dict):
...
def _parse_policy_spec(
    ...
    *,
    ...
    :raises AgentNexusError: On malformed data or unknown
        policy type.
    """
    ...
    if policy_type == "prompt":
        raise AgentNexusError(
            ...
            f"'agentnexus.policies.builtins.prompt.prompt_policy' instead.",
            code=ErrorCode.INVALID_INPUT,
        )
...
def _parse_function_policy(
    ...
    :raises AgentNexusError: On missing ``function:`` field
...
def _parse_on(
    ...
    *,
    ...
    :raises AgentNexusError: On empty list, unknown phase,
...
def _parse_on_entry(
    ...
    *,
    ...
    :raises AgentNexusError: On non-string entry, empty
...
def _resolve_phase(
    ...
    *,
    ...
    :raises AgentNexusError: When *phase_str* is not a
        valid phase.
    """
    try:
...
def _parse_condition(
    ...
    *,
    ...
    :raises AgentNexusError: On a non-dict value.
    """
    if raw is None:
        return None
    if not isinstance(raw, dict):
...
def _parse_writable_labels(
    ...
    *,
    ...
    :returns: List of allowed label keys, or ``None`` when
        *raw* is absent.
    :raises AgentNexusError: When *raw* is not a list.
    """
    if raw is None:
        return None
    if not isinstance(raw, list):
...
def _parse_function_ref(
    ...
    *,
    ...
    :raises AgentNexusError: On malformed shape — non-string
...
def parse_default_policies(
    ...
    *,
    ...
    :raises AgentNexusError: On any malformed policy entry — unknown
