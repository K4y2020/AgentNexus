...
class ProviderAuth:
    ...
    # Required: provider name from ~/.agentnexus/config.yaml providers:.
    name: str
...
class ExecutorSpec:  # type: ignore[explicit-any]  # config: dict[str, Any] field (see below)
    ...
    :param type: Executor type. ``"agentnexus"`` (default),
    ...
    :param config: Executor-type-specific configuration. For
        ``type == "agentnexus"`` this carries ``"harness"`` (e.g.
        ...
        os_env data for the AgentNexus integration (see
        ``designs/AGENTNEXUS_INTEGRATION.md``). A free-form
        ``dict[str, Any]`` on a spec dataclass is the kind of
        bag-of-values escape hatch we'd otherwise reject in
        review — acceptable here only because the AgentNexus
        executor is a temporary bridge with explicit sunset
        criteria. Once AgentNexus is consolidated (phase 6 of the
    ...
    :param auth: Explicit LLM authentication configuration. When set,
        ...
        ``~/.agentnexus/config.yaml``). ``None`` means fall back to
    ...
    # Remove when AgentNexus consolidation lands; do NOT extend.
    ...
    def harness_kind(self) -> str:
        ...
        For ``type == "agentnexus"`` the kind lives in
...
class SandboxConfig:
    ...
    :param container_runtime: The container CLI to use, either
        ...
        via the ``AGENTNEXUS_CONTAINER_RUNTIME`` environment variable;
        the constructor argument takes precedence.
    """

    _ENV_VAR = "AGENTNEXUS_CONTAINER_RUNTIME"
...
class AgentSpec:  # type: ignore[explicit-any]  # params: dict[str, Any] field (see below)
    ...
    :param async_enabled: Whether the LLM-callable async-dispatch
        ...
        whether the user opts into AgentNexus mode or runs the legacy
    ...
    :param os_env: The agent's OS environment, e.g.
        ...
        Native AgentNexus YAML accepts this as a top-level ``os_env:``
        mapping; the omnigent-compat translator copies
        ``AgentDef.os_env`` into this field directly. The
        ``"inherit"`` string sentinel from agentnexus inline
    ...
    :param terminals: Declared interactive tmux terminals
        ...
        ``AgentDef.terminals``. Native AgentNexus YAML support is
        deferred — see ``designs/AGENTNEXUS_TERMINAL_BRIDGE.md`` §3.
    :param timers: Whether the LLM-callable timer builtins
        ...
        stack uses in-memory asyncio tasks; AgentNexus supersedes that
