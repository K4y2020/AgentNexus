...
translator in :mod:`omnigent.spec.agentnexus` routes every
...
# Maps AgentNexus' :class:`Phase` enum to the string literal
...
def build(
    ...
    *factory_kwargs*, then decides whether the resolved callable
    ...
    :param configured_phases: The YAML ``on:`` list (verbatim
        ...
        (:func:`omnigent.spec.agentnexus._translate_function_policy_yaml`)
        always passes a list.
    :returns: A callable matching AgentNexus'
...
def _v0_event_to_legacy_phase(event: Mapping[str, object]) -> str:
    ...
    AgentNexus' ``Phase`` enum. Provide a direct mapping here.
...
def _convert_args(
    ...
    *,
    ...
    ``(content, phase, context)`` from AgentNexus'
...
def _legacy_context(
    ...
    *,
    ...
    # behave identically across legacy and AgentNexus mode.
