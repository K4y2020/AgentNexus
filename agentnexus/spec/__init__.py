...
from agentnexus.spec.parser import expand_env_vars, parse, parse_default_policies, parse_server_llm
...
from agentnexus.spec.validator import ValidationResult, validate
...
def load(
    ...
    *,
    ...
    :raises AgentNexusError: If the spec fails validation, if a policy
    ...
    if isinstance(source, bytes):
        ...
        # AgentNexus single-file YAML dispatch — see
    ...
    # AgentNexus single-file YAML dispatch (extracted-bundle variant)
    ...
    *spec* in place: the failing sub-agent is removed from
    ...
    for sa in spec.sub_agents:
        ...
        _logger.warning(
            ...
            "produced by a newer AgentNexus server with a feature this client does "
...
def _reject_unregistered_spec_policy_handlers(spec: AgentSpec) -> None:
    ...
    :raises AgentNexusError: If a function policy names an unregistered
