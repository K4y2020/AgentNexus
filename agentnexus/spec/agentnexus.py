...
- :func:`agent_spec_to_agent_def` — forward direction, consumed by
  ``AgentNexusExecutor`` at executor-construction time.
...
Design invariants (see designs/AGENTNEXUS_INTEGRATION.md §1 and §2):
...
- Unsupported concepts **fail loud** with an :class:`AgentNexusError`
...
from agentnexus.llms.routing import infer_harness_from_model as _infer_harness_from_model
...
# selects ``AgentNexusExecutor``. Both directions of the translator
# treat this as the discriminator between native omnigent specs
# and omnigent-sourced ones.
AGENTNEXUS_EXECUTOR_TYPE = "agentnexus"
...
AGENTNEXUS_TOOL_LANGUAGE = "agentnexus-python-callable"

# Version stamped on AgentSpecs synthesized from agentnexus YAMLs.
# AgentNexus YAMLs do not declare ``spec_version`` (it's an
...
# AgentNexus loader policy-type discriminators. Used to dispatch
...
def agent_spec_to_agent_def(spec: AgentSpec) -> AgentDef:
    ...
    :param spec: The omnigent spec to translate. ``spec.llm``
        ...
        ``spec.executor.type`` must be ``"agentnexus"`` with
    ...
    :returns: A populated :class:`omnigent.datamodel.AgentDef`
        ...
        rejected upstream.
    :raises AgentNexusError: If the spec uses an unsupported
    ...
    if spec.executor.model is None:
        raise AgentNexusError(
            "executor.type='agentnexus' requires a model (set executor.model)",
            code=ErrorCode.INVALID_INPUT,
        )

    # For native AgentNexus v1 specs ``executor.config`` is empty; infer the
...
def _reject_unsupported_concepts(spec: AgentSpec) -> None:
    ...
    :raises AgentNexusError: On any of:
    ...
    # :class:`AgentNexusExecutor` is invoked with tool calls
    ...
    # ``cancellable_function`` is an omnigent concept — AgentNexus'
    ...
    for tool in spec.local_tools:
        ...
        if _is_cancellable_function_path(tool.path):
            raise AgentNexusError(
                ...
                "agentnexus; cancellation support is an omnigent spec extension",
                code=ErrorCode.INVALID_INPUT,
            )
...
def _translate_tools_to_omnigent(spec: AgentSpec) -> dict[str, Tool]:
    """
    Build the ``AgentDef.tools`` dict from AgentNexus' tool model.
    ...
    :returns: A ``dict[str, Tool]`` mapping tool name to
        ...
        local tools.
    :raises AgentNexusError: If a tool path is a filesystem path
    ...
    # ``AgentNexusExecutor.from_spec`` calls this reverse path when
...
def _mcp_server_to_mcp_tool(config: MCPServerConfig) -> MCPTool:
    ...
    :raises AgentNexusError: If the config is missing the
...
def _sub_spec_to_agent_tool(sub: AgentSpec) -> AgentTool:
    ...
    # (native AgentNexus v1 specs). Also forward os_env so sub-sessions get
...
def _resolve_dotted_attr(dotted_path: str, tool_name: str) -> Any:
    ...
    :returns: The resolved attribute object (callable or
        runner or other).
    :raises AgentNexusError: If the path is not a dotted import
    ...
    if "/" in dotted_path or dotted_path.endswith(".py"):
        raise AgentNexusError(
            ...
            "agentnexus translator requires dotted import paths like "
            "'examples._shared.tool_functions.get_current_time'",
            code=ErrorCode.INVALID_INPUT,
        )
    ...
    try:
        ...
        # single AgentNexusError so the spec author sees a
...
def _resolve_dotted_callable(
    ...
    :raises AgentNexusError: If the module can't be imported,
...
def _translate_guardrails_yaml(
    ...
    *,
    ...
    AgentNexus declares labels/policies at the top level
    ...
    :param raw_policies: AgentNexus YAML ``policies:`` mapping,
    ...
    :param raw_labels: AgentNexus YAML top-level ``labels:`` map
        ...
        ``initial: None``.
    :param raw_label_schema: AgentNexus YAML top-level
        ``label_schema:`` map of
        ``{label_key: {values: [...], monotonic: "max"|"min"|"none"}}``.
    :param raw_ask_timeout: AgentNexus YAML top-level
    ...
    :returns: A validated :class:`GuardrailsSpec`, or ``None``
        ...
        source YAML.
    :raises AgentNexusError: On malformed entries — same error
        shape the omnigent YAML parser produces for a native
        ``guardrails:`` block, so YAML authors see consistent
        error messages whether their spec came from agentnexus
        or omnigent.
    """
    ...
    from agentnexus.spec.parser import _parse_guardrails
...
def _translate_labels_yaml(
    ...
    ``label_schema:`` (schemas) into AgentNexus' unified
...
def _translate_policy_entry_yaml(
    ...
    *,
    ...
    :raises AgentNexusError: When the entry declares a policy
    ...
    if policy_type not in _KNOWN_POLICY_TYPES:
        raise AgentNexusError(
            f"agentnexus policy {policy_name!r}: unknown type "
...
def _translate_function_policy_yaml(
    ...
    # convention keep working under AgentNexus'
...
def _resolve_profile_to_connection(profile: str) -> dict[str, str] | None:
    ...
    self-contained — AgentNexus' LLM adapters take credentials
    ...
    from agentnexus.inner.databricks_executor import _read_databrickscfg
...
def agent_def_to_agent_spec(
    ...
    *,
    ...
    ``AgentNexusExecutor`` at runtime.

    The produced spec sets ``executor.type = "agentnexus"`` so the
    runtime picks ``AgentNexusExecutor``, with ``harness`` and
    ...
    :param raw_yaml: Optional raw YAML dict the ``agent_def`` was
        ...
        AgentNexus' ``guardrails:`` block — the omnigent
    ...
    :raises AgentNexusError: When *agent_def* uses an omnigent
        concept AgentNexus' :class:`AgentSpec` cannot currently
    ...
    if executor_spec.type == "agentnexus":
    ...
    # translation time — the AgentNexus path spawns each child as an
    ...
    #         AgentNexus' ToolManager, srt-wrapped when
    ...
    # top-level YAML fields into AgentNexus' ``guardrails:``
    ...
    if raw_yaml is not None:
        guardrails = _translate_guardrails_yaml(
            ...
            # Baked-in at translation time so AgentNexus'
    ...
    # Per designs/AGENTNEXUS_TERMINAL_BRIDGE.md §6.1 step 2 + §3 non-goal
...
def _translate_skills_filter_from_yaml(
    ...
    :raises AgentNexusError: When the value isn't one of the
...
def _self_agent_tool_to_sub_spec(
    ...
    *,
    ...
    the AgentNexus runtime spawns when the parent's LLM dispatches
...
def _agent_tool_to_sub_spec(
    ...
    *,
    ...
    :class:`AgentSpec` with ``executor.type == "agentnexus"`` so the
    :class:`AgentNexusExecutor` runs it when spawned.
    ...
    :param parent_profile: Databricks profile carried on the
        ...
        empty field does.
    :param parent_harness: AgentNexus harness carried on the
...
def _resolve_inline_agent_tool_os_env(
    ...
    AgentNexus' :class:`AgentTool.os_env` is
...
def _fail_on_unsupported_concepts_def(agent_def: AgentDef) -> None:
    """
    Raise :class:`AgentNexusError` for every omnigent concept
    AgentNexus' :class:`AgentSpec` cannot currently represent.

    Each unsupported concept gets its own clear error message
    naming the specific field.

    :param agent_def: Parsed omnigent agent definition.
    :raises AgentNexusError: On the first unsupported concept
...
def _fail_on_unsupported_tool(
    ...
    """
    Raise :class:`AgentNexusError` when *tool* uses an omnigent
    ...
    :raises AgentNexusError: On MCP tools or cancellable_function
        tools.
    """
    ...
    if isinstance(tool, MCPTool):
        ...
        # Databricks-managed MCP endpoint that AgentNexus'
        ...
        if tool.databricks_server is not None:
            raise AgentNexusError(
                f"agentnexus agent {agent_name!r} declares tool "
                f"{tool_name!r} of type `mcp` with "
                f"``databricks_server={tool.databricks_server!r}`` — "
                f"AgentNexus' MCPServerConfig doesn't understand the "
...
def _translate_llm_from_def(
    ...
    *,
    ...
    ``type = "agentnexus"`` plus the ``harness`` / ``profile``
    config.
...
def _translate_executor_from_def(
    ...
    *,
    ...
    Returns ``type="agentnexus"`` when a harness can be resolved,
    routing the agent through ``AgentNexusExecutor``. Raises
    :class:`AgentNexusError` when a model is declared but no
    ...
    The ``harness`` and ``profile`` fields from agentnexus land in
    ``executor.config`` so :meth:`AgentNexusExecutor.from_spec` can
    ...
    3. :func:`_infer_harness_from_model` on the resolved model
       ...
       that declare only a model continue to work under AgentNexus mode.
    ...
    :returns: An :class:`ExecutorSpec` with ``type="agentnexus"``
        when a harness is known.
    :raises AgentNexusError: When a model is declared but no
        harness can be inferred from it.
    """
    # AgentNexus' :class:`ExecutorSpec.{harness,profile,model}`
    ...
    if oa_executor is not None and oa_executor.auth is not None:
        ...
        from agentnexus.spec.parser import _parse_executor_auth
...
def _translate_mcp_tool_from_def(
    ...
    :raises AgentNexusError: If *tool* has neither ``url`` nor
    ...
    raise AgentNexusError(
        f"agentnexus MCP tool {tool_name!r} has neither 'url' nor "
...
def _translate_function_tool_from_def(
    ...
    :returns: A :class:`LocalToolInfo` with ``name``, ``path``
        ...
        :data:`AGENTNEXUS_TOOL_LANGUAGE`.
    :raises AgentNexusError: When *tool* is not a
        :class:`FunctionTool` (defensive guard).
    """
    if isinstance(tool, CancellableFunctionTool):
        raise AgentNexusError(
            f"agentnexus tool {tool_name!r}: "
    ...
    if not isinstance(tool, FunctionTool):
        raise AgentNexusError(
            f"agentnexus tool {tool_name!r}: expected FunctionTool, got {type(tool).__name__}.",
            code=ErrorCode.INVALID_INPUT,
        )
    ...
    if tool.runtime == "client":
        return LocalToolInfo(
            name=tool_name,
            path=None,
            language=AGENTNEXUS_TOOL_LANGUAGE,
...
def _recover_callable_path(
    ...
    :returns: The dotted module path, e.g.
        ``"examples._shared.tool_functions.get_current_time"``.
    :raises AgentNexusError: When the callable is missing
    ...
    raise AgentNexusError(
        f"agentnexus tool {tool_name!r}: callable {callable_obj!r} "
