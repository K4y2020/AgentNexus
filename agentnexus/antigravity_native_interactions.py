...
AgentNexus elicitation, waits for the human's verdict, and delivers it back to agy
...
WAITING step at a HIGHER ``stepIndex``**. AgentNexus elicitations wait on a human
...
from agentnexus.antigravity_native_steps import PendingInteraction, pending_interaction
from agentnexus.server.routes._antigravity_elicitation import (
...
def tui_injector_for(bridge_dir: Path) -> InjectTui:
    ...
    async def _inject(keys: list[str]) -> None:
        ...
        from agentnexus.antigravity_native_bridge import send_interaction_keys_via_tui
...
async def _inject_via_tui(keys: list[str]) -> None:
    ...
    from agentnexus.antigravity_native_bridge import send_interaction_keys_via_tui
    from agentnexus.inner.antigravity_native_executor import _bridge_dir_from_env
...
def agy_elicitation_id(cascade_id: str, trajectory_id: str, step_index: int) -> str:
    """
    Build the AgentNexus elicitation id for one agy WAITING interaction.
