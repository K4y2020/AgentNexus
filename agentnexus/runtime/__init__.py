...
def get_terminal_registry() -> TerminalRegistry:
    ...
    ``designs/AGENTNEXUS_TERMINAL_BRIDGE.md`` §4.2.
...
def set_runner_router(router: RunnerRouter | None) -> None:
    ...
    Production AgentNexus servers use this instead of a process-wide runner
...
def get_harness_process_manager() -> HarnessProcessManager:
    ...
    :raises RuntimeError: If the lifespan startup hasn't run yet
        (e.g. workflow invoked before AgentNexus boot completed, or in a
    ...
    if pm is None:
        raise RuntimeError(
            "HarnessProcessManager not initialized — AgentNexus lifespan startup "
