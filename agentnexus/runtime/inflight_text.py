...
Lets the AgentNexus server replay the text streamed so far when a client
...
the durable transcript. The index lives only in the AgentNexus process, so it
...
``yield``/``await``*. On the single AgentNexus event loop where the relay
...
def discard(conversation_id: str) -> None:
    ...
    Called from the AgentNexus relay's teardown (``_relay_runner_stream``'s
