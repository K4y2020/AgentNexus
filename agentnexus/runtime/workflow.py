...
from agentnexus.onboarding.ucode_state import UcodeAgentState, read_ucode_state
from agentnexus.runtime import (
    get_artifact_store,
    get_conversation_store,
    get_file_store,
)
from agentnexus.runtime.compaction import (
    CompactionResult,
    SummaryMetadata,
    _CompactionState,
    compact,
...
from agentnexus.runtime.content_resolver import resolve_content_references
from agentnexus.runtime.prompt import build_instructions, history_to_input_items
...
# Translates AgentNexus spec executor types (underscore) to harness subprocess names
...
def _get_runner_client_for_compaction(
    ...
    profile) instead of the AgentNexus server's.
    ...
    Propagates :class:`AgentNexusError` when the pinned runner is
    ...
    if conversation_id is None:
        return None
    from agentnexus.runtime import get_runner_router
...
_HARNESS_DATABRICKS_PROFILE: dict[AgentHarnessType, str] = {
    ...
    # so AgentNexus cannot thread a Databricks gateway through. Users configure
    # providers via ``kimi provider add`` in ``~/.kimi/config.toml``
    # (AgentNexus-side provider injection is a deferred follow-up).
}
...
def _provider_auth_command(family: FamilyConfig) -> str:
    ...
    :raises AgentNexusError: If the family carries neither a static
...
def configure_agent_harness_with_provider(
    ...
    *,
    ...
    of ``~/.agentnexus/config.yaml``) and emits the **same** vendor-neutral
    ...
    :param harness_type: Canonical harness type, e.g. ``"claude-sdk"``.
    :raises AgentNexusError: If an inline-family provider lacks the family
    ...
    if entry.kind == BEDROCK_KIND:
        ...
        raise AgentNexusError(
            ...
            f"native 'agentnexus claude' terminal, not the {harness_type!r} harness. "
            "For agents / 'agentnexus run', use a 'gateway' provider "
    ...
    if entry.kind == CLI_CONFIG_KIND:
        ...
        if harness_type != "codex":
            raise AgentNexusError(
                ...
                "harness in ~/.agentnexus/config.yaml.",
                code=ErrorCode.INVALID_INPUT,
            )
...
def _catalog_default_model(family_name: str) -> str:
    ...
    :raises AgentNexusError: If the family is unknown or discovery has no
        compatible model.
    """
...
def _apply_provider_family(
    ...
    :raises AgentNexusError: If no model is resolvable (neither the spec nor
...
def _optional_provider_family(
...
) -> tuple[FamilyConfig | None, AgentNexusError | None]:
    ...
    keychain secret) raise :class:`AgentNexusError` at family-access time
    ...
    :returns: ``(FamilyConfig, None)`` when the family resolved, or
        ``(None, AgentNexusError)`` when resolution failed, or ``(None, None)``
...
def _apply_provider_to_pi(env: dict[str, str], entry: ProviderEntry) -> None:
    ...
    :raises AgentNexusError: If no configured family's credentials resolve,
...
def _apply_cli_config_databricks_to_pi(env: dict[str, str], entry: ProviderEntry) -> None:
    ...
    :param entry: The resolved ``cli-config`` provider entry (a Databricks
        ...
        reaches here).
    :raises AgentNexusError: If the cli-config entry cannot be translated into a
    ...
    from agentnexus.pi_native_credentials import _cli_config_pi_provider
...
def _resolve_provider_for_build(
    ...
    *for_launch*) and the readout / cost / native paths (without). Precedence,
    ...
    :raises AgentNexusError: If a named :class:`ProviderAuth` references a
    ...
    if isinstance(auth, ProviderAuth):
        ...
        if entry is None:
            raise AgentNexusError(
                ...
                "configured under 'providers:' in ~/.agentnexus/config.yaml. "
...
def _build_claude_sdk_spawn_env(
    ...
    *,
    ...
    design doc), per-spawn env overrides are how AgentNexus threads
    ...
    # 3. Global config ~/.agentnexus/config.yaml auth: — only when spec has
    ...
    # some configurations); routing through AgentNexus mode without a
    # similar default leaves AgentNexus mode users staring at a
    ...
    # active and approval requests surface via AgentNexus elicitation. Read from
...
def _apply_harness_path_override(
    ...
    """Thread a config ``harness.<canonical>.command`` into ``AGENTNEXUS_<NAME>_PATH``.

    The harness wraps read ``AGENTNEXUS_<NAME>_PATH`` to locate their vendor
    CLI (the headless CLI-subprocess family historically read ``HARNESS_*_PATH``;
    both are honored, ``AGENTNEXUS_*`` canonical). A user can set that path via
...
def _build_codex_spawn_env(
    ...
    *,
    ...
    on the AgentNexus server directly (they propagate to the subprocess
...
def _build_qwen_spawn_env(
    ...
    *,
    ...
    # harness fall back to AGENTNEXUS_RUNNER_WORKSPACE — see HARNESS_QWEN_CWD.
    if cwd is not None:
...
def _build_goose_spawn_env(
    ...
    *,
    ...
    # harness fall back to AGENTNEXUS_RUNNER_WORKSPACE — see HARNESS_GOOSE_CWD.
    if cwd is not None:
...
def _build_acp_cli_spawn_env(
    ...
    *,
    ...
    ``AGENTNEXUS_<NAME>_PATH`` env override, then the config
    ...
    # back to AGENTNEXUS_RUNNER_WORKSPACE — see HARNESS_ACP_CWD.
    if cwd is not None:
...
def _build_acp_spawn_env(
    ...
    *,
    ...
    if agent is not None:
        ...
        env["HARNESS_ACP_AGENTNEXUS_MCP"] = "1" if agent.agentnexus_mcp else "0"
...
def _load_global_auth() -> ApiKeyAuth | DatabricksAuth | None:
    """
    Load the ``auth:`` block from ``~/.agentnexus/config.yaml``.

    Reads the user-level global config file (respecting
    ``$AGENTNEXUS_CONFIG_HOME`` for test isolation) and parses the
    ...
    :returns: A :class:`ApiKeyAuth` or :class:`DatabricksAuth`, or
        ...
        file is missing.
    """
    config_home = os.environ.get("AGENTNEXUS_CONFIG_HOME")
    path = (
        Path(config_home) / "config.yaml"
        if config_home
        else Path.home() / ".agentnexus" / "config.yaml"
    )
    if not path.exists():
        return None
    try:
...
def _build_openai_agents_sdk_spawn_env(spec: AgentSpec) -> dict[str, str]:
    ...
    3. Global config ``~/.agentnexus/config.yaml`` ``auth:`` block —
...
def _build_cursor_spawn_env(
    ...
    *,
    ...
    if isinstance(spec.executor.auth, ApiKeyAuth):
        ...
        from agentnexus.onboarding.cursor_auth import resolve_cursor_api_key
...
def _build_kimi_spawn_env(
    ...
    *,
    ...
    :param cwd: Runtime working directory for the kimi subprocess — the
        ...
        When unset, the harness wrap falls back to ``AGENTNEXUS_RUNNER_WORKSPACE``.
        Mirrors :func:`_build_pi_spawn_env`'s ``cwd`` handling.
    :returns: A dict of env-var overrides.
    :raises AgentNexusError: If the spec declares ``executor.auth`` —
    ...
    if spec.executor.auth is not None:
        raise AgentNexusError(
            ...
            "resulting model id in the agent spec. AgentNexus-side provider "
...
def _load_initial_history(
    ...
    # the empty synthetic pair — the AgentNexus thinks context is ~0 tokens
...
async def compact_conversation_now(
    *,
    ...
    if preserve_recent_window is not None:
        ...
        from agentnexus.spec.types import CompactionConfig
    ...
    from agentnexus.llms.context_window import get_model_context_window
...
def _maybe_persist_compaction_item(
    ...
    # session while the AgentNexus thinks context is near-zero and never
