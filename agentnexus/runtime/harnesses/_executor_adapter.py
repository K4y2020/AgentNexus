...
from agentnexus.inner.tracing import TracingContext, is_tracing_enabled
from agentnexus.policies.types import FAIL_CLOSED_PHASES
from agentnexus.runtime.harnesses._scaffold import HarnessApp, PolicyVerdictPayload, TurnContext
...
class ExecutorAdapter(HarnessApp):
    ...
    async def run_turn(self, request: CreateResponseRequest, ctx: TurnContext) -> None:
        """Drive the inner executor for one turn, translating its events to AgentNexus SSE.
        ...
        from agentnexus.runtime.telemetry import current_session_id, session_scope
        ...
        try:
            ...
            if tctx:
                try:
                    from agentnexus.runtime.telemetry import trace_context_for_response
                ...
                async for event in executor.run_turn(
                    ...
                    if ctx.cancelled.is_set():
                        if tctx is not None and agent_span is not None:
                            from agentnexus.runtime.telemetry import record_cancellation
                    ...
                    if tctx is not None:
                        if isinstance(event, ToolCallRequest):
                            ...
                            if event.usage is not None and agent_span is not None:
                                from agentnexus.runtime.telemetry import record_llm_usage
    ...
    def _translate_event(self, event: ExecutorEvent, ctx: TurnContext) -> None:
        """Translate one inner ExecutorEvent into AgentNexus SSE events via ``ctx.emit``."""
        if isinstance(event, TextChunk):
            ...
            # Emit bare names (strip MCP prefix) to match the AgentNexus wire shape.
            tool_use_id = _call_id_from_metadata(event.metadata)
            # Observed native tools already ran inside their harness. They must
            # never enter the queue consumed by AgentNexus's dispatch bridge.
            ...
            from agentnexus.server.schemas import CompactionInProgressEvent
            ...
            from agentnexus.server.schemas import CompactionCompletedEvent
    ...
    def _build_error_detail(self, exception: BaseException) -> Any:
        """Map an exception to a semantic code the AgentNexus retry allowlist recognizes.

        AgentNexusError uses its own ``code``; others go through ``classify_inner_exception``.
        Unknown types fall back to base class (``type(exception).__name__``).
        """
        from agentnexus.error_layers import classify_error_layer
        from agentnexus.errors import AgentNexusError
        from agentnexus.server.schemas import ErrorDetail

        if isinstance(exception, AgentNexusError):
            return ErrorDetail(
                code=exception.code,
...
def _classify_openai_exception(exception: BaseException) -> str | None:
    ...
    if isinstance(exception, openai.InternalServerError):
        return "server_error"
    from agentnexus.llms.errors import is_context_length_exceeded
...
def _extract_role_keyed_messages(
    ...
    """Extract role-keyed message items from an AgentNexus input list.
