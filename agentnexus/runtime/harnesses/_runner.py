...
- ``--conversation-id <id>``: the conversation this process serves.
  ...
  doc. AgentNexus allocates the id; the runner does NOT parse it from the
...
from agentnexus.process_logging import env_truthy
from agentnexus.runner._zygote import ZYGOTE_HARNESS_FORKED_ENV_VAR
...
_GRACEFUL_SHUTDOWN_TIMEOUT_S = float(os.environ.get("AGENTNEXUS_HARNESS_SHUTDOWN_TIMEOUT_S", "5"))
...
_HARD_EXIT_TIMEOUT_S = float(
    os.environ.get(
        "AGENTNEXUS_HARNESS_HARD_EXIT_TIMEOUT_S",
...
def _load_harness_app(harness: str, module_path: str, conversation_id: str) -> FastAPI:
    ...
    :param module_path: Fully-qualified Python module to import,
        e.g. ``"agentnexus.inner.claude_sdk_harness"``. Must
    ...
    # (``AGENTNEXUS_HARNESS_AUTH_TOKEN``); the scaffold's auth check compares
    ...
    # the gate inert.
    app.state.harness_auth_token = os.environ.get("AGENTNEXUS_HARNESS_AUTH_TOKEN")
    return app
...
def _configure_logging(harness: str, conversation_id: str) -> None:
    ...
    Writes to ``AGENTNEXUS_PROCESS_LOG_FILE`` when the parent published one (the
    ...
    try:
        ...
        configure_process_logging(
            "harness",
            log_path=log_path,
            logger_names=("agentnexus", "uvicorn"),
        )
