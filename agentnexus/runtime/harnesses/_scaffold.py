...
# streaming. Matches the existing AgentNexus cadence at
...
# AgentNexus server → runner → harness). Held at one day (86400s) — matching
...
class PolicyVerdictPayload:
    ...
    evaluates the policy on the AgentNexus server and sends the verdict back
...
class TurnContext:
    ...
    :param response_id: Server-allocated id for this turn,
        ...
        ``response.created`` envelope so AgentNexus can correlate
    ...
    async def dispatch_tool(self, call_id: str, name: str, arguments: str, agent: str) -> str:
        ...
        the parked Future when AgentNexus delivers an event carrying the
        matching ``call_id``. See §How AgentNexus resolves action_required
        tool calls in the design doc.

        :param call_id: Unique identifier for this tool call,
            e.g. ``"call_abc123"``. AgentNexus echoes it back in the
    ...
    async def evaluate_policy(
        ...
        :returns: The verdict payload from the AgentNexus server.
        :raises asyncio.CancelledError: If the turn is cancelled.
        """
...
class HarnessApp:
    ...
    def build(self) -> FastAPI:
        ...
        """
        app = FastAPI(title="agentnexus-harness", lifespan=self._lifespan)
        ...
        # (Callable[[Request, Exception], ...]) — the AgentNexusError
        # subclass annotation is what we actually want at runtime, but
        # the static type doesn't accept it. Cast at the boundary.
        app.add_exception_handler(AgentNexusError, _handle_omnigent_error)  # type: ignore[arg-type]
    ...
    def _check_conversation_id(self, request: Request, conversation_id: str) -> None:
        ...
        same shape the AgentNexus side uses for unknown conversations
        ...
        :raises AgentNexusError: 500 if ``app.state.conversation_id``
    ...
    async def _post_session_event(
        ...
        :raises AgentNexusError: 404 on conversation_id mismatch,
    ...
    async def _handle_interrupt_event(self) -> Response:
        ...
        :raises AgentNexusError: 404 if no turn is in flight.
        """
    ...
    async def _build_terminal_event(
        ...
        if status_value == "completed":
            ...
            from agentnexus.server.schemas import CancelledEvent
    ...
    async def _resolve_elicitation(
        ...
        :returns: 204 on success.
        :raises AgentNexusError: 404 if no pending elicitation
...
async def _handle_omnigent_error(_request: Request, exc: AgentNexusError) -> Response:
    """
    Convert :class:`AgentNexusError` into a JSON response with the
    correct HTTP status.
