...
orphan sweep so a previous AgentNexus crash doesn't leave runner processes
behind.
...
from agentnexus.harness_plugins import missing_install_packages
from agentnexus.inner import _proc
from agentnexus.inner._subprocess_lifecycle import close_subprocess_transport
from agentnexus.runner.identity import strip_runner_auth_secrets
...
# sockets (POSIX) and the AP_PID sentinel file. Each AgentNexus instance gets a
# uuid-named subdir so concurrent AgentNexus processes (zero-downtime restarts,
...
if IS_WINDOWS:
    _TMP_PARENT = Path(tempfile.gettempdir()) / "agentnexus"
else:
    _TMP_PARENT = Path(f"/tmp/omnigent-{os.getuid()}")
_TMP_PARENT_ENV_VAR = "AGENTNEXUS_HARNESS_TMP_PARENT"
...
# depth.
_HARNESS_AUTH_TOKEN_ENV = "AGENTNEXUS_HARNESS_AUTH_TOKEN"

# Sentinel file the AgentNexus instance writes into its subdir on boot. The
# orphan sweep uses it to tell whether a sibling subdir belongs to
# a still-running AgentNexus (leave alone) or a crashed one (kill its
...
_HARNESS_IDLE_TIMEOUT_ENV = "AGENTNEXUS_HARNESS_IDLE_TIMEOUT_S"


def _resolve_harness_idle_timeout_s() -> float:
    """Resolve the harness idle-reap window in seconds.

    Honors :envvar:`AGENTNEXUS_HARNESS_IDLE_TIMEOUT_S` (``0`` disables reaping);
...
# enough that a wedged subprocess doesn't block AgentNexus shutdown.
...
# or model-thinking gaps. AgentNexus doesn't bound harness-side latency —
...
def _default_tmp_parent() -> Path:
    ...
    Operators can set :envvar:`AGENTNEXUS_HARNESS_TMP_PARENT` when
...
def _resolve_module_path(harness: str) -> str:
    ...
    raise RuntimeError(
        ...
        f"agentnexus.runtime.harnesses._HARNESS_MODULES)"
    )
    ...
    while True:
        if process.returncode is not None:
            ...
            raise RuntimeError(
                ...
                f"{process.returncode} during spawn (see AgentNexus stderr)"
            )
...
class _SubprocessEntry:
    ...
    :param client: The :class:`httpx.AsyncClient` AgentNexus uses to talk
...
class HarnessProcessManager:
    ...
    Use ``start()`` once at AgentNexus boot to create the per-instance
    ...
    terminal state, and ``shutdown()`` at AgentNexus shutdown to release
    ...
    :param tmp_parent: Override for the parent ``/tmp/omnigent``
        ...
        :envvar:`AGENTNEXUS_HARNESS_TMP_PARENT` to choose a
    ...
    def __init__(
        ...
        *,
        ...
        # AGENTNEXUS_HARNESS_IDLE_TIMEOUT_S env var, else the 1-hour default.
    ...
    async def forward_cancel(
        ...
        :param conversation_id: The AgentNexus conversation id whose
    ...
    async def release(
        ...
        conversation ids the AgentNexus instance has ever spawned for. If
    ...
    async def shutdown(self) -> None:
        ...
        AgentNexus exits after shutdown).
        """
        if not self._started:
            return
        ...
        # rmtree leaves it behind; the next AgentNexus boot's orphan
        # sweep handles it.
    ...
    async def _spawn_entry(
        ...
        # with AgentNexus logs. The runner doesn't currently prefix lines
    ...
    async def _idle_reaper_loop(self) -> None:
        ...
        ``AGENTNEXUS_HARNESS_IDLE_TIMEOUT_S=0``) disables reaping — each
        pass is a no-op.
        ...
        while True:
            ...
            # (``AGENTNEXUS_HARNESS_IDLE_TIMEOUT_S=0``). Without this guard a 0
    ...
    async def _sweep_orphans(self) -> None:
        ...
        for child in self._tmp_parent.iterdir():
            ...
            try:
                ...
                continue
            if _pid_alive(pid):
                # Sibling AgentNexus is still running — leave it alone.
                continue
            _logger.info(
                "sweeping orphaned AgentNexus instance dir %s (pid %d not running)",
                child,
                pid,
            )
