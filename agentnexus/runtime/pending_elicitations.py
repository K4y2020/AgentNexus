...
Lets the AgentNexus server answer "which sessions have a pending approval
...
lifecycle: when the AgentNexus process dies, both the index and every parked
...
* In-memory only; multi-replica AgentNexus deploys would each see their own
...
* Events emitted before the AgentNexus server starts (e.g. between turns,
...
def record_publish(conversation_id: str, event: dict[str, Any]) -> None:
    ...
    * ``response.elicitation_resolved`` — drop the elicitation id
      ...
      AgentNexus server picks it up via ``_relay_runner_stream`` without
