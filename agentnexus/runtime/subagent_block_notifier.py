...
``wake_dispatch`` coroutine (kept free of HTTP/server knowledge) wired up by the AgentNexus server
...
from agentnexus.runtime.subagent_questions import ordinary_question
...
class SubagentBlockNotifier:
    ...
    One instance is constructed and registered per AgentNexus server process.
    ...
    :param loop: The AgentNexus server's event loop, captured at registration
