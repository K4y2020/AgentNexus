"""
Fast ``MessageDisplay`` hook for the native AgentNexus Claude wrapper.
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        ...
        print(f"agentnexus message-display hook: malformed JSON: {exc}", file=sys.stderr)
        return 0
    if not isinstance(payload, dict):
        print("agentnexus message-display hook: expected JSON object", file=sys.stderr)
        return 0
    ...
    try:
        ...
        print(f"agentnexus message-display hook: write failed: {exc}", file=sys.stderr)
        return 0
    return 0
