...
from agentnexus._platform import IS_WINDOWS, resolve_repo_symlink
from agentnexus.cli_common import (
...
from agentnexus.cli_invocation import WRAPPER_COMMAND_ENV, cli_invocation
from agentnexus.cli_native import register_native_commands as _register_native_commands
from agentnexus.cli_sandbox import lakebox as _lakebox_alias_group
from agentnexus.cli_sandbox import sandbox as _sandbox_group
from agentnexus.config import (
...
from agentnexus.host.local_server import (
    _DEFAULT_LOCAL_PORT,
    _pid_alive,
...
from agentnexus.inner import _proc, ui
from agentnexus.integration_daemon import IntegrationDaemon
from agentnexus.json_types import JsonObject as _JsonObject
from agentnexus.onboarding.sandboxes import available_providers as _sandbox_providers
...
from agentnexus.server_url import ServerUrl
from agentnexus.server_url import org_id_from_url as _org_id_from_url
...
if TYPE_CHECKING:
    ...
    from agentnexus.install_ledger import InstallLedger
    from agentnexus.onboarding.acp_auth import AcpAgentEntry
    from agentnexus.server.smart_routing import LLMRoutingClient
    from agentnexus.smart_routing_cli import ArmedSession
    from agentnexus.spec.types import LLMConfig
    from agentnexus.update_check import _InstalledWheelInfo
...
def _databricks_provider_profile(
    ...
    if not isinstance(providers, dict):
        from agentnexus.onboarding.provider_config import load_config as load_provider_config
...
def _build_external_routing_client(
    ...
    from agentnexus.server.smart_routing import _bearer_auth
...
def _server_uvicorn_log_config(
    ...
    *,
    ...
    Uvicorn emits the FastAPI access line itself, so AgentNexus standardizes
    ...
    formatters["default"] = {
        "()": "agentnexus.process_logging.TerminalLogFormatter",
...
# Tests may set ``AGENTNEXUS_CONFIG_HOME`` to isolate subprocesses from a
...
# Overridable via AGENTNEXUS_SERVER_SHUTDOWN_TIMEOUT_S for deployments that
...
_SERVER_GRACEFUL_SHUTDOWN_TIMEOUT_S = int(
    os.environ.get(
        "AGENTNEXUS_SERVER_SHUTDOWN_TIMEOUT_S",
...
def _effective_global_config_path() -> Path:
    """
    Return the path to the user-level AgentNexus config.

    :returns: ``$AGENTNEXUS_CONFIG_HOME/config.yaml`` when the env
...
def _display_path(path: Path) -> str:
    ...
    hardcoded ``~/.agentnexus/...`` literal, this reflects the *actual*
    effective path — so a state dir outside ``$HOME`` (an
    ``AGENTNEXUS_CONFIG_HOME`` / ``AGENTNEXUS_DATA_DIR`` override) renders as
    its real location rather than a misleading ``~``.

    :param path: The path to display, e.g.
        ``Path("/Users/alice/.agentnexus/logs/server/server-ab12.log")``.
    :returns: ``"~/.agentnexus/..."`` when *path* is under ``$HOME``,
        otherwise ``str(path)``.
    """
    try:
        return f"~/{path.relative_to(Path.home())}"
    except ValueError:
        # Not under $HOME (e.g. an AGENTNEXUS_DATA_DIR outside home).
        return str(path)
...
def _display_config_path(path: Path) -> str:
    ...
    :param path: The config path to display, e.g.
        ``Path("/Users/alice/.agentnexus/config.yaml")``.
    :returns: ``"~/.agentnexus/config.yaml"`` when *path* is under
...
def _load_local_config() -> dict[str, Any]:  # type: ignore[explicit-any]
    """
    Load the project-level config from ``.agentnexus/config.yaml`` in cwd.
...
def _load_effective_config() -> dict[str, Any]:  # type: ignore[explicit-any]
    ...
    Precedence (highest last): global (``~/.agentnexus/config.yaml``)
    → local (``.agentnexus/config.yaml`` in cwd).  Project config
...
def _peek_default_agent_harness(target: str) -> str | None:
    ...
    :param target: The configured ``default_agent`` value, e.g.
        ``"/Users/me/.agentnexus/agents/databricks_coding_agent.yaml"``.
...
def _bundled_example_path(name: str) -> str:
    ...
    resource = importlib.resources.files("agentnexus.resources.examples").joinpath(name)
...
def _pick_first_run_harness() -> _FirstRunPlan | None:
    ...
    :returns: A :class:`_FirstRunPlan`, or ``None`` when no harness has a usable
        credential.
    """
    from agentnexus.onboarding.detected import effective_config_with_detected
    ...
    from agentnexus.onboarding.harness_install import KIMI_KEY, harness_cli_installed
...
def _resolve_default_agent_target(
    ...
    if default_harness is not None:
        click.echo(
            f"agentnexus: default agent '{default_agent}' uses harness "
...
def _normalize_harness_scalar_on_write(
    ...
    click.echo(
        f"agentnexus: migrated `harness:` to the new mapping form in {path} (behavior unchanged)",
        err=True,
    )
    return True
...
def _save_global_config(  # type: ignore[explicit-any]
    ...
    """
    Merge *settings* into ``~/.agentnexus/config.yaml`` and remove any
...
def _materialize_bundled_example(name: str) -> Path:
    ...
    user-editable copy under ``~/.agentnexus/agents`` and never overwrite an
    ...
    resource = resources.files("agentnexus.resources.examples").joinpath(name)
    text = resource.read_text(encoding="utf-8")
    executable_placeholder = "__AGENTNEXUS_PYTHON_EXECUTABLE__"
    text = text.replace('"${AGENTNEXUS_HOME:-$PWD}/.venv/bin/python"', executable_placeholder)
    text = text.replace("${AGENTNEXUS_HOME:-$PWD}/.venv/bin/python", executable_placeholder)
...
def _save_local_config(
    ...
    """
    Merge *settings* into ``.agentnexus/config.yaml`` in cwd and remove
    any keys listed in *unset_keys*.

    Creates the ``.agentnexus/`` directory if it does not exist. Mirrors
...
def _default_db_uri() -> str:
    ...
    :returns: e.g. ``"sqlite:////home/alice/.agentnexus/chat.db"``.
    """
    from agentnexus.host.local_server import _local_data_dir
...
def _default_artifact_location() -> str:
    ...
    :returns: e.g. ``"/home/alice/.agentnexus/artifacts"``.
    """
    from agentnexus.host.local_server import _local_data_dir
...
def _ensure_sqlite_parent_dir(db_uri: str) -> None:
    ...
    ``<data_dir>/chat.db`` (machine-global, honoring ``AGENTNEXUS_DATA_DIR``),
    ...
    :param db_uri: The resolved store DB URI, e.g.
        ``"sqlite:////home/alice/.agentnexus/chat.db"`` or
        ``"postgresql://host/db"``.
    :returns: None.
    """
...
def _apply_bind_auth_defaults(host: str) -> None:
    ...
    ``AGENTNEXUS_AUTH_PROVIDER`` keeps header/oidc,
    ``AGENTNEXUS_AUTH_ENABLED`` pins auth on/off, and a truthy
    ``AGENTNEXUS_LOCAL_SINGLE_USER`` declares a single-user server.
    ...
    - **Non-loopback + a truthy ``AGENTNEXUS_LOCAL_SINGLE_USER``** → leave
    ...
    # Compose-style deploys pass AGENTNEXUS_AUTH_PROVIDER as an empty
    ...
    _single_user_requested = env_var_is_truthy("AGENTNEXUS_LOCAL_SINGLE_USER")

    # Non-loopback + no explicit auth → accounts (login) mode.
    _auth_enabled_explicit = bool(os.environ.get("AGENTNEXUS_AUTH_ENABLED", "").strip())
    if (
        ...
        click.echo(
            ...
            "account.\n"
            "    Set AGENTNEXUS_AUTH_ENABLED=0 first to override and stay in "
            "single-user mode.",
            err=True,
        )
    else:
...
def _preregister_agent(  # type: ignore[explicit-any]  # agent_store / artifact_store / agent_cache typed Any to avoid import cycle
    ...
    # see designs/RUN_AGENTNEXUS_SESSION_RESUMPTION.md. Reusing the
...
def _format_version() -> str:
    ...
    :returns: Either ``"agentnexus 0.1.0"`` (no build info), or
        ``"agentnexus 0.1.0 (010cf77c, built 2026-05-21T14:34:45Z)"``.
    """
    import datetime

    from agentnexus.update_check import _read_build_info
    from agentnexus.version import VERSION

    version_str = VERSION
    info = _read_build_info()
    if info is None:
        return f"agentnexus {version_str}"
    epoch, sha = info
    ...
    if sha:
        ...
        return f"agentnexus {version_str} ({sha[:8]}, built {when})"
    # _build_info exists but has no SHA (built without git available).
    return f"agentnexus {version_str} (built {when})"
...
def _print_version_callback(ctx: click.Context, _param: click.Parameter, value: bool) -> None:
    ...
    attribute exists, ``from agentnexus import _build_info`` short-
...
def _harness_extra_checks() -> dict[str, Callable[[], bool]]:
    ...
    """
    from agentnexus.onboarding.cursor_auth import cursor_sdk_installed
...
class _AgentNexusCLI(click.Group):
    ...
    def format_help(self, ctx: click.Context, formatter: click.HelpFormatter) -> None:
        ...
        if ui.show_banner():
            ...
            epilogue = [("Get started", "agentnexus setup")]
            if VERSION:
...
def _finish_cli_profile(profiler: Any, output_path: Path) -> None:  # type: ignore[explicit-any]
    ...
    _print_rows("Top AgentNexus call paths", omnigent_rows)
    _print_rows("Top AgentNexus functions by self time", self_time_rows)
...
@click.group(cls=_AgentNexusCLI)
@click.option(
    "--profiling",
    is_flag=True,
    is_eager=True,
...
def _warn_deprecated_harness_path_env_vars() -> None:
    ...
    replaced by ``AGENTNEXUS_<NAME>_PATH`` (one var per binary, ``-native`` suffix
    ...
    if not sys.stderr.isatty():
        return
    from agentnexus.harness_startup_config import legacy_harness_path_env_vars_set

    for legacy, canonical in legacy_harness_path_env_vars_set():
        click.echo(
            f"agentnexus: {legacy} is deprecated; set {canonical} instead. "
            f"{legacy} support will be removed in v0.8.0.",
            err=True,
        )


REQUIRE_WRAPPER_ENV = "AGENTNEXUS_REQUIRE_WRAPPER"
WRAPPER_BYPASS_ENV = "AGENTNEXUS_WRAPPER_BYPASS"
...
def _wrapper_guard_error(env: Mapping[str, str], prog: str) -> str | None:
    ...
    ``AGENTNEXUS_REQUIRE_WRAPPER`` so direct calls are refused; the wrapper sets
    ``AGENTNEXUS_WRAPPER_BYPASS`` around its own invocation to pass through, and
    ``AGENTNEXUS_WRAPPER_COMMAND`` names the command to suggest instead.
    """
...
def main() -> None:
    ...
    # and warnings to ~/.agentnexus/logs/cli/cli-*.log even when --log
    ...
    from agentnexus.cli_diagnostics import (
        log_cli_error_hint,
        log_cli_exception,
    ...
    if not _should_skip_update_check(argv) and sys.stderr.isatty():
        from agentnexus.update_check import maybe_show_update_notice

        maybe_show_update_notice()

    # Terminal-visible deprecation notice for legacy ``HARNESS_*_PATH`` env
    # vars (now ``AGENTNEXUS_<NAME>_PATH``). Same gating as the update notice so
    ...
    try:
        ...
        from agentnexus.crash_handler import handle_crash
...
# for a daemon that owns a local AgentNexus server. Daemon reuse is keyed on this
...
class _HostDaemonRecord:
    ...
    :param log_path: Daemon log file path, e.g.
        ``"/Users/me/.agentnexus/logs/host/host-abc.log"``.
    :param started_at: Unix epoch seconds when the daemon was spawned,
        e.g. ``1710000000``.
    :param host_id: Local host id advertised to AgentNexus servers, e.g.
...
class _HostHttpResult:
    """
    Decoded AgentNexus management HTTP response.
...
def _normalize_daemon_target(server_url: str | None) -> str:
    ...
    :param server_url: Requested AgentNexus server URL, e.g.
...
def _daemon_host_online(record: _HostDaemonRecord, *, timeout_s: float = 2.0) -> bool:
    ...
    tunnel to the AgentNexus server is up: the server only reports the host
    ...
    """
    from agentnexus.claude_native_bridge import url_component
...
def _read_daemon_record(path: Path) -> _HostDaemonRecord | None:
    ...
    :param path: JSON file path to read, e.g.
        ``Path("~/.agentnexus/daemons/abc.json")``.
...
def _update_daemon_resolved_server_url(target: str, server_url: str) -> None:
    """
    Record the concrete AgentNexus server URL served by a daemon target.
...
def _load_existing_host_id() -> str | None:
    ...
    from agentnexus.host.identity import CONFIG_PATH
...
def _daemon_host_identity_changed(record: _HostDaemonRecord) -> bool:
    ...
    A live daemon can outlast edits to ``~/.agentnexus/config.yaml``. Reusing
...
def _terminate_host_unit(record: _HostDaemonRecord, *, reason: str) -> None:
    """
    Tear down a daemon and, in local mode, the AgentNexus server it owns.

    The ``--local`` daemon spawns its AgentNexus server once and never respawns
...
class _DaemonReuseDecision:
    ...
    :param config_changed: ``True`` when the existing daemon was torn down
        ...
        invocation (e.g. the user flipped ``AGENTNEXUS_AUTH_ENABLED``).
...
def _reuse_existing_daemon_record(target: str) -> _DaemonReuseDecision:
    ...
    ``AGENTNEXUS_AUTH_ENABLED``) would silently keep its old auth
...
def _foreground_daemon_record(
    *,
    ...
    :param target: Normalized daemon target, e.g.
        ``"https://example.databricksapps.com"`` or ``"local"``.
    :param server_url: Concrete AgentNexus server URL being connected to, e.g.
        ``"http://127.0.0.1:8123"``.
...
def _load_or_create_host_id() -> str | None:
    ...
    if host_id is not None:
        return host_id
    from agentnexus.host.identity import CONFIG_PATH, load_or_create_host_identity
...
def _ensure_host_daemon(server_url: str | None) -> bool:
    ...
    :param server_url: AgentNexus server URL the daemon connects to, or ``None``
        for local mode — the daemon starts (or reuses) a persistent local
        AgentNexus server and connects to that.
    ...
    args = [sys.executable, "-m", "agentnexus.host._daemon_entry", *mode_args]
...
def _build_host_daemon_env(
    *,
    ...
    Remote daemons connect to an already-running AgentNexus server, so they only
    need process essentials, TLS trust, and Databricks auth. Local daemons
    also start the local AgentNexus server; that server is the user's local runtime
    and must inherit AgentNexus config plus provider credentials such as
    ...
    :param server_url: AgentNexus server URL for remote mode, e.g.
...
def _ensure_databricks_server_auth(server: str, *, non_interactive: bool = False) -> None:
    ...
    from agentnexus.chat import _remote_headers
    from agentnexus.cli_auth import load_databricks_org_id, store_databricks_auth
...
def _ensure_backend(server: str | None) -> str:
    """Ensure the host daemon is running and return the AgentNexus server URL.

    The daemon is the single backend for ``attach`` / ``run`` / ``claude`` /
    ``codex``: it spawns the runner and, in local mode, the AgentNexus server too.
    ...
    :param server: ``--server`` value after config fallback. A non-empty
        ...
        persistent local AgentNexus server and this returns its discovered loopback
        URL.
    ...
    :raises click.ClickException: If local mode's server never becomes
        reachable.
    """
    from agentnexus._runner_startup import (
    ...
    if server:
        ...
        return server
    # Local mode: the daemon spawns (or reuses) a persistent local AgentNexus server.
...
def _exit_for_auth_mode_change(base_url: str) -> None:
    ...
    The local AgentNexus server bakes its auth posture (header vs accounts, cookie
    secret) at boot, so an ``AGENTNEXUS_AUTH_ENABLED`` flip restarts it
    ...
    :param base_url: The freshly-restarted AgentNexus server URL, e.g.
        ``"http://127.0.0.1:6767"``.
...
def _discover_local_server_url(
    ...
    """Poll until the daemon-started local AgentNexus server is reachable.

    In local mode the daemon owns the AgentNexus server; the CLI discovers its URL
    ...
    while time.monotonic() < deadline:
        ...
        if not _host_daemon_alive():
            raise click.ClickException(
                "The local daemon exited before its AgentNexus server became ready. "
                f"See logs under {process_log_dir_reference('host')} and "
                f"{process_log_dir_reference('server')}."
            )
        time.sleep(0.2)
    raise click.ClickException(
        f"Timed out after {timeout:.0f}s waiting for the local AgentNexus server to "
...
def _start_cli_runner_process(
    *,
    ...
    :param log_dir: Optional base log directory to use when
        ...
        ``~/.agentnexus/logs`` location; tests should pass a
    ...
    try:
        with child_logging_popen_kwargs(env) as logging_kwargs:
            runner_proc: subprocess.Popen[bytes] = subprocess.Popen(
                ...
                [sys.executable, "-P", "-m", "agentnexus.runner._entry"],
                env=env,
                stdout=log_fh,
                stderr=log_fh,
                **_proc.spawn_kwargs(),
    ...
    if runner_proc.poll() is not None:
        from agentnexus._runner_startup import format_runner_log_tail
...
def _adopt_cli_runner_process(proc: subprocess.Popen[bytes]) -> None:
    ...
    :returns: None.
    """
    from agentnexus.runner.identity import RUNNER_ADOPT_SIGNAL
...
@click.option(
    ...
    help=(
        ...
        "(alternative to AGENTNEXUS_ACCOUNTS_INIT_ADMIN_PASSWORD). Only "
...
@click.option(
    ...
    help=(
        ...
        "local server recorded in ~/.agentnexus/local_server.pid) instead of "
...
def server(
    ...
    :param auto_open: Whether to open the magic-redeem URL in the
        ...
        into the ``AGENTNEXUS_ACCOUNTS_AUTO_OPEN`` env var so the
    ...
    :param admin_password: Optional first-run accounts admin password
        ...
        ``AGENTNEXUS_ACCOUNTS_INIT_ADMIN_PASSWORD`` env var that
    ...
    if admin_password:
        os.environ.setdefault("AGENTNEXUS_ACCOUNTS_INIT_ADMIN_PASSWORD", admin_password)
    ...
    if config_path:
        os.environ["AGENTNEXUS_CONFIG"] = str(Path(config_path).resolve())
    ...
    _tunnel_token = os.environ.get("AGENTNEXUS_RUNNER_TUNNEL_TOKEN")
    ...
    # (AGENTNEXUS_AUTH_ENABLED=1 without OIDC config, or an explicit
    # AGENTNEXUS_AUTH_PROVIDER=accounts), supply sensible defaults
    ...
    if resolve_auth_source() == "accounts":
        ...
        os.environ.setdefault("AGENTNEXUS_ACCOUNTS_BASE_URL", f"http://{host}:{port}")
    ...
    from agentnexus.process_logging import configure_process_logging

    server_log_path = configure_process_logging(
        "server",
        logger_names=("agentnexus", "uvicorn", "uvicorn.error", "uvicorn.access"),
    )
    ...
    # static dir.
    from agentnexus.server.app import _WEB_UI_DIST
    ...
    class _ShutdownSignalingServer(uvicorn.server.Server):
        ...
        async def shutdown(self, sockets: list[socket.socket] | None = None) -> None:
            ...
            from agentnexus.runtime import session_stream as _session_stream
            from agentnexus.server import shutdown_state as _shutdown_state
    ...
    try:
        ...
        if _is_canonical_local_server:
            from agentnexus.host.local_server import clear_local_server_record

            clear_local_server_record()


def _stop_local_server_and_daemon(*, force: bool) -> bool:
    """Stop the background AgentNexus server and the local host daemon that owns it.
    ...
    reconnect-flap against a dead server), then the detached AgentNexus server
    recorded in ``~/.agentnexus/local_server.pid``. Best-effort and
...
def server_start() -> None:
    ...
    """
    click.echo(
        f"agentnexus: `server start` is deprecated; use `{cli_invocation()} server --background`.",
        err=True,
    )
    _run_background_server()
...
def server_stop(force: bool) -> None:
    """Stop the background AgentNexus server and the local host daemon.
...
def server_status(json_output: bool) -> None:
    """Show whether the background AgentNexus server is running.

    Reports the recorded pid/port, URL, live-session count, and whether a
    local host daemon is attached. Reads ``~/.agentnexus/local_server.pid``
    and probes ``/health``.
...
@cli.command("start")
@click.option("--server", default=None, help="AgentNexus server URL to host on.")
@click.option(
    "--non-interactive",
    "non_interactive",
    is_flag=True,
...
def start(server: str | None, non_interactive: bool) -> None:
    """Start AgentNexus on this machine, in the background.
...
def stop(force: bool) -> None:
    """Stop everything AgentNexus is running on this machine.
...
def _uninstall_script_path() -> Path:
    ...
    if repo_script.exists():
        return repo_script
    try:
        resource = resources.files("agentnexus.resources.scripts").joinpath("uninstall_oss.sh")
    except ModuleNotFoundError as exc:
        raise click.ClickException("uninstall script is missing from this installation") from exc
    with resources.as_file(resource) as path:
        if path.exists():
            temp_dir = Path(tempfile.mkdtemp(prefix="agentnexus-uninstall-"))
...
def _write_uninstall_manifest(ledger: InstallLedger) -> Path:
    ...
    fd, manifest_name = tempfile.mkstemp(prefix="agentnexus-uninstall-ledger-", suffix=".tsv")
...
def _maybe_fast_backfill_install_ledger(argv: Sequence[str]) -> None:
    ...
    with contextlib.suppress(Exception):
        from agentnexus.install_ledger import backfill_install_ledger
...
def _internal_write_ledger(from_env: bool) -> None:
    ...
    del from_env
    from agentnexus.install_ledger import ledger_path, write_install_ledger_from_env
...
def doctor(
    ...
    from agentnexus.install_ledger import backfill_install_ledger, backfill_ledger_path
    ...
    if json_output:
        ...
        click.echo("No AgentNexus install detected; no ledger written.")
    elif apply_changes:
...
def uninstall(
    ...
    """Uninstall AgentNexus from this machine."""
    from agentnexus.install_ledger import resolve_uninstall_ledger
                ...
                json.dumps(
                    {
                        ...
                        "exit_code": 3,
                        "error": "no AgentNexus install detected",
                    },
                    indent=2,
                )
            )
            raise SystemExit(3)
        click.echo("No AgentNexus install detected; nothing to uninstall.", err=True)
        raise SystemExit(3)
    ...
    for enabled, flag in (
        ...
        if enabled:
            args.append(flag)
    env = os.environ.copy()
    env["AGENTNEXUS_UNINSTALL_LEDGER_SOURCE"] = ledger.ledger_source
    ...
    try:
        ...
        if (
            ...
            and script_path.parent.name.startswith("agentnexus-uninstall-")
...
def _upgrade_vcs_install(
    ...
    *,
    ...
    if remote_sha and current_sha and remote_sha == current_sha:
        click.echo(f"agentnexus is up to date (git {cur_short}, tracking {info.vcs_url}).")
        return
    if known_behind:
        click.echo(
...
def _upgrade_to_nightly(
    ...
    *,
    ...
    current = importlib.metadata.version("agentnexus")
    ...
    if up_to_date:
        click.echo(f"agentnexus is already on the newest nightly (v{current}).")
        return
    ...
    if info.vcs_url is None:
        ...
        if info.detected_installer == "pip":
            click.echo(
                "agentnexus was installed with pip, and pip does not record which "
        ...
        if info.detected_installer == "uv" and _uv_tool_receipt_path() is None:
            click.echo(
                "agentnexus was installed with `uv pip`, not `uv tool install`. "
    ...
    if target_version:
        ...
        if latest is None:
            raise click.ClickException(
                ...
                "connection (or AGENTNEXUS_INDEX_URL / your configured index) and try again."
            )
        if not _is_newer(latest, current):
            click.echo(f"agentnexus is up to date (v{current}).")
            return
...
def _bundle(source: Path) -> bytes:
    ...
    AgentNexus YAML file, or pass through an existing tarball.
    ...
    :param source: Path to an agent image directory,
        standalone AgentNexus YAML file, or an existing
        ``.tar.gz`` bundle file.
    :returns: The gzipped tarball bytes.
    :raises AgentNexusError: If a required env var is
    ...
    if source.is_file() and source.suffix.lower() in {".yaml", ".yml"}:
        from agentnexus.spec import materialize_bundle
...
def _resolve_bundle_env_vars(source: Path) -> dict[str, str]:
    ...
    :raises AgentNexusError: If a ``${VAR}`` reference
...
def resume(
    ...
    #
    # :param target: Optional AgentNexus conversation id, e.g.
    #     ``"conv_abc123"``. None falls through to the picker.
    # :param server: Remote AgentNexus server URL (optional in id mode;
    #     required in picker mode).
    """Resume an AgentNexus conversation, auto-dispatching by runtime.
...
@click.option(
    ...
    default=None,
    help=(
        "AgentNexus server URL. Defaults to the configured server, an existing "
...
def import_session_command(
    ...
    The source transcript is converted to ordinary AgentNexus items and stored
    ...
    from agentnexus.session_import.local import (
    ...
    def _import_one(target: tuple[ImportSource, str]) -> _SessionImportResult:
        ...
        try:
            ...
            return _SessionImportResult(
                sid,
                "unreachable",
                message=f"Could not reach the AgentNexus server: {exc}",
                raw_exc=exc,
            )
    ...
    if unreachable is not None:
        raise click.ClickException(unreachable.message or "Could not reach the AgentNexus server")
...
def usage(limit: int, server: str | None, as_json: bool) -> None:
    """Show your AgentNexus usage and costs.
...
def _import_agent_aliased_types() -> frozenset[str]:
    ...
    """
    from agentnexus.entities.conversation import ITEM_TYPE_TO_DATA_CLS
...
@click.option(
    ...
    required=True,
    metavar="FILE",
    help="Path to a JSONL file produced by 'agentnexus session export'.",
)
@click.option(
    "--title",
    default=None,
...
_FROM_OPENCLAW_HELP = (
    "Launch one agent from the OpenClaw/acpx registry without saving it to AgentNexus config."
)
...
def _run_smart_routing_removed() -> str:
    # Wrapper resolved at call time so the hint honors AGENTNEXUS_WRAPPER_COMMAND.
    return (
...
_LOG_HELP = "Write a JSON dump of the conversation to ~/.agentnexus/logs/ on exit."


_DEFAULT_HARNESS_PROMPTS = {
    "claude-sdk": (
        "You are Claude Code, running through AgentNexus. "
    ...
    "qwen": (
        "You are Qwen Code, running through AgentNexus. "
        "Help the user with software engineering tasks."
    ),
    "goose": (
        "You are Goose, running through AgentNexus. Help the user with software engineering tasks."
    ),
}
_DEFAULT_HARNESS_PROMPT = "You are a helpful coding agent running through AgentNexus."
...
# operations route through the AgentNexus dispatch path (runner
...
def _validate_harness(harness: str) -> None:
    """
    Fail fast when *harness* is not a supported AgentNexus harness.
    ...
    if canonicalize_harness(harness) in AGENTNEXUS_HARNESSES:
        return
    allowed = ", ".join(sorted(AGENTNEXUS_HARNESSES))
...
def _default_harness_prompt(harness: str) -> str:
    ...
    :returns: Prompt text for the generated AgentNexus YAML.
    """
...
def _materialize_harness_launcher_file(
    *,
    ...
    """
    Create a temporary standalone AgentNexus YAML for no-AGENT ``run``.
    ...
    request — routing file/shell operations through the AgentNexus
    ...
    tmpdir = Path(tempfile.mkdtemp(prefix="agentnexus-harness-launcher-"))
    ...
    if acp_agent is not None:
        ...
        if not acp_agent.agentnexus_mcp:
            agent_dict["agentnexus_mcp"] = acp_agent.agentnexus_mcp
...
def _build_resume_parts() -> list[str]:
    ...
    :returns: Argument list prefix, e.g.
        ``["agentnexus", "run", "agent.yaml", "--server",
        "https://example.com"]``.
    """
...
def _dispatch_native_terminal_harness(
    *,
    ...
    AgentNexus turn per message — which persists its own user item — *while*
    ...
    if session_id is None and not resume_picker and resume_latest:
        from agentnexus.chat import _remote_headers, _resolve_latest_conversation_id
...
def _smart_routing_decision(*, server: str, harness: str) -> ArmedSession:
    ...
    *harness*'s own first-message hook picks the model once the user types,
    ...
    :param server: Resolved AgentNexus server base URL.
    ...
    host_id: str | None
    try:
        from agentnexus.host.identity import load_or_create_host_identity
    ...
    click.echo(
        armed.notice
        or "agentnexus: Smart Routing is on for this session; your first message picks the model.",
        err=True,
    )
    return armed
...
def _reject_agent_with_native_terminal_harness(harness: str) -> None:
    ...
    double-record every message (AgentNexus turn + forwarder mirror). So an
...
def _dispatch_run(
    *,
    ...
    :param server: Server URL from ``--server`` or config. With a local
        target, this is the AgentNexus server used for upload/session setup; with
    ...
    :param resume_parts: Pre-built argument list prefix for the
        ...
        ``["agentnexus", "run", "agent.yaml", "--harness", "codex"]``.
    ...
    if target is None:
        ...
        if server_from_cli and server and harness is None:
            ...
            if resume_conversation_id is not None and is_interactive_shape:
                from agentnexus.chat import _redirect_native_resume_if_needed, run_attach
            ...
            from agentnexus.chat import run_chat
        ...
        # user message (AgentNexus turn + forwarder mirror). Returns False for
        ...
        # every message (AgentNexus turn + forwarder mirror, same as the no-AGENT
    ...
    if prompt is not None:
        ...
        from agentnexus.chat import run_prompt
...
def _resolve_attach_server(server: str | None, configured_server: str | None) -> str | None:
    """
    Resolve the AgentNexus server URL ``attach`` should join.

    Resolution order: an explicit ``--server`` value, then the configured
    ``server`` default, then a local AgentNexus server already running in the
...
@click.option(
    ...
    help=(
        ...
        "overlay, JSONL event log (~/.agentnexus/debug/), and "
...
def attach(
    ...
    from agentnexus.chat import run_attach
...
def run(
    ...
    """Start a session with an AgentNexus agent.
    ...
    if acp_agent is None and harness is not None and harness.startswith("acp:"):
        from agentnexus.onboarding.acp_auth import resolve_acp_agent
    ...
    if harness is None and not direct_server_cli:
        from agentnexus.harness_startup_config import resolve_harness_config
    ...
    # NOTE: the host daemon + AgentNexus server are ensured inside ``run_chat``'s
...
def _prompt_stop_local_server() -> None:
    """Ask whether to also stop the detached local AgentNexus server after exit.
...
def _maybe_open_host_web_ui(
    ...
    *,
    ...
    if _resolve_auto_open_conversation_setting(cfg) is False:
        return
    from agentnexus.conversation_browser import open_conversation_url
    from agentnexus.server_url import display_server_url
    ...
    try:
        ...
        opened = False
    if not opened:
        click.echo(f"Open the AgentNexus web UI: {web_url}", err=True)
...
def _run_background_host(
    ...
    *,
    ...
    :param server: Resolved AgentNexus server URL, e.g.
        ``"https://example.databricksapps.com"``. ``None`` or ``""`` selects
        local mode (the daemon starts or reuses a local AgentNexus server).
...
def _host_stop_command(explicit_server: str | None) -> str:
    ...
    :returns: A copy-pasteable command, e.g. ``"agentnexus host stop"``.
    """
...
def host(
    ...
    :param server: Remote AgentNexus server URL, e.g.
    ...
    from agentnexus.host.connect import run_host_process

    # ``host`` IS the daemon (foreground). With no server URL, start (or
    # reuse) the local AgentNexus server here and connect to it; otherwise connect to
...
def _daemon_base_url(record: _HostDaemonRecord) -> str | None:
    """
    Resolve the AgentNexus server URL for a daemon record.

    :param record: Daemon registry record to inspect.
    :returns: AgentNexus server URL, e.g. ``"http://127.0.0.1:8123"``, or
...
def _host_http_json(
    *,
    ...
    """
    Send one management request to an AgentNexus server.
...
def _host_error_text(body: _HostJsonObject | str) -> str:
    """
    Extract a concise error string from an AgentNexus response body.
    ...
    *,
    ...
    if base_url is None:
        return _DaemonSessionsResult(
            base_url=None,
            sessions=[],
            error="local AgentNexus server is not reachable",
        )
    host_id = record.host_id or _load_existing_host_id()
    if not host_id:
        return _DaemonSessionsResult(
...
def _add_daemon_host_status(
    ...
    if not isinstance(base_url, str):
        payload["error"] = "local AgentNexus server is not reachable"
        return
...
def _host_truncate(text: _HostJsonValue, *, max_chars: int) -> str:
    ...
    :param text: Value to truncate, e.g. an AgentNexus error message.
...
def host_enable(
    ...
    from agentnexus.host.service import HostServiceError, enable_user_host_service
    ...
    click.echo(f"Enabled the AgentNexus host user service for {_host_display_url(target)}.")
...
def host_disable() -> None:
    ...
    from agentnexus.host.service import HostServiceError, disable_user_host_service
    ...
    for record in _list_daemon_records():
        if not _pid_alive(record.pid):
            _delete_daemon_record(record)
    click.echo("Disabled the AgentNexus host user service.")
...
def _stop_session_on_server(
    *,
    ...
    """
    Stop one AgentNexus session via the server lifecycle event API.
...
def host_stop_session(
    ...
    :param server: AgentNexus server URL that owns the sessions, e.g.
        ``"https://example.databricksapps.com"``. ``None`` falls back
    ...
    if resolved_server is None:
        ...
        if resolved_server is None:
            raise click.ClickException(
                "No server was supplied and no local AgentNexus server is reachable."
            )
...
def version() -> None:
    """Print the installed AgentNexus version."""
...
def _resolve_harness_startup_args(
    ...
    *cli_args* append *after* so a per-invocation flag wins for last-wins CLIs.
    ...
    """
    from agentnexus.harness_startup_config import resolve_harness_args
...
def _print_config_defaults() -> None:
    ...
    # (``cwd/.agentnexus/config.yaml``) resolves to the SAME file as the
    # user-level path (``~/.agentnexus/config.yaml``). Dedup on the resolved
...
def _integration_state_dir() -> Path:
    """Runtime dir for integration daemon records (honors AGENTNEXUS_DATA_DIR)."""
    from agentnexus.host.local_server import _local_data_dir
...
def integration(ctx: click.Context) -> None:
    """Run and manage AgentNexus chat integrations.
...
def slack(ctx: click.Context, background: bool) -> None:
    ...
    Config (Slack tokens, AGENTNEXUS_SERVER_URL, …) comes from environment
    ...
    click.echo("Starting the AgentNexus Slack bot (foreground). Press Ctrl-C to stop.")
...
def _start_slack_background() -> None:
    ...
    click.echo(f"Started the AgentNexus Slack bot in the background (pid {record.pid}).")
...
def slack_stop() -> None:
    ...
    if record is None:
        click.echo("Slack bot: not running.")
        return
    click.echo(f"Stopped the AgentNexus Slack bot (pid {record.pid}).")
...
@click.option(
    ...
    is_flag=True,
    default=False,
    help="Write to ~/.agentnexus/config.yaml (user-level) instead of the project config.",
)
@click.argument("settings", nargs=-1, required=True, metavar="KEY=VALUE...")
def config_set(is_global: bool, settings: tuple[str, ...]) -> None:
    """Set one or more AgentNexus defaults.

    Without ``--global``, pairs are written to ``.agentnexus/config.yaml``
    in the current directory (project-level, like ``.git/config``); with
    ``--global`` to ``~/.agentnexus/config.yaml`` (user-level, like
    ...
    :param is_global: When ``True``, write to ``~/.agentnexus/config.yaml``;
        when ``False``, to ``.agentnexus/config.yaml`` in cwd.
    :param settings: ``KEY=VALUE`` pairs to set, e.g.
        ``("default_agent=examples/hello.yaml", "model=gpt-5.4-mini")``.
...
@click.option(
    ...
    is_flag=True,
    default=False,
    help="Remove from ~/.agentnexus/config.yaml (user-level) instead of the project config.",
)
@click.argument("keys", nargs=-1, required=True, metavar="KEY...")
def config_unset(is_global: bool, keys: tuple[str, ...]) -> None:
    """Remove one or more AgentNexus defaults.

    :param is_global: When ``True``, remove from ``~/.agentnexus/config.yaml``;
        when ``False``, from ``.agentnexus/config.yaml`` in cwd.
...
def setup(internal_beta: bool) -> None:
    """
    Launch the AgentNexus first-time setup flow.
    ...
    if internal_beta:
        ...
        try:
            import agentnexus.onboarding.internal_beta  # type: ignore[import-not-found]  # noqa: F401
        except ImportError:
            raise click.ClickException(
        ...
        from agentnexus.onboarding.databricks_config import (
        ...
        _warn_missing_harness_dependencies()
        from agentnexus.onboarding.internal_beta import _INTERNAL_BETA_DEFAULT_SERVER
        from agentnexus.onboarding.sandboxes.lakebox import (  # type: ignore[import-not-found]
            install_demo_databricks_cli,
        )
        from agentnexus.onboarding.setup import run_onboarding
...
#
# ``db-upgrade`` runs manual schema operations on an AgentNexus tracking
...
def debug_db_upgrade(url: str) -> None:
    """
    Upgrade the schema of an AgentNexus tracking database to the
    ...
    from agentnexus.db.utils import _run_migrations
...
def debug_migrate_accounts_to_oidc(
    ...
    against your live DB *before* flipping ``AGENTNEXUS_AUTH_PROVIDER``.
    ...
    if not report.committed:
        ...
        click.echo("\nDone. Flip AGENTNEXUS_AUTH_PROVIDER=oidc and restart.\n")
...
def debug_logs(
    ...
    \b
    Log locations (relative to ~/.agentnexus or $AGENTNEXUS_DATA_DIR):
...
def _cached_workspace_bearer(workspace_host: str) -> str | None:
    ...
    """
    from agentnexus.onboarding.databricks_config import databricks_sdk_installed
...
def _resolve_server_url(server: str) -> ServerUrl:
    ...
    """
    from agentnexus.conversation_browser import strip_conversation_path
    from agentnexus.server_url import display_server_url
...
def _databricks_default_workspace_id(workspace_host: str) -> str | None:
    ...
    """
    from agentnexus.inner.databricks_executor import databrickscfg_workspace_id_for_host
...
def _workspace_hosted_profile_org_id(
    ...
    """Return the CLI-recorded workspace id for workspace-hosted AgentNexus."""
    from agentnexus.server_url import is_workspace_hosted_url

    if not is_workspace_hosted_url(server):
        return None
    if profile_name:
        from agentnexus.inner.databricks_executor import databrickscfg_workspace_id_for_profile
...
def _databricks_login(server: str, workspace_host: str, org_id: str | None = None) -> None:
    """Log in to a Databricks-fronted AgentNexus server.
    ...
    stored in ``~/.agentnexus/auth_tokens.json`` — no profile name is
    ...
    from agentnexus.cli_auth import is_workspace_hosted_url
...
def login(server_url: str) -> None:
    """Authenticate with a remote AgentNexus server.
    ...
    - accounts mode: prompts for username + password (no browser
      ...
      ``~/.agentnexus/auth_tokens.json`` keyed by server URL.
    ...
    if probe.status_code == 401:
        ...
        # 401 with non-JSON body — probably not an AgentNexus server.
    ...
    try:
        ...
        raise click.ClickException(
            ...
            f"Is the server running with AGENTNEXUS_AUTH_PROVIDER=oidc?"
        ) from exc
...
def pane_split(direction: str | None, parent_pane: str) -> None:
    ...
    ``run-shell 'agentnexus pane-split -<v|h|w> -p #{pane_id}'`` when
    ...
    # "agentnexus.cli"]`` as a fallback that always works.
    picker_argv = [
...
def _strip_resume_flags(argv: list[str]) -> list[str]:
    ...
    :param argv: Parent's launch argv, e.g.
        ``["python", "-m", "agentnexus.cli", "run", "agent.yaml",
...
def _ensure_bundled_agent_brain_credential(name: str) -> None:
    ...
    from agentnexus.onboarding.configure_models import family_label
    from agentnexus.onboarding.detected import effective_config_with_detected
    from agentnexus.onboarding.provider_config import (
        default_provider_for_harness,
        harness_family,
    ...
    try:
        ...
        return
    except (OSError, yaml.YAMLError, AgentNexusError):
        return
...
def _reject_reserved_kiro_resume_args(kiro_args: tuple[str, ...]) -> None:
    ...
    if any(arg == flag or arg.startswith(f"{flag}=") for arg in kiro_args for flag in reserved):
        raise click.UsageError(
            "Kiro resume flags are reserved for AgentNexus resume handling; use "
