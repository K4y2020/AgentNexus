...
vendor CLI inside an AgentNexus-managed terminal. They were extracted from
...
top-level ``import agentnexus.cli`` (no import cycle) and a test's
...
def register_native_commands(cli: click.Group) -> None:
    ...
    @click.option(
        ...
        default=None,
        help=(
            "Resume a prior AgentNexus conversation. With a conversation id "
    ...
    @click.option(
        ...
        help=(
            ...
            "``AGENTNEXUS_CLAUDE_PATH`` env var or the "
    ...
    def claude(
        ...
        # :param server: Remote AgentNexus server URL, or None for local.
        ...
        """Launch Claude Code with AgentNexus.
        ...
        # remote otherwise) and resolve the concrete AgentNexus server URL. The daemon
        ...
        from agentnexus.claude_native import run_claude_native
        from agentnexus.harness_startup_config import resolve_harness_command

        startup_profiler.mark("native module imported")

        if claude_command:
            click.echo(
                "agentnexus: `claude --command` is deprecated; set AGENTNEXUS_CLAUDE_PATH "
    ...
    def codex(
        ...
        from agentnexus.codex_native import run_codex_native
        from agentnexus.harness_startup_config import resolve_harness_command
        ...
        # Ensure the host daemon (local when ``--server`` is omitted/empty,
        # remote otherwise) and resolve the concrete AgentNexus server URL. Codex follows
    ...
    def opencode(
        ...
        # ``AGENTNEXUS_OPENCODE_PATH`` or ``harness.opencode-native.command`` config.
        ...
        """Launch OpenCode with AgentNexus.
        ...
        """
        from agentnexus.opencode_native import run_opencode_native
    ...
    def pi(
        ...
        from agentnexus.harness_startup_config import resolve_harness_command
        from agentnexus.pi_native import run_pi_native

        cfg = _load_effective_config()
        # Thread ``harness.pi-native.command`` config into the runner via the
        # canonical ``AGENTNEXUS_PI_PATH`` env var (set before ``_ensure_backend``
        ...
        # override via ``AGENTNEXUS_PI_PATH`` or config.
        _resolved = resolve_harness_command("pi-native", default="", explicit=None, cfg=cfg)
        if _resolved:
            os.environ["AGENTNEXUS_PI_PATH"] = _resolved
        if server is None:
            server = cfg.get("server")
        auto_open_conversation = _resolve_auto_open_conversation_from_config(cfg)
    ...
    def cursor(
        ...
        from agentnexus.cursor_native import run_cursor_native
        from agentnexus.harness_startup_config import resolve_harness_command

        cfg = _load_effective_config()
        # Thread ``--command`` / ``harness.cursor-native.command`` config into the
        # runner via the canonical ``AGENTNEXUS_CURSOR_PATH`` env var (set before
        ...
        if _resolved:
            os.environ["AGENTNEXUS_CURSOR_PATH"] = _resolved
        if server is None:
            server = cfg.get("server")
    ...
    def kiro(
        ...
        from agentnexus.harness_startup_config import resolve_harness_command
        from agentnexus.kiro_native import run_kiro_native

        cfg = _load_effective_config()
        # Thread ``--command`` / ``harness.kiro-native.command`` config into the
        # runner via the canonical ``AGENTNEXUS_KIRO_PATH`` env var (set before
        # ``_ensure_backend`` so a locally-spawned daemon inherits it; a remote
        # ``--server`` runner reads its own host env, so set the var there).
        _resolved = resolve_harness_command("kiro-native", default="", explicit=None, cfg=cfg)
        if _resolved:
            os.environ["AGENTNEXUS_KIRO_PATH"] = _resolved
        if server is None:
            server = cfg.get("server")
        if model is None:
            model = cfg.get("model")
    ...
    def goose(
        ...
        from agentnexus.goose_native import run_goose_native
        from agentnexus.harness_startup_config import resolve_harness_command

        cfg = _load_effective_config()
        # Thread ``--command`` / ``harness.goose-native.command`` config into the
        # runner via the canonical ``AGENTNEXUS_GOOSE_PATH`` env var (set before
        # ``_ensure_backend`` so a locally-spawned daemon inherits it; a remote
        # ``--server`` runner reads its own host env, so set the var there).
        _resolved = resolve_harness_command("goose-native", default="", explicit=None, cfg=cfg)
        if _resolved:
            os.environ["AGENTNEXUS_GOOSE_PATH"] = _resolved
        if server is None:
            server = cfg.get("server")
        auto_open_conversation = _resolve_auto_open_conversation_from_config(cfg)
    ...
    def hermes(
        ...
        from agentnexus.harness_startup_config import resolve_harness_command
        from agentnexus.hermes_native import run_hermes_native

        cfg = _load_effective_config()
        # Thread ``--command`` / ``harness.hermes-native.command`` config into the
        # runner via the canonical ``AGENTNEXUS_HERMES_PATH`` env var (set before
        # ``_ensure_backend`` so a locally-spawned daemon inherits it; a remote
        # ``--server`` runner reads its own host env, so set the var there).
        _resolved = resolve_harness_command("hermes-native", default="", explicit=None, cfg=cfg)
        if _resolved:
            os.environ["AGENTNEXUS_HERMES_PATH"] = _resolved
        if server is None:
            server = cfg.get("server")
        auto_open_conversation = _resolve_auto_open_conversation_from_config(cfg)
    ...
    def antigravity(
        ...
        """Launch Antigravity (agy) with AgentNexus.
        ...
        from agentnexus.antigravity_native import run_antigravity_native
        from agentnexus.harness_startup_config import resolve_harness_command
    ...
    def qwen(
        ...
        """Launch Qwen Code with AgentNexus.
        ...
        from agentnexus.harness_startup_config import resolve_harness_command
        from agentnexus.qwen_native import run_qwen_native

        cfg = _load_effective_config()
        # Thread ``--command`` / ``harness.qwen-native.command`` config into the
        # runner via the canonical ``AGENTNEXUS_QWEN_PATH`` env var (set before
        # ``_ensure_backend`` so a locally-spawned daemon inherits it; a remote
        # ``--server`` runner reads its own host env, so set the var there).
        _resolved = resolve_harness_command("qwen-native", default="", explicit=None, cfg=cfg)
        if _resolved:
            os.environ["AGENTNEXUS_QWEN_PATH"] = _resolved
        if server is None:
            server = cfg.get("server")
        auto_open_conversation = _resolve_auto_open_conversation_from_config(cfg)
    ...
    def kimi(
        ...
        """Launch Kimi Code with AgentNexus.

        Boots Moonshot AI's interactive ``kimi`` TUI
        (https://github.com/MoonshotAI/Kimi-Code) in a runner-owned terminal and
        attaches your TTY — the native experience, embedded in the AgentNexus web
        UI. No AgentNexus provider config is needed: kimi authenticates against its
        ...
        from agentnexus.harness_startup_config import resolve_harness_command
        from agentnexus.kimi_native import run_kimi_native

        cfg = _load_effective_config()
        # Thread ``--command`` / ``harness.kimi-native.command`` config into the
        # runner via the canonical ``AGENTNEXUS_KIMI_PATH`` env var (set before
        # ``_ensure_backend`` so a locally-spawned daemon inherits it; a remote
        # ``--server`` runner reads its own host env, so set the var there).
        _resolved = resolve_harness_command("kimi-native", default="", explicit=None, cfg=cfg)
        if _resolved:
            os.environ["AGENTNEXUS_KIMI_PATH"] = _resolved
        if server is None:
            server = cfg.get("server")
        auto_open_conversation = _resolve_auto_open_conversation_from_config(cfg)
