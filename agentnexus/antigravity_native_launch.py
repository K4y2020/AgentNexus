...
from agentnexus.onboarding.gemini_auth import gemini_auth_has_credential
...
# so there is no per-tool / web-routed gate. AgentNexus therefore maps its
# ``permission_mode`` onto this single flag.
_SKIP_PERMISSIONS_FLAG = "--dangerously-skip-permissions"
# AgentNexus permission mode that maps to agy's all-or-nothing bypass.
...
def should_skip_permissions(
    ...
    * the launch is **headless** — no interactive client will attach to answer
    ...
    AgentNexus elicitation by the RPC read driver's interaction bridge (see
    :mod:`omnigent.antigravity_native_reader` /
    :mod:`omnigent.antigravity_native_interactions`). agy exposes no firing
    pre-tool hook, so AgentNexus cannot pre-empt a tool before it runs; the
    ...
    :param permission_mode: The session's effective AgentNexus permission mode,
...
def build_agy_launch(
    ...
    **Permission mode → agy flag.** ``--dangerously-skip-permissions`` is
    ...
    cannot be routed to the web UI or made per-tool (the genuine AgentNexus policy
