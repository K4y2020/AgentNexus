...
Per ``designs/AGENTNEXUS_TERMINAL_BRIDGE.md`` §4.2, this rewrite swaps to:
...
- No idle reaper: terminals are explicit-launch only and the LLM is
  expected to ``sys_terminal_close`` when done. AgentNexus shutdown still
  closes everything; per-conversation cleanup runs from the workflow's
  finally block.

The registry is constructed once at AgentNexus startup
...
from agentnexus.inner.terminal import TerminalInstance, create_terminal_instance

logger = logging.getLogger(__name__)

# Per-instance close timeout. Bounds the cleanup window so a wedged
# tmux session can't block AgentNexus shutdown or workflow finalization
...
def conversation_link_for_id(
    ...
    *,
    ...
    :param conversation_id: Conversation/session id, e.g.
        ``"conv_abc123"``.
    :param base_url: Optional AgentNexus server base URL, e.g.
...
class TerminalRegistry:
    ...
    def __init__(self, *, conversation_link_base_url: str | None = None) -> None:
        ...
        :param conversation_link_base_url: Optional AgentNexus server base URL
        ...
        # See ``designs/AGENTNEXUS_TERMINAL_BRIDGE.md`` §9.1.
    ...
    def native_panes(self) -> list[tuple[str, str, Path]]:
        ...
        """
        from agentnexus.terminals.pane_reaper import NATIVE_PANE_TERMINAL_NAMES
    ...
    def transfer(
        ...
        process and tmux pane keep running, but the AgentNexus session that owns
    ...
    async def cleanup_conversation(self, conversation_id: str) -> None:
        ...
        Per ``designs/AGENTNEXUS_TERMINAL_BRIDGE.md`` §4.4, this is **not**
    ...
    async def shutdown(self) -> None:
        ...
        a stuck instance shouldn't block the rest of AgentNexus shutdown.
        """
        with self._lock:
