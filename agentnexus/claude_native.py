"""Native Claude Code terminal wrapper for the AgentNexus CLI.

The wrapper deliberately treats Claude Code as a terminal-first
program. It creates or binds an AgentNexus session, launches ``claude``
...
from agentnexus.llms.adapters._content import redact_binary_payloads
from agentnexus.runtime.tool_result_replay import (
...
from agentnexus._native_resume_hint import echo_native_resume_hint
from agentnexus._runner_startup import RunnerStartupProgress, runner_startup_progress
from agentnexus._startup_profile import StartupProfiler
from agentnexus._terminal_picker_theme import (
...
from agentnexus.claude_model_vocabulary import (
...
from agentnexus.claude_native_state import (
    read_launch_state,
    redirect_launch_state,
    write_launch_state,
)
from agentnexus.cli_invocation import cli_invocation
from agentnexus.conversation_browser import conversation_url, open_conversation_link_if_enabled
...
from agentnexus.native_coding_agents import native_shell_terminal_spec
from agentnexus.native_terminal import (
...
from agentnexus.process_logging import log_info_once
...
# other sessions inside the TUI.  AgentNexus owns session switching in its own
...
def _managed_settings_paths() -> tuple[Path, ...]:
    ...
    if override is not None:
        return override
    from agentnexus.onboarding.ambient import CLAUDE_CODE_MANAGED_SETTINGS_PATHS
...
_CLAUDE_STARTUP_PROFILE_ENV_VAR = "AGENTNEXUS_CLAUDE_STARTUP_PROFILE"
...
class PreparedClaudeTerminal:
    ...
    :param cold_resumed: ``True`` when we launched a fresh terminal
        against an existing AgentNexus session (i.e. ``--resume <conv>`` with
...
def claude_catalog_serves_model(
    ...
    """
    from agentnexus.model_catalog_store import catalog_contains
...
def resolve_claude_native_model_selection(
    ...
    """Resolve an AgentNexus picker id to a model identifier Claude accepts.

    Claude Code understands its built-in family aliases directly, but the
    extra Sonnet 5 row uses AgentNexus's ``sonnet_5`` id because it occupies
...
def claude_config_with_routed_arms_pinned(
    ...
    """
    from agentnexus.claude_model_vocabulary import normalized_model_id
...
def managed_claude_gateway_signal() -> tuple[str | None, bool]:
    ...
    """
    from agentnexus.onboarding.ambient import claude_managed_gateway
...
def claude_catalog_fingerprint(claude_config: ClaudeNativeUcodeConfig | None) -> str:
    ...
    """
    from agentnexus.claude_launcher import resolve_claude_launch
    from agentnexus.model_catalog_store import binary_identity, fingerprint_of
...
def build_native_claude_terminal_env(
    ...
    terminal stays pinned to the session the AgentNexus UI is showing, and
...
def run_claude_native(
    *,
    ...
    """
    Launch Claude Code in an AgentNexus terminal and attach locally.

    :param server: Optional remote AgentNexus server URL. ``None`` starts a
        local AgentNexus server using the existing chat server machinery.
    ...
    :param claude_args: Args after ``claude``, e.g.
        ...
        ``-r`` is stripped defensively (AgentNexus owns resume).
    ...
    :param startup_profiler: Optional shared startup profiler from the
        ...
        ``AGENTNEXUS_CLAUDE_STARTUP_PROFILE``.
    ...
    with TemporaryDirectory(prefix="agentnexus-claude-native-") as tmpdir:
...
def _align_working_directory_with_session(
    ...
    * A user resuming from a different machine has no recorded
    ...
    :param session_id: Resolved AgentNexus conversation id, e.g.
        ``"conv_abc123"``.
    :param base_url: AgentNexus server base URL used to look up Claude's
...
def _fetch_external_session_id_for_redirect(
    *,
    ...
    :param base_url: AgentNexus server base URL, or ``None`` when unavailable.
    :param headers: HTTP headers for the AgentNexus request.
...
def _record_launch_for_fresh_session(session_id: str) -> None:
    ...
    :param session_id: Newly created AgentNexus conversation id, e.g.
        ``"conv_abc123"``.
    :returns: None.
    """
    try:
...
def _ucode_config_for_profile(
    ...
    *,
    ...
    from agentnexus.onboarding.databricks_config import get_workspace_url_for_profile
    from agentnexus.onboarding.ucode_state import read_ucode_state
    ...
    if refresh_models:
        ...
        try:
            from agentnexus.databricks_model_discovery import (
...
def _provider_config_for_native_claude(entry: ProviderEntry) -> ClaudeNativeUcodeConfig | None:
    ...
    """
    from agentnexus.onboarding.provider_config import ANTHROPIC_FAMILY
    ...
    return ClaudeNativeUcodeConfig(
        ...
        routable_models=tuple(
            dict.fromkeys(
                [
                    *pin_env.values(),
                    *[k for k in family.models.keys() if isinstance(k, str) and k not in ("default", "sonnet", "opus", "haiku", "fable")],
                    *[v for v in family.models.values() if isinstance(v, str)],
...
def resolve_native_claude_config(
    *,
    ...
    from agentnexus.runtime.workflow import _load_global_auth, _resolve_provider_for_build
...
def _wrapper_spec_raw_instructions(spec_path: Path) -> str | None:
    ...
    from agentnexus.spec import load as load_agent_spec
...
def _run_with_local_server(
    ...
    *,
    ...
    """
    Start a local AgentNexus server, launch Claude, and attach to it.
    ...
    startup_profiler = startup_profiler or StartupProfiler(name="agentnexus claude", enabled=False)
    ...
    try:
        ...
        if resolved_session_id is not None:
            ...
            # independent (writes to ``~/.agentnexus/``), so chdiring
            # now is safe.
...
class _AttachOutcome(Enum):
    ...
    :cvar EXITED: The user quit (stdin EOF / Ctrl-D), the terminal is
        ...
        launcher tears down the runner and AgentNexus terminal resource.
...
async def _attach_direct_tmux(
    ...
    *,
    ...
    """
    from agentnexus.terminals.ws_common import _check_pane_dead_definitive, _tmux_session_alive
...
async def _attach_with_transcript_forwarder(
    *,
    ...
    :param base_url: AgentNexus server base URL.
    :param headers: Static HTTP auth headers for AgentNexus requests. For
    ...
    :param auth: Optional httpx Auth that mints a fresh bearer token
        ...
        transcript forwarder's HTTP client so AgentNexus posts continue to
    ...
    # offset 0 contains turns AgentNexus already has from the previous run).
    ...
    try:
        ...
        # the AgentNexus terminal resource must stay live (the web UI keeps
...
async def _attach_with_reconnect(
    *,
    ...
    :param base_url: AgentNexus server URL for the post-close terminal probe;
    ...
    :param active_session_id_reader: Optional callback that returns
        the latest active AgentNexus session id, e.g. ``"conv_new"``. This is
            ...
            if (
                ...
                async def _terminal_gone_probe(
                    *,
                    ...
                    :returns: ``True`` when the AgentNexus terminal resource
...
async def _is_terminal_resource_gone(
    *,
    ...
    :param base_url: AgentNexus server base URL.
    :param headers: HTTP auth headers for the AgentNexus server. Mutated in
...
async def _prepare_claude_terminal_via_daemon(
    *,
    ...
    :param base_url: AgentNexus server base URL.
    :param headers: Static HTTP auth headers for AgentNexus requests.
...
def _run_with_remote_server(
    ...
    *,
    ...
    """
    Launch Claude on a remote AgentNexus server via the connect daemon.
    ...
    :param base_url: Remote AgentNexus server base URL without a trailing
    ...
    :returns: None.
    """
    from agentnexus.chat import _bundle_agent, _remote_headers, _server_auth
...
async def _prepare_claude_terminal(
    *,
    ...
    :param base_url: AgentNexus server base URL.
    :param headers: Static HTTP auth headers for the AgentNexus server.
    :param session_id: Optional existing session id.
    ...
    async with open_server_client(base_url, headers=headers, timeout=timeout) as client:
        ...
        # AgentNexus already holds the prior conversation history from the
        # earlier run. The forwarder must not re-read whatever the new
        # transcript file contains at startup and republish it as new
        # AgentNexus events. Both subcases — injected ``--resume <claude_sid>``
...
async def _fetch_claude_session_labels(
    ...
    """
    Fetch labels for an existing Claude-native AgentNexus session.

    :param client: HTTP client for the AgentNexus server.
    :param session_id: AgentNexus conversation id, e.g. ``"conv_abc123"``.
...
async def _resolve_cold_resume_args(
    ...
    if not isinstance(external_session_id, str) or not external_session_id:
        # AgentNexus conv survives; claude side starts fresh. Warn on
    ...
    if transcript is None:
        ...
        # starting. Launch fresh — the AgentNexus conv survives.
        message = (
...
async def _ensure_local_claude_resume_transcript(
    ...
    *,
    ...
    Cross-machine resume has the AgentNexus conversation and Claude external
    ...
    wrapper always rewrites it from committed AgentNexus items before launch so
    AgentNexus remains the source of truth when a previous local Claude JSONL
    has diverged.

    :param client: HTTP client pointed at the AgentNexus server.
    :param session_id: AgentNexus conversation id, e.g.
        ``"conv_abc123"``.
    ...
    :param workspace: Resolved directory Claude will run in — its
        ...
        ``AGENTNEXUS_RUNNER_WORKSPACE``. Pass an already-resolved
    ...
    :returns: Path to the local transcript that was written; ``None`` if
        *external_session_id* is not a safe transcript stem, or if the AP
        ...
        fresh).
    :raises click.ClickException: If AgentNexus history cannot be fetched or
...
async def _resolve_session_item_file_references(
    ...
    *,
    ...
    """
    from agentnexus.inner.native_attachments import has_unresolved_file_id, resolve_file_id_block
...
def _claude_transcript_records_from_session_items(
    ...
    *,
    ...
    """
    Convert AgentNexus session items into Claude Code transcript records.

    :param items: Flat AgentNexus item dicts in chronological order, e.g.
...
def _claude_transcript_record_from_session_item(
    ...
    *,
    ...
    """
    Convert one AgentNexus item into one Claude transcript record.

    No ``message.model`` is emitted. An item's wire ``model`` is the
    AgentNexus *agent* name (``MessageData.agent`` serializes under that
    ...
    :param item: Flat AgentNexus item dict, e.g.
...
def _synthetic_claude_transcript_uuid(
    *,
    ...
    :param external_session_id: Claude-native session id, e.g.
        ``"02857840-6362-408f-b41f-309e396ed7c6"``.
    :param item: AgentNexus item dict. ``id`` is used when present.
    ...
    return str(
        uuid.uuid5(
            uuid.NAMESPACE_URL,
            f"agentnexus-claude-resume:{session_id}:{external_session_id}:{stable_item_id}",
        )
    )
...
def _claude_user_content_from_api_blocks(
    ...
    """
    Convert AgentNexus user message blocks into Claude message content.
    ...
    :param content: AgentNexus ``content`` value, e.g.
...
def _claude_attachment_text_blocks_from_api_content(
    ...
    :param content: AgentNexus content array, e.g.
    ...
    """
    from agentnexus.inner.native_attachments import attachment_reference_line
...
def _claude_assistant_content_from_api_blocks(
    ...
    """
    Convert AgentNexus assistant message blocks into Claude text blocks.
...
def _claude_text_blocks_from_api_content(
    ...
    *,
    ...
    """
    Extract text blocks from an AgentNexus content array.

    :param content: AgentNexus content array, e.g.
        ``[{"type": "input_text", "text": "hello"}]``.
    :param api_type: AgentNexus block type to include, e.g.
...
def _json_object_from_string(value: object) -> _JsonObject:
    ...
    :param value: JSON string from an AgentNexus function-call item, e.g.
        ``"{\"file_path\":\"README.md\"}"``.
...
def _preflight_local_tools(command: str) -> None:
    ...
    if shutil.which(command) is None:
        raise click.ClickException(
            ...
            "--server selects the AgentNexus server only; Claude still runs locally."
        )
...
async def _launch_claude_terminal(
    ...
    *,
    ...
    :param client: HTTP client pointed at the AgentNexus server. Its
        ``base_url`` and ``headers`` are reused as the
        ``PermissionRequest`` command hook's AgentNexus URL and auth. The hook
...
async def attach_local_terminal(
    ...
    *,
    ...
    """
    Attach the local TTY to an AgentNexus terminal WebSocket.
    ...
    :param terminal_gone_probe: Optional async callback returning
        ``True`` once the AgentNexus terminal resource is stopped. When set,
...
async def _close_ws_when_terminal_gone(
    ...
    *,
    ...
    """
    Close the client WebSocket when the AgentNexus terminal resource stops.
...
def _websocket_connect(
    ...
    *,
    ...
    from agentnexus.tls import client_ssl_context
    ...
    handshake_headers = {**headers, "Origin": AGENTNEXUS_INTERNAL_WS_ORIGIN}
