"""Native Pi TUI wrapper for the AgentNexus CLI."""
...
from agentnexus._wrapper_labels import PI_NATIVE_WRAPPER_VALUE as _WRAPPER_LABEL_VALUE
...
from agentnexus.native_terminal import bind_session_runner as _bind_session_runner
from agentnexus.native_terminal import (
    normalize_extra_args as _normalize_extra_args,
)
from agentnexus.native_terminal import url_component
from agentnexus.pi_native_bridge import bridge_dir_for_session_id

_logger = logging.getLogger(__name__)


_DEFAULT_PI_COMMAND = "pi"
_PI_PATH_ENV = "AGENTNEXUS_PI_PATH"
...
def _configured_pi_command(env: Mapping[str, str]) -> str:
    ...
    Reads ``AGENTNEXUS_PI_PATH`` (canonical) then the deprecated
    ...
    if legacy:
        from agentnexus.harness_startup_config import _warn_legacy_path
...
def run_pi_native(
    *,
    ...
    """
    Launch Pi TUI in an AgentNexus terminal.

    :param server: Resolved AgentNexus server URL.
    ...
    if server is None:
        raise click.ClickException(
            "Pi requires a resolved AgentNexus server URL. The CLI should call "
            "_ensure_backend before run_pi_native."
        )
    with TemporaryDirectory(prefix="agentnexus-pi-native-") as tmpdir:
...
def _run_with_remote_server(
    ...
    *,
    ...
    """
    Launch Pi on an AgentNexus server via a daemon-spawned runner.
...
async def _create_pi_session(
    ...
    *,
    ...
    :param client: HTTP client pointed at AgentNexus.
