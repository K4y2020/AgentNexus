...
# ``from agentnexus_ui_sdk import FormattedItem``.
from agentnexus_ui_sdk.terminal._completer import FileMentionCompleter
from agentnexus_ui_sdk.terminal._formatter import FormattedItem
from agentnexus_ui_sdk.terminal._theme import LIGHT_THEME, get_theme
...
if TYPE_CHECKING:
    ...
    from agentnexus.server.schemas import SessionStatusEvent
...
def _load_startup_theme() -> TerminalTheme:
    ...
    On first launch (no persisted theme in ``~/.agentnexus/config.yaml``),
    ...
    from agentnexus.repl._theme_picker import startup_theme_picker
...
def _render_startup_banner_ansi(
    ...
    *,
    ...
    """
    from agentnexus.inner.banner import BannerLine, startup_banner_strings
    from agentnexus.server_url import display_server_url, is_workspace_hosted_url
...
async def _fetch_server_version(client: AgentNexusClient) -> str | None:
    ...
    Routed through the REPL's already-connected :class:`AgentNexusClient` so
...
def _build_elicitation_content_from_schema(
    ...
    """
    from agentnexus.tools._elicitation_schema import build_accept_content_from_schema
...
def _server_event_to_sdk_event(event: object) -> object | None:
    ...
    from agentnexus_client._types import Response as SDKResponse
...
class _SessionsChatReplAdapter:
    ...
    def __init__(
        ...
        :param client: The :class:`AgentNexusClient` used to
    ...
    async def _runner_recover_watch(self) -> None:
        ...
        :returns: None.
        """
        _dbg = bool(os.environ.get("AGENTNEXUS_SESSIONS_ADAPTER_DEBUG"))
        while True:
            try:
    ...
    async def _ensure_session(self) -> str:
        ...
        Set ``AGENTNEXUS_SESSIONS_ADAPTER_DEBUG=1`` to trace
    ...
    def _emit_runner_recovery_error_once(self, exc: Exception) -> None:
        ...
        :param exc: Failure raised while relaunching or rebinding a
            runner, e.g. an SDK ``AgentNexusError`` from
    ...
    def _is_terminal_runner_recovery_error(self, exc: Exception) -> bool:
        ...
        :returns: ``True`` for typed server runner-state errors,
            ...
            retrying.
        """
        if not isinstance(exc, AgentNexusError):
            return False
        code = exc.code or ""
    ...
    async def send(
        ...
        *,
        ...
        try:
            ...
            # _on_event.
            from agentnexus_client._events import ResponseCompleted
            from agentnexus_client._types import Response as SDKResponse
    ...
    async def send_skill_slash_command(
        ...
        The AgentNexus server persists a visible ``slash_command`` item and
    ...
    async def _handle_elicitation(
        ...
        from agentnexus_client._tool_handler import ElicitationRequestCtx
...
SlashCommandHandler = Callable[
    [str, _ReplSession, AgentNexusClient, TerminalHost, RichBlockFormatter],
    Awaitable[None],
]
...
def _render_failed_status_error(
    ...
    """
    from agentnexus_client import ErrorBlock
...
async def run_repl(
    ...
    *,
    ...
    :param client: Connected AgentNexusClient.
    ...
    :param resume_conversation_id: When set, the REPL opens
        ...
        ``--resume <id>`` (see designs/RUN_AGENTNEXUS_SESSION_RESUMPTION.md).
    ...
    :param server_url: Base URL of the AgentNexus server the REPL is
    ...
    from agentnexus.repl._event_tape import EventTape, PipelineCounters, TapeEntry
        ...
        * ``ClientTaskCancelEvent`` -> cancel local tool task
        ...
        if isinstance(event, _StatusEv):
            ...
            if event.status == "running":
                from agentnexus_client import BlockContext, ResponseStartBlock
                ...
                from agentnexus_client import TextDone
        ...
        from agentnexus.server.schemas import ElicitationResolvedEvent
        ...
        if isinstance(sdk_ev, _TD):
            from agentnexus_client import TextChunk
        ...
        if isinstance(sdk_ev, _RS):
            from agentnexus_client import BlockContext, ReasoningStartBlock
        ...
        if isinstance(sdk_ev, _RD | _RSD):
            from agentnexus_client import BlockContext
            from agentnexus_client import ReasoningChunk as _RC
        ...
        from agentnexus_client._events import ResponseCompleted as _Completed
        ...
        from agentnexus_client._events import ErrorEvent as _ErrorEvent

        if isinstance(sdk_ev, _ErrorEvent):
            from agentnexus_client import BlockContext, ErrorBlock
        ...
        from agentnexus_client._events import ResponseFailed as _Failed
    ...
    def _maybe_log_tape_entry(entry: TapeEntry) -> None:
        ...
        if _event_log_fh is not None:
            from agentnexus.repl._event_tape import log_entry_jsonl
    ...
    from agentnexus_ui_sdk import Overlay

    async def _overview_builder(target: OverlayTarget | None) -> RenderableType:
        from agentnexus.cli_diagnostics import current_cli_log_path
    ...
    from agentnexus_ui_sdk import OverlayAction
    ...
    if debug_events and _event_tape is not None:
        from agentnexus.repl._event_tape import build_tape_detail, build_tape_targets
        ...
        async def _tape_targets() -> list[OverlayTarget]:
            ...
            """
            from agentnexus.repl._event_tape import _OverlayTargetLike
    ...
    async with host:
        ...
        if debug_events:
            ...
            from agentnexus.repl._event_tape import open_event_log
        ...
        # AgentNexus REPL feels identical at boot. Raw stdout write
        ...
        from agentnexus_ui_sdk import StreamingText
...
async def _maybe_write_session_log(
    ...
    :param client: Connected AgentNexusClient — REUSED, not opened
...
async def _cmd_help(
    ...
    client: AgentNexusClient,  # noqa: ARG001
    host: TerminalHost,
    fmt: RichBlockFormatter,
) -> None:
    from rich.text import Text
...
async def _cmd_theme(
    ...
    client: AgentNexusClient,  # noqa: ARG001 — dispatch-contract params
    host: TerminalHost,
    fmt: RichBlockFormatter,
) -> None:
    ...
    from agentnexus.repl._theme_picker import _build_preview, build_theme_confirmation
...
def _build_model_readout_lines(
    ...
    if harness and harness_owns_its_credential(harness):
        # The external agent authenticates itself, so there is no AgentNexus
...
def _match_configured_provider(config: dict[str, object], token: str) -> str | None:
    ...
    """
    from agentnexus.onboarding.configure_models import provider_display_name
...
def _model_validation_warning(model: str) -> str | None:
    ...
    from agentnexus.llms.routing import parse_model_string
...
async def _cmd_model(
    ...
    from agentnexus.onboarding.configure_models import kind_glyph, provider_display_name
    from agentnexus.onboarding.detected import effective_config_with_detected
    from agentnexus.onboarding.provider_config import (
...
async def _attach_to_conversation(
    ...
    *,
    ...
    - ``run_repl(resume_conversation_id=...)`` on startup
      ...
      designs/RUN_AGENTNEXUS_SESSION_RESUMPTION.md) — passes
    ...
    :param client: The :class:`AgentNexusClient` for fetching
        items.
    ...
    if cw:
        ...
        if last_total is not None:
            ...
            from agentnexus.runtime.compaction import count_tokens
...
# Mirrors _DEFAULT_TRIGGER_THRESHOLD from agentnexus.runtime.compaction.
...
async def _refresh_session_metadata(
    ...
    :param client: AgentNexus HTTP client used to fetch the snapshot.
...
async def _update_context_ring_estimate(
    ...
    :param client: AgentNexus HTTP client used to query items.
...
def _build_github_issue_url(
    ...
    body_parts: list[str] = [
        ...
        *info_lines,
        ...
        "     --debug-events for a JSONL event log in ~/.agentnexus/debug/ -->",
    ]
...
async def _cmd_logs(
    ...
    from agentnexus.cli_diagnostics import current_cli_log_path
    from agentnexus.repl._session_log import write_logs_zip, write_session_log
...
async def _cmd_report(
    arg: str,
    session: _ReplSession,
    client: AgentNexusClient,  # noqa: ARG001 — dispatch-contract param
    host: TerminalHost,
    fmt: RichBlockFormatter,
) -> None:
    ...
    try:
        version = _pkg_version("agentnexus")
    except Exception:  # noqa: BLE001
        version = None
...
async def _open_terminal_in_tmux(
    ...
    *,
    ...
    same behavior under AgentNexus mode.
    ...
    :param client: AgentNexus HTTP client — used to re-walk the
...
async def _build_debug_overview(
    ...
    *,
    ...
    :param cli_log_path: Optional path to the always-on CLI
        diagnostics log (``~/.agentnexus/logs/cli/cli-*.log``).
...
_BANG_TIMEOUT_S: float = float(os.environ.get("AGENTNEXUS_BANG_TIMEOUT_S") or 120.0)
_BANG_DISPLAY_MAX: int = int(os.environ.get("AGENTNEXUS_BANG_DISPLAY_MAX") or 30_000)
_BANG_CONTEXT_MAX: int = int(os.environ.get("AGENTNEXUS_BANG_CONTEXT_MAX") or 16_000)
...
def _write_bang_overflow(cmd: str, stdout: str, stderr: str) -> str | None:
    ...
    if len(plain_out) + len(plain_err) <= _BANG_CONTEXT_MAX:
        return None
    fd, path = tempfile.mkstemp(prefix="agentnexus-bang-", suffix=".log")
