...
Conversations come from AgentNexus' conversation store.
...
if TYPE_CHECKING:
    ...
    from agentnexus.entities.conversation import ConversationItem
...
from agentnexus.claude_native_state import read_launch_state as _read_claude_launch_state
from agentnexus.codex_native_state import read_launch_state as _read_codex_launch_state
...
def _runtime_badge(row: _ConversationRow) -> str:
    ...
    The badge surfaces which AgentNexus wrapper owns the session so a
...
async def pick_conversation_by_wrapper_label_from_sdk(
    ...
    *,
    ...
    (``~/.agentnexus/claude-native/``); sessions created on a
...
def _render_workspace_cell(row: _ConversationRow, *, current_cwd: Path) -> Text | None:
    ...
    *current_cwd* (resolved). When state is absent -- legacy session
    ...
    ``~/.agentnexus/claude-native/<hash>/launch.json`` -- fast on
