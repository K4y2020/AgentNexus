"""
JSON dump of an omnigent conversation — the AgentNexus mode port of the
legacy ``--log`` feature (see designs/RUN_AGENTNEXUS_REPL_PARITY.md).

The legacy non-AP path wrote a session-shaped JSON to
``~/.agentnexus/logs/`` at REPL exit (`_write_session_log` /
...
.. code-block:: json

    {
      ...
      "format": "agentnexus-conversation",
...
from agentnexus_ui_sdk import state_dir
...
_LOG_FORMAT = "agentnexus-conversation"

# Default log directory. Derives from the shared ``state_dir()`` so
# ``AGENTNEXUS_DATA_DIR`` isolates it without replacing ``HOME``. The
# fallback remains the legacy ``~/.agentnexus/logs/`` location. Created
# on first write.
...
def default_log_zip_path(
    ...
    *,
    ...
    :data:`DEFAULT_LOG_ZIP_DIR`, i.e. ``~/.agentnexus/logs``.
    ...
    if slug is not None:
        return base / f"agentnexus-logs-{slug}-{timestamp}.zip"
    return base / f"agentnexus-logs-{timestamp}.zip"
...
def collect_log_files(log_paths: list[Path]) -> list[tuple[Path, str]]:
    ...
    This helper does **not** walk ``~/.agentnexus/logs`` or
    ``~/.agentnexus/debug`` wholesale; doing so would include unrelated
    sessions.
...
async def write_session_log(
    ...
    *,
    ...
    :param client: A connected :class:`AgentNexusClient`. The
    ...
    :param agent_name: Agent name to include in the dump (for
        ...
        e.g. ``"resume_test"``.
    :param log_dir: Override the default location. ``None`` uses
        :data:`DEFAULT_LOG_DIR` (``~/.agentnexus/logs/``).
...
def _extract_child_conversation_ids(items: list[dict[str, object]]) -> list[str]:
    ...
    from agentnexus.repl._repl import _parse_sub_agent_handle
...
def _fetch_all_items_sync(
    ...
    self-contained — a reader doesn't need any of AgentNexus'
...
async def _fetch_all_items_via_sessions(
    ...
    :param client: The connected :class:`AgentNexusClient`.
