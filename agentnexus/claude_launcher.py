...
:data:`CLAUDE_LAUNCHER_ENTRY_POINT_GROUP` group::
    ...
    [project.entry-points."agentnexus.claude_launcher"]
...
environment the runner runs in -- no AgentNexus code change, no in-tree import
path. At launch time, the ``AGENTNEXUS_CLAUDE_LAUNCHER`` environment variable
selects *which* registered launcher to use, by entry-point name (e.g.
``AGENTNEXUS_CLAUDE_LAUNCHER=isaac``). Unset -> default launch. The selected
...
the command preserves the AgentNexus bridge unchanged.
...
CLAUDE_LAUNCHER_ENV_VAR = "AGENTNEXUS_CLAUDE_LAUNCHER"

#: setuptools entry-point group launcher plugins register themselves in.
CLAUDE_LAUNCHER_ENTRY_POINT_GROUP = "agentnexus.claude_launcher"
...
class ClaudeLauncher(abc.ABC):
    ...
    AgentNexus instantiates the subclass (no-arg constructor) and calls
    ...
    def launch(self, command: str, args: list[str]) -> tuple[str, list[str]]:
        ...
        :param command: Default terminal command AgentNexus would otherwise spawn,
            e.g. ``"claude"``.
        :param args: Fully-augmented Claude CLI args (MCP config, hook settings
            ...
            the AgentNexus bridge.
        :returns: The ``(command, args)`` AgentNexus should spawn instead.
        """
