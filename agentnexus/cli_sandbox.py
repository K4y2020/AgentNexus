...
def sandbox() -> None:
    """
    Run an AgentNexus host inside a remote sandbox.

    \b
    Subcommands:
      create   Provision a sandbox + bootstrap AgentNexus into it.
    ...
    Provider notes:
      modal    Sandboxes live at most 24 hours (platform cap). Needs
               `pip install 'agentnexus[modal]'` + `modal token new`.
      daytona  No lifetime cap (idle auto-stop is disabled). Needs
               `pip install 'agentnexus[daytona]'` + DAYTONA_API_KEY.
...
def sandbox_connect(
    ...
    """
    from agentnexus.onboarding.sandboxes import connect_sandbox_host, derive_workspace
