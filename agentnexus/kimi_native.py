"""Native Kimi TUI wrapper for the AgentNexus CLI.
...
from agentnexus._wrapper_labels import KIMI_NATIVE_WRAPPER_VALUE as _WRAPPER_LABEL_VALUE
...
_KIMI_PATH_ENV = "AGENTNEXUS_KIMI_PATH"
...
def run_kimi_native(
    *,
    ...
    """
    Launch the Kimi TUI in an AgentNexus terminal.
    ...
    if server is None:
        raise click.ClickException(
            "Kimi requires a resolved AgentNexus server URL. The CLI should call "
            "_ensure_backend before run_kimi_native."
        )
    with TemporaryDirectory(prefix="agentnexus-kimi-native-") as tmpdir:
...
def _run_with_remote_server(
    ...
    *,
    ...
    """
    Launch Kimi on an AgentNexus server via a daemon-spawned runner.
