...
Every AgentNexus Python entrypoint (server / runner / host / harness) writes its
...
can be queried in one place (see the AgentNexus Debuggability Plan, OMNI-4198).

The sink is enabled only when the ``AGENTNEXUS_DEBUG_LOG_*`` environment variables
...
CLIENT_ID_ENV_VAR = "AGENTNEXUS_DEBUG_LOG_CLIENT_ID"
CLIENT_SECRET_ENV_VAR = "AGENTNEXUS_DEBUG_LOG_CLIENT_SECRET"
WORKSPACE_URL_ENV_VAR = "AGENTNEXUS_DEBUG_LOG_WORKSPACE_URL"
ENDPOINT_ENV_VAR = "AGENTNEXUS_DEBUG_LOG_ENDPOINT"
...
PRIMARY_SESSION_ID_ENV_VAR = "AGENTNEXUS_RUNNER_PRIMARY_SESSION_ID"
...
USER_ID_ENV_VAR = "AGENTNEXUS_USER_ID"
_user_id_var: ContextVar[str | None] = ContextVar("agentnexus_debug_user_id", default=None)
...
def runner_primary_session_id() -> str | None:
    ...
    The host sets ``AGENTNEXUS_RUNNER_PRIMARY_SESSION_ID`` when it spawns a runner
...
def current_user_id() -> str | None:
    ...
    process-constant ``AGENTNEXUS_USER_ID`` env (single-user runner/host). Both
...
def debug_event(
    ...
    **attributes: object,
    ...
    fallback (a request-scoped ContextVar on the server, the ``AGENTNEXUS_USER_ID``
...
class ZerobusLogHandler(logging.Handler):
    ...
    def _start_worker(self) -> None:
        ...
        self._thread = threading.Thread(target=self._run, name="agentnexus-debug-log", daemon=True)
        self._thread.start()
...
SSE_LOGGER_NAME = "agentnexus.sse_events"
...
def attach_debug_log_sink(loggers: list[logging.Logger], *, source: str, level: int) -> None:
    ...
    A no-op unless the ``AGENTNEXUS_DEBUG_LOG_*`` variables are set. Reuses one
