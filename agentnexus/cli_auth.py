...
def _token_file_path() -> Path:
    ...
    Uses the shared AgentNexus state directory, honoring
    ``AGENTNEXUS_DATA_DIR``.

    :returns: Path to ``<data-dir>/auth_tokens.json``.
    """
    from agentnexus_ui_sdk.terminal._config import state_dir
...
def _write_tokens_file(path: Path, data: dict[str, dict[str, str | float]]) -> None:
    ...
    :param path: Destination file, i.e. ``~/.agentnexus/auth_tokens.json``.
...
def _store_entry(server_url: str, entry: dict[str, str | float]) -> None:
    ...
    Writes ``~/.agentnexus/auth_tokens.json`` with user-only
...
AGENTNEXUS_SLICE_KEY_HEADER = "X-Databricks-AgentNexus-Slice-Key"
...
DATABRICKS_EXTRA_HEADERS_ENV_VAR = "AGENTNEXUS_DATABRICKS_EXTRA_HEADERS"
...
def databricks_request_headers(
    ...
    *,
    ...
    :param host_id: The host a request is scoped to (its control tunnel, its
        ...
        :data:`AGENTNEXUS_SLICE_KEY_HEADER` routing header) only when *server_url*
        ...
        host_id inside a runner process (via ``AGENTNEXUS_RUNNER_SLICE_KEY``) and
        otherwise leaves routing to the default.
    :returns: A header dict carrying ``Authorization``, ``X-Databricks-Org-Id``,
        ``X-Databricks-AgentNexus-Slice-Key``, and/or the configured extra headers
    ...
    #      AGENTNEXUS_RUNNER_SLICE_KEY) — keys the runner's server traffic
    ...
    if host_id is None and on_workspace_mount:
        from agentnexus.host.identity import load_host_identity_if_present

        identity = load_host_identity_if_present()
        if identity is not None:
            host_id = identity.host_id
    # Kill switch: slice-key emission is ON by default; export
    # ``AGENTNEXUS_HOST_SLICE_KEY_ENABLED=0`` to turn it off and fall back to the
    ...
    slice_key_enabled = os.environ.get("AGENTNEXUS_HOST_SLICE_KEY_ENABLED", "1") != "0"
    if host_id and on_workspace_mount and slice_key_enabled:
        headers[AGENTNEXUS_SLICE_KEY_HEADER] = host_id
...
def open_server_client(
    ...
    *,
    ...
    """Open an :class:`httpx.AsyncClient` to an AgentNexus server, keyed for routing.
