...
def run_resume(
    *,
    ...
    :param server: Optional remote AgentNexus server URL. Required in the
...
def _dispatch_by_runtime(
    *,
    ...
    :param server: Optional remote AgentNexus server URL. ``None`` when
...
def _dispatch_wrapper(
    *,
    ...
    :param wrapper: Value from ``labels.agentnexus.wrapper``.
    :param server: AgentNexus server base URL without trailing slash, or
...
def _read_wrapper_label_local(*, conv_id: str) -> str | None:
    ...
    :param conv_id: Local AgentNexus conversation id, e.g. ``"conv_abc123"``.
    :returns: Value of ``labels.agentnexus.wrapper``, or ``None`` when
        no wrapper label is present.
    :raises click.ClickException: If the conversation id is not found
        in the local persistent store.
    """
    from agentnexus.chat import _omnigent_persistent_dir
    ...
    if conversation is None:
        raise click.ClickException(
            ...
            "Pass --server if the conversation lives on a remote AgentNexus server.",
        )
...
def _read_wrapper_label_remote(
    *,
    ...
    :param server: Remote AgentNexus server URL.
    :param conv_id: AgentNexus conversation id.
