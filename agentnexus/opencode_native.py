...
from agentnexus._wrapper_labels import OPENCODE_NATIVE_WRAPPER_VALUE as _WRAPPER_LABEL_VALUE
...
from agentnexus.opencode_native_state import read_launch_state, write_launch_state
...
def run_opencode_native(  # pragma: no cover
    *,
    ...
    """
    Launch the OpenCode TUI in an AgentNexus terminal (the ``omnigent opencode`` path).
    ...
    :param server: Resolved AgentNexus server URL. ``None`` is an error (the CLI
        must resolve a backend first).
    :param session_id: Optional existing AgentNexus conversation id to resume.
    ...
    if server is None:
        raise click.ClickException(
            "OpenCode requires a resolved AgentNexus server URL. The CLI should resolve "
            "a backend before run_opencode_native."
        )
    with TemporaryDirectory(prefix="agentnexus-opencode-native-") as tmpdir:
...
def _run_with_remote_server(  # pragma: no cover
    ...
    *,
    ...
    """Launch OpenCode on an AgentNexus server via a daemon-spawned runner."""
...
def _resolve_session_id_for_resume(
    *,
    ...
    # e2e, not unit tests (it opens an AgentNexusClient and an arrow-key picker).
    from agentnexus_client import AgentNexusClient  # pragma: no cover
