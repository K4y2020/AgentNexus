...
surface (``~/.agentnexus/config.yaml``) and writes a per-session ``models.json``
...
from agentnexus.databricks_ai_gateway import (
...
from agentnexus.model_override import normalize_model_for_provider
...
if TYPE_CHECKING:
    ...
    from agentnexus.onboarding.ambient import CodexConfigTransport
...
_PI_OPENAI_PROVIDER_ID = "agentnexus-openai"
...
_PI_COMPLETIONS_PROVIDER_ID = "agentnexus-completions"
_PI_MLFLOW_PROVIDER_ID = "agentnexus-mlflow"
...
def _split_pi_native_model_selection(selection: str | None) -> tuple[str, str] | None:
    """Split an AgentNexus-managed ``provider/model`` picker value."""
    if not selection:
        return None
...
def _default_claude_model_from(entries: list[_PiModelEntry]) -> str | None:
    ...
    """
    from agentnexus.databricks_model_discovery import _natural_model_key
...
def _databricks_pi_provider(entry: ProviderEntry, *, model: str | None) -> PiProviderConfig | None:
    ...
    from agentnexus.inner.codex_executor import _databricks_codex_auth_command
    from agentnexus.inner.databricks_executor import _read_databrickscfg_host
    ...
    *,
    ...
    try:
        ...
        if resolved is None:
            ...
            from agentnexus.onboarding.provider_config import _parse_provider
