...
from agentnexus.claude_native_status import sync_raw_status_context
...
# Claude Code hook event names → AgentNexus session-status values
...
class _ForwardHealth:
    """
    Process-level health of AgentNexus transcript/usage forwarding (#1120).
...
def _note_forward_failure(retry_key: str) -> None:
    ...
    if (
        ...
        _logger.error(
            "claude-native forward sync degraded: %d consecutive AgentNexus "
...
class CompactionForwardState:
    ...
    both reconcile against one durable token so exactly one AgentNexus
...
class SubagentEntry:
    ...
    One of these per Claude-side sub-agent. Tracks the AgentNexus child
    ...
    :param child_conversation_id: AgentNexus child Conversation id minted
...
class SubagentForwardState:
    ...
    :param subagents: Map from Claude-side ``subagent_id`` to the
        ...
        inserted here after the AgentNexus server returns a child
        Conversation id.
    """
...
class DeltaForwardState:
    ...
    Claude process and keeps growing across AgentNexus session rotations, so the
    offset stays monotonic and each new chunk is forwarded to whatever
    AgentNexus session is active when it is read.
...
class _PostRetryEntry:
    """
    In-memory retry state for one outbound AgentNexus event.
...
class _PostRetryDecision:
    """
    Result of recording one outbound AgentNexus post failure.
...
class _PostRetryTracker:
    """
    Track bounded retries and backoff for AgentNexus event posts.
    ...
    backoff so AgentNexus outages do not silently drop transcript data.
...
async def forward_claude_transcript_to_session(
    *,
    ...
    :param headers: Static HTTP headers for AgentNexus requests. Authorization
...
def _parse_json_response(resp: httpx.Response, *, context: str) -> dict[str, object]:
    """
    Parse an AgentNexus JSON response, failing loudly on a non-JSON body.
...
async def _post_external_subagent_start(
    ...
    *,
    ...
    """
    POST ``external_subagent_start`` to the AgentNexus server and return the
    ...
    :returns: The AgentNexus child conversation id, e.g. ``"conv_child456"``.
    :raises httpx.HTTPError: If the AgentNexus request fails or is rejected.
...
async def _forward_available_subagents(
    *,
    ...
    """
    Discover new Claude Task-tool sub-agents on disk, mint AgentNexus child
    ...
    ``subagent_forwarder.json``) holds the AgentNexus child id and byte
...
async def _maybe_rotate_session_on_clear(
    *,
    ...
    """
    Rotate the active AgentNexus session when Claude reports ``/clear``.

    :param client: AgentNexus HTTP client.
    :param session_id: Currently active AgentNexus session id, e.g.
        ``"conv_old"``.
    :param bridge_dir: Native Claude bridge directory.
...
async def _seed_fork_transcript_forward_state(
    *,
    ...
    """
    Seed transcript forwarding after AgentNexus has forked history.

    Claude fork transcripts start with copied source-session records.
    The AgentNexus fork endpoint has already copied those conversation items,
...
async def _create_clear_replacement_session(
    *,
    ...
    """
    Create the fresh AgentNexus session for a Claude ``/clear`` event.
    ...
    :raises httpx.HTTPError: If AgentNexus rejects session creation, new-session
    ...
    clear_resp = await client.patch(
        ...
        json={
            ...
            # ``agentnexus claude --resume``) would put a second forwarder on the
...
async def _maybe_rotate_session_on_fork(
    *,
    ...
    """
    Fork the active AgentNexus session when Claude reports ``/fork``/``/branch``.
...
async def _create_fork_replacement_session(
    *,
    ...
    """
    Create the forked AgentNexus session for a Claude ``/fork``/``/branch``.
    ...
    :raises RuntimeError: If the AgentNexus fork response is malformed.
    """
    old = await _fetch_session_snapshot(client, old_session_id)
    runner_id = old.get("runner_id")
...
def _is_fork_hook_record(record: ClaudeHookRecord) -> bool:
    ...
    completed the AgentNexus fork.

    :param record: Claude hook record read from hooks.jsonl.
    :returns: ``True`` when the active AgentNexus session should be forked.
    """
...
async def _fetch_session_snapshot(
    ...
    """
    Fetch one AgentNexus session snapshot.

    :param client: AgentNexus HTTP client.
    :param session_id: AgentNexus session id, e.g. ``"conv_abc123"``.
    :returns: Parsed JSON snapshot.
    :raises httpx.HTTPError: If AgentNexus returns a non-2xx status.
...
async def _maybe_mirror_external_session_id(
    *,
    ...
    """
    Mirror Claude's native session id onto the AgentNexus conversation row.

    Reads the latest captured Claude-native session id from the
    bridge state file and, if present, PATCHes
    ``external_session_id`` on the AgentNexus conversation. Best-effort: a
...
async def _ensure_state_for_transcript(
    *,
    ...
    if start_at_offset is not None:
        ...
        # in the TUI pane, absent from the AgentNexus DB, with no error anywhere.
        ...
        # is content AgentNexus already holds and a live end-offset is correct.
        byte_offset = await asyncio.to_thread(_transcript_end_offset, transcript_path)
    state = TranscriptForwardState(
        transcript_path=transcript_path,
        line_cursor=0,
...
async def _handle_compact_summary_item(
    ...
    *,
    ...
    persists exactly one AgentNexus ``compaction`` boundary carrying the
...
async def _forward_available_items(
    *,
    ...
    for item in items:
        ...
        # Persist a durable AgentNexus ``compaction`` boundary here (never
...
async def _post_clear_supersession(
    ...
    *,
    ...
    :param client: AgentNexus HTTP client (``base_url`` = AP server).
    ...
    *,
    ...
    with (
        ...
        telemetry.span(
            ...
            attributes={"agentnexus.response_id": item.response_id},
        ),
    ):
        resp = await client.post(
...
async def _forward_available_deltas(
    *,
    ...
    :param client: AgentNexus HTTP client.
    :param session_id: AgentNexus session/conversation id deltas are forwarded
...
async def _post_external_permission_mode_change(
    ...
    *,
    ...
    Code terminal, which AgentNexus has no other way to observe.
...
async def _forward_permission_mode_from_pane(
    ...
    *,
    ...
    A shift+tab pressed inside the TUI produces no event AgentNexus can see, so
...
async def _post_external_session_title(
    ...
    *,
    ...
    Mirrors a ``/rename`` typed in the Claude Code pane onto the AgentNexus
...
async def _post_title_change_if_new(
    ...
    *,
    ...
    try:
        ...
        _logger.warning(
            "Failed to mirror /rename to AgentNexus session=%s; the web session "
...
async def _post_model_change_if_new(
    ...
    *,
    ...
    try:
        ...
        _logger.warning(
            "Failed to mirror model change to AgentNexus session=%s; model pill / "
...
async def _post_external_compaction_status(
    ...
    *,
    ...
    ``SessionStart`` (``source == "compact"``) hook. The AgentNexus server maps
...
async def _patch_external_session_id(
    ...
    *,
    ...
    """
    PATCH the AgentNexus conversation row with the Claude-native session id.
...
async def _maybe_sync_effort_from_slash_command(
    ...
    *,
    ...
    """
    Mirror an in-pane ``/effort`` change onto the AgentNexus session row.
    ...
    try:
        ...
        _logger.warning(
            "Failed to mirror in-pane /effort=%s to AgentNexus session=%s; "
...
def _is_permanent_http_error(exc: httpx.HTTPError) -> bool:
    """
    Return whether ``exc`` is a permanent AgentNexus rejection.

    :param exc: HTTP exception raised while posting an AgentNexus event.
...
def _usage_from_status_state(state: dict[str, object]) -> dict[str, float] | None:
    """
    Convert statusLine ``current_usage`` (+ cost) into the AgentNexus usage shape.
