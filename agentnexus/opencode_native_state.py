...
the shared AgentNexus server. Mirrors :mod:`omnigent.codex_native_state`.

Layout (per conversation):

    ~/.agentnexus/opencode-native/<sha256(conv_id)[:32]>/launch.json
"""
...
_STATE_ROOT_ENV_VAR = "AGENTNEXUS_OPENCODE_NATIVE_STATE_DIR"
...
def _opencode_native_state_root() -> Path:
    ...
    Otherwise follows ``AGENTNEXUS_DATA_DIR``, falling back to
