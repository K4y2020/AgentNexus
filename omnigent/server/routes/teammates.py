...
from omnigent.native_coding_agents import is_execution_harness_agent_name
    ...
    try:
        ...
        a2a_id = existing_a2a.id if existing_a2a else next(
            (
                conv.id
                for conv in conversations
                if conv.labels.get("omnigent.teammate.channel") == "a2a"
            ),
            None,
        )
        primary_id = existing_primary.id if existing_primary else next(
            (
                ...
                and conv.id != a2a_id
                and conv.labels.get("omnigent.teammate.primary") == "true"
            ),
            None,
        )
    ...
    *,
    ...
    async def update_bot(bot_id: str, body: UpdateBotRequest, request: Request) -> dict[str, Any]:
        ...
        if binding is None:
            ...
            if agent is None:
                raise OmnigentError("Bot agent not found", code=ErrorCode.NOT_FOUND)
