...
- [UI / Bug fix / Docs] Sign in to Google-backed AgentNexus servers from the iOS app without hitting `disallowed_useragent` (#2556)
...
- [Feature] Renaming a session with `/rename` inside a native Claude Code terminal now updates its name in the AgentNexus UI (#4104)
...
- [Bug fix] `agentnexus host` now stays connected when optional harness detection fails and surfaces server-side tunnel setup errors. (#4769)
...
- [Chore] `agentnexus host` conflict error now shows the exact `agentnexus host stop --server (#4821)
...
- [Bug fix] `agentnexus host stop` no longer crashes when a stale daemon record points at another user's process — it warns and cleans up the record instead (#5012)
...
- [Bug fix] `agentnexus login` to a Databricks workspace now writes a per-workspace `~/.databrickscfg` profile instead of overwriting the shared `DEFAULT`. (#5112)
...
- [Bug fix] The Electron app now returns you to AgentNexus when a Databricks workspace navigation lands at the workspace root. (#5125)
...
- [Breaking] The deprecated `agentnexus[memory]` extra has been removed; use `agentnexus[hindsight]` instead. (#5139)
...
- [Chore] Native harness launches (`agentnexus claude`, `codex`, `pi`, …) notice the host, runner, and terminal becoming ready sooner. (#5190)
...
- [Bug fix] `agentnexus resume` now works against servers that key sessions on non-uuid ids (#5218)
- [Bug fix] The shell no longer hangs on `exit` after an agentnexus codex session ends. (#5228)
...
- [Bug fix] Older AgentNexus clients now explain when a database was migrated by a newer version instead of recommending an impossible schema upgrade. (#5377)
...
- [UI / Feature / Docs / Test/CI] White-label the web UI from server config with custom names, headings, safe logo assets, favicon, and optional AgentNexus attribution. (#2857)
...
- [Bug fix] `agentnexus pi` now uses each model's real context window and output limit, instead of compacting early and truncating long replies on high-context models. (#4178)
...
- [Bug fix] `agentnexus server --host 0.0.0.0` with `AGENTNEXUS_LOCAL_SINGLE_USER=1` no longer 401s every request and 403s the host tunnel; the single-user marker is honored on network-exposed binds, with a warning that the server serves unauthenticated requests (#4224)
...
- [Bug fix] `agentnexus chat <remote-url>` can now start a new conversation with an agent registered on the remote server (#4260)
...
- [UI / Feature] Organizations can preconfigure AgentNexus server URLs through Android managed configuration, and they show up ready to tap in the app's server list. (#4315)
- [Feature] `agentnexus host --background` starts the local server and registers this machine as a host without tying up a terminal. (#4317)
...
- [Feature] `agentnexus start` starts the local server and registers this machine as a host — the on switch to go with `agentnexus stop`. (#4321)
...
- [Bug fix / Feature] `agentnexus run --server local` runs against a local server, overriding any configured server default — and the no-AGENT `agentnexus run --server ""` no longer fails with `Agent path not found: https:` (#4387)
...
- [Bug fix] The `build-agentnexus` skill is available again in native `agentnexus claude` and `agentnexus codex` sessions (#4391)
...
- [Bug fix / Feature] The Copilot harness now authenticates with your existing `gh auth login` session, and a GitHub Enterprise host can be set via `agentnexus setup` (#4396)
...
- [Bug fix / Test/CI] Honor AgentNexusClient timeout for ordinary Python SDK HTTP requests. (#4509)
...
- [Bug fix] `agentnexus` no longer crashes on startup when your shell sets a SOCKS proxy (e.g. `ALL_PROXY=socks5://…`) and the `httpx[socks]` extra isn't installed (#4517)
...
- [Bug fix] Harness logs now go to a file under `~/.agentnexus/logs/harness/` (or the runner's log), and a failing ACP turn quotes the agent's own error output instead of dropping it. (#4523)
...
- [Bug fix] Upgrading agentnexus in place no longer breaks harness launches on already-running runners (#4539)
...
- [Feature] The Android app now opens Databricks workspaces on their `/agentnexus` app instead of the workspace landing page. (#4543)
- [Bug fix] `agentnexus host` pointed at a local server that has exited now stops after ~5 minutes with a clear error instead of reconnecting forever. (#4544)
...
- [Bug fix] The Android app no longer shows the Databricks workspace navigation bar around AgentNexus when connecting to a workspace-hosted server. (#4555)
...
- [UI] Connecting the iOS app to a Databricks workspace now opens AgentNexus directly and hides the workspace navigation bar (#4559)
- [Bug fix] kiro sessions no longer fail the first message with a connection error when the kiro TUI is slow to start (#4562)
- [Bug fix] `agentnexus host` now warns once and backs off when a server accepts connections but never responds, waits out (up to 120s) a slow-booting local server instead of stranding it — stopping it if it truly fails — recovers fast runner starts after a zygote crash, and no longer leaves empty log files behind. (#4563)
...
- [Bug fix] Upgrading agentnexus in place no longer breaks runner launches on already-running hosts (#4587)
...
- [Chore] Removed the "A new version of AgentNexus is available" prompt and browser PWA install support; the desktop and mobile apps remain the installable clients. (#4617)
- [Chore] OpenTelemetry exporters and automatic instrumentors are now installed through `agentnexus[tracing]` instead of the default package. (#4621)
- [Bug fix] Development builds no longer show an update reminder for the matching final release. (#4628)
- [Bug fix] `omni host` no longer prints a zygote traceback — and keeps copy-on-write runner forking — when started from a directory that contains an `agentnexus` checkout (#4631)
...
- [Bug fix] Sessions whose workspace is an agentnexus checkout no longer run a different agentnexus than the one you installed (#4688)
- [Bug fix] `agentnexus run --harness acp:<agent>` now launches the ACP agent you asked for instead of the first one configured (#4689)
...
- [Bug fix] `agentnexus run --harness acp:<slug>` now works with remote servers by resolving the slug client-side and embedding the full agent config in the spec. ACP agent settings (session_id_mode, send_model, agentnexus_mcp, env_passthrough) are now preserved through embedding. (#4702)
...
- [Bug fix] `agentnexus resume` now lists only your own sessions, not ones shared with you (#4709)
...
- [Feature] New `AGENTNEXUS_REQUIRE_WRAPPER` env var lets operators require the CLI be run through a wrapper (e.g. `isaac omni`) and refuse direct `omni` calls (#4766)
...
- [UI / Feature / Docs] Usage and web-driven harness setup can now be enabled per deployment with `AGENTNEXUS_FEATURES`. (#4775)
...
- [Feature / Docs] ArgoCD quick-start overlay for deploying AgentNexus with the kubernetes sandbox provider (#4788)
- [Bug fix] `agentnexus[antigravity]` no longer crashes with a protobuf gencode/runtime version mismatch on startup. (#4795)
...
- [Bug fix] `omnidev agentnexus` commands now keep runtime state and configuration inside their development pod. (#4822)
...
- [Feature / Breaking] Managed runner Pods are now labelled with the agent they are running, so admission policies and Pod selectors can target a single agent's sandboxes. **Breaking:** session create and patch now reject client-supplied `agentnexus.sandbox.*` labels, a namespace reserved for server-internal sandbox lifecycle state. (#3361)
...
- [Bug fix / Feature] `agentnexus run --profile <name>` authenticates a remote `--server` with a named Databricks profile (enables headless service-principal access to a deployed app) (#4017)
...
- [Bug fix] `agentnexus host` now survives a dropped VPN — a 401/403 on an already-connected host retries and auto-reconnects instead of exiting (#4025)
...
- [Bug fix] `brew install agentnexus` works again, and installs prebuilt wheels instead of (#4080)
...
- [Bug fix] `agentnexus codex` now keeps the terminal's session link and the exit `--resume` (#4138)
...
- [Bug fix] Runner tunnel HTTP 401 errors now suggest stopping stale host instances with `agentnexus stop`. (#4175)
...
- [Docs / Chore] Resuming a goose / hermes / antigravity / qwen / opencode native session now attaches to the live TUI instead of double-posting each message through the AgentNexus REPL. (#3314)
...
- [UI / Bug fix / Feature / Test/CI] `agentnexus setup` can import OpenClaw/acpx coding agents from an auto-detected or user-selected config into the generic ACP harness picker, and `agentnexus run --from-openclaw <agent>` can try one without saving it. (#3354)
...
- [Bug fix / Feature / Docs / Test/CI] ACP agents can opt out of AgentNexus’s MCP relay with `agentnexus_mcp: false`, enabling compatible OpenClaw Gateway ACP registrations. (#3420)
...
- [Bug fix] `/model` and the startup header no longer name an AgentNexus provider and model for ACP-backed sessions, which run on the agent's own auth and model. (#3431)
- [Bug fix] Session tokens saved by `agentnexus login` are now created owner-only, so a JWT is never briefly world-readable on first login, and an interrupted write no longer discards every stored token. (#3441)
...
- [Feature / Docs] Minimal `agentnexus run` agents now discover their default model instead of using a release-specific built-in endpoint. (#3455)
...
- [Bug fix] `agentnexus sandbox create --provider openshell` no longer crashes against openshell SDK >=0.0.86; workspace is configurable via `sandbox.openshell.workspace` or `$AGENTNEXUS_OPENSHELL_WORKSPACE` (defaults to `"default"`). (#3524)
- [Bug fix] Shared-session agents distinguish speakers without treating claimed roles as authorization. (#3527)
- [Bug fix] Direct `agentnexus.llms.Client` Anthropic reasoning requests now select adaptive or fixed-budget thinking from live model capabilities. (#3529)
...
- [Feature] `agentnexus import --force` replaces a previously imported chat with the latest local transcript. (#3576)
...
- [UI / Feature] `agentnexus --help` now groups launch commands under a **Harnesses** section, colorizes the output, hides harnesses whose optional extra isn't installed (with a notice pointing at `agentnexus setup`), drops the duplicate `update` alias line, and tidies the command descriptions (#3795)
...
- [UI / Chore] `agentnexus --help` command descriptions are tidied (harness rows read "Launch <Name> with AgentNexus"), and the duplicate `update` alias line is hidden from the listing (#3797)
...
- [Bug fix] Pi-native sessions now retain AgentNexus system tools and the comment relay. (#3920)
...
- [Feature] `agentnexus diagnose` prints a secret-free environment snapshot (CLI/server versions, OS, auth mode) for bug reports (#3928)
...
- [UI / Feature] Voice dictation in the composer now works in Electron, Firefox, and Chromium via optional server-side transcription (`agentnexus[dictation]`) — local models, live streaming partials, audio never leaves your server. (#2093)
...
- [Feature] Set `AGENTNEXUS_CONTAINER_RUNTIME=podman` to use Podman (or another supported runtime) globally instead of Docker, without editing every agent's YAML. (#2949)
...
- [Feature / Test/CI] `agentnexus` benchmark harness gains `--network-delay-ms` and per-journey HTTP request counts (#2977)
...
- [Feature] Import local Qwen, Kiro, Pi, and Kimi coding chats into AgentNexus (#3032)
...
- [Feature] Import existing OpenCode chats, including files and tool activity, with `agentnexus import` (#3046)
...
- [Bug fix / Feature] The Slack bot can now run against an AgentNexus server deployed on Databricks Apps, (#3051)
...
- [Bug fix] Databricks OAuth CLI profiles no longer fail with a misleading "malformed profile" error; the message now explains the real fix (install `agentnexus[databricks]` or refresh the OAuth session). (#3059)
...
- [Bug fix] `agentnexus server --host 0.0.0.0` now enables accounts (login) mode automatically instead of silently 401-ing every request (#3107)
...
- [Feature] `omnidev agentnexus <args…>` runs an agentnexus command against the current checkout's pod via `uv run agentnexus`, with the pod's isolated env applied (#3110)
...
- [Feature] `agentnexus session import` loads a `session export` JSONL back into a server as a new session (#3141)
...
- [Feature] `agentnexus claude` keeps tool search enabled when launched with `CLAUDE_CODE_USE_GATEWAY=1`. (#3161)
- [Test/CI] Fix `test_session_stream_emits_heartbeat_on_idle` after `_session_labels_for_runner_spawn` was extracted into `agentnexus.runner.native.orchestration`; patch the heartbeat cadence on `agentnexus.runner.app` where it is defined and consumed. (#3163)
...
- [Chore / Breaking] Slack-on-Databricks deploy: renamed `AGENTNEXUS_SLACK_WEBAUTH_BASE_URL` to `AGENTNEXUS_SLACK_DATABRICKS_APP_URL` (`--app-url`), removed the `WEBAUTH_PORT` / `DATABRICKS_WORKSPACE_HOST` overrides, and dropped deploy-time `uv lock` in favor of in-container `uv run`. (#3206)
...
- [Feature] Smart routing now activates automatically when a server `llm:` block or an external `routing:` block is configured — no `AGENTNEXUS_SMART_ROUTING` env var needed (#3215)
...
- [Feature] Add a top-level `justfile` with recipes for launching the iOS Simulator, running the Android debug build, starting the agentnexus dev pod, and running pre-commit/lockfile normalization. (#3310)
...
- [Feature] `agentnexus server` now tells users they can set `AGENTNEXUS_AUTH_ENABLED=0` to override automatic multi-user mode when binding to a non-loopback interface. (#3320)
- [Chore / Breaking] [Breaking] The deprecated `AGENTNEXUS_ACCOUNTS_ENABLED` environment variable has been removed; use `AGENTNEXUS_AUTH_ENABLED` instead. (#3322)
...
- [Bug fix] `sys_os_shell` commands no longer inherit agentnexus's own `PYTHONPATH` entry, so project subprocesses resolve their own installed packages instead of agentnexus's. (#1861)
...
- [Bug fix] Fixed the headless Hermes harness registering no AgentNexus MCP server (agent had no builtin tools). (#2216)
...
- [Bug fix] `agentnexus server` now accepts documented boolean server-config values like `sandbox.kubernetes.in_cluster: false` instead of rejecting them at startup. (#2314)
...
- [Bug fix / Feature] `agentnexus run --harness <name>-native` now launches every registered native harness with matching prompt and model behavior. (#2379)
- [Feature / Docs / Test/CI] Harness bench now reports AgentNexus MCP relay support separately from vendor-native tool calling. (#2380)
...
- [Bug fix] Pi no longer shows a blocking "Trust project folder?" dialog when launched via AgentNexus in a project with `.pi/` settings or extensions (requires Pi 0.79+) (#2529)
...
- [Feature / Docs / Test/CI] Add `agentnexus uninstall` for safely removing the CLI, installer-managed shell/config edits, and optionally local AgentNexus state. (#2550)
...
- [UI / Bug fix] The file editor now follows AgentNexus's selected web and desktop color theme instead of the operating-system theme. (#2594)
...
- [Chore / Breaking] `agentnexus[memory]` is renamed to `agentnexus[hindsight]`; the Hindsight memory extra is now the canonical name. (#2605)
- [Bug fix] `agentnexus host status` now hides the session table by default (use `--sessions` to show it), making the command significantly faster. (#2606)
- [Feature] `agentnexus://<hostname>/c/<session_id>` links open that session in the desktop app, reusing an open window on that server in-place (#2607)
...
- [Feature] Session creation requests from the web UI now send an explicit `X-AgentNexus-Client` header, enabling the server to record the exact client surface (web/desktop/ios/android) in telemetry without relying on User-Agent parsing. (#2615)
...
- [Bug fix] Clicking an AgentNexus session URL printed inside the embedded terminal now stays in the web app instead of opening a duplicate browser tab/window. (#2639)
...
- [Feature] `agentnexus://<hostname>/c/<session_id>` links open that session in the iOS app, reusing the open window on that server in-place (#2661)
...
- [Feature] `agentnexus setup` can now install Hermes directly before configuring its model provider. (#2751)
...
- [Bug fix] Native Codex and Claude now resolve their CLI from common global install dirs and an `AGENTNEXUS_CODEX_PATH` / `AGENTNEXUS_CLAUDE_PATH` override when it isn't on the host daemon's PATH (#2788)
...
- [Feature / Breaking] Per-harness startup command/args overrides via a polymorphic `harness:` config key + `AGENTNEXUS_<NAME>_PATH` env-var standardization (legacy `HARNESS_*_PATH` deprecated, slated for v0.8.0 removal) (#2933)
- [Bug fix] `agentnexus[memory]` is restored as a backwards-compat alias for `agentnexus[hindsight]` (the `memory` extra was dropped by #2605); it will be removed in 0.70. (#2938)
- [Feature] `AGENTNEXUS_SESSION_RENAME=on` re-enables the automatic first-turn session rename (disabled by default) (#2944)
...
- [Feature] `AGENTNEXUS_OIDC_SKIP_EMAIL_VERIFICATION=1` lets OIDC logins through when the IdP omits the `email_verified` claim (e.g. standard-tier Okta with directory-provisioned users) (#1859)
...
- [UI / Bug fix] "Back to AgentNexus" from Settings now returns you to the conversation you were viewing instead of the home page (#1905)
...
- [Bug fix / Chore] Fixed harness workers (claude, codex, etc.) failing to start when agentnexus is launched from a macOS or Linux GUI client due to a stripped PATH. Fix now lives in the Electron launcher (web/electron/src/main.js) per reviewer guidance. (#1935)
...
- [Feature / Test/CI] New official `agentnexus-server-kubernetes` image ships the kubernetes sandbox provider SDK — the `sandbox-runners` overlay now works against published images, no custom build needed. (#2124)
...
- [UI / Feature] Pick a color theme (AgentNexus, Dracula, GitHub, Catppuccin, or Gruvbox) in Appearance settings, independent of light/dark mode. (#2147)
...
- [Feature / Test/CI] Add a performance-benchmark harness for HTTP user journeys, with a seeded corpus, a SQLite+Postgres backend matrix, and a nightly workflow (`uv run dev/benchmarks/agentnexus/run.py`) (#2159)
...
- [Feature / Test/CI] Add a performance-benchmark harness for HTTP + full-turn user journeys (`uv run dev/benchmarks/agentnexus/run.py`), with a seeded corpus and SQLite+Postgres backend matrix (#2202)
- [UI / Bug fix] The new-session picker now remembers the host you last picked instead of resetting to the default. (#2218)
- [Bug fix] Fixed the Hermes `pre_tool_call` hook double-gating AgentNexus relay tools, which parked a (#2220)
...
- [Feature] `agentnexus debug logs` tails runner, server, or CLI diagnostic logs; `--session` scopes runner logs to a specific session across relaunches (#2273)
...
- [Feature] omnidev dev pods now get their own isolated `config.yaml` (seeded from `~/.agentnexus/config.yaml`), so server-config edits while testing in a pod no longer touch your real config (#2360)
...
Highlights and full notes: <https://github.com/agentnexus-ai/agentnexus/releases/tag/v0.4.0>

## [v0.3.0] — 2026-06-26

Highlights and full notes: <https://github.com/agentnexus-ai/agentnexus/releases/tag/v0.3.0>

## [v0.2.0] — 2026-06-19

Highlights and full notes: <https://github.com/agentnexus-ai/agentnexus/releases/tag/v0.2.0>
